...
beforeEach(() => {
  profileCalls = 0
  dir = mkdtempSync(path.join(tmpdir(), "koda-telemetry-identity-"))
  Identity.reset()
...
describe("Identity.updateFromkodaAuth profile cache", () => {
  test("fetches profile and writes cache keyed by token hash", async () => {
    await Identity.updateFromkodaAuth("token-a")
  ...
  test("refetches when the token changes", async () => {
    await Identity.updateFromkodaAuth("token-a")
    Identity.reset()
    await Identity.updateFromkodaAuth("token-b")
  ...
  test("clears identity when token is null", async () => {
    await Identity.updateFromkodaAuth("token-a")
    Identity.reset()
    await Identity.updateFromkodaAuth(null)
