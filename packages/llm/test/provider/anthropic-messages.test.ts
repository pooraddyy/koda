    ...
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
        LLM.request({
          ...
          messages: [
            ...
            Message.user("Summarize the result."), // koda_change - preserve Anthropic role alternation
          ],
          cache: "none",
        }),
      )
      ...
      expect(prepared.body).toEqual({
        ...
        messages: [
          ...
          // koda_change start - adjacent user content is coalesced with tool results
          {
            role: "user",
            content: [
