...
// `component/dialog-model.tsx` so the koda Gateway grouping/search rules are
// testable without mounting the dialog.
//
// Two rules here exist because the TUI used to hide live koda Gateway models:
//   1. Recently used models are no longer stripped from their provider section.
//      Selecting a koda sonnet once used to remove it from "Recommended" /
//      "koda Gateway" entirely, leaving those sections looking empty. This
...
//      the title and the section header, so typing `koda` finds
...
export const koda_PROVIDER_ID = "koda"
        ...
        map(([modelID, model]) =>
          build(provider, modelID, model, {
            description: favorites.some((item) => sameRef(item, { providerID: provider.id, modelID }))
              ...
              : undefined,
            category: connected
              ? provider.id === koda_PROVIDER_ID && model.recommendedIndex !== undefined
