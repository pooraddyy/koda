...
export interface DialogSelectProps<T> {
  ...
  actions?: DialogSelectAction<T>[] // koda_change - supports actions without a selected option
  footerHints?: {
    title: string
    label: string
    side?: "left" | "right"
...
// koda_change start - support list-level actions when no option is selected
  ...
  function scrollToSelection(center: boolean) {
    ...
    if (center) {
      ...
      if (y < 0) {
        scroll.scrollBy(y)
        if (flat()[0] === selected()) {
          // koda_change - reference identity; duplicate values are legal (see `active`)
          scroll.scrollTo(0)
        }
      }
    }
      ...
      commands: [
        ...
        ...visible.map((item) => ({
          ...
          run() {
            ...
            setStore("input", "keyboard")
            // koda_change start - allow actions such as scope toggles on empty lists
            ...
            <For each={grouped()}>
              {([category, options], index) => (
                <>
                  ...
                  <For each={options}>
                    {(option) => {
                      // koda_change start - match the selected row by reference, not by value: the
                      ...
                      return (
                        <box
                          ...
                          onMouseOver={() => {
                            ...
                            const index = flat().indexOf(option) // koda_change - see `active` above
                            if (index === -1) return
                            moveTo(index)
                          }}
                          onMouseDown={() => {
