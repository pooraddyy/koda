...
import { useNudge } from "@/koda/cli/cmd/tui/context/nudge" // koda_change
...
import type { AssistantMessage, FilePart, UserMessage } from "@koda/sdk/v2"
...
// koda_change start
import { kodaSessionTuiSync } from "@/koda/session/tui-sync"
import { slashMatches } from "@/koda/cli/cmd/command-display"
import { createCostAlertController } from "@/koda/cli/cmd/tui/cost-alert"
import { MemoryPrompt } from "@/koda/cli/cmd/tui/component/memory-prompt"
// koda_change end
import { koda_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, useOpencodeKeymap } from "../../keymap"
import { useTuiConfig } from "../../config"
// koda_change start - vim modal editing for the prompt
import { useVim, VimModeIndicator, vimToggleCommand } from "@/koda/cli/cmd/tui/component/prompt"
// koda_change end
...
export function Prompt(props: PromptProps) {
  ...
  const nudge = useNudge() // koda_change
  ...
  const [store, setStore] = createStore<{
    ...
    interrupt: number
    exitPress: number // koda_change - track double ctrl+c to exit
    placeholder: number
  }>({
  ...
  // koda_change start - sync local agent/model whenever newest user message changes
  ...
  createEffect(() => {
    ...
    if (!parts) return
    if (!kodaSessionTuiSync.model({ role: msg.role, parts })) return
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      // koda_change start - vim modal editing toggle (palette + /vim)
      vimToggleCommand({
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.koda_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
          workspace.open()
        },
  ...
  useBindings(() => ({
    ...
    bindings: tuiConfig.keybinds.gather("prompt.palette", [
      ...
      "prompt.stash.pop",
      "prompt.stash.list",
      "prompt.vim.toggle", // koda_change
      "prompt.skills", // koda_change
      "session.interrupt",
      "workspace.set",
      "session.move",
    ]),
  ...
  const ref: PromptRef = {
    ...
    reset() {
      ...
      vim.resetVim() // koda_change - return to insert mode after the prompt is cleared
    },
    submit() {
      void submit()
    },
  ...
  // koda_change start - close autocomplete while blocking overlays hide the prompt
  createEffect(() => {
  ...
  // koda_change start - require a double Ctrl+C to exit from an empty focused prompt
  useBindings(() => ({
    target: inputTarget,
    enabled: inputTarget() !== undefined && !props.disabled && store.prompt.input === "",
    bindings: [
      ...
      commands: [
        {
          ...
          run() {
            ...
            vim.resetVim() // koda_change - recalled history starts in insert mode
  ...
  async function submitInner() {
    ...
    // koda_change start - in-memory cost alert command
    ...
    if (store.mode === "shell") {
      ...
      void sdk.client.session.shell({
        sessionID,
        agent: local.agent.current()?.name ?? "", // koda_change
        model: {
          providerID: selectedModel.providerID,
    ...
    toast.dismiss() // koda_change - dismiss persistent config warning on first submit
    history.append({
      ...store.prompt,
      mode: currentMode,
    })
  ...
  async function pasteInputText(text: string) {
    ...
    // koda_change start - a second identical paste expands the collapsed placeholder
    ...
    if (
      (lineCount >= 5 || pastedContent.length > 800) && // koda_change #7252 delay paste summary
  ...
  function clearPrompt() {
    ...
    setStore("extmarkToPartIndex", new Map())
    vim.resetVim() // koda_change - don't leak stale vim mode/selection into an emptied prompt
  }

  // koda_change start - cost-alert logic lives under koda/; only prompt mutation stays here
  ...
  const highlight = createMemo(() => {
    ...
    return local.agent.color(agent.name ?? "") // koda_change
  })
  ...
  const spinnerDef = createMemo(() => {
    const agent =
      status().type !== "idle"
        ...
        : local.agent.current()
    const color = agent ? local.agent.color(agent.name ?? "") : theme.border // koda_change
    return {
      frames: createFrames({
        color,
        style: "blocks",
      ...
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
        ...
        >
          ...
          >
            <textarea
              ...
              /* koda_change */ onCursorChange={() => {
                setCursorVersion((value) => value + 1)
                if (store.mode === "normal") auto()?.onCursorChange()
              }}
              /* koda_change - KeyEvent type for vim key routing */ onKeyDown={(e: KeyEvent) => {
                if (props.disabled) {
                  e.preventDefault()
                  return
                }
                // koda_change start - route keys through the vim layer when enabled
                if (vim.vimOnKey(e)) {
                  e.preventDefault()
                  e.stopPropagation()
                  return
                ...
                <Show when={local.agent.current()} fallback={<box height={1} />}>
                  {(agent) => (
                    <>
                      ...
                      {/* koda_change start - vim mode indicator */}
                      <VimModeIndicator
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          <Switch>
            ...
            {/* koda_change start - koda already shows the working directory in its sidebar */}
            <Match when={true}>{props.hint ?? <text />}</Match>
            {/* koda_change end */}
          </Switch>
          <Show when={status().type !== "retry"}>
            <box gap={2} flexDirection="row">
              {/* koda_change start - show "ctrl+c again to exit" hint */}
