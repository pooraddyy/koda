...
import { slashDisplay } from "@/koda/cli/cmd/command-display" // koda_change
import { createSessionPart, sessionMentionText } from "../../koda/session-mentions" // koda_change
import { DialogSessionMention } from "../../koda/dialog-session-mention" // koda_change
import { useDialog } from "../../ui/dialog" // koda_change
import { Locale } from "../../util/locale"
import type { PromptInfo } from "../../prompt/history"
import { useFrecency } from "../../prompt/frecency"
import { useBindings, useCommandSlashes, useOpencodeModeStack } from "../../keymap"
import { displayCharAt, mentionTriggerIndex } from "../../prompt/display"
import type { FileSystemEntry } from "@koda/sdk/v2"
...
export type AutocompleteRef = {
  ...
  // koda_change start - validate cursor moves and close overlays without mutating draft text
  onCursorChange: () => void
  dismiss: () => void
  // koda_change end
  visible: false | "@" | "/"
...
export function Autocomplete(props: {
  ...
  // koda_change start - "Past chats" opens a searchable session picker dialog
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    ...
    for (const serverCommand of sync.data.command) {
      // koda_change start - preserve suffixes like :skill when inserting selected slash commands
  ...
  const options = createMemo((prev: AutocompleteOption[] | undefined) => {
    ...
    const nonFileOptions: AutocompleteOption[] =
      store.visible === "@" ? [...referenceAliasesValue, ...agentsValue, ...mcpResources(), pastChatsOption] : [...commandsValue] // koda_change - add past chats option
  ...
  useBindings(() => ({
    ...
    bindings: [
      ...
      // koda_change start - close stale suggestions while allowing normal cursor movement
      {
        key: "right",
        fallthrough: true,
        cmd: () => {
  ...
  // koda_change start - keep slash text intact when overlays hide the prompt,
  ...
  onMount(() => {
    ...
    props.ref({
      ...
      // koda_change start - dismiss stale popup after cursor leaves active filter region
      onCursorChange() {
        if (!store.visible) return
        ...
        >
          {(option, index) => (
            <box
              ...
              onMouseUp={() => select()}
              gap={1} // koda_change - keep descriptions separated from labels in flex layout
            >
              ...
              <Show when={option().description}>
                <text fg={index === store.selected ? selectedForeground(theme) : theme.textMuted} wrapMode="none">
                  {option().description?.trimStart()}{/* koda_change */}
                </text>
              </Show>
            </box>
          )}
