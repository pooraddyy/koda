...
export function DialogSessionList() {
  ...
  const [global, setGlobal] = createSignal(false) // koda_change - show current worktree by default
  const deleteHint = useCommandShortcut("session.delete")
  const quickSwitch1 = useCommandShortcut("session.quick_switch.1")
  const quickSwitch9 = useCommandShortcut("session.quick_switch.9")

  // koda_change start - experimental sessions retain worktree labels and scope
  ...
  // koda_change - support local and global sessions
  ...
  const options = createMemo(() => {
    ...
    function buildOption(id: string, category: string) {
      ...
      return {
        ...
        description: all && "worktreeName" in x && x.worktreeName ? `(${x.worktreeName})` : undefined, // koda_change
  ...
  return (
    <DialogSelect
      title={global() ? "Sessions (all worktrees)" : "Sessions (current worktree)"} // koda_change
      options={options()}
      skipFilter={true}
      ...
      // koda_change start - preserve Ctrl+A worktree scope toggle with the upstream keymap engine
