...
// koda_change start — guard against missing renderer context in ErrorBoundary fallback
...
export function ErrorComponent(props: { error: Error; reset: () => void; mode?: "dark" | "light" }) {
  ...
  // koda_change start — guard against missing keyboard context in ErrorBoundary fallback
  try {
    useKeyboard((evt) => {
...
function buildIssueURL(message: string, stack: string) {
  ...
  // koda_change start
  const url = new URL("https://github.com/koda-Org/koda/issues/new?template=bug-report.yml")
  url.searchParams.set("title", `koda TUI crash: ${message}`)
  url.searchParams.set("koda-version", InstallationVersion)
  // koda_change end
  url.searchParams.set("os", describeOS())
  url.searchParams.set("terminal", describeTerminal())
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the koda crash screen. If you can, describe what you were doing when it crashed.",
  )
  ...
  const marker = "\n... (truncated)"
  const head = `The koda TUI crashed with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
