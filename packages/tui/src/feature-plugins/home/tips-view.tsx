import type { TuiPluginApi } from "@koda/plugin/tui"
import { createMemo, For, type Accessor } from "solid-js"
import { DEFAULT_THEMES, useTheme } from "../../context/theme"
import { koda_TIPS } from "@/koda/cli/cmd/tui/feature-plugins/home/tips" // koda_change
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const tip = createMemo(() => {
    ...
    // koda_change start
    const tips = [...koda_TIPS, ...TIPS, process.platform !== "win32" ? TERMINAL_SUSPEND_TIP : INPUT_UNDO_TIP].flatMap(
      (item) => {
...
const TIPS: Tip[] = [
  /* koda_change hide the entire list for if it is accidentally used
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.koda/commands/{/highlight} to define reusable custom prompts", // koda_change
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.koda/agents/{/highlight} for specialized AI personas", // koda_change
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.koda/tools/{/highlight} to define new LLM tools", // koda_change
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.koda/plugins/{/highlight} for event hooks", // koda_change
  ...
  "Create JSON theme files in {highlight}.koda/themes/{/highlight} directory", // koda_change
