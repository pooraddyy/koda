import type { Event } from "@koda/sdk/v2"
import type { TuiAttentionSoundName, TuiPlugin, TuiPluginApi } from "@koda/plugin/tui"
...
const tui: TuiPlugin = async (api) => {
  ...
  // koda_change start - only completed turns need completion attention
  ...
  api.event.on("session.error", (event) => {
    ...
    if (event.properties.error?.name === "MessageAbortedError") return // koda_change - manual stops do not need attention
