...
import { createStore, produce, reconcile } from "solid-js/store" // koda_change
import { createSimpleContext } from "./helper"
import { useSDK } from "./sdk"
import { useEvent } from "./event"
import { createSignal, onCleanup, onMount } from "solid-js"
import { hydrate } from "../koda/hydration" // koda_change
...
// koda_change start - "global" is an event-routing sentinel, not a filesystem location.
...
// koda_change start - suppress only refreshes canceled by normal TUI disposal
...
export const { use: useData, provider: DataProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const syncing = new Map<string, Promise<void>>() // koda_change
    ...
    function handleEvent(event: V2Event) {
      switch (event.type) {
        ...
        case "session.next.compaction.ended": {
          // koda_change start - legacy v1 compaction events do not carry a projectable message identity.
    ...
    onMount(() => {
      const unsub = events.subscribe((event, metadata) => {
        handleEvent({
          ...
          location: eventLocation({ directory: metadata.directory, workspace: metadata.workspace }), // koda_change
        } as V2Event)
      })
      onCleanup(unsub)
    })
    ...
    const result = {
      session: {
        ...
        message: {
          ...
          async refresh(sessionID: string) {
            // koda_change start - reconcile the snapshot without losing live updates received while it was loading
    ...
    // koda_change start - classify each rejection when it occurs so later disposal cannot hide earlier failures
    let disposed = false
    onCleanup(() => {
      disposed = true
    })
