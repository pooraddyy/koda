import type {
  ...
  PermissionRequest,
  QuestionRequest,
  SuggestionRequest, // koda_change
  SessionNetworkWait, // koda_change
  LspStatus,
  McpStatus,
  McpResource,
  FormatterStatus,
  ...
  SnapshotFileDiff,
  ConsoleState,
  BackgroundProcessInfo, // koda_change
  InteractiveTerminalSnapshot, // koda_change
  IndexingStatus, // koda_change
} from "@koda/sdk/v2"
...
import { batch, createEffect, on, onMount } from "solid-js" // koda_change
import path from "path"
import { useKV } from "./kv"
import { handleSuggestionEvent } from "@/koda/suggestion/tui/sync" // koda_change
import { appendTerminalOutput } from "@/koda/interactive-terminal/output" // koda_change
import { at, recent, slot } from "../koda/message-order" // koda_change
import { useToast } from "../ui/toast" // koda_change
...
export const {
  ...
  init: () => {
    ...
    const [store, setStore] = createStore<{
      ...
      // koda_change end
      config: Config
      globalConfig: Config // koda_change
      session: Session[]
      session_status: {
      ...
      indexing: IndexingStatus // koda_change
    }>({
      provider_next: {
        all: [],
        default: {},
      ...
      capabilities: {
        experimentalBackgroundSubagents: true, // koda_change - background subagents are enabled by default
      },
      provider_auth: {},
      config: {},
      globalConfig: {}, // koda_change
      status: "loading",
      agent: [],
      permission: {},
      question: {},
      suggestion: {}, // koda_change
      network: {}, // koda_change
      command: [],
      provider: [],
      provider_default: {},
      ...
      session_diff: {},
      todo: {},
      background_process: {}, // koda_change
      interactive_terminal: {}, // koda_change
      message: {},
      part: {},
      lsp: [],
      mcp: {},
      ...
      formatter: [],
      vcs: undefined,
      indexing: { state: "Disabled", message: "Indexing disabled.", processedFiles: 0, totalFiles: 0, percent: 0 }, // koda_change
    })

    const event = useEvent()
    const project = useProject()
    const sdk = useSDK()
    const toast = useToast() // koda_change
    ...
    const deleted = new Set<string>() // koda_change
    const terminalDeleted = new Set<string>() // koda_change
    let syncedWorkspace = project.workspace.current() // koda_change
    let vcsVersion = 0 // koda_change
    ...
    event.subscribe((event, { directory, workspace }) => {
      switch (event.type) {
        ...
        case "session.deleted": {
          ...
          evict(event.properties.info.id) // koda_change
          break
        }
        ...
        case "message.updated": {
          ...
          const result = slot(messages, event.properties.info) // koda_change - order by created time, ids wrap
          if (result.found) {
        ...
        case "message.removed": {
          ...
          const result = at(messages, event.properties.messageID) // koda_change - list is time-ordered, not id-sorted
          if (result.found) {
            setStore(
              "message",
              event.properties.sessionID,
    ...
    // koda_change start - retain versioned Sync events used by koda clients
    event.sync((event) => {
      switch (event.name) {
        ...
        case "session.updated.1": {
          ...
          setStore(
            "session",
            match.index,
            reconcile(event.data.info), // koda_change - session.updated carries a full snapshot, including omitted optional fields
          )
          break
        }
        ...
        case "message.updated.1": {
          ...
          const match = slot(messages, info) // koda_change - order by created time, ids wrap
          if (match.found) {
        ...
        case "message.removed.1": {
          ...
          if (!messages) break
          const match = at(messages, event.data.messageID) // koda_change - list is time-ordered, not id-sorted
          if (!match.found) break
          setStore(
            "message",
    ...
    async function bootstrap(input: { fatal?: boolean } = {}) {
      ...
      const workspace = project.workspace.current()
      // koda_change start - isolate workspace-scoped koda state
      ...
      const version = vcsVersion // koda_change
      ...
      const globalConfigPromise = sdk.client.global.config.get({ throwOnError: true }) // koda_change
      await Promise.all([
        providersPromise,
        providerListPromise,
        capabilitiesPromise,
        agentsPromise,
        configPromise,
        globalConfigPromise, // koda_change
        projectPromise,
        ...
        .then(async () => {
          ...
          const globalConfigResponse = globalConfigPromise.then((x) => x.data!) // koda_change
          ...
          return Promise.all([
            ...
            agentsResponse,
            configResponse,
            globalConfigResponse, // koda_change
            ...
            const globalConfig = responses[6] // koda_change
            ...
            batch(() => {
              ...
              // koda_change start - fail closed when the backend omits the capability
              setStore(
                "capabilities",
              ...
              setStore("globalConfig", reconcile(globalConfig)) // koda_change
    ...
    // koda_change start - re-bootstrap when Agent Manager changes workspace
    createEffect(
      on(
            ...
            setStore(
              produce((draft) => {
                ...
                const infos = (messages.data ?? []).flatMap((message) => {
                  if (!tracker.messages.has(message.info.id)) return [strip(message.info)] // koda_change
                ...
                // koda_change start - window by created time so wrapped ids stay visible
