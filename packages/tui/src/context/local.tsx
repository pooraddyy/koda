...
import { useProject } from "./project" // koda_change
...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    const sync = useSync()
    const sdk = useSDK()
    const project = useProject() // koda_change
    ...
    function createAgent() {
      ...
      return {
        ...
        current() {
          // koda_change start - fall back to first agent when current is removed (e.g. org switch)
        ...
        move(direction: 1 | -1) {
          batch(() => {
            ...
            if (!value) return // koda_change - guard against empty agent list during org switch
    ...
    function createModel() {
      const [modelStore, setModelStore] = createStore<{
        ready: boolean
        // koda_change start - persisted picks plus process-local overrides
        model: Record<
          string,
          | {
              providerID: string
      ...
      const state = {
        pending: false,
        writer: Promise.resolve() as Promise<unknown>, // koda_change - serialize writes
      }

      // koda_change start - keep configured-agent selections process-local
      ...
      function save() {
        ...
        state.pending = false
        // koda_change start - serialize writes so a slow first write cannot overwrite a later one
        const data = {
      ...
      readJson<unknown>(filePath)
        .then((x) => {
          ...
          if (typeof value.model === "object" && value.model !== null)
            setModelStore("model", value.model as Record<string, { providerID: string; modelID: string } | undefined>) // koda_change
        })
        .catch(() => {})
        .finally(() => {
      ...
      const currentModel = createMemo(() => {
        ...
        if (!a) return fallbackModel() // koda_change - guard against empty agent list
        // koda_change start - configured models beat stale persisted picks
        return (
          getFirstValidModel(
      ...
      return {
        ...
        // koda_change start - expose persisted per-agent pick separately from overrides
        saved(name: string) {
          return modelStore.model[name]
        },
        // koda_change end
        // koda_change start - resolve once all queued writes (atomic write+rename) have settled.
        ...
        cycle(direction: 1 | -1) {
          ...
          const a = agent.current()
          if (!a) return
          apply(a.name, val, !a.model) // koda_change
          save() // koda_change
        },
        cycleFavorite(direction: 1 | -1) {
          ...
          const a = agent.current()
          if (!a) return
          apply(a.name, next, !a.model) // koda_change
        ...
        set(model: { providerID: string; modelID: string }, options?: { recent?: boolean }) {
          batch(() => {
            ...
            const a = agent.current()
            if (!a) return
            apply(a.name, model, !a.model) // koda_change
            if (options?.recent) {
    ...
    createEffect(() => {
      // koda_change start - configured models resolve directly without persistence
