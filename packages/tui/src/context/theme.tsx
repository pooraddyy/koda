...
const themeSource: ThemeSource = {
  async discover() {
    ...
    for (let current = process.cwd(); ; current = path.dirname(current)) {
      // koda_change start - discover koda config roots, not OpenCode roots
      directories.push(path.join(current, ".koda"), path.join(current, ".koda"))
      // koda_change end
...
export async function discoverThemes(directories: string[]) {
  ...
  for (const directory of directories) {
    ...
    for (const file of files) {
      // koda_change start - one malformed custom theme must not discard all themes
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "koda") // koda_change
        draft.active = typeof active === "string" ? active : "koda" // koda_change
        draft.ready = false
      }),
    )
        ...
        .then((themes) => {
          setCustomThemes(
            Object.entries(themes).reduce<Record<string, ThemeJson>>((result, [name, theme]) => {
              // koda_change - protect built-ins and require core theme colors
              if (!(name in DEFAULT_THEMES) && isValidTheme(theme)) result[name] = theme
              return result
            }, {}),
          )
        })
        .catch(() => setStore("active", "koda")) // koda_change
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "koda") // koda_change
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.koda, store.mode) // koda_change
    })
...
// koda_change start - custom themes must contain the colors required for safe fallback rendering
