...
import { ApprovalBadge, describeApproval, stateMetadata } from "../../koda/tool-approval" // koda_change
import { useEpilogue } from "../../context/epilogue"
import { normalizePath } from "../../util/path"
import { PermissionPrompt } from "./permission"
import { QuestionPrompt } from "./question"
// koda_change start
import { Suggest } from "@/koda/suggestion/tui/render"
import { SuggestPrompt } from "@/koda/suggestion/tui/prompt"
...
import { koda_BASE_MODE, useBindings, useCommandShortcut, useOpencodeKeymap } from "../../keymap"
import { usePathFormatter } from "../../context/path-format"
// koda_change start
import { KodaErrorBlock } from "@/koda/components/koda-error-display"
import { splitDiffHunks } from "@/koda/tui/diff"
import { RoutedModelMeta } from "@/koda/cli/cmd/tui/routes/session/routed-model-meta"
import { submitFeedback } from "@/koda/cli/cmd/tui/feedback"
import { MemorySessionTui } from "@/koda/cli/cmd/tui/routes/session/memory"
...
const sessionBindingCommands = [
  ...
  "messages.copy",
  // koda_change start - message feedback
...
export function Session() {
  ...
  onCleanup(MemorySessionTui.attach({ event, toast, sessionID: route.sessionID })) // koda_change

  // koda_change start - background processes are scoped to the visible session
  ...
  const sessionCommandList = createMemo(() => [
    ...
    },
    {
      ...
      value: "messages.copy",
      category: "Session",
      // koda_change start - /copy copies the latest assistant response
      slash: {
        name: "copy",
      },
      // koda_change end
    ...
    // koda_change end
    {
      ...
      category: "Session",
      slash: {
        name: "copy-session", // koda_change - transcript copy moved off /copy
      },
      run: async () => {
        try {
          const sessionData = session()
          if (!sessionData) return
          // koda_change start - fetch all messages from server instead of truncated sync store
        ...
        <box flexDirection="row" flexGrow={1} minHeight={0}>
          <box flexGrow={1} minHeight={0} paddingBottom={1} paddingLeft={2} paddingRight={2} gap={1}>
            <Show when={session()}>
              ...
              <box flexShrink={0}>
                {/* koda_change start - arbitrate koda terminal, question, suggestion, and network input */}
...
function AssistantMessage(props: { message: AssistantMessage; parts: Part[]; last: boolean }) {
  ...
  const routed = createMemo(() => RoutedModelMeta.info(ctx.providers(), props.parts, ctx.showDetails(), props.message)) // koda_change
  const route = createMemo(() => routed().footer) // koda_change
  ...
  return (
    <>
      {/* koda_change start - provide compact routed-model metadata to part renderers */}
      ...
      {/* koda_change start - koda-specific error display */}
        ...
        <Match when={props.last || final() || props.message.error?.name === "MessageAbortedError"}>
          <box ref={(el: BoxRenderable) => alwaysSeparate.add(el)} paddingLeft={3}>
            <text marginTop={1}>
              ...
              {/* koda_change start - show routed model in regular assistant footer */}
              <Show when={route()}>
...
// koda_change start - register rendered step-finish parts
const PART_MAPPING = {
  text: TextPart,
  tool: ToolPart,
...
// koda_change start - show concrete routed models reported by gateway/provider responses
function StepFinishPart(props: { last: boolean; part: StepFinishPart; message: AssistantMessage }) {
  ...
  const routed = createMemo(() => {
    if (props.message.providerID !== "koda") return undefined
    if (!props.message.modelID.startsWith("koda-auto/")) return undefined
...
function ReasoningPart(props: { last: boolean; part: ReasoningPart; message: AssistantMessage }) {
  ...
  return (
    // Minimal mode keeps completed reasoning compact, but the active Thinking
    // row remains visible so the user can see that the agent is working.
    <Show when={!inMinimal() || !isDone()}>
      <Show when={content() || !isDone()}>
        <box
          ref={(el: BoxRenderable) => alwaysSeparate.add(el)}
          paddingLeft={3}
          marginTop={1}
...
function TextPart(props: { last: boolean; part: TextPart; message: AssistantMessage }) {
  ...
  // koda_change start - format markdown tables with fixed-width columns
...
function ToolPart(props: { last: boolean; part: ToolPart; message: AssistantMessage }) {
  ...
  // Keep routine file inspection quiet in the live terminal transcript. Errors
  // remain visible, while the underlying tool result is still retained for
  // transcript export and session state.
  const quietInspection = createMemo(() => ["glob", "read", "grep"].includes(display()))
  const shouldHide = createMemo(() => {
    if (quietInspection()) return props.part.state.status !== "error"
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        {/* koda_change start - preserve koda tool-specific status rendering */}
...
// koda_change start - koda tool-specific status rendering
...
function InlineTool(props: {
  ...
  // koda_change - explain why the call was auto-approved or denied
  ...
  return (
    <InlineToolRow
      ...
      note={approvalNote()} // koda_change
      noteColor={theme.textMuted} // koda_change
...
export function InlineToolRow(props: {
  ...
  spinner?: boolean
  separate?: boolean
  note?: string // koda_change - why the call was auto-approved or denied
  noteColor?: RGBA // koda_change
  children: JSX.Element
  onMouseOver?: () => void
  onMouseOut?: () => void
  onMouseUp?: () => void
          ...
          >
            <box flexDirection="row">
              ...
              >
                ...
                {/* koda_change - explain why the call was auto-approved or denied, inline on the header */}
...
function BlockTool(props: {
  ...
  part?: ToolPart
  spinner?: boolean
  hideApproval?: boolean // koda_change - suppress the auto-approval note (e.g. todowrite)
}) {
  const { theme } = useTheme()
  const renderer = useRenderer()
  const [hover, setHover] = createSignal(false)
...
function Task(props: ToolProps) {
  ...
  const content = createMemo(() => {
    ...
    } else if (isRunning()) content.push(`↳ Starting...`) // koda_change
...
function Edit(props: ToolProps) {
  ...
  const hunks = createMemo(() => splitDiffHunks(diffContent())) // koda_change

  return (
    <Switch>
      <Match when={stringValue(props.metadata.diff) !== undefined}>
        <BlockTool title={"← Edit " + pathFormatter.format(stringValue(props.input.filePath))} part={props.part}>
          {/* koda_change start - preserve separated multi-hunk edit rendering */}
  ...
  return (
    <Switch>
      <Match when={parseTodos(props.metadata.todos).length}>
        {/* koda_change - todo writes are orchestration, not a mutating action to explain */}
...
function Question(props: ToolProps) {
  ...
  // koda_change start - preserve dismissed content and the compact expandable summary
...
const toolDisplays = new Set([
  ...
  "question",
  "skill",
  // koda_change start - retain dedicated koda tool renderers
  "execute",
  "background_process",
