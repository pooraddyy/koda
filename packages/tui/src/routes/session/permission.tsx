...
import type { PermissionRequest } from "@koda/sdk/v2"
...
import { normalizeUrls } from "@/koda/util/url"
import { MemoryPermissionRegistry } from "@/koda/cli/cmd/tui/routes/session/memory-permission"
import { skillShellPrompt } from "@/koda/skills/display"
// koda_change end
import { koda_BASE_MODE, useBindings, useCommandShortcut } from "../../keymap"
...
function EditBody(props: { request: PermissionRequest }) {
  ...
  const hunks = createMemo(() => splitDiffHunks(diff())) // koda_change
        ...
        <Prompt
          ...
          onSelect={(option) => {
            ...
            void sdk.client.permission.reply({
              ...
              interactive: true, // koda_change - human answered this prompt
            })
          }}
        />
      </Match>
      ...
      <Match when={store.stage === "permission"}>
        {(() => {
          const info = () => {
            ...
            if (permission === "bash") {
              // koda_change start - skill shell batches show the verbatim, escaped commands + skill title
            ...
            // koda_change start - show sandbox escalation details and keep approval one-shot
            ...
            if (permission === "webfetch") {
              const url = normalizeUrls(typeof data.url === "string" ? data.url : "") // koda_change
              return {
                icon: "%",
                title: `WebFetch ${url}`,
                body: (
            ...
            const custom = MemoryPermissionRegistry.render(permission, props.request) // koda_change
            if (custom) return custom // koda_change
          ...
          const header = () => (
            <box flexDirection="column" gap={0}>
              ...
              {/* koda_change start - explain protected koda configuration access */}
                ...
                <box paddingLeft={4} flexShrink={0}>
                  <text fg={theme.textMuted}>
                    {props.request.permission === "edit"
                      ...
                      : "koda configuration access always requires approval"}
                  </text>
                </box>
              </Show>
              {/* koda_change end */}
          ...
          // koda_change start - skill shell batches are never persisted: only Allow / Reject
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell koda what to do differently</text>
        </box>
      </box>
      <box
