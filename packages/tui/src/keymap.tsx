...
export const koda_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const koda_MODE_KEY = "opencode.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(koda_MODE_KEY, koda_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(koda_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(koda_MODE_KEY, stack.at(-1)?.mode ?? koda_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? koda_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(koda_MODE_KEY, undefined)
...
export function getOpencodeModeStack(keymap: OpenTuiKeymap) {
  ...
  if (!value) throw new Error("koda mode stack is not registered for this keymap") // koda_change
  return value
}
