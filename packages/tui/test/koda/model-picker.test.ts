...
import {
  ...
  type ModelPickerRef,
} from "../../src/koda/model-picker"

const koda: ModelPickerProvider = {
  id: "koda",
  name: "koda Gateway",
  models: {
...
const providers = [koda, BEDROCK]

const sonnet45: ModelPickerRef = { providerID: "koda", modelID: "anthropic/claude-sonnet-4-5" }
const sonnet4: ModelPickerRef = { providerID: "koda", modelID: "anthropic/claude-sonnet-4" }
...
describe("model picker options", () => {
  ...
  test("keeps recommended koda models in their section after they are used", () => {
  ...
  test("finds a recently used recommended koda model when filtering by provider name", () => {
    const options = build({ recents: [sonnet45], query: "koda" })
    ...
    expect(inCategory(options, "koda Gateway")).toContain("openai/gpt-5")
  })

  test("filtering by provider name does not leak other providers", () => {
    const options = build({ query: "koda" })

    expect(options.every((option) => option.providerID === "koda")).toBe(true)
  })

  test("keeps the koda Gateway section populated after a Bedrock model is used", () => {
    const options = build({ recents: [bedrockSonnet] })

    expect(inCategory(options, "Recent")).toEqual([bedrockSonnet.modelID])
    expect(inCategory(options, "koda Gateway")).toEqual(["openai/gpt-5"])
    expect(inCategory(options, RECOMMENDED_CATEGORY)).toEqual([sonnet45.modelID, sonnet4.modelID])
