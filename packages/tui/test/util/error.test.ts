...
describe("util.error", () => {
  ...
  // koda_change start - preserve koda-branded CLI guidance
  test("uses koda commands and capability messaging", () => {
    const model = cliErrorMessage({
      _tag: "ProviderModelNotFoundError",
      providerID: "anthropic",
      modelID: "claude-sonnet-4",
    })
    expect(model).toContain("koda models")
    expect(model).toContain("koda.json")
