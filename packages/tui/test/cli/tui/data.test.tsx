...
import type { Event, GlobalEvent } from "@koda/sdk/v2"
import { createSignal, onMount, Show, type ParentProps } from "solid-js" // koda_change
import { ProjectProvider, useProject } from "../../../src/context/project" // koda_change
...
// koda_change start - initialize koda's project filter before mounting V2 event consumers
...
test("refreshes effective catalog data after catalog updates", async () => {
  ...
  const app = await testRender(() => (
    <TestTuiContexts>
      <SDKProvider url="http://test" directory={directory} events={events.source} fetch={calls.fetch}>
        <ProjectProvider>
          {/* koda_change start - initialize koda's project filter before consuming catalog events */}
          <Ready>
            <DataProvider>
              <box />
            </DataProvider>
...
// koda_change start - retain promoted prompt compatibility used by koda sync
...
// koda_change start - preserve koda's V2 hydration-race coverage after the DataProvider extraction
