...
// test/preload.ts pins koda_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.koda_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.koda_DISABLE_MODELS_FETCH
// koda_change start - isolate the mutable cache fixture from parallel package test processes
...
// koda_change end
beforeAll(() => {
  Flag.koda_MODELS_PATH = undefined
  Flag.koda_DISABLE_MODELS_FETCH = true
  Global.Path.cache = root // koda_change
})
afterAll(() => {
  Flag.koda_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.koda_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
  Global.Path.cache = original // koda_change
})

const cacheFile = path.join(root, "models.json") // koda_change
...
afterAll(async () => {
  await rm(root, { recursive: true, force: true }) // koda_change
})
...
describe("ModelsDev Service", () => {
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.koda_DISABLE_MODELS_FETCH = false
        }),
  ...
  it.live("refresh(true) fetches via HttpClient and updates the cache", () =>
    Effect.gen(function* () {
      ...
      expect(final.calls[0].userAgent).toEndWith(`/${Flag.koda_CLIENT}`)
    }),
  )
