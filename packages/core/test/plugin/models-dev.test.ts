...
// koda_change - Catalog pulls Credential, which imports Global.data/auth.json on startup, so
// without this the suite reads the developer's real credential store.
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "koda-modelsdev-test-"))
...
const layer = AppNodeBuilder.build(LayerNode.group([Catalog.node, Integration.node, EventV2.node]), [
  ...
  [Global.node, globalLayer], // koda_change
])
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.koda_MODELS_PATH,
          disabled: Flag.koda_DISABLE_MODELS_FETCH,
        }
        Flag.koda_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.koda_MODELS_PATH = previous.path
          Flag.koda_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
