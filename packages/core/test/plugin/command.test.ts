...
describe("CommandPlugin.Plugin", () => {
  it.effect("registers built-in init and review commands", () =>
    Effect.gen(function* () {
      ...
      expect((yield* command.get("init"))?.template).toContain("future koda sessions") // koda_change
      expect((yield* command.get("init"))?.template).toContain("`koda.json`") // koda_change
      expect((yield* command.get("init"))?.template).not.toContain("OpenCode") // koda_change
