...
const PTY_TEST_TIMEOUT = "15 seconds" // koda_change - PTY startup can exceed the default test timeout on macOS CI
...
const waitForEvents = (events: Queue.Queue<PtyEvent>, id: PtyID, count: number) =>
  Effect.gen(function* () {
    ...
    Effect.timeoutOrElse({
      duration: PTY_TEST_TIMEOUT, // koda_change
...
// koda_change start - preserve collected PTY output in timeout diagnostics
...
describe("pty", () => {
  ...
  // koda_change start - explicit commands must not acquire implicit login-shell arguments.
  ...
  // koda_change start - canonical attachments replay retained exited output, then end without accepting input.
