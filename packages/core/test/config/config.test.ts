...
describe("Config", () => {
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(async () => {
            ...
            await Promise.all([
              fs.writeFile(path.join(project, "koda.json"), JSON.stringify({ model: "project/model" })),
              fs.writeFile(path.join(global, "koda.json"), JSON.stringify({ model: "global/model" })),
            ])
          })

          const prior = process.env.koda_DISABLE_PROJECT_CONFIG
          process.env.koda_DISABLE_PROJECT_CONFIG = "1"
          yield* Effect.addFinalizer(() =>
            Effect.sync(() => {
              if (prior === undefined) delete process.env.koda_DISABLE_PROJECT_CONFIG
              else process.env.koda_DISABLE_PROJECT_CONFIG = prior
            }),
          )
  ...
  // koda_change - koda keeps loading config.json for released installs (see the names list in config.ts)
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents).toHaveLength(1) // koda_change
          }).pipe(Effect.provide(testLayer(tmp.path)))
        }),
      ),
    ),
  ...
  // koda_change start - V2 config discovery follows koda roots and precedence
  it.live("loads koda configuration roots up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await Promise.all(
              [root, directory].flatMap((dir) =>
                [".koda", ".koda", ".opencode"].map((name) => fs.mkdir(path.join(dir, name), { recursive: true })),
              ),
            )
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "opencode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "koda.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "koda.json"), JSON.stringify({ $schema: "root" })),
              ...
              fs.writeFile(path.join(root, ".koda", "opencode.json"), JSON.stringify({ $schema: "root-koda" })),
              fs.writeFile(path.join(root, ".koda", "koda.json"), JSON.stringify({ $schema: "root-koda" })),
              fs.writeFile(
                path.join(directory, ".koda", "opencode.jsonc"),
                JSON.stringify({ $schema: "directory-koda" }),
              ),
              fs.writeFile(path.join(directory, ".koda", "koda.jsonc"), JSON.stringify({ $schema: "directory-koda" })),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".koda")),
              AbsolutePath.make(path.join(root, ".koda")),
              AbsolutePath.make(path.join(directory, ".koda")),
              AbsolutePath.make(path.join(directory, ".koda")),
            ])
            ...
            expect(Config.latest(entries, "$schema")).toBe("directory-koda")
