    ...
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          ...
          expect(matches.items.map((item) => item.entry.path)).toContain(RelativePath.make(".opencode/config")) // koda_change
          expect(matches.items.map((item) => item.entry.path)).not.toContain(RelativePath.make(".git/config")) // koda_change
        }),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ),
  )

  // koda_change start - surfaced error keeps the underlying reason
