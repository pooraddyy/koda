...
describe("SessionRunnerLLM", () => {
  ...
  // koda_change start - prevent a source-location context epoch after the session moves mid-load
  ...
  it.effect("forces a text response on an agent's configured final step", () =>
    Effect.gen(function* () {
      ...
      expect(requests[1]?.messages.at(-1)).toMatchObject({
        role: "user", // koda_change - max-step instructions must not become assistant prefill
