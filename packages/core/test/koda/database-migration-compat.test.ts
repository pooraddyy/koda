...
import { ensure } from "@opencode-ai/core/koda/database-compat"
...
describe("database migration compatibility", () => {
  ...
  test("keeps released context epoch writes compatible on fresh current databases", async () => {
    ...
    const filename = path.join(tmp.path, "koda.db")
  ...
  test("skips a migration another writer recorded after the completed snapshot", async () => {
    await run(
      Effect.gen(function* () {
        const db = yield* make
        yield* db.run(sql`CREATE TABLE session (id text PRIMARY KEY)`)
        // stands in for a second koda process committing the later migration's journal row
