...
import { kodaPtyTermination } from "../../src/koda/pty/termination"

function fake(pid = 123) {
  const calls: Array<string | undefined> = []
  const proc: kodaPtyTermination.Process = {
    pid,
    onExit: () => ({ dispose() {} }),
...
function runtime(
  ...
  const sleeps: number[] = []
  let alive = true
  const value: kodaPtyTermination.Runtime = {
    platform,
...
describe("pty process-tree termination", () => {
  test("uses hidden taskkill for Windows process trees", async () => {
    ...
    await kodaPtyTermination.terminate(item.proc, input.value)
