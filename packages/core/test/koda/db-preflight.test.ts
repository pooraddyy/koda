...
import { DbPreflight } from "@opencode-ai/core/koda/db-preflight"
import { Database as kodaDatabase } from "@opencode-ai/core/database/database"
...
// reproducing the state a crashed koda process leaves behind
...
describe("DbPreflight", () => {
  ...
  test("accepts a writable database", async () => {
    ...
    const file = path.join(tmp.path, "koda.db")
    createWalDb(file)
    expect(() => DbPreflight.assertWritable(file)).not.toThrow()
  })

  test("names the offending file for a read-only sidecar outside the koda data dir", async () => {
    if (skip) return
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "koda.db")
  ...
  test("reports a missing directory as missing, not as read-only", async () => {
    ...
    const file = path.join(dir, "koda.db")
  ...
  test("pending WAL frames with a read-only sidecar fail with the actionable error, and repair recovers the data", async () => {
    ...
    expect(() => kodaDatabase.layerFromPath(file)).toThrow(`Database file is not writable: ${file}-wal`)
