...
import { ProviderUsage } from "../src/koda/provider-usage"
...
const catalog = (input?: CatalogInput) => {
  const providers = () => {
    ...
    const kodaInfo = ProviderV2.Info.make({
      id: ProviderV2.ID.koda,
      name: "koda",
      api: { type: "native", settings: {} },
      request: { headers: {}, body: organization ? { kodaOrganizationId: organization } : {} },
    })
    ...
    return input?.china ? [info, china, kodaInfo] : [info, kodaInfo]
  }
...
const connections = (input: DirectInput, accountID?: string, failure?: () => "global" | "china" | "koda" | undefined) =>
  Layer.mock(Integration.Service)({
    connection: {
      active: (id) =>
        Effect.sync(() => {
          const direct = directValue(input, id)
          return id === koda || ((id === integration || id === chinaIntegration) && direct)
            ? {
                type: "credential" as const,
                id: Credential.ID.make(
                  id === koda ? "cred_koda" : id === chinaIntegration ? "cred_direct_cn" : "cred_direct",
                ),
                label: "test",
              }
            : undefined
      ...
      resolve: (connection) =>
        Effect.suspend(() => {
          ...
          const kind =
            connection.type === "credential" && connection.id === "cred_koda"
              ? "koda"
...
const configuredLayer = (input: {
  ...
  accountID?: string
  config?: CatalogInput
  failure?: () => "global" | "china" | "koda" | undefined
...
describe("ProviderUsage location service", () => {
  ...
  it.live("preserves managed usage across metadata and credential failures while direct usage refreshes", () =>
    Effect.gen(function* () {
      ...
      let credentialFailure: "koda" | undefined
      ...
      expect(partial.items.find((item) => item.sourceKind === "koda_managed")).toMatchObject({ fetchState: "ready" })
      ...
      expect(degraded.items.find((item) => item.sourceKind === "koda_managed")).toMatchObject({ fetchState: "stale" })
      expect(JSON.stringify(degraded)).not.toContain("private usage failure")

      byokFailure = false
      usageFailure = false
      credentialFailure = "koda"
      const credential = yield* usage.get()
      expect(credential.items.find((item) => item.sourceKind === "koda_managed")).toMatchObject({
        fetchState: "stale",
      })
      ...
      expect(organizationResult.items.find((item) => item.sourceKind === "koda_managed")).toBeUndefined()
