...
import * as StoredMessage from "@opencode-ai/core/koda/session-message" // koda_change
  ...
  it.effect("projects durable context messages supported by the updater", () =>
    Effect.gen(function* () {
      ...
      const rows = yield* db
        ...
        .all()
        .pipe(Effect.orDie)
      // koda_change start - assert the projector itself writes the released-reader compaction shape.
      const compaction = rows.find((row) => row.type === "compaction")
      expect(compaction?.data).toMatchObject({
        summary: "summary\n\nRecent context:\nrecent context",
        koda_summary: "summary",
        recent: "recent context",
      })
