...
import { Database as SQLite } from "bun:sqlite" // koda_change
import { describe, expect } from "bun:test"
import { eq } from "drizzle-orm" // koda_change
import { Effect, Layer } from "effect"
import { Credential } from "@opencode-ai/core/credential"
import { CredentialTable } from "@opencode-ai/core/credential/sql" // koda_change
...
function localLayer(directory: string) {
  return LayerNode.compile(Credential.node, [
    ...
    Layer.fresh, // koda_change - rebuild so process-local credentials are re-read
  )
}
...
describe("Credential", () => {
  ...
  // koda_change start - process-provided credentials remain isolated from durable storage
  it.live("keeps valid koda_AUTH_CONTENT credentials and isolated mutations process-local", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = process.env.koda_AUTH_CONTENT
        process.env.koda_AUTH_CONTENT = JSON.stringify({
          koda: {
            type: "oauth",
            refresh: "refresh",
            access: "access",
        ...
        Effect.acquireRelease(
          ...
          Effect.flatMap((tmp) =>
            Effect.gen(function* () {
              ...
              const koda = Integration.ID.make("koda")
              const listed = yield* service.list(koda)
              ...
              expect(yield* service.list(koda)).toEqual([created])
              yield* service.update(created.id, { label: "Updated" })
              expect((yield* service.list(koda))[0]?.label).toBe("Updated")
              yield* service.remove(created.id)
              expect(yield* service.list(koda)).toEqual([])

              delete process.env.koda_AUTH_CONTENT
      ...
      (previous) =>
        Effect.sync(() => {
          if (previous === undefined) delete process.env.koda_AUTH_CONTENT
          else process.env.koda_AUTH_CONTENT = previous
        }),
    ),
  )
    ...
    Effect.acquireUseRelease(
      ...
      (tmp) => {
        ...
        const write = (key: string) =>
          Effect.promise(() => Bun.write(auth, JSON.stringify({ koda: { type: "api", key } })))
        return Effect.gen(function* () {
          ...
          const before = yield* store.db
            ...
            .where(eq(CredentialTable.integration_id, Integration.ID.make("koda")))
            .get()
          yield* Layer.build(Layer.fresh(layer))
