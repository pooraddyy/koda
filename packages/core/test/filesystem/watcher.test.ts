...
const describeWatcher =
  Watcher.hasNativeBinding() && (!process.env.CI || process.env.koda_TEST_PROFILE === "darwin") // koda_change
    ? describe
    : describe.skip
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    koda_EXPERIMENTAL_FILEWATCHER: "true",
    koda_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
    ...
    withTmp(
      (directory) =>
        Effect.gen(function* () {
          ...
          // koda_change start - FSEvents may classify this overwrite as an add.
