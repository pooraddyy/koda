...
// koda_change start - koda dual-writes stored credentials into Global.data/auth.json and re-imports
...
// koda_AUTH_CONTENT is koda's process-local credential mode: it skips the auth.json dual-write.
...
const directory = fs.mkdtempSync(path.join(os.tmpdir(), "koda-integration-test-"))
let previous: string | undefined
beforeAll(() => {
  previous = process.env.koda_AUTH_CONTENT
