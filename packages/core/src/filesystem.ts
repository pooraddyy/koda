...
import { Context, Effect, Layer, Option, Schema } from "effect" // koda_change
import { FSUtil } from "./fs-util"
import { Location } from "./location"
import { PositiveInt, RelativePath } from "./schema"
import { FileSystemSearch } from "./filesystem/search"
import { Entry, FileSystem, FindInput, Match } from "@opencode-ai/schema/filesystem"
import * as SearchTarget from "./koda/search-target" // koda_change
...
export const DEFAULT_SEARCH_LIMIT = 100 // koda_change - preserve bounded koda tool searches
export const MAX_SEARCH_LIMIT = 100 // koda_change
export const SearchLimit = PositiveInt.check(Schema.isLessThanOrEqualTo(MAX_SEARCH_LIMIT)) // koda_change
...
const baseLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const resolve = Effect.fnUntraced(function* (input?: RelativePath) {
      ...
      const target = yield* SearchTarget.inspect(fs, real).pipe(Effect.orDie) // koda_change
      return { absolute, real, directory: location.directory, root, target } // koda_change
    })
    return Service.of({
      find: search.find,
      glob: search.glob,
      ...
      read: Effect.fn("FileSystem.read")(function* (input) {
        ...
        if (target.target.type !== "file") return yield* Effect.die(new Error("Path is not a file")) // koda_change
        // koda_change start - read from the validated descriptor, not a second pathname lookup.
      ...
      list: Effect.fn("FileSystem.list")(function* (input = {}) {
        const target = yield* resolve(input.path)
        if (target.target.type !== "directory") return yield* Effect.die(new Error("Path is not a directory")) // koda_change
        // koda_change start - reject directory replacement during enumeration
