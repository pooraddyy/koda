...
export default {
  up(tx) {
    return Effect.gen(function* () {
      ...
      yield* tx.run(`
        CREATE TABLE \`koda_collaboration_graph\` (
      ...
      `)
      yield* tx.run(`
        CREATE TABLE \`koda_collaboration_node\` (
          ...
          \`started_at\` integer,
          \`finished_at\` integer,
          CONSTRAINT \`koda_collaboration_node_pk\` PRIMARY KEY(\`graph_id\`, \`node_id\`),
      ...
      `)
      yield* tx.run(`
        CREATE TABLE \`koda_collaboration_event\` (
          \`id\` text PRIMARY KEY NOT NULL,
          \`graph_id\` text NOT NULL,
      ...
      `)
      yield* tx.run(
        `CREATE INDEX \`koda_collaboration_graph_root_idx\` ON \`koda_collaboration_graph\` (\`root_session_id\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`koda_collaboration_node_state_idx\` ON \`koda_collaboration_node\` (\`graph_id\`,\`state\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`koda_collaboration_node_session_idx\` ON \`koda_collaboration_node\` (\`session_id\`);`,
      )
      yield* tx.run(
        `CREATE UNIQUE INDEX \`koda_collaboration_event_sequence_idx\` ON \`koda_collaboration_event\` (\`graph_id\`,\`sequence\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`koda_collaboration_event_node_idx\` ON \`koda_collaboration_event\` (\`graph_id\`,\`node_id\`);`,
      )
      yield* tx.run(
