...
import { existsSync, writeFileSync } from "fs" // koda_change
import fs from "fs/promises"
import * as Global from "../global"
import { Schema } from "effect"
import { Glob } from "./glob"
import { createStream } from "rotating-file-stream" // koda_change
import { koda_RUN_ID } from "./opencode-process" // koda_change
...
const keep = 10
const initializedRunID = "koda_LOG_INITIALIZED_RUN_ID"
...
let write = stderr
let stream: ReturnType<typeof createStream> | undefined // koda_change

export async function init(options: Options) {
  if (options.level) level = options.level
  void cleanup(Global.Path.log)
  // koda_change start - initialize one rotating stream and truncate dev.log once per koda run
  if (stream) {
    const active = stream
    stream = undefined
