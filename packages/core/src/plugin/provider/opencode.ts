...
// koda_change start - koda keeps only the free-tier catalog gate from upstream's opencode plugin.
//
...
// drive koda's provider/model catalog. koda routes providers through the koda gateway and does not
// offer a competitor's account system as a sign-in option, so none of that is registered here.
//
// What remains is the behavior koda actually relies on and shipped before the v1.17.13 merge:
