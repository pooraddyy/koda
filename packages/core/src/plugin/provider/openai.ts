...
import type { IntegrationOAuthMethodRegistration } from "@koda/plugin/v2/effect/integration"
import { define } from "@koda/plugin/v2/effect/plugin"
import { Deferred, Effect } from "effect"
import type { Scope } from "effect"
import { Credential } from "../../credential"
import { InstallationVersion } from "../../installation/version"
import { Integration } from "../../integration"
import { kodaOauthCallbackPage } from "../../koda/oauth/page" // koda_change
  ...
  authorize: () =>
    Effect.gen(function* () {
      ...
      const server = createServer((request, response) => {
        ...
        // koda_change start - unrelated localhost requests must not terminate the active OAuth attempt
        if (url.searchParams.get("state") !== state) {
          response.writeHead(400, { "Content-Type": "text/html" }).end(kodaOauthCallbackPage.error("Invalid OAuth state"))
          return
        }
        // koda_change end
        ...
        if (error) {
          ...
          response
            ...
            .end(kodaOauthCallbackPage.error(error, { provider: "ChatGPT" })) // koda_change
          return
        }
        if (!value) {
          ...
          response
            .writeHead(400, { "Content-Type": "text/html" })
            .end(kodaOauthCallbackPage.error(message, { provider: "ChatGPT" })) // koda_change
          return
        }
        Effect.runFork(Deferred.succeed(code, value))
        response
          .writeHead(200, { "Content-Type": "text/html" })
          .end(kodaOauthCallbackPage.success({ provider: "ChatGPT" })) // koda_change
      })
...
function headers(contentType: string) {
  return { "Content-Type": contentType, "User-Agent": `koda/${InstallationVersion}` } // koda_change
}
