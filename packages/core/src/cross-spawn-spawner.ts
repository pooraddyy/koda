...
import { prepareCommand as prepareSandbox } from "@koda/sandbox" // koda_change
import { tap as tapStdio, tapped } from "./koda/stdio-tap" // koda_change - Bun drops buffered stdio on close
import * as SpawnExit from "./koda/spawn-exit" // koda_change
import * as SpawnValidation from "./koda/spawn-validation" // koda_change
import { settle } from "./koda/exit-code" // koda_change - settle signal termination as 128 + signum
...
export const make = Effect.gen(function* () {
  ...
  const setupOutput = (
    ...
    let stdout = proc.stdout
      ? NodeStream.fromReadable({
          evaluate: () => tapped(proc, "stdout"), // koda_change - read the spawn-time tap
          onError: (cause) => toPlatformError("fromReadable(stdout)", toError(cause), command),
        })
      : Stream.empty
    let stderr = proc.stderr
      ? NodeStream.fromReadable({
          evaluate: () => tapped(proc, "stderr"), // koda_change - read the spawn-time tap
  ...
  const spawn = (
    ...
    direct: boolean, // koda_change - avoid shadowing settle
  ) =>
    Effect.callback<readonly [NodeChildProcess.ChildProcess, ExitSignal], PlatformError.PlatformError>((resume) => {
      const signal = Deferred.makeUnsafe<readonly [code: number | null, signal: NodeJS.Signals | null]>()
      const proc = launch(command.command, command.args, opts)
      tapStdio(proc) // koda_change - must run in the same tick as spawn
      let end = false
      ...
      proc.on("exit", (...args) => {
        exit = args
        if (direct) Deferred.doneUnsafe(signal, Exit.succeed(args)) // koda_change - bounded grep must not await inherited pipes
      })
      proc.on("close", (...args) => {
        if (end) return
        end = true
  ...
  // koda_change start - inspect descendants owned by commands that settle on direct exit
  ...
  const spawnCommand: (
    ...
    function* (command) {
      switch (command._tag) {
        case "StandardCommand": {
          const validation = SpawnValidation.take(command) // koda_change - retain target validation through preparation
          const direct = SpawnExit.take(command) // koda_change - opt selected commands into direct-exit settlement
          const dir = yield* cwd(command.options)
          // koda_change start - prepare agent-scoped commands through the selected sandbox backend
          ...
          // koda_change start - close target-swap races at the raw spawn boundary
          if (validation)
          ...
          const [proc, signal] = yield* Effect.acquireRelease(
            // koda_change start - spawn the prepared command and options
            spawn(
              target,
              {
                cwd: dir,
            ...
            Effect.fnUntraced(function* ([proc, signal]) {
              ...
              const kill = timeout(proc, command, target.options) // koda_change
              if (done) {
                const [code] = yield* Deferred.await(signal)
                if (process.platform === "win32") return yield* Effect.void
                // koda_change start - clean up only descendants owned by direct-settling commands
              ...
              // koda_change start - preserve kill options from the prepared command
          ...
          return makeHandle({
            ...
            exitCode: Effect.flatMap(Deferred.await(signal), settle), // koda_change - signal termination settles as 128 + signum
