...
import type { ProviderUsage } from "@opencode-ai/schema/koda/provider-usage"
...
function base() {
  if (!process.env.koda_API_URL) return "https://app.koda.ai"
  try {
    return new URL(process.env.koda_API_URL).origin
  } catch {
    return "https://app.koda.ai"
  }
}
...
export async function managed(
  ...
  const planState = subscription.cancelAtPeriodEnd
    ...
    : subscription.status === "past_due"
      ? "past_due"
      : "active"
  const id = `koda-managed:${subscription.id}`
