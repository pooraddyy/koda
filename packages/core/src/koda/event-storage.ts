// koda_change start - released readers persist tool content in the pre-1.17.13 shapes
//
// koda has shipped durable rows whose `content` uses `{type:"media"}` or `{type:"file",source}`.
