 ...
 * had a junction at e.g. `~/.koda` pointing to a deleted directory).
 *
...
async function writable(p: string) {
  const probe = path.join(p, `.koda-write-${process.pid}-${randomUUID()}`)
...
export async function resolveState(p: string, fallback?: string) {
  ...
  console.warn(`[koda] Cannot use state directory "${p}"; using "${fallback}" instead: ${msg}`)
  return fallback
}
