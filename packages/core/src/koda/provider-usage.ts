...
import { ProviderUsage as Contract } from "@opencode-ai/schema/koda/provider-usage"
...
const managed: Adapter = {
  cachePrefixes: ["koda-managed:"],
  cloudScoped: true,
  async run(ctx) {
    if (!ctx.cloud || !ctx.token || !ctx.cloudIdentity) {
      return { items: ctx.cloudReliable ? [] : ctx.preserve("koda-managed:") }
    }
    const state = await ctx.cloud()
    if (!ctx.identityCurrent(ctx.cloudIdentity)) return { items: [] }
    if (!state.plans.ok || !state.byok.ok) return { items: ctx.preserve("koda-managed:", ctx.cloudIdentity) }
    const token = ctx.token
    const identity = ctx.cloudIdentity
    const detected = Cloud.plans(state)
    const ids = detected.map((subscription) => `koda-managed:${subscription.id}`)
    ctx.prune("koda-managed:", ids)
    return {
      items: await Promise.all(
        detected.map((subscription) =>
          ctx.source(`koda-managed:${subscription.id}`, () => Cloud.managed(token, subscription, ctx.usage), identity),
        ),
      ),
    }
  },
...
export class Service extends Context.Service<Service, Interface>()("@koda/ProviderUsage") {}
...
export class Transport extends Context.Service<Transport, TransportInterface>()("@koda/ProviderUsageTransport") {}
...
const inputs = Effect.fn("ProviderUsage.inputs")(function* (
  ...
  const koda = yield* resolve(integrations, Integration.ID.make("koda"))
  const kodaProvider = byID.get(ProviderV2.ID.koda)
  const configuredOrg = nonempty(process.env.koda_ORG_ID) ?? nonempty(kodaProvider?.request.body.kodaOrganizationId)
  const organization =
    configuredOrg !== undefined ||
    (koda.ok && koda.value?.type === "oauth" && nonempty(koda.value.metadata?.accountID) !== undefined)
  const cloudReliable = organization || koda.ok
  const token =
    koda.ok && koda.value?.type === "oauth" && !organization && koda.value.access ? koda.value.access : undefined
  return {
