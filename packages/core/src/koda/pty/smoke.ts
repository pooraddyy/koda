...
import { kodaPtyTermination } from "./termination"
...
export async function smoke() {
  const proc = spawn(Shell.preferred(), [], {
    name: "xterm-256color",
    cwd: process.cwd(),
    env: { ...process.env, TERM: "xterm-256color", koda_TERMINAL: "1" } as Record<string, string>,
    cols: 80,
    rows: 24,
  })
  ...
  const data = proc.onData((chunk) => {
    ...
    if (lines.some((line) => line.trim() === "koda_PTY_READY")) output.resolve()
  })
  ...
  try {
    proc.resize(100, 40)
    proc.write("echo koda_PTY_READY\r")
    await Promise.race([
      output.promise,
  ...
  let stopped = false
  try {
    await kodaPtyTermination.terminate(active)
    stopped = true
  } finally {
