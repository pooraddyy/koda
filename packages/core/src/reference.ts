...
export interface Interface extends State.Transformable<Draft> {
  readonly replace: (sources: readonly (readonly [string, Source])[]) => Effect.Effect<void> // koda_change
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      ...
      // koda_change start - reconcile koda's effective config without a request-scoped transform slot.
      replace: (sources) =>
        state.mutate((editor) =>
          Effect.sync(() => {
