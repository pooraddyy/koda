          ...
          return yield* flock
            .withLock(
              Effect.gen(function* () {
                ...
                const root = yield* fs.existsSafe(path.join(localPath, ".git")) // koda_change - avoid Windows path spelling differences
                // koda_change start - canonicalize file remotes rewritten by Git on Windows
                const match =
