...
import { Flag } from "./flag/flag" // koda_change
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["config.json", "koda.json", "koda.jsonc", "opencode.json", "opencode.jsonc"] // koda_change
    ...
    const discovered = locationIsGlobal || Flag.koda_DISABLE_PROJECT_CONFIG // koda_change
      ? []
      : yield* fs
          .up({
            targets: [".koda", ".koda", ...names.toReversed()], // koda_change
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => [".koda", ".koda"].includes(path.basename(item))) // koda_change
        .toReversed()
    ...
    // koda_change start
    const directPaths = discovered.filter((item) => ![".koda", ".koda"].includes(path.basename(item))).toReversed()
    // koda_change end
    ...
    // global config, project files, then koda config-directory files. // koda_change
