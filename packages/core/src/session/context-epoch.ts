...
import { Location } from "../location" // koda_change
import { SessionSchema } from "./schema"
import { SessionContextEpochTable, SessionTable } from "./sql"

type DatabaseService = Database.Interface["db"]

class LocationMismatch extends Error {} // koda_change - a concurrent Session move must abort the epoch
...
export function initialize(
  ...
  location: Location.Ref, // koda_change
...
const prepareOnce = Effect.fnUntraced(function* (
  ...
  if (!stored) {
    ...
    const baselineSeq = yield* insert(db, sessionID, generation, location) // koda_change
...
const insert = Effect.fnUntraced(function* (
  ...
  // koda_change start - upstream only checks the Location before loading system context, so a move
