...
export const fromCatalogModel = (
  ...
  // koda_change start - koda Gateway consumes the migrated OAuth account as its organization route.
  ...
  const resolved =
    credential?.type === "oauth" && credential.metadata?.accountID !== undefined
      ? produce(merged, (draft) => {
          draft.request.body.kodaOrganizationId = String(credential.metadata!.accountID)
