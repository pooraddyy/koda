...
import { and, desc, eq, isNotNull, gt, or, sql } from "drizzle-orm" // koda_change
...
import * as StoredMessage from "../koda/session-message" // koda_change
...
function sessionRow(info: SessionV1.SessionInfo): typeof SessionTable.$inferInsert {
  return {
    ...
    share_url: info.share?.url ?? null, // koda_change - full session updates must clear removed shares
...
function run(db: DatabaseService, event: SessionEvent.Event) {
  return Effect.gen(function* () {
    const decodeRow = (row: typeof SessionMessageTable.$inferSelect) =>
      decodeMessage(StoredMessage.normalize({ ...row.data, id: row.id, type: row.type })) // koda_change
    ...
    yield* events.project(SessionEvent.RevertEvent.Committed, (event) =>
      Effect.gen(function* () {
        const boundary = yield* db
          ...
          .get()
          .pipe(Effect.orDie)
        // koda_change - koda keeps session_message.seq nullable for released-data compatibility
