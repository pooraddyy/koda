...
// koda_change start - bind approved reads to filesystem identity
...
export const inspect = Effect.fn("ReadTool.inspect")(function* (fs: FSUtil.Interface, input: string) {
  ...
  // koda_change start - retain the approved identity for descriptor verification
...
// koda_change - reject targets replaced after permission approval
...
export const read = Effect.fn("ReadTool.read")(function* (
  ...
  return yield* Effect.scoped(
    Effect.gen(function* () {
      // koda_change start - open the approved path and verify the descriptor identity
...
// koda_change - verify approved directory identity before listing
