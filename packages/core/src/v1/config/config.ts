...
import { IndexingConfig as kodaIndexingConfig, IndexingSchema as kodaIndexingSchema } from "@koda/koda-indexing/config"
import z from "zod"
// koda_change end
...
// koda_change start - indexing configuration
export const Indexing = kodaIndexingConfig
...
const Percent = Schema.Number.check(Schema.isGreaterThan(0), Schema.isLessThanOrEqualTo(100)) // koda_change

const IndexingRef = kodaIndexingSchema.annotate({ [ZodOverride]: kodaIndexingConfig }) // koda_change
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServerV1.Server).annotate({
    description: "Server configuration for the koda serve command", // koda_change
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommandV1.Info)).annotate({
    description: "Command configuration, see https://koda.ai/docs/customize/workflows", // koda_change
  }),
  ...
  // koda_change start
  // NOTE: Any new koda_change key added to Config.Info must also be mirrored in
  // apps/web/src/app/config.json/extras.ts in the cloud repo, otherwise
  // $schema: https://app.koda.ai/config.json will not recognize it.
  remote_control: Schema.optional(Schema.Boolean).annotate({
    description: "Enable remote control of sessions via koda Cloud. Equivalent to running /remote on startup.",
  }),
    ...
    Schema.Struct({
      context_sidebar_width: Schema.optional(
        Schema.Int.check(Schema.isBetween({ minimum: 250, maximum: 800 })).annotate({
          description: "Width of the koda Console project context sidebar in pixels",
        }),
      ),
      diff_style: Schema.optional(Schema.Literals(["unified", "split"])).annotate({
        description: "Default diff layout in koda Console project reviews",
      }),
    }),
  ).annotate({ description: "koda Console user interface configuration" }),
  ...
  hide_prompt_training_models: Schema.optional(Schema.Boolean).annotate({
    description: "Hide koda Gateway models that may train on your prompts from model listings",
  }),
  ...
  collaboration: Schema.optional(
    Schema.Struct({
      ...
      max_nodes: Schema.optional(NonNegativeInt),
      max_concurrency: Schema.optional(PositiveInt),
      retries: Schema.optional(Schema.Number.check(Schema.isInt(), Schema.isGreaterThanOrEqualTo(0))),
    }),
  ).annotate({
    description: "Opt-in proactive collaboration graph planning for complex requests",
  }),
  hooks: Schema.optional(
      ...
      definitions: Schema.optional(
        Schema.Array(
          Schema.Struct({
            ...
            on_error: Schema.optional(Schema.Literals(["ignore", "warn", "block"])),
            timeout_ms: Schema.optional(PositiveInt),
            max_output_bytes: Schema.optional(PositiveInt),
            enabled: Schema.optional(Schema.Boolean),
    ...
    description: "Opt-in Koda lifecycle hooks; config-defined hooks observe and warn without blocking execution",
  }),
  ...
  agent: Schema.optional(
    ...
    // koda_change start
  ).annotate({ description: "Agent configuration, see https://koda.ai/docs/customize/custom-subagents" }), // koda_change
  ...
  commit_message: CommitMessageSchema, // koda_change
  ...
  experimental: Schema.optional(
    Schema.Struct({
      ...
      sandbox: Schema.optional(Schema.Boolean).annotate({
        description:
          "Run agent tools inside a sandbox that restricts writes to project and koda state directories and can restrict outbound network access",
      }),
