...
import { PowerShell } from "./koda/powershell" // koda_change
...
function win() {
  return Array.from(
    new Set(
      // koda_change - probe known PowerShell 7 install locations so legacy 5.1 is not picked when pwsh is off PATH
      [PowerShell.pwsh(), which("powershell"), gitbash(), process.env.COMSPEC || "cmd.exe"] // koda_change
...
export function gitbash() {
  ...
  if (Flag.koda_GIT_BASH_PATH) return Flag.koda_GIT_BASH_PATH
  const git = which("git")
  if (!git) return
...
export function args(file: string, command: string, cwd: string) {
  ...
  if (ps(file)) return PowerShell.args(command) // koda_change - preserve UTF-8 and script prologues
  return ["-c", command]
}
