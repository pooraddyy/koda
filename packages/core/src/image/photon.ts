...
import { DecodeError, PixelLimitError, ResizerUnavailableError, SizeError } from "../image" // koda_change
import { allowed, dimensions, MAX_DIMENSION, MAX_PIXELS } from "../koda/image-size" // koda_change
...
export const make = Effect.gen(function* () {
  ...
  return Effect.fn("Image.Photon.normalize")(function* (
    ...
    // koda_change start - reject decompression bombs before Photon allocates native pixels
    ...
    const decoded = yield* Effect.try({
      try: () => photon.PhotonImage.new_from_byteslice(input), // koda_change
      catch: () => new DecodeError({ resource }),
    })
    try {
