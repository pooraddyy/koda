...
import * as kodaGrep from "./koda/ripgrep-grep" // koda_change
import * as SpawnExit from "./koda/spawn-exit" // koda_change
import * as SpawnValidation from "./koda/spawn-validation" // koda_change
...
const RawMatch = Schema.Struct({
  type: Schema.Literals(["match", "context"]), // koda_change - retain requested context records
  data: Schema.Struct({
...
type RawMatchData = (typeof RawMatch.Type)["data"] & { readonly context: boolean } // koda_change
...
export interface GlobInput {
  ...
  readonly follow?: boolean
  readonly signal?: AbortSignal
  readonly validate?: Effect.Effect<void, unknown> // koda_change - bind approved searches at spawn
}

export interface GrepInput extends kodaGrep.Options {
  // koda_change
  readonly cwd: string
  readonly pattern: string
...
export interface Interface {
  ...
  readonly glob: (input: GlobInput) => Effect.Effect<SearchResult<Entry>, Error> // koda_change
  readonly grep: (input: GrepInput) => Effect.Effect<SearchResult<kodaGrep.GrepMatch>, Error | InvalidPatternError> // koda_change
}

// koda_change start - retain truncation state through model-facing tools
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = <A>(input: {
      ...
      readonly stop?: (item: A) => boolean // koda_change - stop bounded searches at the overflow match
      readonly validate?: Effect.Effect<void, unknown> // koda_change - spawn-bound target validation
    }) => {
      const program = Effect.scoped(
        Effect.gen(function* () {
          const filepath = yield* binary.filepath
          // koda_change start - validate approved targets after all spawn preparation
          const command = ChildProcess.make(filepath, input.args, {
            cwd: input.cwd,
            extendEnv: true,
            stdin: "ignore",
            forceKillAfter: input.stop ? Duration.seconds(1) : undefined, // koda_change - bound grep interruption
          })
          const validated = input.validate ? SpawnValidation.attach(command, input.validate) : command
          const spawned = input.stop ? SpawnExit.attach(validated) : validated // koda_change
          ...
          let observed = 0
          let stopped = false // koda_change
          const take = input.stop // koda_change start
            ? Stream.takeUntil<A>((row) => {
                stopped = input.stop?.(row) ?? false
                return stopped
              })
            : Stream.take(input.limit + 1) // koda_change end
          ...
          if (stopped) return { items: rows, truncated: true, partial: false } // koda_change
          const truncated = input.stop ? false : rows.length > input.limit // koda_change - custom stop owns truncation
      ...
      return abortable.pipe(
        // koda_change start - surface the underlying reason instead of a bare wrapper message
    ...
    return Service.of({
      glob: (input) =>
        run<string>({
          ...
          limit: input.limit,
          signal: input.signal,
          validate: input.validate, // koda_change - preserve spawn-bound target validation
          args: [
            "--no-config",
            "--files",
            ...(input.hidden ? ["--hidden"] : []),
          ...
          // koda_change start - retain spawn metadata after mapping paths
          Effect.map((result) => ({
            ...result,
      ...
      grep: (input) =>
        run<RawMatchData>({
          ...input,
          stop: kodaGrep.stop(input.limit), // koda_change
          args: [
            "--no-config",
            "--json",
            "--hidden",
            "--no-messages",
            ...kodaGrep.flags(input), // koda_change
            ...
            (Buffer.byteLength(line, "utf8") > MAX_RECORD_BYTES
              ...
              Effect.flatMap((json) => {
                if (
                  ...
                  typeof json !== "object" ||
                  !("type" in json) ||
                  (json.type !== "match" && json.type !== "context") // koda_change
                )
                ...
                return Schema.decodeUnknownEffect(RawMatch)(json).pipe(
                  Effect.map((match) => ({
                    ...
                    context: match.type === "context", // koda_change
                  })),
          ...
          // koda_change start - retain spawn metadata after mapping matches
          Effect.map((result) => ({
            ...result,
            items: kodaGrep.select(input, result.items).map((match) => {
              ...
              return kodaGrep.decorate(item, match.context, match.lines.text.length > 2_000)
            }),
          })),
          // koda_change end
        ),
