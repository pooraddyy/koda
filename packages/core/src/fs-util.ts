...
import { decorateFileSystem, ensureDirectory } from "@koda/sandbox" // koda_change
...
export namespace FSUtil {
  ...
  const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const fs = decorateFileSystem(yield* FileSystem.FileSystem) // koda_change
      ...
      const ensureDir = Effect.fn("FileSystem.ensureDir")(function* (path: string) {
        yield* ensureDirectory(fs, path) // koda_change - mutate through the sandbox-confined filesystem
      })
      ...
      const writeWithDirs = Effect.fn("FileSystem.writeWithDirs")(function* (
        ...
        yield* write.pipe(
          Effect.catchIf(
            ...
            () =>
              Effect.gen(function* () {
                yield* ensureDirectory(fs, dirname(path)) // koda_change - sandbox-confined mkdir
                yield* write
              }),
          ),
        )
  ...
  export const defaultLayer = layer.pipe(Layer.provide(NodeFileSystem.layer)) // koda_change - legacy koda runtime compatibility
