...
import { fetchkodaPassState, parsekodaPassState } from "../../src/api/koda-pass"

describe("parsekodaPassState", () => {
  test("parses batched tRPC subscription data", () => {
    const state = parsekodaPassState([
      {
        result: {
          data: {
            json: {
  ...
  test("returns null without period amounts", () => {
    expect(parsekodaPassState({ status: "none" })).toBeNull()
  })
  ...
  test("hides canceled and expired subscriptions that still report period credits", () => {
    ...
    expect(parsekodaPassState(payload("canceled"))).toBeNull()
    expect(parsekodaPassState(payload("expired"))).toBeNull()
    expect(parsekodaPassState(payload("past_due"))).toMatchObject({ currentPeriodBaseCreditsUsd: 19 })
  })
  ...
  test("silently ignores transport failures", async () => {
    ...
    try {
      await expect(fetchkodaPassState("token")).resolves.toBeNull()
