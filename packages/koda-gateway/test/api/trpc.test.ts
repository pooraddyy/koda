...
describe("Cloud tRPC client", () => {
  test("uses unbatched GET queries without an organization header", async () => {
    const fn = mock(() =>
      Promise.resolve(
        result([
          {
            ...subscription,
            routeLabel: "MiniMax via koda Gateway",
    ...
    expect(new Headers(call[1].headers).has("x-koda-organizationid")).toBe(false)
  ...
  test("decodes procedure errors even when HTTP is successful", async () => {
    ...
    expect(error).toMatchObject({ kind: "procedure", message: "koda Cloud data is temporarily unavailable." })
