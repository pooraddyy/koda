...
import { EMPTY_koda_EMBEDDING_MODEL_CATALOG, fetchkodaEmbeddingModelCatalog } from "../../src/api/embedding-models"
...
describe("fetchkodaEmbeddingModelCatalog", () => {
  test("fetches catalog from koda Gateway", async () => {
    const prev = global.fetch
    const fn = mock(() => Promise.resolve(response())) as unknown as typeof fetch
    global.fetch = fn

    try {
      const catalog = await fetchkodaEmbeddingModelCatalog({ baseURL: "https://example.test" })
  ...
  test("bounds caller-controlled retry attempts", async () => {
    ...
    try {
      await fetchkodaEmbeddingModelCatalog({ baseURL: "https://example.test", attempts: Number.POSITIVE_INFINITY })
  ...
  test("reports a final failure without writing to the console", async () => {
    ...
    try {
      await expect(
        fetchkodaEmbeddingModelCatalog({ baseURL: "https://example.test", attempts: 1, onError: issue }),
      ).resolves.toEqual(EMPTY_koda_EMBEDDING_MODEL_CATALOG)
      expect(issue).toHaveBeenCalledWith({
        code: "http",
        message: "Unable to load koda embedding models (HTTP 500).",
        status: 500,
      })
  ...
  test("fallback catalog is empty so Cloud owns model metadata", () => {
    expect(EMPTY_koda_EMBEDDING_MODEL_CATALOG).toEqual({
      defaultModel: "",
      models: [],
      aliases: {},
    })
