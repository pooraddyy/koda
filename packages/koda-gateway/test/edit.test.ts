...
describe("Edit target resolution", () => {
  ...
  test("does NOT route the FIM Mercury model to the edit endpoint", () => {
    // `mercury-edit-2` (kind: fim) must fall through to the koda placeholder,
    // not the edit endpoint — only `mercury-next-edit` (kind: edit) is NES.
    expect(resolveEditTarget("inception", "mercury-edit-2").provider).toBe("koda")
  })

  test("routes the koda Gateway next-edit model to the gateway proxy", () => {
    const target = resolveEditTarget("koda", "inception/mercury-next-edit")
    expect(target.provider).toBe("koda")
    expect(target.model).toBe("inception/mercury-edit-2")
    expect(target.url).toMatch(/\/api\/edit\/completions$/)
  })

  test("falls back to a koda placeholder (no upstream) for non-edit models", () => {
    expect(resolveEditTarget("koda", "mistralai/codestral-2508")).toEqual({
      provider: "koda",
  ...
  test("routes the default model to the gateway proxy", () => {
    expect(resolveEditTarget()).toEqual({
      ...
      url: "https://api.koda.ai/api/edit/completions",
    })
  })
})
