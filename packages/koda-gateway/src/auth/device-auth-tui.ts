...
import { getkodaProfile, getkodaDefaultModel, defaultOrganizationId } from "../api/profile.js"
import { koda_API_BASE, POLL_INTERVAL_MS } from "../api/constants.js"
import type { AuthOuathResult } from "@koda/plugin"
...
async function initiateDeviceAuth(): Promise<DeviceAuthInitiateResponse> {
  const response = await fetch(`${koda_API_BASE}/api/device-auth/codes`, {
    method: "POST",
    headers: {
...
async function pollDeviceAuth(code: string): Promise<DeviceAuthPollResponse> {
  const response = await fetch(`${koda_API_BASE}/api/device-auth/codes/${code}`)
...
export async function authenticateWithDeviceAuthTUI(inputs?: Record<string, string>): Promise<AuthOuathResult> {
  ...
  return {
    ...
    async callback() {
      ...
      const profile = await getkodaProfile(token).catch(() => undefined)
      const organizationId = profile ? defaultOrganizationId(profile) : undefined

      // Fetch default model
      await getkodaDefaultModel(token, organizationId)
