...
import { getkodaProfile, getkodaDefaultModel, promptOrganizationSelection } from "../api/profile.js"
...
export async function authenticateWithDeviceAuth(): Promise<DeviceAuthResult> {
  ...
  const profileData = await getkodaProfile(token)
  ...
  const model = await getkodaDefaultModel(token, organizationId)
