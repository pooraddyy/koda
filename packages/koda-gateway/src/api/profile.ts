...
import type { kodaProfile, Organization, kodaBalance } from "../types.js"
import { koda_API_BASE, DEFAULT_MODEL, DEFAULT_FREE_MODEL } from "./constants.js"

/**
 * Fetch user profile from koda API
 */
export async function fetchProfile(token: string): Promise<kodaProfile> {
  const response = await fetch(`${koda_API_BASE}/api/profile`, {
    headers: {
  ...
  // Transform to flat kodaProfile structure
  return {
 ...
 */
export const getkodaProfile = fetchProfile
 ...
 */
export function defaultOrganizationId(profile: kodaProfile): string | undefined {
...
/**
 * Fetch user balance from koda API
 * @param token - Authentication token
 * @param organizationId - Optional organization ID for team balance
 */
export async function fetchBalance(token: string, organizationId?: string): Promise<kodaBalance | null> {
  try {
    ...
    if (organizationId) {
      headers["x-koda-organizationid"] = organizationId
    }

    const response = await fetch(`${koda_API_BASE}/api/profile/balance`, { headers })
 ...
 * Alias for compatibility with existing code
 */
export const getkodaBalance = fetchBalance
 ...
 * Alias for compatibility with existing code
 */
export const getkodaDefaultModel = fetchDefaultModel
