...
import { koda_API_BASE, koda_OPENROUTER_BASE, MODELS_FETCH_TIMEOUT_MS, PROMPTS, AI_SDK_PROVIDERS } from "./constants.js"

export type kodaModelsResult = {
...
/**
 * Fetch models from koda API (OpenRouter-compatible endpoint)
 *
 * @param options - Configuration options
 * @returns Typed result with models and optional error info
 */
export async function fetchkodaModels(options?: {
  kodaToken?: string
  kodaOrganizationId?: string
  baseURL?: string
}): Promise<kodaModelsResult> {
  const raw = await fetchRawkodaModels(options)
  ...
  for (const model of raw.data) {
    // Skip models that explicitly don't support tools — koda requires tool calling
...
export type kodaImageModel = {
  id: string
  name: string
  description?: string
}

export type kodaImageModelsResult = {
  models: kodaImageModel[]
  error?: { kind: "unauthorized" | "network" | "schema" | "http"; status?: number }
}

export type kodaTranscriptionModel = {
  id: string
  name: string
}

export type kodaTranscriptionModelsResult = {
  models: kodaTranscriptionModel[]
  error?: { kind: "unauthorized" | "network" | "schema" | "http"; status?: number }
}

/**
 * Fetch image-capable models from koda API (OpenRouter-compatible endpoint).
 * Uses the same raw fetch as {@link fetchkodaModels} but keeps only models
 * whose `output_modalities` include `"image"`.
 */
export async function fetchkodaImageModels(options?: {
  kodaToken?: string
  kodaOrganizationId?: string
  baseURL?: string
}): Promise<kodaImageModelsResult> {
  const raw = await fetchRawkodaModels(options)
  if (raw.error) return { models: [], error: raw.error }

  const models: kodaImageModel[] = []
...
export async function fetchkodaTranscriptionModels(options?: {
  kodaToken?: string
  kodaOrganizationId?: string
  baseURL?: string
}): Promise<kodaTranscriptionModelsResult> {
  const token = options?.kodaToken
  const organizationId = options?.kodaOrganizationId
  const url = new URL("transcription-models", resolvekodaGatewayBaseUrl({ baseURL: options?.baseURL, token }))
  const response = await fetch(url, {
    headers: {
      ...getDefaultHeaders(),
      ...buildkodaHeaders(undefined, { kodaOrganizationId: organizationId }),
...
/**
 * Shared raw fetch + validate used by both {@link fetchkodaModels} and {@link fetchkodaImageModels}.
 */
async function fetchRawkodaModels(options?: {
  kodaToken?: string
  kodaOrganizationId?: string
  baseURL?: string
}): Promise<
  { data: OpenRouterModel[]; error?: undefined } | { data?: undefined; error: NonNullable<kodaModelsResult["error"]> }
> {
  const token = options?.kodaToken
  const organizationId = options?.kodaOrganizationId

  // Construct base URL
  const defaultBaseURL = organizationId ? `${koda_API_BASE}/api/organizations/${organizationId}` : koda_OPENROUTER_BASE

  const baseURL = options?.baseURL ?? defaultBaseURL

  // Transform URL with token if available
  const finalBaseURL = token ? getkodaUrlFromToken(baseURL, token) : baseURL
