...
const koda_FIM_URL = koda_API_BASE + "/api/fim/completions"
const INCEPTION_FIM_URL = "https://api.inceptionlabs.ai/v1/fim/completions"

function kodaTarget(model?: string): FimTarget {
  return { provider: "koda", model: model ?? "mistralai/codestral-2501", url: koda_FIM_URL }
}

export function resolveFimTarget(provider?: string, model?: string): FimTarget {
  if (!provider || provider === "koda") return kodaTarget(model)
