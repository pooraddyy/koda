    ...
    return fetch(url, {
      ...
      headers: {
        ...
        ...(target.provider === "koda"
          ? buildkodaHeaders(undefined, { kodaOrganizationId: input.organizationId })
          : {}),
...
export function createFimHandler(Auth: Auth) {
  return async (c: any) => {
    ...
    const proxy = target.provider === "koda" ? await getProxyAuth(Auth) : undefined
    const token = target.provider === "koda" ? proxy?.token : await getProviderKey(Auth, target.provider)

    if (target.provider === "koda" && !proxy?.auth) {
      return c.json({ error: "Not authenticated with koda Gateway" }, 401)
    }

    if (target.provider === "koda" && !token) {
