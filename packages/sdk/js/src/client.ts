...
function rewrite(request: Request, directory?: string) {
  ...
  const value = pick(request.headers.get("x-koda-directory"), directory)
  ...
  const next = new Request(url.href, request) // koda_change
  next.headers.delete("x-koda-directory")
  return next
}

export function createkodaClient(config?: Config & { directory?: string }) {
  if (!config?.fetch) {
