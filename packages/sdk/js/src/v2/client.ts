...
import { kodaClient } from "./gen/sdk.gen.js"
import { wrapClientError } from "../error-interceptor.js"
export { type Config as kodaClientConfig, kodaClient }
...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-koda-directory", "directory"],
    ["x-koda-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-koda-directory")
  next.headers.delete("x-koda-workspace")
  return next
}

export function createkodaClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-koda-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-koda-workspace": config.experimental_workspaceID,
    }
  }
