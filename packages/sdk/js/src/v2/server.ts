...
// koda_change start - Merge existing koda_CONFIG_CONTENT with new config
// This preserves koda-injected modes when spawning nested CLI instances
...
function parseExistingConfig(): Config | undefined {
  const content = process.env.koda_CONFIG_CONTENT
...
export async function createkodaServer(options?: ServerOptions) {
  ...
  // koda_change start
  const proc = launch(`koda`, args, {
    // koda_change end
    env: {
      ...process.env,
      koda_CONFIG_CONTENT: buildConfigEnv(options.config), // koda_change
    },
  })
  let clear = () => {}
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    proc.stdout?.on("data", (chunk) => {
      ...
      for (const line of lines) {
        // koda_change start
        if (line.startsWith("koda server listening")) {
          // koda_change end
...
export function createkodaTui(options?: TuiOptions) {
  const args = []
