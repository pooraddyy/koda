...
import { SessionDurable } from "@opencode-ai/schema/durable-event-manifest" // koda_change - released history keys
...
>(
  ...
  locationMiddleware: Context.Key<LocationId, LocationService>, // koda_change - supply the configured location for session creation
) =>
  HttpApiGroup.make("server.session")
    ...
    .add(
      HttpApiEndpoint.post("session.create", "/api/session", {
        ...
        .middleware(locationMiddleware), // koda_change - use the SDK's configured directory when payload location is omitted
    )
    .add(
    ...
    .add(
      HttpApiEndpoint.get("session.history", "/api/session/:sessionID/history", {
        ...
        success: Schema.Struct({
          data: Schema.Array(SessionDurable.schema), // koda_change
    ...
    .add(
      HttpApiEndpoint.get("session.events", "/api/session/:sessionID/event", {
        ...
        success: HttpApiSchema.StreamSse({ data: SessionDurable.schema }), // koda_change
        error: SessionNotFoundError,
      })
        .middleware(sessionLocationMiddleware)
        .annotateMerge(
