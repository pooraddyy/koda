...
import { Location } from "@opencode-ai/core/location" // koda_change
  ...
  Effect.gen(function* () {
    ...
    return handlers
      ...
      .handle(
        "session.create",
        Effect.fn(function* (ctx) {
          const location = yield* Location.Service // koda_change
          return {
            data: yield* session.create({
              id: ctx.payload.id,
              agent: ctx.payload.agent,
              model: ctx.payload.model,
              // koda_change start - honor createkodaClient's configured Location
              location:
