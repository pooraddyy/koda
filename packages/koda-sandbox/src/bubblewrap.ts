...
declare const koda_BWRAP_SHA256: string | undefined
declare const koda_SANDBOX_NETWORK_RELAY_PATH: string | undefined
declare const koda_SANDBOX_SECCOMP_PATH: string | undefined
...
function relay() {
  if (typeof koda_SANDBOX_NETWORK_RELAY_PATH === "undefined") {
    return { path: fileURLToPath(new URL("./koda-sandbox-network-relay.ts", import.meta.url)), environment: {} }
  }
  const target = koda_SANDBOX_NETWORK_RELAY_PATH.startsWith(".")
    ? fileURLToPath(new URL(koda_SANDBOX_NETWORK_RELAY_PATH, import.meta.url))
    : path.resolve(path.dirname(process.execPath), koda_SANDBOX_NETWORK_RELAY_PATH)
  return { path: target, environment: { BUN_BE_BUN: "1" } }
}

function seccomp() {
  if (typeof koda_SANDBOX_SECCOMP_PATH !== "undefined") {
    return path.resolve(path.dirname(process.execPath), koda_SANDBOX_SECCOMP_PATH)
  }
...
function digest() {
  return typeof koda_BWRAP_SHA256 === "undefined" ? undefined : koda_BWRAP_SHA256
}
...
function select(): Selection {
  const override = process.env.koda_BWRAP_PATH
