...
declare const koda_SANDBOX_MUTATION_WORKER_PATH: string

function worker() {
  if (typeof koda_SANDBOX_MUTATION_WORKER_PATH === "undefined") {
    return { path: fileURLToPath(new URL("./koda-sandbox-mutation-worker.ts", import.meta.url)), environment: {} }
  }
  const path = koda_SANDBOX_MUTATION_WORKER_PATH.startsWith(".")
    ? fileURLToPath(new URL(koda_SANDBOX_MUTATION_WORKER_PATH, import.meta.url))
    : resolve(dirname(process.execPath), koda_SANDBOX_MUTATION_WORKER_PATH)
...
const CurrentRunner = Context.Reference<Runner>("@koda/sandbox/CurrentMutationRunner", {
  defaultValue: () => mutate,
})
