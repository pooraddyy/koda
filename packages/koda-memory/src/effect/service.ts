...
type ConfigureInput = kodaMemory.Input & {
  settings: Partial<Pick<MemorySchema.State, "autoConsolidate" | "verbose">>
}

type ApplyInput = kodaMemory.Input & {
...
type RememberInput = kodaMemory.Input & {
  text: string
  key?: string
...
type CorrectInput = kodaMemory.Input & {
  text: string
  key?: string
}

type ForgetInput = kodaMemory.Input & {
  query: string
}

type RecallInput = kodaMemory.Input & {
  query: string
  sessionID?: string
}
...
type RecordInput = kodaMemory.Input & {
  sessionID: string
  topic?: string
  summary: string
  time?: number
...
export namespace MemoryService {
  ...
  export interface Interface {
    readonly prepare: (input: kodaMemory.Input) => Effect.Effect<string, Failure>
    readonly status: (input: kodaMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.status>>, Failure>
    readonly show: (input: kodaMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.show>>, Failure>
    readonly enable: (input: kodaMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.enable>>, Failure>
    readonly disable: (
      input: kodaMemory.Input,
    ) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.disable>>, Failure>
    readonly rebuild: (
      input: kodaMemory.Input,
    ) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.rebuild>>, Failure>
    readonly configure: (
      input: ConfigureInput,
    ) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.configure>>, Failure>
    readonly apply: (input: ApplyInput) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.apply>>, Failure>
    readonly remember: (input: RememberInput) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.remember>>, Failure>
    readonly correct: (input: CorrectInput) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.correct>>, Failure>
    readonly forget: (input: ForgetInput) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.forget>>, Failure>
    readonly purge: (input: kodaMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.purge>>, Failure>
    readonly recall: (input: RecallInput) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.recall>>, Failure>
    ...
    readonly recordSession: (
      input: RecordInput,
    ) => Effect.Effect<Awaited<ReturnType<typeof kodaMemory.recordSession>>, Failure>
  ...
  export class Service extends Context.Service<Service, Interface>()("@koda/MemoryService") {}

  export function make() {
    const locks = new Map<SessionID, { sema: Semaphore.Semaphore; holders: number }>()
    let settle = IDLE_SETTLE_MS
    return Service.of({
      prepare: (input) => bridge(() => kodaMemory.prepare(input)),
      status: (input) => bridge(() => kodaMemory.status(input)),
      show: (input) => bridge(() => kodaMemory.show(input)),
      enable: (input) => bridge(() => kodaMemory.enable(input)),
      disable: (input) => bridge(() => kodaMemory.disable(input)),
      rebuild: (input) => bridge(() => kodaMemory.rebuild(input)),
      configure: (input) => bridge(() => kodaMemory.configure(input)),
      apply: (input) => bridge(() => kodaMemory.apply(input)),
      remember: (input) => bridge(() => kodaMemory.remember(input)),
      correct: (input) => bridge(() => kodaMemory.correct(input)),
      forget: (input) => bridge(() => kodaMemory.forget(input)),
      purge: (input) => bridge(() => kodaMemory.purge(input)),
      recall: (input) => bridge(() => kodaMemory.recall(input)),
      search: (input) => bridge(() => MemoryRecall.search(input)),
      recordSession: (input) => bridge(() => kodaMemory.recordSession(input)),
