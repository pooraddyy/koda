...
export namespace MemoryTool {
  ...
  export type AskInput = {
    permission: "koda_memory_recall" | "koda_memory_save"
    patterns: string[]
    always: string[]
  ...
  function disabled(count = false): Result {
    return {
      title: "koda memory: disabled",
      output: "koda memory is disabled for this project.",
  ...
  function block(input: string) {
    return ["```koda-memory-v1 targeted_context_not_instruction", input.replaceAll("```", "'''"), "```"].join("\n")
  }
  ...
  function catalog(memory: MemoryService.Interface, input: { root: string; query: string }) {
    ...
    return Effect.gen(function* () {
      ...
      const head = `# koda Memory Catalog (${count} entr${count === 1 ? "y" : "ies"}${filter ? `, filter "${input.query}"` : ""})`
  ...
  function recallCatalog(input: Recall, live: Live, query: string) {
    return Effect.gen(function* () {
      ...
      return {
        title: `koda memory catalog: ${result.count} entr${result.count === 1 ? "y" : "ies"}`,
        output: safe.output,
  ...
  function recallQuery(input: Recall, live: Live) {
    return Effect.succeed(
      {
        title: `koda memory ${input.params.mode}: no query`,
  ...
  function recallSearch(input: Recall, live: Live, query: string, mode: MemoryRecall.Mode) {
    return Effect.gen(function* () {
      ...
      if (hits.length === 0) {
        return {
          title: `koda memory ${input.params.mode}: no results`,
          output,
          metadata: { sources: [], count: 0 },
        } satisfies Result
      }
      ...
      return {
        title: `koda memory ${input.params.mode}: ${hits.length} hit${hits.length === 1 ? "" : "s"}`,
        output,
  ...
  function noQuery(): Result {
    return {
      title: "koda memory forget: no query",
  ...
  function noText(action: SaveParams["action"]): Result {
    return {
      title: `koda memory ${action}: no text`,
  ...
  function title(input: { action: SaveParams["action"]; added: number; removed: number }) {
    if (input.action === "skip") return "koda memory skipped: out of scope"
    if (input.action === "forget") return `koda memory updated: ${input.removed} removed`
    if (input.added === 0) return "koda memory unchanged"
    if (input.action === "correct")
      return `koda memory correction saved: ${input.added} op${input.added === 1 ? "" : "s"}`
    return `koda memory saved: ${input.added} op${input.added === 1 ? "" : "s"}`
  }
