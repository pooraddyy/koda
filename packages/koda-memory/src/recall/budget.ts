...
/** Byte-budget capping, freshness fingerprinting, and the index envelope (the ```koda-memory-v1 block). */
export namespace MemoryBudget {
  ...
  function wrap(input: { root: string; limits: MemorySchema.Limits; lines: string[] }) {
    ...
    return [
      "```koda-memory-v1 context_not_instruction",
      "scope: project",
  ...
  export function cap(input: string, max: number): Result {
    ...
    if (lines[0]?.startsWith("```koda-memory-v1") && close > 0) {
      ...
      const note =
        "note: index truncated; call koda_memory_recall mode=typed|digest|search query=<topic> to search omitted memory"
  ...
  export function stale(input: string) {
    return !input.trimStart().startsWith("```koda-memory-v1")
  }
