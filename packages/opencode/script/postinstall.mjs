...
// koda_change start - variant detection matching bin/koda logic
const platformMap = {
  darwin: "darwin",
  linux: "linux",
  win32: "windows",
...
const base = `@koda-code/cli-${platform}-${arch}`
const sourceBinary = platform === "windows" ? "koda.exe" : "koda"
const targetBinary = path.join(__dirname, "bin", ".koda")
...
// koda_change start - copy runtime resources next to cached binary
function copyResources(source) {
  ...
  const worker = path.join(path.dirname(source), "koda-sandbox-mutation-worker.js")
  if (fs.existsSync(worker)) fs.copyFileSync(worker, path.join(__dirname, "bin", "koda-sandbox-mutation-worker.js"))
}
...
function main() {
  ...
  for (const name of packageNames()) {
    try {
      ...
      const temp = fs.mkdtempSync(path.join(os.tmpdir(), "koda-install-"))
      try {
  ...
  throw new Error(
    `It seems your package manager failed to install the right koda CLI package. Try manually installing ${packageNames()
...
try {
  ...
  console.error("Failed to setup koda binary:", error.message)
  process.exit(1)
}
