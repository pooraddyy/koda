...
// Launch the koda CLI dev build against a locally running cloud dev server.
//   bun dev:local <project-dir> [--cloud <dir>] [--no-ingest] [--no-events] [--print] [-- <koda args>]
//
// Reads ports from <cloud>/dev/logs/manifest.json (+ .dev-port), probes the web
// server, and points the CLI at it (koda_API_URL / koda_SESSION_INGEST_URL /
// EVENT_SERVICE_URL).
// Auth/config/state/cache are isolated under ~/.koda-dev so it can't clash with
// your main koda install; real HOME is kept so git/ssh still work.
...
const koda = path.resolve(import.meta.dir, "../../..")
const home = path.join(os.homedir(), ".koda-dev")
...
async function main() {
  ...
  env.koda_API_URL = `http://localhost:${webPort}`
  env.koda_DEV_CWD = project
  env.koda_DISABLE_AUTOUPDATE = "1"
  if (ingestPort) env.koda_SESSION_INGEST_URL = `http://localhost:${ingestPort}`
  else env.koda_DISABLE_SESSION_INGEST = "1"
  if (eventsPort) {
    env.EVENT_SERVICE_URL = `ws://localhost:${eventsPort}`
    delete env.koda_DISABLE_PRESENCE
    delete env.koda_EVENT_SERVICE_URL
  } else env.koda_DISABLE_PRESENCE = "1"
  ...
  process.exit(await Bun.spawn({ cmd: ["bun", "run", "--cwd", "packages/opencode", "--conditions=browser", "src/index.ts", ...pass], cwd: koda, env, stdio: ["inherit", "inherit", "inherit"] }).exited)
}
