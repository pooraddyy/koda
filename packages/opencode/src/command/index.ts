...
import { legacyReviewCommand, reviewCommand } from "@/koda/review/command" // koda_change
import { apply as applyOverride, type Override } from "@/koda/command/override" // koda_change
import PROMPT_INITIALIZE from "./template/initialize.txt"
import PROMPT_LONG_RUN from "@/koda/session/long-run.txt"
...
export const Info = Schema.Struct({
  ...
  variant: Schema.optional(Schema.String), // koda_change
  source: Schema.optional(Schema.Literals(["command", "mcp", "skill"])),
  trusted: Schema.optional(Schema.Boolean), // koda_change - skill-sourced templates only run `!`cmd`` shell when trusted
...
// koda_change start - skills can share names with slash commands
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const init = Effect.fn("Command.state")(function* (ctx: InstanceContext) {
      ...
      commands["long-run"] = {
        name: "long-run",
        description: "incremental, checkpointed execution for large engineering tasks",
        source: "command",
        template: PROMPT_LONG_RUN,
        hints: hints(PROMPT_LONG_RUN),
      }
      commands["resume-claude"] = SessionResume.resumeClaude
      commands["resume-codex"] = SessionResume.resumeCodex
      // koda_change end

      // koda_change start - defer partial overrides until all command sources are registered
      const overrides: Array<{ name: string; command: Override }> = []
      for (const [name, command] of Object.entries(cfg.command ?? {})) {
        if (!applyOverride(commands, name, command, hints)) overrides.push({ name, command }) // koda_change
      }
      // koda_change end
      ...
      for (const item of yield* skill.all()) {
        ...
        commands[item.name] = fromSkill(item, directory(item)) // koda_change
      }

      // koda_change start - apply deferred overrides to their registered source
      for (const item of overrides) {
        ...
        if (skillTarget) {
          ...
          if (found) {
            if (commands[skillTarget]?.source !== "skill") {
              ...
              applyOverride(commands, item.name, item.command, hints) // koda_change
            } else {
              applyOverride(commands, skillTarget, item.command, hints) // koda_change
            }
          }
          continue
        }
        ...
        if (mcpTarget) {
          ...
          applyOverride(commands, mcpTarget, item.command, hints) // koda_change
          continue
        }
        applyOverride(commands, item.name, item.command, hints) // koda_change
      }
    ...
    const get = Effect.fn("Command.get")(function* (name: string) {
      ...
      const exact = s.commands[name] // koda_change
      if (exact) return exact // koda_change
      const alias = legacyReviewCommand(name) // koda_change
      if (alias) return alias // koda_change
