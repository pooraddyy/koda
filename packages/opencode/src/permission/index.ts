...
import * as Config from "@/config/config" // koda_change
import { InstanceState } from "@/effect/instance-state"
import { Wildcard } from "@opencode-ai/core/util/wildcard"
import { Deferred, Effect, Layer, Context } from "effect"
import os from "os"
import z from "zod" // koda_change
import { zod } from "@opencode-ai/core/effect-zod" // koda_change
import { PermissionV1 } from "@opencode-ai/core/v1/permission"
import { Database } from "@opencode-ai/core/database/database" // koda_change
import { EventV2Bridge } from "@/event-v2-bridge"
import { SessionID } from "@/session/schema" // koda_change - used by AllowEverythingInput
// koda_change start
import { ConfigProtection } from "@/koda/permission/config-paths"
import { kodaHeadless } from "@/koda/permission/headless"
import { drainCovered } from "@/koda/permission/drain"
import { ReadPermission } from "@/koda/permission/read"
import { AgentManagerPermission } from "@/koda/permission/agent-manager" // koda_change
import { ExternalDirectoryPermission } from "@/koda/permission/external-directory"
// koda_change end

export const Event = PermissionV1.Event
// koda_change start - upstream moved these types into PermissionV1; re-export them here so existing
// koda callers that import off `Permission.*` keep working without a repo-wide rewrite
...
// koda extends upstream's AskInput with an optional hardRuleset (consumed by drain + session/prompt)
...
// koda_change start - describe why a call was allowed so clients can explain auto-approval
...
export interface Interface {
  readonly ask: (input: AskInput) => Effect.Effect<AskOutcome, Error> // koda_change - was Effect<void>; returns the decision
...
interface State {
  ...
  approved: Rule[]
  session: Record<string, Ruleset> // koda_change
}
...
export function resolve(permission: string, pattern: string, ruleset: Ruleset, ...overrides: Ruleset[]): Rule {
  ...
  const saved = AgentManagerPermission.harden(permission, pattern, evalFn(permission, pattern, ...overrides)) // koda_change
...
function covered(entry: PendingEntry, approved: Ruleset, local: Ruleset) {
  ...
  if (entry.info.metadata?.["skillShell"] === true) return false // koda_change - skill batch needs an explicit reply
  if (entry.info.metadata?.["sandboxEscalation"] === true) return false // koda_change - host access needs an explicit reply
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const database = yield* Database.Service // koda_change
    const state = yield* InstanceState.make<State>(
      Effect.fn("Permission.state")(function* (ctx) {
        void ctx
        const state = {
          pending: new Map<PermissionV1.ID, PendingEntry>(),
          approved: [] as Rule[], // koda_change - upstream dropped DB-seeded approvals; koda persists via config.updateGlobal
          session: {} as Record<string, Ruleset>, // koda_change
        }
    ...
    const ask = Effect.fn("Permission.ask")(function* (input: AskInput) {
      ...
      // koda_change end
      let needsAsk = false
      let approvedRule: Rule | undefined // koda_change - remember the rule that auto-approved

      // koda_change start - protect config access while honoring explicit global skill trust
      ...
      const forceAsk = request.metadata?.["skillShell"] === true || request.metadata?.["sandboxEscalation"] === true // koda_change
      for (const pattern of request.patterns) {
        const rule = resolve(request.permission, pattern, ruleset, approved, local) // koda_change — include session-scoped rules
        yield* Effect.logInfo("evaluated", { permission: request.permission, pattern, action: rule })
        // koda_change start — saved/session approvals cannot override hard Ask/Plan denials
        if (veto(request.permission, pattern, hardRuleset)) {
          return yield* new DeniedError({ ruleset: subset(request.permission, hardRuleset ?? []) })
        }
        // koda_change end
        if (rule.action === "deny") {
          // koda_change - carry the deciding rule (not just the permission subset) for provenance
          return yield* new DeniedError({ ruleset: rule })
        }
        // koda_change start - skill shell forces a prompt instead of honoring an allow/auto-approve rule
        if (forceAsk) {
          needsAsk = true
          continue
        }
        // koda_change end
        // koda_change start - override "allow" to "ask" for protected config paths
      ...
      if (!needsAsk) return { manual: false, rule: approvedRule } // koda_change - report auto-approval

      // koda_change start - headless subagent asks fail instead of queuing for a reply that never comes (#11903)
      if (yield* kodaHeadless.denies(request.sessionID).pipe(Effect.provideService(Database.Service, database))) {
      ...
      const info: PermissionV1.Request = {
        ...
        // koda_change start - disable persistence for protected config paths outside one exact global skill
        metadata: {
          ...request.metadata,
          ...(skill ? { rules: [skill] } : {}),
        ...
        // koda_change end
        always: skill ? [skill] : request.always, // koda_change - persist only the exact global skill subtree
        tool: request.tool,
      }
      yield* Effect.logInfo("asking", { id, permission: info.permission, patterns: info.patterns })

      const deferred = yield* Deferred.make<void, RejectedError | CorrectedError>()
      pending.set(id, { info, ruleset, hardRuleset, deferred }) // koda_change
      yield* events.publish(Event.Asked, info) // koda_change - was bus.publish
      // koda_change start - was `return yield* Effect.ensuring(...)`; report the manual decision to callers
    ...
    const reply = Effect.fn("Permission.reply")(function* (input: PermissionV1.ReplyInput) {
      ...
      // koda_change start - skill-shell batches must be answered by a human; ignore machine approvals
      ...
      // koda_change start - downgrade "always" to "once" for protected config paths
      if (ConfigProtection.isRequest(existing.info) && !ConfigProtection.isGlobalSkillRequest(existing.info)) return
      // koda_change end

      for (const pattern of existing.info.always) {
        // koda_change start — saveAlwaysRules may have already persisted selected always-rules
        if (!existing.saved) {
          approved.push({
      ...
      ) // koda_change - drain publishes replies through the same EventV2Bridge channel
    ...
    const saveAlwaysRules = Effect.fn("Permission.saveAlwaysRules")(function* (
      ...
      // koda_change - drain publishes replies through the same EventV2Bridge channel (was DeniedError)
      yield* drainCovered(
    ...
    return Service.of({ ask, reply, list, saveAlwaysRules, allowEverything, pending }) // koda_change
  }),
)
...
export function fromConfig(permission: ConfigPermissionV1.Info) {
  ...
  for (const [key, value] of Object.entries(permission)) {
    ...
    if (value === null) continue // koda_change — null is a delete sentinel
    ruleset.push(
      // koda_change start — filter out null entries (delete sentinels)
...
// koda_change start — inverse of fromConfig: convert rules back to config format
...
export const node = LayerNode.make({
  service: Service,
  layer,
  deps: [EventV2Bridge.node, Config.node, Database.node], // koda_change
})
