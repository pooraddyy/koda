...
import { isSafeSegment, isSafeRelativePath } from "@/koda/skill/discovery-validate" // koda_change
...
const layer: Layer.Layer<Service, never, FSUtil.Service | Path.Path | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const pull = Effect.fn("Discovery.pull")(function* (url: string) {
      ...
      // koda_change start - resolve the index origin so file downloads can be pinned to it
      ...
      // koda_change start - the remote index controls skill.name and file, so validate every segment,
      ...
      // koda_change start - download each validated, origin-pinned, cache-confined plan
