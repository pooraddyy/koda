...
import { SessionMessage } from "@opencode-ai/core/session/message" // koda_change - shared Revert.State brand

import type { Provider } from "@/provider/provider"
import { Permission } from "@/permission"
import { Global } from "@opencode-ai/core/global"
// koda_change start - koda session behavior extensions
import { BackgroundProcess } from "@/koda/background-process"
import * as SandboxInheritance from "@/koda/sandbox/inheritance"
import { InteractiveTerminal } from "@/koda/interactive-terminal"
import { kodaSession } from "@/koda/session"
import { kodaSessionFork } from "@/koda/session/fork-command"
import { kodaSessionEvent } from "@/koda/session/event"
import { SessionExport } from "@/koda/session-export"
import * as SandboxPolicy from "@/koda/sandbox/policy"
import { carryForkDiff } from "@/koda/session-portability/cumulative-diff" // koda_change
// koda_change end
import { Effect, Layer, Option, Context, Schema, Types } from "effect"
import { NonNegativeInt, optionalOmitUndefined } from "@opencode-ai/core/schema"
import { AbsolutePath } from "@opencode-ai/core/schema" // koda_change
...
export function fromRow(row: SessionRow): Info {
  ...
  // koda_change start - the shared column stores the upstream Revert.State brand; project it to the v1 shape
...
export function toRow(info: Info) {
  return {
    ...
    // koda_change - re-brand the v1 messageID to the shared Revert.State brand for the column
    revert: info.revert
...
const Summary = Schema.Struct({
  ...
  files: Schema.Finite,
  diffs: optionalOmitUndefined(Schema.Array(Snapshot.SummaryFileDiff)), // koda_change - lightweight diff without patch
})
...
const Revert = Schema.Struct({
  ...
  workspace: optionalOmitUndefined(Schema.Literals(["restored", "snapshots-disabled", "unavailable"])), // koda_change
})
...
export const GlobalInfo = Schema.Struct({
  ...
  worktreeName: Schema.optional(Schema.String), // koda_change - basename of the specific worktree directory
...
export const CreateInput = Schema.optional(
  Schema.Struct({
    ...
    platform: Schema.optional(Schema.String), // koda_change - per-session platform override for telemetry attribution
    // koda_change start - server-issued sandbox inheritance grant
...
export type GlobalListInput = {
  // koda_change start - worktree-family filters for the Agent Manager
  projectID?: string
  directory?: string
  directories?: string[]
...
export const Event = {
  ...
  // koda_change start
  TurnOpen: kodaSessionEvent.TurnOpen,
  TurnClose: kodaSessionEvent.TurnClose,
  // koda_change end
}

export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".koda", "plans") // koda_change
...
export const getUsage = (input: {
  ...
  provider?: Provider.Info // koda_change
}) => {
  ...
  // koda_change start - Use provider-reported cost when available for OpenRouter/koda
  const reported = kodaSession.providerCost({
...
export interface Interface {
  ...
  // koda_change start - session create metadata and sandbox inheritance extensions
  ...
  readonly create: (input?: {
    ...
    permission?: PermissionV1.Ruleset
    platform?: string // koda_change - per-session platform override for telemetry attribution
    workspaceID?: WorkspaceV2.ID
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // koda_change start - inherited sandbox policy source
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      permission?: PermissionV1.Ruleset
      platform?: string // koda_change - per-session platform override for telemetry attribution
      sourceID?: SessionID // koda_change - inherited sandbox policy source
      sourceDirectory?: string
      sandboxFallback?: SandboxPolicy.Snapshot // koda_change - confinement to seed when source state lives in another directory
    }) {
      ...
      // koda_change start - legacy sessions must satisfy the upstream project foreign key
      yield* db
        .insert(ProjectTable)
        .values({
          id: ctx.project.id,
      ...
      // koda_change start - initialize inherited state before session.created subscribers run
      kodaSession.register({ id: result.id, parentID: result.parentID, platform: input.platform })
    ...
    // koda_change start - preserve koda's cross-project worktree-family filtering
    const listGlobal = Effect.fn("Session.listGlobal")((input?: GlobalListInput) =>
      kodaSession.listGlobal<GlobalInfo>({ ...input, fromRow }).pipe(Effect.provideService(Database.Service, database)),
    )
    // koda_change end

    // koda_change start - scope children by persisted parent project_id
    ...
    const remove: Interface["remove"] = Effect.fnUntraced(function* (sessionID: SessionID) {
      ...
      try {
        ...
        yield* SandboxPolicy.dispose(
          sessionID,
          Effect.gen(function* () {
            yield* Effect.promise(() => kodaSession.removeSession(sessionID)).pipe(Effect.ignore)
            kodaSession.clearPlatformOverride(sessionID)
            if (hasInstance) {
            ...
            // koda_change - migrated from legacy sync.run/sync.remove to EventV2 (events.publish/remove)
            yield* events.publish(SessionV1.Event.Deleted, { sessionID, info: session })
            // koda_change - capture final session-export workspace delta on close/delete
            const workspaceKey = hasInstance ? yield* InstanceState.directory : undefined // koda_change
            yield* Effect.promise(() => SessionExport.onSessionClose(sessionID, workspaceKey)) // koda_change
    ...
    const updateMessage = <T extends SessionV1.Info>(msg: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        // koda_change start - ignore FK errors when session was deleted while processor was still running
        yield* kodaSession.runSyncSafe(
    ...
    const create = Effect.fn("Session.create")(function* (input?: {
      ...
      // koda_change end
      // koda_change start - propagate trusted sandbox inheritance grant
      const session = yield* createNext({
        ...
        platform: input?.platform, // koda_change
        sourceID: grant?.sessionID, // koda_change
        sourceDirectory: grant?.directory, // koda_change
    ...
    const fork = Effect.fn("Session.fork")(function* (input: { sessionID: SessionID; messageID?: MessageID }) {
      ...
      // koda_change start - forks into another directory cannot read the source confinement from the new dir, so carry it over explicitly
      const sandboxFallback = yield* SandboxPolicy.peek(original.directory, input.sessionID)
      // koda_change end
      // koda_change start - historical forks must use the model from retained context, not a later source-session selection
      ...
      const session = yield* createNext({
        ...
        model, // koda_change - preserve the model + variant active at the fork point
        sourceID: input.sessionID, // koda_change - forks preserve initialized confinement
        sandboxFallback, // koda_change - seed confinement from the source session's original directory
        platform: kodaSession.resolvePlatform(original.id), // koda_change - inherit platform telemetry attribution
      })
      ...
      for (const msg of msgs) {
        ...
        const cloned = yield* updateMessage({
          ...
          id: newID,
          ...(msg.info.role === "assistant" && { cost: 0 }), // koda_change - count only spend incurred after the fork
          ...(parentID && { parentID }),
        })

        for (const part of msg.parts) {
          // koda_change - detach task calls + drop transient parts before copying the forked transcript
          const prepared = kodaSession.prepareForkedPart(part)
          ...
          const p: SessionV1.Part = {
            ...
            messageID: cloned.id,
            sessionID: session.id,
            ...(prepared.type === "step-finish" && { cost: 0 }), // koda_change - exclude pre-fork spend from model stats
          }
      ...
      // koda_change - preserve imported/cumulative diffs when forking (self-contained Storage runtime keeps this shared file off the legacy Storage layer)
      yield* carryForkDiff(input.sessionID, session.id)
      // koda_change start - fork terminal task children under the new parent and remap their references
      yield* kodaSession.remapChildren({
        sessionID: session.id,
...
function listByProject(
  ...
  // koda_change start - kodaSession.filters keeps sessions visible across project_id changes
  ...
  const conditions =
    input.path !== undefined
      ...
      : kodaSession.filters({ projectID: input.projectID, directory: input.directory })
  // koda_change end
  ...
  if (input.path !== undefined) {
    ...
    // koda_change start - directory filtering handled by kodaSession.filters above
    // if (input.directory) {
...
// koda_change start - delegate to kodaSession.listGlobal (adds projectID worktree family + directories[])
export function listGlobal(input?: {
  ...
  return kodaSession.listGlobal<GlobalInfo>({ ...input, fromRow })
}
// koda_change end

// koda_change - delegate the exported Promise facade to the koda session runtime
export const fork = kodaSessionFork
