...
import { RuntimeFlags } from "@/effect/runtime-flags" // koda_change - configured output token ceiling
...
// koda_change start
import { kodaSessionProcessor, type ReviewTelemetry } from "@/koda/session/processor"
import { PermissionProvenance } from "@/koda/permission/provenance" // koda_change
import { kodaSessionOverflow } from "@/koda/session/overflow"
import { kodaRoutedModel } from "@/koda/session/routed-model"
import { kodaResponseMetadata } from "@/koda/session/response-metadata"
import { Suggestion } from "@/koda/suggestion"
// koda_change end
...
export interface Handle {
  ...
  readonly compactError?: () => ReturnType<typeof MessageV2.ContextOverflowError.prototype.toObject> | undefined // koda_change
}
...
interface ProcessorContext extends Input {
  ...
  toolmeta: Record<string, { title?: string; metadata?: Record<string, any> }> // koda_change
  shouldBreak: boolean
  snapshot: string | undefined
  blocked: boolean
  needsCompaction: boolean
  compactionError: ReturnType<typeof MessageV2.ContextOverflowError.prototype.toObject> | undefined // koda_change
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const flags = yield* RuntimeFlags.Service // koda_change

    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      // Pre-capture snapshot before the LLM stream starts. The AI SDK
      // may execute tools internally before emitting start-step events,
      // so capturing inside the event handler can be too late.
      // koda_change start - pass turn context for slow-snapshot UI/policy handling
      ...
      const ctx: ProcessorContext = {
        ...
        model: input.model,
        toolcalls: {},
        toolmeta: {}, // koda_change
        shouldBreak: false,
        snapshot: initialSnapshot,
        blocked: false,
        needsCompaction: false,
        compactionError: undefined, // koda_change
      ...
      let aborted = false
      const ac = new AbortController() // koda_change — abort controller for offline handler
      let attempt = kodaSessionProcessor.attempt() // koda_change

      // koda_change start
      const parse = (e: unknown) =>
        kodaSessionProcessor.parseError(e, {
          providerID: input.model.providerID,
          aborted,
        })
      const retryParse = (e: unknown) => {
        const error = parse(e)
        if (e instanceof kodaSessionProcessor.IncompleteResponseError) return kodaSessionProcessor.blockRetry(error)
        if (attempt.text || attempt.reasoning || attempt.tool) return kodaSessionProcessor.blockRetry(error)
        return error
      }
      // koda_change end
      ...
      const settleToolCall = Effect.fn("SessionProcessor.settleToolCall")(function* (toolCallID: string) {
        ...
        delete ctx.toolmeta[toolCallID] // koda_change
      ...
      // koda_change start - tolerate deleted sessions during subagent cost reconciliation (#6321)
      ...
      // koda_change start - buffer metadata emitted before tool-call registration
      ...
      const completeToolCall = Effect.fn("SessionProcessor.completeToolCall")(function* (
        ...
        // koda_change start - preserve approval provenance recorded during permission checks
        ...
        yield* session.updatePart({
          ...
          state: {
            ...
            metadata, // koda_change - merged to keep approval
            title: output.title,
            time: { start: match.part.state.time.start, end: Date.now() },
            attachments: output.attachments,
          },
        })
        // koda_change start - accepted suggest review actions tag following LLM completion telemetry
        if (match.part.tool === "suggest") {
          ctx.telemetry = kodaSessionProcessor.suggestionReviewTelemetry(output.metadata) ?? ctx.telemetry
        }
        // koda_change end
      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        kodaSessionProcessor.observe(attempt, value) // koda_change
        switch (value.type) {
          ...
          case "reasoning-delta":
            ...
            if (value.text.trim()) ctx.step.reasoning = true // koda_change
          ...
          // koda_change start - upstream calls ensureToolCall here, which creates a part when none
          ...
          case "tool-call": {
            ...
            ctx.step.tool = true // koda_change
            yield* ensureToolCall(value)
            const input = isRecord(value.input) ? value.input : { value: value.input }
            // koda_change start - apply metadata buffered before the running transition
          ...
          case "tool-result": {
            ...
            // koda_change start — send_file delivery attachments (up to 4 MiB raw)
            ...
            // koda_change start - dismissed suggestions stop the turn after persisting normalized output
          ...
          case "step-start":
            ...
            yield* session.updatePart({
              ...
              type: "step-start",
              time: { start: ctx.stepStartDate }, // koda_change
            })
            return

          case "step-finish": {
            // koda_change start - retry only terminally incomplete attempts before settlement
            if (
              kodaSessionProcessor.replayable({
              ...
              return yield* Effect.fail(
                new kodaSessionProcessor.IncompleteResponseError(kodaResponseMetadata.read(value.providerMetadata)),
              )
            // koda_change end
            // koda_change start - pass turn context for slow-snapshot UI/policy handling
            ...
            // koda_change start
            const model = kodaRoutedModel.readAuto(value.providerMetadata, {
              providerID: ctx.model.providerID,
              modelID: ctx.model.id,
              selected: ctx.assistantMessage.modelID,
            })
            const generationID = kodaSessionProcessor.generationID(value.providerMetadata)
            const vercelID = kodaResponseMetadata.read(value.providerMetadata)
            // koda_change end
            // koda_change start - guard against finish-step without start-step:
            ...
            const metrics = kodaSessionProcessor.computeMetrics({
              providerMetadata: value.providerMetadata,
              tokens: usage.tokens,
              elapsedMs,
            })
            kodaSessionProcessor.trackStep({
              sessionID: ctx.sessionID,
              model: ctx.model,
              tokens: usage.tokens,
              cost: usage.cost,
            ...
            // koda_change start - capture any subagent cost propagated by tool calls during this step (#6321)
            yield* reconcile()
            // koda_change end
            ...
            yield* session.updatePart({
              ...
              sessionID: ctx.assistantMessage.sessionID,
              type: "step-finish",
              time: { start: startDate, end: endDate, elapsed: elapsedMs }, // koda_change
              ...(model ? { model } : {}), // koda_change
              ...(generationID ? { generationID } : {}), // koda_change
              ...(vercelID ? { vercelID } : {}), // koda_change
              ...(metrics ? { metrics } : {}), // koda_change
              tokens: usage.tokens,
              cost: usage.cost,
            })
            // koda_change start - surface output limit stops, with a stronger message for reasoning-only stops
            const warn = kodaSessionProcessor.lengthWarning({ msg: ctx.assistantMessage, step: ctx.step })
            if (warn) {
              yield* session.updatePart({
                id: PartID.ascending(),
                messageID: ctx.assistantMessage.id,
            ...
            const providerError = kodaSessionProcessor.providerFinishError(ctx.assistantMessage)
            if (providerError) {
          ...
          case "text-delta":
            ...
            if (value.text.trim()) ctx.step.text = true // koda_change
      ...
      const cleanup = Effect.fn("SessionProcessor.cleanup")(function* () {
        ...
        ctx.toolcalls = {}
        ctx.toolmeta = {} // koda_change
        // koda_change start - read parts through the upstream Effect database
        kodaSessionProcessor.guardEmptyToolCalls(
          ctx.assistantMessage,
          yield* MessageV2.parts(ctx.assistantMessage.id).pipe(Effect.provideService(Database.Service, database)),
        )
        // koda_change end
        ctx.assistantMessage.time.completed = Date.now()
        // koda_change start - reconcile cost with any subagent propagation written during tool calls (#6321)
        yield* reconcile()
        // koda_change end
        yield* session.updateMessage(ctx.assistantMessage)
      })
      ...
      const halt = Effect.fn("SessionProcessor.halt")(function* (e: unknown) {
        // koda_change start - internal preflight signal, not a provider error
        if (e instanceof kodaSessionOverflow.PreflightError) {
          ctx.needsCompaction = true
          return
        }
        // koda_change end
        ...
        const error = parse(e)
        // koda_change start
        if (e instanceof kodaSessionProcessor.IncompleteResponseError) ctx.assistantMessage.finish = "unknown"
      ...
      const process = Effect.fn("SessionProcessor.process")(function* (streamInput: LLM.StreamInput) {
        ...
        // koda_change start - a deleted session cannot accept EventV2 writes under core FK enforcement
        ...
        ctx.compactionError = undefined // koda_change
        ctx.shouldBreak = (yield* config.get()).experimental?.continue_loop_on_deny !== true

        return yield* Effect.gen(function* () {
          // koda_change start - publish retry state consistently for provider and empty-response retries
          ...
          const setRetry = (info: {
            ...
            Effect.gen(function* () {
              ...
              const error =
                info.error && SessionV1.APIError.isInstance(info.error)
                  ? info.error.data
                  : {
                      message: info.message,
                      isRetryable: true,
          ...
          const request = () =>
            Effect.gen(function* () {
              ...
              Effect.onInterrupt(() =>
                Effect.gen(function* () {
                  aborted = true
                  ac.abort() // koda_change — also abort offline handler
              ...
              Effect.retry(
                SessionRetry.policy({
                  ...
                  parse: retryParse,
                  ...kodaSessionProcessor.retryOpts({
                    sessionID: ctx.sessionID,
                    abort: ac.signal,
                    set: status.set,
          ...
          const recover = () => {
            ...
            return kodaSessionProcessor.recover({
              run: Effect.fn("SessionProcessor.incompleteAttempt")(function* () {
                ...
                attempt = kodaSessionProcessor.attempt()
                yield* request()
              }),
              replayable: () =>
                kodaSessionProcessor.replayable({
