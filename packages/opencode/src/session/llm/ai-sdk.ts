...
import { kodaRoutedModel } from "@/koda/session/routed-model" // koda_change
import { kodaResponseMetadata } from "@/koda/session/response-metadata" // koda_change
...
function usage(value: unknown) {
  ...
  const item = value as {
    ...
    raw?: Record<string, unknown> // koda_change - preserve provider billing details
  }
  const entries = Object.entries({
    ...
    providerMetadata: item.raw ? { aiSdk: item.raw } : undefined, // koda_change - retain koda billing details
      ...
      return Effect.sync(() => {
        ...
        return [
          LLMEvent.stepFinish({
            ...
            // koda_change start
            providerMetadata: kodaResponseMetadata.write(
              kodaRoutedModel.write(metadata, event.response?.modelId),
