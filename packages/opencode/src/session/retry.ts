...
import { iskodaError } from "@/koda/koda-errors" // koda_change
...
export type RetryReason = string & {} // koda_change - koda does not support OpenCode Go upsell reasons
...
// koda_change - koda does not emit OpenCode Go actions
export function retryable(error: Err, _provider?: string): Retryable | undefined {
  // context overflow errors should not be retried
  if (SessionV1.ContextOverflowError.isInstance(error)) return undefined
  if (SessionV1.APIError.isInstance(error)) {
    const status = error.data.statusCode
    // koda_change start - Current koda errors require user action (login/signup), don't retry
    if (iskodaError(error)) return undefined
    // koda_change end
    ...
    // koda_change start - koda does not support OpenCode Go upsells. FreeUsageLimitError is not retryable: retrying
...
export function policy(opts: {
  ...
  return Schedule.fromStepWithMetadata(
    Effect.succeed((meta: Schedule.InputMetadata<unknown>) => {
      // koda_change start — enforce retry limit
      ...
      return Effect.gen(function* () {
        // koda_change start — handle network disconnect via offline handler
        if (opts.offline && SessionNetwork.disconnected(meta.input)) {
          ...
          yield* opts.set({ attempt: 0, message: "Reconnected", next: Date.now(), error })
