...
export { EditorContext } from "@/koda/editor-context" // koda_change
...
import { SessionNetwork } from "./network" // koda_change
import { CodexAuthExpiredError } from "@/koda/provider/codex-refresh" // koda_change
import { kodaSessionMessageOrder } from "@/koda/session/message-order" // koda_change
import * as TextStream from "@/koda/text-stream" // koda_change
...
// koda_change - upstream moved these message/part types to SessionV1; re-export them so the
...
function truncateToolOutput(text: string, maxChars?: number) {
  ...
  // koda_change start - avoid persisting malformed Unicode in compacted tool output
...
// koda_change start - strip bloated metadata fields from stored parts to prevent multi-MB payloads
...
export function stripPartMetadata(part: Part): Part {
  // koda_change - exported for testing
...
// koda_change - apply stripping inside helpers so all read paths are covered
...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  for (const msg of input) {
    ...
    if (msg.info.role === "assistant") {
      ...
      for (const part of msg.parts) {
        // koda_change - !part.ignored keeps local UI warnings out of future prompts
        ...
        if (part.type === "tool") {
          ...
          if (part.state.status === "completed") {
            ...
            // koda_change start — do not replay send_file delivery attachments to the model;
...
export function parts(messageID: MessageID) {
  return Effect.gen(function* () {
    ...
    const rows = yield* db
      ...
      .all()
      .pipe(Effect.orDie)
    return rows.map(part) // koda_change - part() applies stripPartMetadata to cover all read paths
  })
}
...
export function filterCompacted(msgs: Iterable<WithParts>) {
  ...
  result.reverse()
  kodaSessionMessageOrder.annotate(result) // koda_change - preserve chronology before retained-tail projection
...
export function fromError(
  ...
  switch (true) {
    ...
    case e instanceof CodexAuthExpiredError: // koda_change start
      return new AuthError(
        {
          providerID: "openai",
          message: e.message,
        ...
        { cause: e },
      ).toObject() // koda_change end
    case SessionNetwork.disconnected(e): // koda_change start
      return new APIError(
        {
          message: SessionNetwork.message(e), // koda_change end
          isRetryable: true,
          metadata: {
