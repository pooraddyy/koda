...
import { kodaInstruction } from "@/koda/session/instruction" // koda_change
import type { kodaMarkdown } from "@/koda/config/markdown" // koda_change
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const globalFiles = [
      // koda_change start - prefer koda_CONFIG_DIR profile when set
      ...(Flag.koda_CONFIG_DIR ? [path.join(Flag.koda_CONFIG_DIR, "AGENTS.md")] : []),
      // koda_change end
    ...
    const relative = Effect.fnUntraced(function* (instruction: string) {
      ...
      // koda_change - prefer koda_CONFIG_DIR profile when set, else fall back to global.config
      const root = Flag.koda_CONFIG_DIR ?? global.config
      return yield* fs.globUp(instruction, root, root).pipe(Effect.catch(() => Effect.succeed([] as string[]))) // koda_change
    })

    // koda_change start - project instructions cannot read env or files outside the project root
    const options = Effect.fnUntraced(function* (filepath: string, origin?: kodaMarkdown.Source) {
      const ctx = yield* InstanceState.context
    ...
    const read = Effect.fnUntraced(function* (filepath: string, origin?: kodaMarkdown.Source) {
      const opts = yield* options(filepath, origin)
      return yield* Effect.promise(() => kodaInstruction.read(filepath, opts).catch(() => ""))
    })
    // koda_change end
    ...
    // koda_change start - retain declaration provenance through instruction path expansion
    const systemSources = Effect.fn("Instruction.systemSources")(function* () {
      const config = yield* cfg.get()
      const ctx = yield* InstanceState.context
      const root = ctx.worktree === "/" ? ctx.directory : ctx.worktree
      const paths = new Map<string, kodaMarkdown.Source>()
      const add = (item: string, origin: kodaMarkdown.Source) => {
      ...
      if (config.instructions) {
        for (const raw of config.instructions) {
          ...
          const trusted = declared.trusted && (path.isAbsolute(instruction) || Flag.koda_DISABLE_PROJECT_CONFIG)
    ...
    const system = Effect.fn("Instruction.system")(function* () {
      const config = yield* cfg.get()
      const sources = yield* systemSources() // koda_change
      const paths = Array.from(sources.keys()) // koda_change
