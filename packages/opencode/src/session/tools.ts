...
import { MemoryMarker } from "@/koda/memory/marker" // koda_change
...
import { McpApps } from "@/koda/mcp/apps"
// koda_change end
...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
  ...
  processor: Pick<SessionProcessor.Handle, "message" | "metadata" | "completeToolCall"> // koda_change
  bypassAgentCheck: boolean
  messages: SessionV1.WithParts[]
  promptOps: TaskPromptOps
  memoryCache: MemoryMarker.Cache // koda_change
}) {
  ...
  // koda_change start - permission provenance
  const config = yield* Config.Service
  const cfg = yield* config.get()
  const permissionOrigins = cfg.permission_origins
  // koda_change end
  const flags = yield* RuntimeFlags.Service
  const restricted = yield* SandboxPolicy.networkRestricted(input.session.id) // koda_change
  const sandboxed = (yield* SandboxPolicy.status(input.session.id)).enabled // koda_change
  const context = (args: Record<string, unknown>, options: ToolExecutionOptions): Tool.Context => {
    const extra = {
      model: input.model,
      bypassAgentCheck: input.bypassAgentCheck,
      promptOps: input.promptOps,
      sandboxed, // koda_change
  ...
  for (const item of yield* registry.tools({
    ...
    family: input.model.family, // koda_change
    agent: input.agent,
    permission: input.session.permission,
    networkRestricted: restricted, // koda_change - let the registry suppress code-mode in restricted sessions
  })) {
      ...
      execute(args, options) {
        return run.promise(
          Effect.gen(function* () {
            ...
            // koda_change - mark successful targeted memory recalls for the assistant badge
            if (item.id === "koda_memory_recall") MemoryMarker.recall({ result: output, cache: input.memoryCache }) // koda_change
            yield* plugin.trigger(
              "tool.execute.after",
  ...
  const mcpTools = restricted ? {} : yield* mcp.tools() // koda_change
    ...
    item.execute = (args, opts) =>
      run.promise(
        Effect.gen(function* () {
          ...
          // koda_change start - propagate MCP App UI metadata so hosts can preload the UI resource
          ...
          const result: Awaited<ReturnType<NonNullable<typeof execute>>> = yield* SandboxPolicy.executeMcp(
            ctx.sessionID,
            entry, // koda_change - retain the native entry's local/remote network authority marker
            Effect.gen(function* () {
          ...
          const metadata = {
            ...
            truncated: truncated.truncated,
            ...(truncated.truncated && { outputPath: truncated.outputPath }),
            ...mcpAppMeta, // koda_change - MCP App UI metadata
          }
