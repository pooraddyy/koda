...
import { Config } from "@/config/config" // koda_change
...
import { kodaSessionRevert } from "@/koda/session/revert" // koda_change
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const revert = Effect.fn("SessionRevert.revert")(function* (input: RevertInput) {
      ...
      // koda_change start - keep the entire workspace transition atomic
      const prior = session.revert ? kodaSessionRevert.files(all, session.revert) : []
      ...
      yield* kodaSessionRevert.apply(
        snap,
        baseline,
        files,
        Effect.gen(function* () {
          if (session.revert?.snapshot) yield* kodaSessionRevert.restore(snap, session.revert.snapshot, prior)
    ...
    const unrevert = Effect.fn("SessionRevert.unrevert")(function* (input: { sessionID: SessionID }) {
      ...
      // koda_change start - preserve the reverted workspace if redo cannot complete
      const all = yield* sessions.messages({ sessionID: input.sessionID }).pipe(Effect.orDie)
      const files = kodaSessionRevert.files(all, session.revert)
      ...
      yield* kodaSessionRevert.apply(
        ...
        files,
        Effect.gen(function* () {
          if (session.revert?.snapshot) yield* kodaSessionRevert.restore(snap, session.revert.snapshot, files)
    ...
    const cleanup = Effect.fn("SessionRevert.cleanup")(function* (session: Session.Info) {
      ...
      if (session.revert.partID && target) {
        ...
        if (idx >= 0) {
          ...
          // koda_change start - clear a reverted provider error from the retained assistant message
