...
import { appendSessionDiffs, readSessionDiffBase } from "@/koda/session-portability/cumulative-diff" // koda_change
import { Storage } from "@/storage/storage" // koda_change
...
export interface Interface {
  ...
  readonly diff: (input: DiffInput) => Effect.Effect<Snapshot.FileDiff[]> // koda_change - full-content detail input
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const storage = yield* Storage.Service // koda_change
    ...
    const summarize = Effect.fn("SessionSummary.summarize")(function* (input: {
      ...
      if ((yield* config.get()).snapshot === false) return // koda_change - respect snapshot config toggle

      // koda_change start - preserve imported cumulative diffs when summarizing cloud-forked sessions
      ...
      yield* storage.write(["session_diff", input.sessionID], diffs).pipe(Effect.ignore) // koda_change
      yield* events.publish(Session.Event.Diff, { sessionID: input.sessionID, diff: diffs })

      if (!target || target.info.role !== "user") return
      const msgDiffs = base.length > 0 ? local : yield* computeDiff({ messages }) // koda_change
      target.info.summary = { ...target.info.summary, diffs: msgDiffs }
      yield* sessions.updateMessage(target.info)
    })

    const diff = Effect.fn("SessionSummary.diff")(function* (input: DiffInput) { // koda_change - full-content detail input
      // koda_change start - authoritative full-content detail for one file (editor diff tabs)
      ...
      // koda_change end
      // koda_change start - retain cumulative diffs for legacy TUI and VS Code consumers
      if (!input.messageID) {
...
export const DiffInput = Schema.Struct({
  ...
  full: Schema.optional(Schema.Boolean), // koda_change - request full-content detail
  file: Schema.optional(Schema.String), // koda_change - scope full detail to one file
})
...
export const node = LayerNode.make({
  service: Service,
  layer: layer,
  deps: [Session.node, Snapshot.node, EventV2Bridge.node, Config.node, Storage.node], // koda_change
})
