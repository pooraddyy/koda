...
export namespace SessionNetwork {
  ...
  const urls = ["https://koda.ai", "https://example.com", "https://cloudflare.com/cdn-cgi/trace"]
  const POLL_MS = 3_000
  ...
  export function disconnected(err: unknown) {
    ...
    // koda_change - recognize AbortSignal.timeout() errors
    for (const item of chain(err)) {
  ...
  export function message(err: unknown) {
    // koda_change - check for timeout first
    for (const item of chain(err)) {
  ...
  export const reply = fn(
    ...
    async (input) => {
      ...
      // koda_change start - reconnect failed remote MCP servers after network recovery
