...
import fs from "node:fs" // koda_change
import { SessionV1 } from "@opencode-ai/core/v1/session"
import os from "os"
import { kodaSessionPrompt } from "@/koda/session/prompt" // koda_change
import { SKILL_SHELL_DISABLED, SKILL_SHELL_UNTRUSTED } from "@/koda/skills/display" // koda_change
import { kodaSessionMessageOrder } from "@/koda/session/message-order" // koda_change
import { kodaSessionPromptQueue } from "@/koda/session/prompt-queue" // koda_change
import { kodaSession } from "@/koda/session" // koda_change
import { SessionTranscript } from "@/koda/session/transcript" // koda_change
...
import * as kodaWorkflowVariant from "@/koda/session/workflow-variant" // koda_change
import { kodaSessionOverflow } from "@/koda/session/overflow" // koda_change
import { kodaReference } from "@/koda/reference/contains" // koda_change
import { kodaReadObject } from "@/koda/tool/read-object" // koda_change
import { isInterrupted } from "@/koda/effect/cause" // koda_change
import * as SandboxPolicy from "@/koda/sandbox/policy" // koda_change
import { CommandTimeout } from "@/koda/command-timeout" // koda_change
import { Suggestion } from "@/koda/suggestion" // koda_change
import { Question } from "@/question" // koda_change
import { BUILTIN_COMMANDS } from "@/koda/session/builtin-commands" // koda_change
import { legacyReviewMessage } from "@/koda/review/command" // koda_change
import { zod } from "@opencode-ai/core/effect-zod" // koda_change
import { withStatics } from "@opencode-ai/core/schema" // koda_change
...
import * as DateTime from "effect/DateTime" // koda_change
import { SessionEvent } from "@opencode-ai/core/session/event" // koda_change
import { SessionMessage } from "@opencode-ai/core/session/message" // koda_change
import { InstanceState } from "@/effect/instance-state"
import { InstanceRef } from "@/effect/instance-ref"
import { Instance } from "@/koda/instance"
import { EffectBridge } from "@/effect/bridge"
import { TaskTool, type TaskPromptOps } from "@/tool/task"
import { assertExternalDirectoryEffect } from "@/tool/external-directory" // koda_change
...
import * as kodaConfiguredReference from "@/koda/reference" // koda_change
...
import { SessionResume } from "@/koda/session-resume" // koda_change
import * as kodaRunBudget from "@/koda/session/run-budget" // koda_change
...
export const shouldAskPlanFollowup = kodaSessionPrompt.shouldAskPlanFollowup // koda_change - retain koda plan handoff policy

// koda_change start - persistent tool-output pruning when payload is already large
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const question = yield* Question.Service // koda_change - dismiss superseded pending questions through the shared service
    ...
    const cache = Option.getOrUndefined(yield* Effect.serviceOption(RepositoryCache.Service)) // koda_change
    const { db } = database
    ...
    const cancel = Effect.fn("SessionPrompt.cancel")(function* (sessionID: SessionID) {
      ...
      yield* kodaSessionPrompt.cancelTree({ sessionID, sessions, cancel: state.cancel }) // koda_change - stop queued work and subagents
    })

    // koda_change start - preserve configured reference mentions on the Core reference architecture
    const resolveReferenceParts = Effect.fnUntraced(function* (template: string, skip = new Set<string>()) {
      const ctx = yield* InstanceState.context
      const cfg = yield* config.get()
      const refs = kodaConfiguredReference
        .resolveAll({
      ...
      for (const match of ConfigMarkdown.files(template)) {
        ...
        if (reference.kind === "git" && cache) yield* kodaConfiguredReference.ensure(cache, reference) // koda_change
    ...
    const resolvePromptParts = Effect.fn("SessionPrompt.resolvePromptParts")(function* (template: string) {
      const ctx = yield* InstanceState.context
      const roots = yield* resolveReferenceParts(template) // koda_change
      ...
      yield* Effect.forEach(
        ...
        Effect.fnUntraced(function* (match) {
          const name = match[1]
          if (!name) return
          // koda_change start - configured references were already added above
          const alias = name.split("/")[0]
    ...
    const title = Effect.fn("SessionPrompt.ensureTitle")(function* (input: {
      ...
      const msgs = onlySubtasks
        ...
        : yield* MessageV2.toModelMessagesEffect(context, mdl).pipe(
            Effect.provideService(Database.Service, database), // koda_change - provide the migrated message store
          )
      const text = yield* llm
        .stream({
          ...
          tools: {},
          model: mdl,
          sessionID: kodaSessionPrompt.titleID(input.session.id), // koda_change - isolate title requests from the agent task
          retries: 2,
      ...
      // koda_change start - auto-title mark/re-check owned by kodaSessionPrompt
      const fresh = yield* sessions.get(input.session.id).pipe(Effect.orElseSucceed(() => null))
      if (
        !kodaSessionPrompt.prepareAutoTitle({
      ...
      yield* sessions.setTitle({ sessionID: input.session.id, title: t }).pipe(
        Effect.catchCause((cause) => {
          kodaSessionPrompt.clearAutoTitleMark(input.session.id, t)
    ...
    const handleSubtask = Effect.fn("SessionPrompt.handleSubtask")(function* (input: {
      ...
      const taskVariant = task.variant ?? lastUser.model.variant // koda_change
      const assistantMessage: SessionV1.Assistant = yield* sessions.updateMessage({
        ...
        mode: task.agent,
        agent: task.agent,
        variant: taskVariant, // koda_change
      ...
      // koda_change start - distinguish explicit workflow model selection from the effective subtask model
      ...
      // koda_change start - shared reader for the child session id written by task.ts ctx.metadata (#6321)
      const childID = () => {
      ...
      const result = yield* taskTool
        .execute(taskArgs, {
          ...
          // koda_change start - resolve permissions at ask time so active tools see config edits
          ask: (req: any) =>
            kodaSessionPrompt
              .askPermission({
                permission,
        ...
        .pipe(
          ...
          Effect.onInterrupt(() =>
            Effect.gen(function* () {
              ...
              // koda_change start - propagate partial subagent cost on cancel (#6321)
              const cid = childID()
              if (cid) {
                assistantMessage.cost = yield* kodaCostPropagation.childCost(sessions, SessionID.make(cid))
              }
              // koda_change end
      ...
      assistantMessage.finish = "tool-calls"
      assistantMessage.time.completed = Date.now()
      // koda_change start - include subagent total cost on the wrapper message (#6321)
      ...
      const summaryUserMsg: SessionV1.User = {
        ...
        editorContext: lastUser.editorContext, // koda_change — preserve editor context
      }
      ...
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          const { msg, part, cwd } = yield* Effect.gen(function* () {
            ...
            const callID = ulid() // koda_change - correlate v2 shell events with the persisted tool part
            ...
            // koda_change start - dual-write the v2 shell record so the durable timeline correlates with the tool part
          ...
          let output = ""
          let aborted = false
          let timeout: string | undefined // koda_change
    ...
    const getModel = Effect.fn("SessionPrompt.getModel")(function* (
      ...
      if (isInterrupted(exit.cause)) return yield* Effect.interrupt // koda_change
      const err = Cause.squash(exit.cause)
      if (Provider.ModelNotFoundError.isInstance(err)) {
        const hint = err.suggestions?.length ? ` Did you mean: ${err.suggestions.join(", ")}?` : ""
        const empty = err.modelsEmpty ? " No models are currently available." : "" // koda_change
        yield* events.publish(Session.Event.Error, {
          sessionID,
          error: new NamedError.Unknown({
            message: `Model not found: ${err.providerID}/${err.modelID}.${hint}${empty}`, // koda_change
          }).toObject(),
        })
      }
    ...
    const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
      const agentName = input.agent ?? (yield* sessions.get(input.sessionID).pipe(Effect.orDie)).agent // koda_change
      ...
      // koda_change start - retain the source session variant across Agent Manager's model-less fork handoff
      ...
      const info: SessionV1.User = {
        ...
        sessionID: input.sessionID,
        time: { created: Date.now() },
        tools: { ...input.tools, ...input.ephemeralTools }, // koda_change - apply non-persistent remote tool restrictions
        agent: ag.name,
        model: {
          providerID: model.providerID,
          modelID: model.modelID,
        ...
        system: input.system,
        format: input.format,
        editorContext: input.editorContext, // koda_change
      }
      ...
      const ctx = yield* InstanceState.context // koda_change - resolve V1 reference roots for attachment authorization
      const references = kodaConfiguredReference
        .resolveAll({
      ...
      const referenceContextFromFilePart = Effect.fnUntraced(function* (
        ...
        return { root: reference.path } // koda_change - carry the Core reference root for authorization
      })
        ...
        if (part.type === "file") {
          ...
          switch (url.protocol) {
            case "data:":
              ...
              // koda_change start - normalize user image data before persistence
              ...
              // koda_change end
              break
            // koda_change start - resolve @-mentioned past chats into transcript context
            case "session:":
            ...
            case "file:": {
              ...
              // koda_change start - authorize prompt attachments like model-issued read calls
              const controller = new AbortController()
              const ask: Tool.Context["ask"] = (request) =>
                Effect.gen(function* () {
                  const session = yield* sessions.get(input.sessionID)
                  yield* kodaSessionPrompt.askPermission({
                    permission,
                    agents,
                    sessions,
                    agent: ag,
              ...
              const execRead = (args: Parameters<typeof read.execute>[0], extra?: Tool.Context["extra"]) => {
                return read
                  .execute(args, ctx(extra)) // koda_change - enforce read and external_directory permissions
              ...
              // koda_change start - authorize metadata, then reopen and verify before consuming bytes
              const access = yield* Effect.gen(function* () {
                const file = yield* kodaReadObject.file(filepath)
                const instance = yield* InstanceState.context
                const context = ctx()
                const explicit = reference ? yield* kodaReference.path(fsys, reference.root, file.target) : false
                const referenced =
                  explicit || (yield* kodaReference.contains({ fs: fsys, references, target: file.target }))
                ...
                return yield* kodaReadObject.use(file, (bound) =>
                  Effect.gen(function* () {
              ...
              return [
                ...
                access.value, // koda_change - retain the attachment read through the authorized object
              ]
            }
          }
        }
      ...
      // koda_change start - expand references from direct prompt text, not only command templates
      ...
      // koda_change - koda normalizes images inside resolvePart, so there is no separate normalization pass here (unlike upstream)
    ...
    const prompt: Interface["prompt"] = Effect.fn("SessionPrompt.prompt")(
      function* (input: PromptInput) {
        const session = yield* sessions.get(input.sessionID).pipe(Effect.orDie)
        yield* revert.cleanup(session)
        // koda_change start - recover interrupted koda turns before accepting a follow-up
        yield* kodaSessionPrompt.recoverDanglingAssistant({ sessionID: input.sessionID, status, sessions })
        yield* kodaSessionPrompt.recoverProviderFinishError({ sessionID: input.sessionID, status, sessions })
        // koda_change end
        const message = yield* kodaSessionPrompt.intake(input.sessionID, createUserMessage(input)) // koda_change
        ...
        if (permissions.length > 0) {
          // koda_change start - preserve inherited task restrictions while refreshing prompt tool toggles
          const merged = kodaSessionPrompt.mergeToolPermissions({
        ...
        // koda_change start — register the queued follow-up before dismissing blockers.
        ...
        return yield* kodaSessionPromptQueue.enqueue(
          input.sessionID,
          message.info.id,
          bridge.run(
    ...
    const lastAssistant = Effect.fnUntraced(function* (sessionID: SessionID) {
      // koda_change start - retry when cancel races before shellImpl writes messages
    ...
    // koda_change — mutable close-reason per session, set by runLoop and read by loop
    const closeReasons = new Map<string, kodaSession.CloseReason>()

    // koda_change start - retain request-scoped snapshot initialization policy
    const runLoop: (input: LoopInput) => Effect.Effect<MessageV2.WithParts, NotFoundError> = Effect.fn(
      "SessionPrompt.run",
    )(function* (input: LoopInput) {
      const sessionID = input.sessionID
      // koda_change end
      // koda_change — cache environment details per turn (prompt caching)
      const envCache: kodaSessionPrompt.EnvCache = {}
      const memoryCache = kodaSessionPrompt.memoryCache() // koda_change
      closeReasons.delete(sessionID) // koda_change
      let compactionAttempts = 0 // koda_change - cap compaction attempts per turn to avoid infinite loops
      let runBudget: kodaRunBudget.State | undefined
      ...
      while (true) {
        ...
        // koda_change start - provide the upstream Effect database to koda's retained prompt loop
        let msgs = yield* MessageV2.filterCompactedEffect(sessionID).pipe(
          Effect.provideService(Database.Service, database),
        )
        // koda_change end
        msgs = kodaSessionPromptQueue.scope(sessionID, msgs) // koda_change - hide later queued prompts
        msgs = kodaSessionPrompt.trimBeforeLastSummary(msgs) // koda_change - trim on any completed summary (e.g. manual /compact against a text user)
        // koda_change start - select loop state by chronology after retained-tail projection
        const latest = kodaSessionMessageOrder.latest(msgs)
        ...
        // koda_change start - compare chronology, not generated IDs
        const userBeforeAssistant =
          latest.userMessage &&
          latest.assistantMessage &&
          kodaSessionMessageOrder.compare(latest.userMessage, latest.assistantMessage) < 0
        // koda_change end
        // koda_change start - carry local review command marker into LLM telemetry
        const telemetry =
          kodaSessionProcessor.extractReviewTelemetry(
            msgs.findLast((m) => m.info.role === "user" && m.info.id === lastUser.id)?.parts ?? [],
          ) ?? kodaSessionProcessor.extractSuggestionReviewTelemetry(lastAssistantMsg?.parts ?? [])
        // koda_change end
        ...
        // koda_change start - plan_exit is a hard stop before another model call
        if (
          lastAssistant?.finish &&
          hasToolCalls &&
          lastAssistant.parentID === lastUser.id &&
          userBeforeAssistant &&
          kodaSessionPrompt.shouldAskPlanFollowup({ messages: msgs, abort: AbortSignal.any([]) })
        ) {
          const action = yield* Effect.promise((signal) =>
            kodaSessionPrompt.askPlanFollowup({ sessionID, messages: msgs, abort: signal, question }),
          )
        ...
        if (
          ...
          !hasToolCalls &&
          lastAssistant.parentID === lastUser.id && // koda_change - unrelated later assistants do not answer this turn
          userBeforeAssistant // koda_change - compare chronology, not generated IDs
        ) {
        ...
        if (task?.type === "compaction") {
          ...
          // koda_change start - compaction.process only returns "stop" after
        ...
        if (
          ...
          // koda_change start
          const guard = kodaSessionPrompt.guardCompactionAttempt({
            sessionID,
        ...
        runBudget ??= kodaRunBudget.make(agent.budget)
        if (!budgetSeeded) {
          kodaRunBudget.seed(runBudget, msgs)
          budgetSeeded = true
        }
        const priorBudget = kodaRunBudget.observeMany(runBudget, msgs)
        if (priorBudget.exceeded) {
          const exhausted = priorBudget.exceeded
          yield* state.markTerminal(sessionID, "budget", kodaRunBudget.describe(exhausted))
          yield* Effect.logWarning("agent budget reached", {
            "session.id": sessionID,
            kind: exhausted.kind,
            used: exhausted.used,
        ...
        const handle = yield* processor
          .create({
            ...
            sessionID,
            model,
            telemetry, // koda_change
            snapshotInitialization: input.snapshotInitialization, // koda_change
          })
        ...
        const outcome: "break" | "continue" = yield* Effect.gen(function* () {
          ...
          const tools = yield* SessionTools.resolve({
            ...
            messages: msgs,
            promptOps,
            memoryCache, // koda_change
          }).pipe(
            Effect.provideService(Plugin.Service, plugin),
            Effect.provideService(Permission.Service, permission),
            Effect.provideService(Agent.Service, agents), // koda_change
            Effect.provideService(Session.Service, sessions), // koda_change
            Effect.provideService(ToolRegistry.Service, registry),
            Effect.provideService(MCP.Service, mcp),
            Effect.provideService(Truncate.Service, truncate),
            // koda_change start - provide services used by session tool resolution
            Effect.provideService(Config.Service, config),
          ...
          // koda_change start — ephemeral context injection + post-summary
          ...
          kodaSessionPrompt.injectEditorContext({ msgs, lastUser, sessionID, cache: envCache })
          msgs = kodaSessionPrompt.maybeStripHistoricalMedia(msgs)
          // koda_change end

          // koda_change start - persistently prune stale tool outputs when payload is already large
          const [skills, env, mem, evolution, instructions, mcpInstructions] = yield* Effect.all([
            sys.skills(agent),
            sys.environment(model, lastUser.editorContext), // koda_change
            kodaSessionPrompt.memoryInject({ ctx, sessionID, record: step === 1, cache: memoryCache }), // koda_change
            kodaSessionPrompt.evolutionInject({ ctx, sessionID }),
          ...
          if (size > REQUEST_PRUNE_BYTES) {
            ...
            msgs = kodaSessionPromptQueue.scope(sessionID, msgs)
            msgs = kodaSessionPrompt.trimBeforeLastSummary(msgs)
            yield* plugin.trigger("experimental.chat.messages.transform", {}, { messages: msgs })
            kodaSessionPrompt.injectEditorContext({ msgs, lastUser, sessionID, cache: envCache })
            msgs = kodaSessionPrompt.maybeStripHistoricalMedia(msgs)
          ...
          const result = yield* handle.process({
            // koda_change start - keep Ask/Plan tool filtering hardened against session allows
            user: lastUser,
            agent,
            permission: kodaSessionPrompt.guardPermissions({ agent, session }),
            // koda_change end
            sessionID,
            ...
            messages: [
              ...modelMsgs,
              ...(isLastStep ? [{ role: "user" as const, content: MAX_STEPS_PROMPT }] : []), // koda_change - avoid provider-incompatible assistant prefill
            ],
            tools,
            model,
            toolChoice: format.type === "json_schema" ? "required" : undefined,
            // koda_change start - feed the provider-reported context size from the last finished
            // turn into the output-token cap, so image/vision input is measured by the provider
            // rather than by encoded payload bytes (see kodaLLM.capOutputTokens). Summary messages
            ...
            reportedContextTokens:
              lastFinished && lastFinished.summary !== true
                ? kodaSessionOverflow.count(lastFinished.tokens)
                : undefined,
            // koda_change end
          })

          // The active assistant object does not carry persisted tool parts. Reload this
          // message after processing so delegated task calls are charged in the same turn.
          ...
          if (currentBudget.exceeded) {
            ...
            handle.message.error ??= budgetError
            handle.message.finish ??= "error"
            yield* sessions.updateMessage(handle.message)
            yield* events.publish(Session.Event.Error, { sessionID, error: budgetError })
            yield* state.markTerminal(sessionID, "budget", kodaRunBudget.describe(currentBudget.exceeded))
            return "break" as const
          }

          // koda_change start - persist a lightweight marker when this assistant step had memory context
          const marker = kodaSessionPrompt.memoryPart({ sessionID, message: handle.message, cache: memoryCache })
          ...
          if (finished && !handle.message.error) {
            if (handle.message.finish === "content-filter") {
              ...
              closeReasons.set(sessionID, "error") // koda_change - retain koda close-reason propagation
              return "break" as const
            }
            ...
            if (handle.message.finish === "error") {
              kodaSessionProcessor.providerFinishError(handle.message)
              yield* sessions.updateMessage(handle.message)
              closeReasons.set(sessionID, "error")
              return "break" as const
            }
          ...
          if (result === "compact") {
            ...
            yield* compaction.create({
              ...
              model: lastUser.model,
              auto: true,
              // koda_change - preflight compaction replays the pending turn without treating media as provider overflow
              overflow: !handle.message.finish && handle.compactError?.() !== undefined, // koda_change
            })
          }
          // koda_change start — break out so a newer queued prompt can take over
          ...
          if (kodaSessionPromptQueue.hasFollowup(sessionID)) {
            closeReasons.set(sessionID, "superseded")
            return "break" as const
          }
          // koda_change end
          // koda_change start - guard against providers that end the stream
    ...
    const loop: (input: LoopInput) => Effect.Effect<MessageV2.WithParts, NotFoundError> = Effect.fn(
      ...
      yield* kodaSessionPrompt.recoverDanglingAssistant({ sessionID: input.sessionID, status, sessions })
      yield* kodaSessionPrompt.recoverProviderFinishError({ sessionID: input.sessionID, status, sessions })
      yield* kodaSession.publishTurnOpen({ sessionID: input.sessionID })
      return yield* Effect.onExit(
        ...
        Effect.fnUntraced(function* (exit) {
          yield* kodaSession.publishTurnClose({
            sessionID: input.sessionID,
            parentID: session.parentID,
            reason: kodaSessionPrompt.resolveCloseReason({
              sessionID: input.sessionID,
              closeReasons,
              exit,
            }),
    ...
    // koda_change start - resume command handler
    ...
    const handleResume = Effect.fn("SessionPrompt.handleResume")(function* (input: {
      ...
      // koda_change start - test-only resume roots via Context.Service
      ...
      if (msgs.length > 0) {
        const error = new NamedError.Unknown({
          message: "Start a new koda session, then run the resume command again.",
        })
      ...
      if (trimmed.length === 0) {
        ...
        const answers = yield* question.ask({
          ...
          questions: [
            {
              question: `Which recent ${input.format === "claude" ? "Claude Code" : "OpenAI"} session do you want to resume?`,
      ...
      if (!file) {
        const error = new NamedError.Unknown({
          message: `No ${input.format === "claude" ? "Claude Code" : "OpenAI"} session found with UUID: ${uuid}`,
        })
        yield* events.publish(Session.Event.Error, { sessionID: input.cmdInput.sessionID, error: error.toObject() })
        return yield* Effect.fail(error)
      }
    ...
    const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
      ...
      if (!cmd) {
        ...
        available.push(...BUILTIN_COMMANDS) // koda_change - surface built-in session commands in error hint
        available.sort() // koda_change - alphabetical for stable, easy-to-scan output
      ...
      // koda_change start - resume commands import external transcripts
      const fmt = isResumeCommand(input.command)
      if (fmt) {
        return yield* handleResume({ cmdInput: input, format: fmt })
      }
      // koda_change end
      // koda_change start - deprecated review aliases should display a static notice without an LLM turn
      ...
      if (legacy) {
        ...
        const user = yield* kodaSessionPrompt.intake(
          input.sessionID,
          createUserMessage({
            sessionID: input.sessionID,
            messageID: input.messageID,
      ...
      // koda_change start - skill templates run !`cmd`` only when trusted and the kill-switch is off,
      ...
      const task = yield* getModel(taskModel.providerID, taskModel.modelID, input.sessionID) // koda_change
      ...
      // koda_change start
      const variant = kodaWorkflowVariant.resolve({
        command: cmd,
        agent,
        model: taskModel,
        selected: task,
      ...
      kodaSessionProcessor.markReviewTelemetry(templateParts, input.command) // koda_change - mark review commands for completion telemetry
      ...
      const result = yield* prompt({
        ...
        agent: userAgent,
        parts,
        variant: isSubtask ? input.variant : variant, // koda_change
        snapshotInitialization: input.snapshotInitialization, // koda_change
      })
      yield* events.publish(Command.Event.Executed, {
        name: input.command,
...
export const PromptInput = Schema.Struct({
  ...
  // koda_change start - managed product slow-snapshot policy
  ...
  // koda_change end
  // koda_change start - reuse shared editor context schema
...
// koda_change start - retain precise prompt input types for koda callers
...
export class LoopInput extends Schema.Class<LoopInput>("SessionPrompt.LoopInput")({
  sessionID: SessionID,
  snapshotInitialization: Schema.optional(Schema.Literal("wait")), // koda_change
}) {
...
const repositoryCacheNode = RepositoryCache.node // koda_change
...
export const node = LayerNode.make({
  ...
  deps: [
    ...
    RuntimeFlags.node,
    Database.node,
    Question.node, // koda_change
    repositoryCacheNode, // koda_change
  ],
})
