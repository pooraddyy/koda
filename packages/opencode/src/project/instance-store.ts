...
import { context as instanceContext, type InstanceContext } from "./instance-context" // koda_change
  ...
  Effect.gen(function* () {
    ...
    const boot = (input: LoadInput & { directory: string }) =>
      Effect.gen(function* () {
        ...
        // koda_change start - run bootstrap inside the Instance ALS so kodaBootstrap
    ...
    // koda_change start - complete the deferred and drop failed entries on any exit,
    ...
    const disposeEntry = Effect.fnUntraced(function* (directory: string, entry: Entry, ctx: InstanceContext) {
      if (cache.get(directory) !== entry) return false
      // koda_change start - remove disposed entries even when event publication fails
      ...
      return Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            if (previous) {
              // koda_change start - dispose reloads under the previous instance context
    ...
    const disposeAllOnce = Effect.fnUntraced(function* () {
      ...
      // koda_change start - dispose independent worktrees concurrently without interrupting siblings
