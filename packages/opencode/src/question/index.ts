...
import { kodaQuestion } from "@/koda/question" // koda_change
...
export interface Interface {
  ...
  readonly dismissAll: (sessionID: SessionID) => Effect.Effect<void> // koda_change
}
  ...
  Effect.gen(function* () {
    ...
    const ask = Effect.fn("Question.ask")(function* (input: {
      ...
      const info: Request = {
        ...
        blocking: input.blocking, // koda_change
        tool: input.tool,
      }

      // koda_change start
      yield* kodaQuestion.guardFollowup(input.sessionID, () => new RejectedError())
      // koda_change end
      ...
      return yield* Effect.ensuring(
        ...
        // koda_change start - every asked question gets a terminal event when its waiter is interrupted
        kodaQuestion.finalize({
          pending,
          id,
    ...
    // koda_change start - body lives in @/koda/question/kodaQuestion.makeDismissAll
    const dismissAll = kodaQuestion.makeDismissAll({
      state,
    ...
    return Service.of({ ask, reply, reject, list, dismissAll }) // koda_change
  }),
)
