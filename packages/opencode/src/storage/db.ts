...
import { DbPreflight } from "@opencode-ai/core/koda/db-preflight" // koda_change
...
declare const koda_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export function getChannelPath(flags: Pick<DatabaseFlags, "disableChannelDb"> = readRuntimeFlags()) {
  if (["latest", "beta", "prod"].includes(InstallationChannel) || flags.disableChannelDb)
    return path.join(Global.Path.data, "koda.db") // koda_change
  const safe = InstallationChannel.replace(/[^a-zA-Z0-9._-]/g, "-")
  const next = path.join(Global.Path.data, `koda-${safe}.db`) // koda_change
  const prev = path.join(Global.Path.data, `opencode-${safe}.db`) // koda_change
  if (!existsSync(next) && existsSync(prev)) return prev // koda_change
  return next // koda_change
}

export const getPath = (flags?: Pick<DatabaseFlags, "disableChannelDb">) => {
  if (Flag.koda_DB) {
    if (Flag.koda_DB === ":memory:" || path.isAbsolute(Flag.koda_DB)) return Flag.koda_DB
    return path.join(Global.Path.data, Flag.koda_DB)
  }
...
export const Client = Object.assign(
  (flags: DatabaseFlags = readRuntimeFlags()): Client => {
    ...
    DbPreflight.assertWritable(dbPath) // koda_change - actionable error (and self-heal for koda-owned files) instead of an opaque wal_checkpoint crash on read-only db files
    ...
    const entries =
      typeof koda_MIGRATIONS !== "undefined"
        ? koda_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof koda_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
