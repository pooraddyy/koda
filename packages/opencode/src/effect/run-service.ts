...
import { context as instanceContext } from "@/project/instance-context" // koda_change
...
export function attach<A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R> {
  ...
  // koda_change start - bridge legacy AsyncLocalStorage instance context into Effect runtimes
...
export function makeRuntime<I, S, E>(service: Context.Service<I, S>, layer: Layer.Layer<I, E>) {
  ...
  return {
    ...
    // koda_change start - allow koda-owned service runtimes to release persistent resources
    dispose: async () => {
      const current = rt
      rt = undefined
