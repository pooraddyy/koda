import { Effect, Schema, Scope } from "effect" // koda_change - stable object reads do not use Option
import { NonNegativeInt } from "@opencode-ai/core/schema"
import * as path from "path"
import { Readable } from "stream" // koda_change
...
import { Config } from "@/config/config" // koda_change - optional configured reference authorization
import { assertExternalDirectoryEffect } from "./external-directory"
import { Instruction } from "../session/instruction"
import { isPdfAttachment, sniffAttachmentMime } from "@/util/media"
// koda_change start
import * as Encoding from "../koda/encoding"
import { kodaReference } from "@/koda/reference/contains"
import * as kodaConfiguredReference from "@/koda/reference"
import { kodaReadObject } from "@/koda/tool/read-object"
import * as Extract from "../koda/tool/read-extract"
import * as TextStream from "../koda/text-stream"
// koda_change end

const DEFAULT_READ_LIMIT = 2000
const MAX_LINE_LENGTH = 2000
// koda_change start - report the safe Unicode slice length
...
>(
  ...
  Effect.gen(function* () {
    ...
    // koda_change start - authorize missing paths without enumerating sibling names
    ...
    // koda_change start - extracted formats and text consume the authorized open object
    const lines = Effect.fn("ReadTool.lines")(
      (file: kodaReadObject.File, opts: { limit: number; offset: number }, abort: AbortSignal) =>
        Effect.tryPromise({
          try: async (signal) => {
    ...
    const isBinaryFile = (filepath: string, bytes: Uint8Array) => {
      ...
      // koda_change start - UTF-16/32 BOM: NUL bytes are legitimate, skip the NUL/control-char heuristic
    ...
    const run = Effect.fn("ReadTool.execute")(function* (
      ...
      // koda_change start - resolve V1 configured references without introducing a Core location-layer dependency
      const config = yield* Effect.serviceOption(Config.Service)
      const references =
        config._tag === "Some"
          ? kodaConfiguredReference.resolveAll({
          ...
          : []
      // koda_change end
      // koda_change start - fail before read authorization when the target is missing
      ...
      // koda_change start - directory mentions expose only a bound listing, never child file bodies
      if (info.type === "Directory") {
        ...
        const explicit =
          ...
          (yield* kodaReference.path(fs, ctx.extra["referenceRoot"], target))
        const referenced =
          explicit ||
          (yield* kodaReference.contains({ fs, references, target }))
        ...
        // koda_change start - reject any canonical path change after permission approval
        ...
        const limit = Math.max(1, params.limit ?? DEFAULT_READ_LIMIT) // koda_change - prevent zero-limit loops
      ...
      // koda_change start - authorize metadata, then bind every content read to the same reopened object
      const file = yield* kodaReadObject.file(requested)
      const explicit =
        typeof ctx.extra?.["referenceRoot"] === "string" &&
        (yield* kodaReference.path(fs, ctx.extra["referenceRoot"], file.target))
      const referenced =
        explicit ||
        (yield* kodaReference.contains({ fs, references, target: file.target }))
...
// koda_change start - extracted formats use native readers; ordinary text is supplied by FSUtil above
async function collect(stream: Readable, opts: { limit: number; offset: number }) {
  ...
  try {
    for await (const text of rl) {
      ...
      // koda_change start - keep truncated output valid Unicode
