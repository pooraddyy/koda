import { Effect, Fiber, Stream } from "effect" // koda_change - Fiber
import os from "os"
...
import { normalizeUrls } from "@/koda/util/url" // koda_change
import { CommandTimeout } from "@/koda/command-timeout" // koda_change
import { heredocs } from "@/koda/tool/shell-heredoc" // koda_change
import { unparsed } from "@/koda/tool/shell-unparsed" // koda_change
import { ChildProcess } from "effect/unstable/process"
import { ChildProcessSpawner } from "effect/unstable/process/ChildProcessSpawner"
import { ShellPrompt, type Parameters } from "./shell/prompt"
import { BashArity } from "@/permission/arity"
import { mutates as mutatesGit } from "@/koda/sandbox/git" // koda_change
...
const ask = Effect.fn("ShellTool.ask")(function* (
  ...
  scan: Scan,
  command: string,
  metadata: ReturnType<typeof heredocs>, // koda_change
  description?: string, // koda_change
) {
  // koda_change
  if (scan.dirs.size > 0) {
    ...
    yield* ctx.ask({
      ...
      patterns: globs,
      always: globs,
      // koda_change start - retain read classification alongside upstream permission context
      metadata: {
        command,
  ...
  yield* ctx.ask({
    ...
    metadata: { command: normalizeUrls(command), ...(description ? { description } : {}), ...metadata }, // koda_change
  })
})

// koda_change start - share bash permission scanning with koda interactive terminal
type PermissionInput = {
  command: string
  cwd: string
  shell: string
  description?: string
  escalate?: boolean // koda_change
}
...
export const ShellPermission = Effect.gen(function* () {
  ...
  const collect = Effect.fn("ShellTool.collect")(function* (
    ...
    // koda_change start - fail closed on commands the grammar failed to parse (#12326)
  ...
  const check = Effect.fn("ShellTool.permission")(function* (ctx: Tool.Context, input: PermissionInput) {
    ...
    yield* Effect.scoped(
      Effect.gen(function* () {
        ...
        const metadata = heredocs(tree.rootNode, ShellID.toKind(Shell.name(input.shell))) // koda_change
        if (!containsPath(input.cwd, instance)) {
          scan.dirs.add(input.cwd)
          scan.access = "unknown"
        }
        yield* ask(ctx, scan, input.command, metadata, input.description) // koda_change
        const gitMutation = commands(tree.rootNode).some((node) => mutatesGit(node.text))
        if (input.escalate && gitMutation) {
          yield* ctx.ask({
            permission: "sandbox_escalation", // koda_change
  ...
  // koda_change start - expose the tree-sitter scan (sub-command patterns + external-dir globs) for skill-shell batching
  ...
  return { ask: check, resolve, decompose } // koda_change - decompose for skill-shell
})
// koda_change end

function cmd(shell: string, command: string, cwd: string, env: NodeJS.ProcessEnv) {
  if (process.platform === "win32" && Shell.ps(shell)) {
    // koda_change start - PowerShell args
...
export const ShellTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const permission = yield* ShellPermission // koda_change
    ...
    const shellEnv = Effect.fn("ShellTool.shellEnv")(function* (ctx: Tool.Context, cwd: string) {
      ...
      return modelEnv(extra.env) // koda_change - model shells must not inherit backend credentials
    })
    ...
    const run = Effect.fn("ShellTool.run")(function* (
      ...
      const code: number | null = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const reader = yield* Effect.forkScoped(
            // koda_change - keep the fiber so trailing output can be drained
          ...
          const timeout = Effect.sleep(`${CommandTimeout.duration(input.timeout)} millis`) // koda_change
          ...
          // koda_change start - closing the scope interrupts the reader fiber, which can drop
      ...
      return {
        title: input.description, // koda_change - UI shows the model's description, command goes in metadata
        metadata: {
          output: last || preview(output),
          exit: code,
          description: input.description, // koda_change
          truncated: cut,
        ...
        return {
          ...
          execute: (params: Parameters, ctx: Tool.Context) =>
            Effect.gen(function* () {
              ...
              const timeout = CommandTimeout.clamp(params.timeout ?? defaultTimeoutMs).timeout // koda_change
              ...
              yield* permission.ask(ctx, {
                ...
                escalate: sandboxed, // koda_change
              }) // koda_change
                ...
                run(
                  {
                    ...
                    timeout,
                    description: params.description ?? params.command, // koda_change
                  },
                  ctx,
                ),
              )
