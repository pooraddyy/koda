...
import type { AuthOAuthResult, Hooks } from "@koda/plugin"
...
const layer: Layer.Layer<Service, never, Auth.Service | Plugin.Service | ModelCache.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const callback = Effect.fn("ProviderAuth.callback")(function* (
      ...
      // koda_change start - Update telemetry identity on koda auth
      if (input.providerID === "koda") {
...
export const node = LayerNode.make({ service: Service, layer: layer, deps: [Auth.node, Plugin.node, ModelCache.node] }) // koda_change
