...
import * as ModelsDev from "./models" // koda_change - assemble dynamic koda models around upstream core catalog
...
import { optional, optionalOmitUndefined } from "@opencode-ai/core/schema" // koda_change
...
// koda_change start
import {
  koda_BUNDLED_PROVIDERS,
  kodaCustomLoaders,
  koda_MODEL_SCHEMA_EXTENSIONS,
  patchModelsDevModel as patchkodaModel,
  patchConfigModel as patchkodaConfigModel,
  customProviderVariants,
  patchCustomLoaderResult,
  patchkodaProviderPrivacy,
  kodaSmallModelPriority,
  buildTimeoutSignal,
  requestTimeout,
  wrapFirstByte,
} from "@/koda/provider/provider"
import * as ModelsRefresh from "@/koda/provider/models-refresh"
import { bedrockAuth, providerKey, vertexAuth, vertexCredentials, vertexOptions } from "@/koda/provider/cloud-auth"
// koda_change end
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  ...koda_BUNDLED_PROVIDERS, // koda_change
}
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    azure: Effect.fnUntraced(function* (provider: Info) {
      ...
      // koda_change start - prefer explicit Azure endpoint over resource name to avoid conflicting SDK options
      const endpoint = iife(() => {
        return [
      ...
      if (!resource && !endpoint) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              "Azure resource name or endpoint is missing. Set AZURE_RESOURCE_NAME, AZURE_OPENAI_RESOURCE_NAME, AZURE_OPENAI_ENDPOINT, or reconnect the azure provider.", // koda_change
            )
          },
        }
      }
      ...
      return {
        ...
        options: {
          ...(endpoint ? { baseURL: endpoint } : { resourceName: resource }), // koda_change
        },
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      const stored = bedrockAuth(auth) // koda_change
      const env = yield* dep.env()

      // Region precedence: 1) config file, 2) stored credentials, 3) env var, 4) default // koda_change
      const configRegion = providerConfig?.options?.region
      const envRegion = env["AWS_REGION"]
      const defaultRegion = configRegion ?? stored?.region ?? envRegion ?? "us-east-1" // koda_change
      ...
      // koda_change start - pass stored bearer tokens directly without leaking them into process.env
      ...
      const providerOptions: Record<string, any> = {
        ...
        ...(stored ? { credentialProvider: async () => stored.credentials } : {}), // koda_change
      }

      // Only use credential chain if no bearer token exists
      // Bearer token takes precedence over credential chain (profiles, access keys, IAM roles, web identity tokens)
      // koda_change start - stored static credentials use the credential provider above
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://koda.ai/",
            "X-Title": "koda Code", // koda_change
            "X-BILLING-INVOKE-ORIGIN": "koda", // koda_change
          },
        },
      }),
    vercel: () =>
    ...
    "google-vertex": Effect.fnUntraced(function* (provider: Info) {
      const env = yield* dep.env()
      const stored = vertexAuth(yield* dep.auth("google-vertex")) // koda_change
      ...
      return {
        ...
        options: {
          project,
          location,
          ...(stored ? vertexCredentials(stored.credentials) : {}), // koda_change
          fetch: async (input: RequestInfo | URL, init?: RequestInit) => {
            const { GoogleAuth } = await import("google-auth-library")
            // koda_change start - authenticate OpenAI-compatible Vertex endpoints with stored credentials
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `koda/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`, // koda_change
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `koda auth cloudflare-ai-gateway`.",
        )
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "koda Code", // koda_change
          },
        },
      }),
    koda: () =>
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, koda auth, or provider options.`, // koda_change
            )
          },
        }
      }
...
export const Model = Schema.Struct({
  ...
  ...koda_MODEL_SCHEMA_EXTENSIONS, // koda_change
...
export const Info = Schema.Struct({
  id: ProviderV2.ID,
  name: Schema.String,
  description: optionalOmitUndefined(Schema.String), // koda_change
  source: Schema.Literals(["env", "config", "custom", "api"]),
  env: Schema.Array(Schema.String),
  key: optional(Schema.String),
  metadata: optionalOmitUndefined(ProviderMetadata), // koda_change
  options: Schema.Record(Schema.String, Schema.Any),
...
export const ListResult = Schema.Struct({
  ...
  failed: Schema.Array(Schema.String), // koda_change
})
...
export class ModelNotFoundError extends Schema.TaggedErrorClass<ModelNotFoundError>()("ProviderModelNotFoundError", {
  ...
  modelsEmpty: Schema.optional(Schema.Boolean), // koda_change
...
function fromModelsDevModel(provider: ModelsDev.Provider, model: ModelsDev.Model): Model {
  ...
  Object.assign(base, patchkodaModel(provider.id, model)) // koda_change
  const variants = ProviderTransform.reasoningVariants(model, base) ?? ProviderTransform.variants(base) // koda_change

  return {
    ...base,
    variants: mapValues(variants, (v) => v), // koda_change
  }
}
...
export function fromModelsDevProvider(provider: ModelsDev.Provider): Info {
  ...
  return {
    ...
    source: "custom",
    name: provider.name,
    description: provider.description, // koda_change
    env: [...(provider.env ?? [])],
    options: {},
    models,
  }
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const [providerID, provider] of configProviders) {
          if (!provider) continue // koda_change - null entries are transient delete sentinels
          ...
          for (const [modelID, model] of Object.entries(provider.models ?? {})) {
            if (!model) continue // koda_change - null entries are transient delete sentinels
            ...
            const parsedModel: Model = {
              ...
              // variants: {}, // koda_change, moved into patchkodaConfigModel
              ...patchkodaConfigModel(model, existingModel), // koda_change
            }
            // koda_change start
            ...
            parsedModel.variants = mapValues(
              pickBy(merged, (v): v is NonNullable<typeof v> => !!v && !v.disabled), // koda_change - drop null delete sentinels
              (v) => omit(v, ["disabled"]),
            )
        ...
        // koda_change start - load auths before env so OAuth plugins can override inherited credentials
        ...
        for (const [id, provider] of Object.entries(database)) {
          ...
          if (disabled.has(providerID)) continue
          // koda_change start - prefer explicit OAuth auth over inherited env credentials
          if (
        ...
        for (const [id, provider] of Object.entries(auths)) {
          ...
          if (provider.type === "api") {
            mergeProvider(providerID, {
              source: "api",
              key: providerKey(providerID, provider), // koda_change - keep structured credentials provider-specific
            })
          }
        }
        ...
        // koda_change start - resolve env once for patchCustomLoaderResult (azure env fallback)
        const kodaEnv = yield* env.all()
        // koda_change end
        for (const [id, fn] of Object.entries({ ...custom(dep), ...kodaCustomLoaders(dep) })) {
          // koda_change
          const providerID = ProviderV2.ID.make(id)
          if (disabled.has(providerID)) continue
          ...
          if (result) patchCustomLoaderResult(id, result, kodaEnv) // koda_change
        ...
        for (const [id, provider] of configProviders) {
          if (!provider) continue // koda_change - null entries are transient delete sentinels
          const providerID = ProviderV2.ID.make(id)
          // koda_change start - keep OAuth plugin source when config and Codex auth coexist
          const oauth =
        ...
        patchkodaProviderPrivacy(providers[ProviderV2.ID.make("koda")], cfg) // koda_change

        const gitlab = ProviderV2.ID.make("gitlab")
        if (discoveryLoaders[gitlab] && providers[gitlab] && isProviderAllowed(gitlab)) {
          // koda_change start - keep discovery failures visible instead of swallowing them
    ...
    yield* ModelsRefresh.watch(state) // koda_change
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        vertexOptions(model.providerID, model.api.npm, options) // koda_change - hydrate stored credentials
        ...
        options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
          ...
          const timeout = buildTimeoutSignal(options) // koda_change - use cancellable timeout for connection phase
          // koda_change start - extend the same deadline to the first response byte
          ...
          if (timeout.signal) signals.push(timeout.signal) // koda_change
          if (firstByteCtl) signals.push(firstByteCtl.signal) // koda_change

          const combined = signals.length === 0 ? null : signals.length === 1 ? signals[0] : AbortSignal.any(signals)
          if (combined) opts.signal = combined

          // koda_change start - clear connection-phase timeout once headers arrive
          try {
            ...
            timeout.clear()
            // koda_change start - hand the remaining deadline to the first-byte guard
    ...
    const getModel = Effect.fn("Provider.getModel")(function* (providerID: ProviderV2.ID, modelID: ModelV2.ID) {
      ...
      if (!provider) {
        ...
        const suggestions = catalogProvider
          ...
          : fuzzysort
              ...
              .map((m) => m.target)
        const empty = false // koda_change
        return yield* new ModelNotFoundError({ providerID, modelID, suggestions, modelsEmpty: empty }) // koda_change
      }
      ...
      if (!info) {
        ...
        const empty = Object.keys(provider.models).length === 0 // koda_change
        return yield* new ModelNotFoundError({ providerID, modelID, suggestions, modelsEmpty: empty }) // koda_change
      }
      return info
    })
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      // koda_change start - koda's auto model is an ID, while upstream priorities are model families.
      const kodaPriority = kodaSmallModelPriority(providerID)
      if (kodaPriority) {
        for (const id of kodaPriority) {
          const model = provider.models[id]
      ...
      // koda_change start - fall back to koda's auto small model
      const kodaFallback = s.providers[ProviderV2.ID.make("koda")] ?? s.catalog[ProviderV2.ID.make("koda")]
      if (kodaFallback?.models["koda-auto/small"]) return kodaFallback.models["koda-auto/small"]
      // koda_change end
