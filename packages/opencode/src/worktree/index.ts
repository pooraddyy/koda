...
import { WorktreeCleanup } from "@/koda/worktree-cleanup" // koda_change
import { clearPtys } from "@/koda/worktree/pty-cleanup" // koda_change
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const boot = Effect.fnUntraced(function* (info: Info, startCommand?: string) {
      ...
      // koda_change start - signal full readiness once setup also completes
      GlobalBus.emit("event", {
        directory: info.directory,
        project: ctx.project.id,
        workspace: workspaceID,
    ...
    // koda_change start - use koda cleanup helper for slow Windows handle release
    ...
    const remove = Effect.fn("Worktree.remove")(function* (input: RemoveInput) {
      const ctx = yield* InstanceState.context
      const workspaceID = yield* InstanceState.workspaceID // koda_change
      if (ctx.project.vcs !== "git") {
        return yield* new NotGitError({ message: "Worktrees are only supported for git projects" })
      }
      ...
      if (!entry?.path) {
        yield* clearPtys(directory, workspaceID) // koda_change
      ...
      yield* clearPtys(entry.path, workspaceID) // koda_change
