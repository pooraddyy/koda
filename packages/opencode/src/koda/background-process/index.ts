...
import { model as modelEnv } from "@/koda/process/env"
import { SessionID } from "@/session/schema"
import { PowerShell } from "@/koda/shell/shell"
...
export namespace BackgroundProcess {
  ...
  async function claim(shared: Shared) {
    ...
    const pending = Flock.acquire(shared.key, {
      ...
      .catch((err) => {
        log.debug("persistent process scope is managed by another koda process", { err, scope: shared.key })
        return false
      })
      .finally(() => {
  ...
  function infer() {
    return Flag.koda_CLIENT === "cli" && process.env.koda_BACKGROUND_PROCESS_PORTS === "true"
  }
  ...
  function env(id?: ID, token?: string) {
    const result: NodeJS.ProcessEnv = modelEnv({
      TERM: "dumb",
      ...(id ? { koda_BACKGROUND_PROCESS_ID: id } : {}),
      ...(token ? { koda_BACKGROUND_PROCESS_TOKEN: token } : {}),
    })
    delete result.koda_BACKGROUND_PROCESS_PORTS
    return result
  }
  ...
  async function linux(active: Active): Promise<Probe> {
    ...
    if (leader && pgrp(leader) === pid) {
      ...
      if (data?.toString("utf8").split("\0").includes(`koda_BACKGROUND_PROCESS_TOKEN=${token}`)) return "owned"
    }
  ...
  async function launch(state: State, input: StartInput, id = ID.ascending()) {
    ...
    if (logpath) {
      ...
      if (!(await claim(state.shared)))
        throw new Error("Persistent processes for this project are managed by another koda process")
  ...
  kodaShutdown.register(() => runtime.dispose())
