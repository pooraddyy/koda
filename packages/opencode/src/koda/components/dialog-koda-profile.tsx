...
import type { kodaProfile, kodaBalance } from "@koda/koda-gateway"

interface DialogkodaProfileProps {
  profile: kodaProfile
  balance: kodaBalance | null
  currentOrgId?: string | null
}

export function DialogkodaProfile(props: DialogkodaProfileProps) {
  ...
  const usageUrl = props.currentOrgId
    ? `https://app.koda.ai/organizations/${props.currentOrgId}/usage-details`
    : "https://app.koda.ai/usage"

  return (
    <box paddingLeft={2} paddingRight={2} gap={1}>
      <box flexDirection="row" justifyContent="space-between">
        <text attributes={TextAttributes.BOLD} fg={theme.text}>
          koda Gateway Profile
        </text>
