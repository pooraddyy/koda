import { fetchkodaEmbeddingModelCatalog, resolvekodaGatewayBaseUrl } from "@koda/koda-gateway"
import type { Config, IndexingConfig, kodaEmbeddingModelCatalog } from "@koda/sdk/v2"
...
export async function loadkodaEmbeddingModels(onError?: (message: string) => void) {
  const endpoint = new URL("embedding-models", resolvekodaGatewayBaseUrl()).toString()
  log.info("loading koda embedding model catalog", { endpoint })
  const catalog = await fetchkodaEmbeddingModelCatalog({
    onError: (issue) => {
      log.warn("failed to load koda embedding model catalog", {
        code: issue.code,
        status: issue.status,
  ...
  log.info("loaded koda embedding model catalog", {
...
export function kodaModelOptions(catalog?: kodaEmbeddingModelCatalog) {
...
export function currentkodaModel(catalog: kodaEmbeddingModelCatalog | undefined, model?: string | null) {
