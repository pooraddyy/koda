...
import { DEFAULT_VECTOR_STORE, isFileExtension, parseFileExtensions } from "@koda/koda-indexing/config"
import { useSync } from "@tui/context/sync"
import { useToast } from "@tui/ui/toast"
import { createEffect, createMemo, createResource, createSignal, Show } from "solid-js"
import { reconcile } from "solid-js/store"
import type { IndexingConfig, Config } from "@koda/sdk/v2"
import * as Log from "@opencode-ai/core/util/log"
import { haskodaIndexingAuth, resolvekodaIndexingAuth, shouldDefaultIndexingTokoda } from "../indexing-auth"
import {
...
function haskodaAuth(sync: ReturnType<typeof useSync>, scope: IndexingScope, indexing: IndexingConfig): boolean {
  const provider = sync.data.provider_next.all.find((item) => item.id === "koda")
  const config = indexingScopeConfig(scope, sync.data.config, sync.data.globalConfig, indexing)
  return haskodaIndexingAuth({ config, provider })
}
...
function defaultIndexing(
  ...
  const provider = sync.data.provider_next.all.find((item) => item.id === "koda")
  const config = indexingScopeConfig(scope, sync.data.config, sync.data.globalConfig, indexing)
  const auth = resolvekodaIndexingAuth({ config, provider })
  if (!shouldDefaultIndexingTokoda({ ...global, ...indexing }, auth)) return indexing
  return { ...indexing, provider: "koda", model: null, dimension: null }
}
...
function ProviderSelect(props: SubDialogProps) {
  ...
  const options: DialogSelectOption<EmbeddingProvider>[] = (
    ...
    .filter(([value]) => value !== "koda" || haskodaAuth(sync, props.scope, indexing) || indexing.provider === "koda")
    .map(([value, title]) => ({
      value,
      title,
...
function KodaModelSelect(props: SubDialogProps) {
  const dialog = useDialog()
  const sync = useSync()
  const sdk = props.useSDK()
  const toast = useToast()
  ...
  const [catalog] = createResource(() => loadkodaEmbeddingModels(setError))
  ...
  createEffect(() => {
    ...
    seen.state = state
    log.info("koda embedding model resource changed", {
      state: catalog.state,
      models: cfg?.models.length ?? 0,
      current: currentkodaModel(cfg, indexing.model),
      defaultModel: cfg?.defaultModel || undefined,
      scope: props.scope,
    })
  })
  const options = createMemo(() => kodaModelOptions(catalog()))
  const current = createMemo(() => currentkodaModel(catalog(), indexing.model))

  return (
    <DialogSelect
      title="koda Embedding Model"
      options={options()}
      current={current()}
      renderFilter={(catalog()?.models.length ?? 0) > 0}
      onSelect={async (option) => {
        if (!option.value || !catalog()?.models.some((model) => model.id === option.value)) return
        log.info("selected koda embedding model", { model: option.value, scope: props.scope })
...
function ScopeSelect(props: DialogIndexingProps & { scope: IndexingScope }) {
  ...
  const options: DialogSelectOption<IndexingScope>[] = [
    ...
    { value: "project", title: "Project", description: "Stored in this repo's .koda config" },
  ]
  ...
  const options = createMemo<DialogSelectOption<string>[]>(() => {
    ...
    const result: DialogSelectOption<string>[] = [
      ...
      },
      {
        ...
        category: "Embedding",
        description: mark(
          indexing.provider === "koda" ? (indexing.model ?? "koda catalog") : (indexing.model ?? "default"),
          [["model"]],
        ),
      },
      {
      ...
      onSelect={async (option) => {
        ...
        switch (option.value) {
          ...
          case "providerSettings":
            ...
            break
          case "model": {
            if (indexing.provider === "koda") {
              dialog.replace(() => (
                <KodaModelSelect useSDK={props.useSDK} scope={scope()} indexing={indexing} raw={raw} />
              ))
              break
            }
          ...
          case "dimension": {
            if (indexing.provider === "koda") break
