...
export namespace CloudCatalog {
  ...
  export class Service extends Context.Service<Service, Interface>()("@koda/CloudCatalog") {}
  ...
  export const layer = (options: Options = {}) => {
    ...
    const request = Effect.fn("CloudCatalog.request")(function* <A>(url: string, input: Input, schema: z.ZodType<A>) {
        ...
        try: () =>
          new Request(url, {
            headers: {
              ...
              ...buildkodaHeaders(
                undefined,
                input.organizationID ? { kodaOrganizationId: input.organizationID } : undefined,
              ),
              "X-koda-FEATURE": "koda-cli",
          ...
          new CatalogError({
            kind: "schema",
            message: "koda catalog URL is invalid",
          }),
      })
      const response = yield* Effect.tryPromise({
        try: () => fetcher(req),
          ...
          new CatalogError({
            kind: "network",
            message: "Unable to reach the koda model catalog",
          }),
      })
      if (!response.ok) {
        ...
        return yield* Effect.fail(
          new CatalogError({
            ...
            message:
              kind === "auth"
                ? "koda credentials or organization were rejected by the model catalog"
                : "The koda model catalog is unavailable",
          }),
        )
      }
      ...
      const body = yield* Effect.tryPromise({
        try: () => readBoundedJson(response),
          ...
          new CatalogError({
            kind: "schema",
            message: "The koda model catalog returned an invalid response",
          }),
      })
    ...
    const base = Effect.fn("CloudCatalog.base")(function* (input: Input) {
      const raw = env.koda_API_URL?.trim()
      const fallback = raw || DEFAULT_koda_API_URL
      const value = getkodaUrlFromToken(fallback, Redacted.value(input.token))
      return yield* Effect.try({
        try: () => {
          const url = new URL(resolvekodaOpenRouterBaseUrl({ baseURL: value }))
          ...
          new CatalogError({
            kind: "schema",
            message: "koda catalog URL must be secure",
          }),
      })
    })
