...
export namespace CloudDefaults {
  ...
  export type ModelStateInfo = kodaModelState.State

  export interface ModelStateInterface {
    readonly get: () => Effect.Effect<ModelStateInfo>
  }

  export class ModelState extends Context.Service<ModelState, ModelStateInterface>()("@koda/CloudModelState") {}

  export const modelStateLayer = Layer.succeed(
    ModelState,
    ModelState.of({ get: () => Effect.promise(() => kodaModelState.get()) }),
  )
  ...
  function normalize(value: string) {
    return value.startsWith("koda/") ? value.slice("koda/".length) : value
  }
  ...
  export const resolve = Effect.fn("CloudDefaults.resolve")(function* (input: Input = {}) {
    ...
    return yield* Effect.fail(
      new ResolutionError({
        kind: "model",
        message: "The koda model catalog has no available default model",
      }),
    )
  })
}
