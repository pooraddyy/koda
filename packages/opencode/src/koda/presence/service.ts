...
import { EventServiceClient } from "@/koda/event-service/client"
import { koda_EVENT_SERVICE_URL } from "@koda/koda-gateway"
...
const log = Log.create({ service: "koda-viewers" })
...
function extract(auth: Auth.Info | undefined): { token: string | undefined; identity: string | undefined } {
  const envKey = process.env.koda_API_KEY?.trim()
...
export namespace kodaViewers {
  export interface Interface {
    readonly update: (snapshot: ViewerSnapshot) => Effect.Effect<void>
    readonly invalidateAuth: () => Effect.Effect<void>
  }

  export class Service extends Context.Service<Service, Interface>()("@koda/kodaViewers") {}

  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const auth = yield* Auth.Service
      const kodaSessions = (yield* Effect.promise(() => import("@/koda-sessions/koda-sessions"))).kodaSessions

      const platform = inferPlatform()
      const killSwitch = process.env.koda_DISABLE_PRESENCE === "1"
      // Same endpoint the server envelope hands kodaClaw; koda_EVENT_SERVICE_URL
      // is a presence-specific override on top of the gateway's EVENT_SERVICE_URL.
      const url = process.env.koda_EVENT_SERVICE_URL || koda_EVENT_SERVICE_URL
      ...
      function pushAttached() {
        ...
        if (!sameArr(union, s.prevAttached)) {
          ...
          kodaSessions.setAttachedSessions(union)
        }
      }
      ...
      const readAuth = auth.get("koda").pipe(Effect.orElseSucceed((): Auth.Info | undefined => undefined))

      const update = Effect.fn("kodaViewers.update")(function* (snapshot: ViewerSnapshot) {
      ...
      const invalidateAuth = Effect.fn("kodaViewers.invalidateAuth")(function* () {
        disconnectClient()
      ...
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          ...
          disconnectClient()
          s.viewers.clear()
          kodaSessions.setAttachedSessions([])
        }),
      )
