...
const log = Log.create({ service: "koda-watcher" })

export namespace kodaWatcher {
  export interface Interface {
    readonly init: () => Effect.Effect<void>
  }

  export class Service extends Context.Service<Service, Interface>()("@koda/Watcher") {}

  // The terminal runtime eagerly warms the location stack for live branch-label
  // updates. The experimental disable flag remains available for diagnosis.
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return Service.of({
        init: Effect.fn("kodaWatcher.init")(function* () {
  ...
  export const defaultLayer = Layer.unwrap(
    Effect.gen(function* () {
      if (!eager() || (yield* Flag.koda_EXPERIMENTAL_DISABLE_FILEWATCHER.pipe(Effect.orElseSucceed(() => false))))
