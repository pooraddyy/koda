...
/**
 * Read project ID from .koda/config.json, falling back to .koda/config.json
...
async function getProjectIdFromConfig(directory: string): Promise<string | undefined> {
  // Check .koda first, then legacy .koda
  for (const dir of [".koda", ".koda"]) {
...
/**
 * Resolve project ID with priority: .koda/config.json -> .koda/config.json -> git origin URL
 * @returns Normalized project ID or undefined
 */
async function resolveProjectId(dir: string): Promise<string | undefined> {
  // Priority 1: .koda/config.json (falls back to .koda/config.json)
...
export namespace kodaProjectID {
  export interface Interface {
    readonly get: () => Effect.Effect<string | undefined>
  }

  export class Service extends Context.Service<Service, Interface>()("@koda/kodaProjectID") {}
...
const { runPromise } = makeRuntime(kodaProjectID.Service, kodaProjectID.defaultLayer)

/**
 * Get the project ID for the current Instance context (cached per-project)
 * @returns Normalized project ID or undefined
 */
export async function getkodaProjectId(): Promise<string | undefined> {
