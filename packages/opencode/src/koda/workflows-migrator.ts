...
export namespace WorkflowsMigrator {
  ...
  const koda_WORKFLOWS_DIRS = [".koda/workflows"]
  const globalWorkflowsDirs = () => [path.join(home(), ".koda", "workflows")]

  export interface kodaWorkflow {
    name: string
    path: string
    content: string
  ...
  export async function discoverWorkflows(
    ...
    if (!skipGlobalPaths) {
      // Global terminal Koda workflows load before project-local workflows.
      for (const dir of globalWorkflowsDirs()) {
        workflows.push(...(await loadWorkflowsFromDir(dir, "global", undefined, warnings)))
      }
    }

    // Project workflows load last and take precedence over global definitions.
    for (const dir of koda_WORKFLOWS_DIRS) {
  ...
  export function convertToCommand(workflow: kodaWorkflow): ConfigCommandV1.Info {
    return {
  ...
  export async function migrate(options: {
    ...
    const workflowsByName = new Map<string, kodaWorkflow>()
