...
export function matchLegacykodaOpenApi(input: Record<string, unknown>) {
  rebrand(input)
  ...
  const body = spec.paths?.["/koda/organization"]?.post?.requestBody?.content?.["application/json"]?.schema
  ...
  const profile = json("/koda/profile")?.schema?.properties
  const pass = profile?.kodaPass?.properties
  ...
  if (profile?.kodaPass) profile.kodaPass = nullable(profile.kodaPass)
  if (profile?.currentOrgId) profile.currentOrgId = nullable(profile.currentOrgId)

  const sessions = json("/koda/cloud-sessions")?.schema?.properties
  ...
  const claw = json("/koda/claw/status")?.schema?.properties
  ...
  const credentials = json("/koda/claw/chat-credentials")
  ...
  const fim = spec.paths?.["/koda/fim"]?.post?.responses
  if (!fim) return
  fim["200"] = {
