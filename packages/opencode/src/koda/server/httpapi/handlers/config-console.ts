...
import { kodaConfigOverlay } from "@/koda/config/overlay"
import { kodaConfigWriter } from "@/koda/config/writer"
import { kodaConfigSources } from "@/koda/config/sources"
import { kodaModelState } from "@/koda/config/model-state"
import { ConfigRules } from "@/koda/server/routes/config-rules"
import { kodaKeybinds } from "@/koda/tui/keybinds"
import { kodaTuiConfig } from "@/koda/tui/config"
...
export const configConsoleHandlers = HttpApiBuilder.group(InstanceHttpApi, "config-console", (handlers) =>
  Effect.gen(function* () {
    ...
    const overlayUpdate = Effect.fn("ConfigConsoleHttpApi.overlayUpdate")(function* (ctx: {
      ...
      const patch = kodaConfigOverlay.patch(body)
    ...
    const modelState = Effect.fn("ConfigConsoleHttpApi.modelState")(function* () {
      return yield* Effect.promise(() => kodaModelState.get())
    })
    ...
    const modelStateUpdate = Effect.fn("ConfigConsoleHttpApi.modelStateUpdate")(function* (ctx: {
      ...
      return yield* Effect.promise(() =>
        kodaModelState.update({ favorite: ctx.payload.favorite?.map((item) => ({ ...item })) }),
      )
    })

    const tuiConfigGet = Effect.fn("ConfigConsoleHttpApi.tuiConfigGet")(function* () {
      const instance = yield* InstanceState.context
      return yield* Effect.promise(() => kodaTuiConfig.get({ directory: instance.directory }))
    })

    const tuiKeybindList = Effect.fn("ConfigConsoleHttpApi.tuiKeybindList")(function* () {
      return { keybinds: kodaKeybinds.list() }
    })
