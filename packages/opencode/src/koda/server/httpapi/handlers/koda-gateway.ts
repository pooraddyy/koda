...
import {
  ...
  clearModesCache,
  fetchBalance,
  fetchkodaNotifications,
  fetchkodaPassState,
  fetchOrganizationModes,
  fetchProfile,
} from "@koda/koda-gateway"
import { DIRECT_FIM_ENV, requestMistralFim, resolveFimTarget } from "@koda/koda-gateway/fim"
import { DIRECT_EDIT_ENV, extractFencedBody, resolveEditTarget } from "@koda/koda-gateway/edit"
import { buildMercuryEditPrompt } from "@koda/koda-gateway/edit-prompt"
import { buildkodaHeaders } from "@koda/koda-gateway"
...
import { AudioTranscriptionsBody, ClawStatus, CloudSessionImportError, EditBody, FimBody } from "../groups/koda-gateway"
...
const log = Log.create({ service: "koda-gateway" })
...
export const kodaGatewayHandlers = HttpApiBuilder.group(InstanceHttpApi, "koda", (handlers) =>
  Effect.gen(function* () {
    ...
    const profile = Effect.fn("kodaGatewayHttpApi.profile")(function* () {
      const info = yield* auth.get("koda").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
      if (!info || info.type !== "oauth") return yield* Effect.fail(new HttpApiError.Unauthorized({}))

      const currentOrgId = info.accountId ?? null
      const [profile, balance, kodaPass] = yield* Effect.tryPromise({
        try: () =>
          Promise.all([
    ...
    const authStatus = Effect.fn("kodaGatewayHttpApi.authStatus")(function* () {
      const info = yield* auth.get("koda").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
    ...
    const proxyAuth = Effect.fn("kodaGatewayHttpApi.proxyAuth")(function* () {
      const info = yield* auth.get("koda").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      return {
        auth: info,
        token: getToken(info),
    ...
    const modes = Effect.fn("kodaGatewayHttpApi.modes")(function* () {
      const info = yield* auth.get("koda").pipe(Effect.catch(() => Effect.succeed(undefined)))
    ...
    const fim = Effect.fn("kodaGatewayHttpApi.fim")(function* (ctx: { payload: typeof FimBody.Type }) {
      const target = resolveFimTarget(ctx.payload.provider, ctx.payload.model)
      const info = target.provider === "koda" ? yield* proxyAuth() : undefined
      const token = yield* Effect.gen(function* () {
        if (target.provider === "koda") return info?.token
      ...
      if (target.provider === "koda" && !info?.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
          ...
          const run = async (url: string): Promise<Response> => {
            ...
            return fetch(url, {
              ...
              headers: {
                ...
                ...(target.provider === "koda"
                  ? buildkodaHeaders(undefined, { kodaOrganizationId: info?.organizationId })
                  : {}),
                ...(target.provider === "koda" ? { [HEADER_FEATURE]: "autocomplete" } : {}),
              },
              signal,
              body: JSON.stringify({
                model: target.model,
    ...
    const edit = Effect.fn("kodaGatewayHttpApi.edit")(function* (ctx: { payload: typeof EditBody.Type }) {
      const target = resolveEditTarget(ctx.payload.provider, ctx.payload.model)
      if (target.provider === "koda" && !target.url) {
        return yield* Effect.fail(new HttpApiError.BadRequest({}))
      }
      const proxy = target.provider === "koda" ? yield* proxyAuth() : undefined
      const token = yield* Effect.gen(function* () {
        if (target.provider === "koda") return proxy?.token
        const item = yield* auth.get(target.provider).pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
        if (item?.type === "api") return item.key
        return DIRECT_EDIT_ENV[target.provider].map((key) => process.env[key]).find(Boolean)
      })
      if (target.provider === "koda" && !proxy?.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const audioTranscriptions = Effect.fn("kodaGatewayHttpApi.audioTranscriptions")(function* (ctx: {
      ...
      const response = yield* Effect.tryPromise({
        try: () =>
          fetch(`${koda_API_BASE}/api/gateway/v1/audio/transcriptions`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    const notifications = Effect.fn("kodaGatewayHttpApi.notifications")(function* () {
      // Locally-detected notice about leftover opencode config; appended so it reuses each client's dismissal path.
      const notice = kodaConfig.opencodeConfigNotification({
        directory: Instance.directory,
        worktree: Instance.worktree,
        scanProject: !Flag.koda_DISABLE_PROJECT_CONFIG,
      })
    ...
    const organization = Effect.fn("kodaGatewayHttpApi.organization")(function* (ctx) {
      const info = yield* auth.get("koda").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      if (!info || info.type !== "oauth") return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      yield* cache.clear("koda")
      clearModesCache()
      yield* store.disposeAll().pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      return true
    })

    const clawStatus = Effect.fn("kodaGatewayHttpApi.clawStatus")(function* () {
      const info = yield* auth.get("koda").pipe(Effect.mapError(() => new HttpApiError.ServiceUnavailable({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
        ...
        Effect.match({
          onFailure: (err) => {
            if (err instanceof GatewayError)
              return jsonError(`kodaClaw request failed: ${err.status} ${err.message}`, err.status)
            logError("claw/status", err)
            return jsonError("Failed to reach kodaClaw", 502)
          },
    ...
    const clawChatCredentials = Effect.fn("kodaGatewayHttpApi.clawChatCredentials")(function* () {
      const info = yield* auth.get("koda").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSessions = Effect.fn("kodaGatewayHttpApi.cloudSessions")(function* (ctx) {
      const info = yield* auth.get("koda").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSession = Effect.fn("kodaGatewayHttpApi.cloudSession")(function* (ctx) {
      const info = yield* auth.get("koda").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSessionImport = Effect.fn("kodaGatewayHttpApi.cloudSessionImport")(function* (ctx) {
      const info = yield* auth.get("koda").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const imageModels = Effect.fn("kodaGatewayHttpApi.imageModels")(function* () {
      const info = yield* proxyAuth()
      if (!info.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!info.token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      const result = yield* Effect.tryPromise({
        try: () =>
          fetchkodaImageModels({
            kodaToken: info.token,
            kodaOrganizationId: info.organizationId,
          }),
        catch: () => new HttpApiError.BadRequest({}),
      })
    ...
    const transcriptionModels = Effect.fn("kodaGatewayHttpApi.transcriptionModels")(function* () {
      const info = yield* proxyAuth()
      if (!info.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!info.token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
