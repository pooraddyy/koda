...
import { kodaTitleIcon } from "@/koda/cli/cmd/tui/title-icon"
...
const TuiConfigShape = {
  ...
  title_icon: Schema.optional(kodaTitleIcon.Value),
...
export const TuiKeybindListResponse = Schema.Struct({ keybinds: Schema.Array(kodaKeybinds.Info) }).annotate({
...
export const ConfigConsoleApi = HttpApi.make("config-console")
  .add(
    HttpApiGroup.make("config-console")
      .add(
        ...
        HttpApiEndpoint.get("rules", ConfigConsolePaths.rules, {
          ...
          OpenApi.annotations({
            ...
            description: "List project instruction files used by koda and return their current contents.",
          }),
        ),
        ...
        HttpApiEndpoint.patch("modelStateUpdate", ConfigConsolePaths.modelState, {
          ...
          OpenApi.annotations({
            ...
            description: "Patch model selections consumed by compatible Koda terminal sessions.",
          }),
        ),
      ...
      .annotateMerge(OpenApi.annotations({ title: "config", description: "Koda terminal configuration routes." }))
