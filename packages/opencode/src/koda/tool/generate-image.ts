...
import { koda_OPENROUTER_BASE } from "@koda/koda-gateway"
import DESCRIPTION from "./generate-image.txt"

const log = Log.create({ service: "tool.generate_image" })

const koda_OPENROUTER_URL = `${koda_OPENROUTER_BASE}/chat/completions`
...
function buildRequest(resolved: ResolvedProvider, prompt: string, model: string, inputImage?: string): ResolvedRequest {
  ...
  if (resolved.organizationId) headers["X-koda-ORGANIZATIONID"] = resolved.organizationId
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const auth = yield* authSvc.get("koda")
          ...
          if (!resolved) {
            return {
              ...
              output:
                "No image generation provider available. Log in to koda or set OPENROUTER_API_KEY, then try again.",
