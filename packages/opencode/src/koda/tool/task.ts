...
const log = Log.create({ service: "koda-task-model" })
...
export namespace kodaTask {
   ...
   * koda intentionally differs: parent edit/notebook/MCP denials remain hard ceilings
  ...
  const saved = Effect.fn("kodaTask.savedModel")(function* (name: string) {
    if (Flag.koda_CLIENT !== "cli") return undefined
  ...
  export const resolveModel = Effect.fn("kodaTask.resolveModel")(function* (input: {
    name: string
