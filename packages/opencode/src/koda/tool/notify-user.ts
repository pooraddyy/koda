...
const FAILURE_TEXT =
  "Push notifications are unavailable: this session is not connected to koda cloud. " +
  "Sign in with `koda auth login` and ensure the session is sharing before retrying."

const SUCCESS_TEXT =
  "Notification sent to the user's koda app. Delivery may be suppressed by the user's " +
...
export const NotifyUserTool = Tool.define<typeof Params, Meta, kodaSessions.Service, "notify_user">(
  "notify_user",
  Effect.gen(function* () {
    ...
    return {
      ...
      execute: (params, ctx) =>
        Effect.gen(function* () {
          ...
          if (!kodaSessions.remoteStatus().enabled) {
            return {
              title: title(false),
              output: FAILURE_TEXT,
