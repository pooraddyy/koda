...
import type { OrganizationMode } from "@koda/koda-gateway"

export namespace ModesMigrator {
  // koda mode structure
  export interface kodaMode {
    slug: string
    name: string
  ...
  export interface kodaModesFile {
    customModes: kodaMode[]
  }
  ...
  export function convertPermissions(groups: kodaMode["groups"]): ConfigPermission.Info {
  ...
  export function convertMode(mode: kodaMode): ConfigAgentV1.Info {
  ...
  export async function readModesFile(filepath: string): Promise<kodaMode[]> {
    try {
      const content = await fs.readFile(filepath, "utf-8")
      // Wrap YAML content in frontmatter delimiters so gray-matter can parse it
      const wrapped = `---\n${content}\n---`
      const parsed = matter(wrapped).data as kodaModesFile
  ...
  export async function migrate(options: {
    ...
    /** Skip user-level paths during isolated tests. */
    ...
    const allModes: kodaMode[] = []

    if (!options.skipGlobalPaths) {
      // CLI global settings (fallback/alternative location)
      const cliGlobalPath = path.join(os.homedir(), ".koda", "cli", "global", "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(cliGlobalPath)))

      // Home directory .kodamodes
      const homeModesPath = path.join(os.homedir(), ".kodamodes")
      if (homeModesPath !== options.projectDir) {
        allModes.push(...(await readModesFile(homeModesPath)))
      }
    }

    // Explicit global settings directory for controlled compatibility and testing.
    ...
    // Project .kodamodes
    const projectModesPath = path.join(options.projectDir, ".kodamodes")
    allModes.push(...(await readModesFile(projectModesPath)))

    // Deduplicate by slug (later entries win)
    const modesBySlug = new Map<string, kodaMode>()
