...
import { backendSupport, run as runSandbox, unrestricted, type Profile } from "@koda/sandbox"
import { Bus } from "@/bus"
...
export function profile(
  ...
  return {
    ...
    environment: {
      deny: ["koda_CONFIG", "koda_CONFIG_CONTENT", "koda_CONFIG_DIR", "koda_SERVER_PASSWORD", "koda_SERVER_USERNAME"],
      set: {
...
const snapshot = Effect.fn("SandboxPolicy.snapshot")(function* (sessionID: SessionID) {
  ...
  return yield* locked(
    ...
    Effect.gen(function* () {
      ...
      // A session's create-time koda.sandbox toggle takes precedence over the config default, so a
