...
/**
 * kodaClaw koda Chat client wrapper for the TUI.
 *
 * Hosts the HTTP and WebSocket clients, exposes a multi-conversation API
 * (list/create/select/rename/delete), and translates koda Chat events for
...
import { kodaChatClient } from "./koda-chat-client"
...
export async function connect(input: ConnectInput): Promise<ClawChatClient> {
  ...
  const chat = new kodaChatClient({
    baseUrl: input.envelope.kodaChatUrl,
    getToken: async () => input.envelope.token,
  })
  ...
  const sandboxCtx = `/kodaclaw/${input.sandboxId}`
  ...
  function setActive(id: string | null): void {
    ...
    clearTyping()
    if (id) {
      activeCtx = `/kodaclaw/${input.sandboxId}/${id}`
