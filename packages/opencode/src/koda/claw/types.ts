...
/**
 * kodaClaw TUI types — koda Chat protocol.
 *
 * Mirrors `@koda/koda-chat` from the cloud monorepo. We carry our own
...
export type ClawStatus = {
  ...
  // restore (cloud: `services/kodaclaw/src/index.ts`).
  status:
    | "provisioned"
    | "starting"
    | "restarting"
  ...
  secretCount?: number
  userId?: string
  // User-chosen name for the kodaClaw bot, set during onboarding via the
  ...
  // literal string "kodaClaw" when null.
  botName?: string | null
}

// ── koda Chat token envelope (gateway response) ─────────────────────
...
export type kodaChatEventName = keyof kodaChatEventMap
