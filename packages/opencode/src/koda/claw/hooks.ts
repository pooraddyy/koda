...
/**
 * kodaClaw SolidJS reactive helpers — koda Chat protocol.
 *
 * Exposes:
 *   - `createClawStatus`: polled instance status
 *   - `createClawChat`: koda Chat connection + active-conversation state
 *
...
/**
 * Poll the kodaClaw instance status every `interval` ms.
 */
export function createClawStatus(sdk: SDK, interval = 10_000) {
  ...
  onMount(() => {
    const poll = async () => {
      const res = await sdk.client.koda.claw.status().catch(() => null)
...
export function createClawChat(sdk: SDK) {
  ...
  onMount(async () => {
    ...
    const statusRes = await sdk.client.koda.claw.status().catch(() => null)
    ...
    const credsRes = await sdk.client.koda.claw.chatCredentials().catch((e: unknown) => {
    ...
    if (!envelope.kodaChatUrl) missing.push("kodaChatUrl")
