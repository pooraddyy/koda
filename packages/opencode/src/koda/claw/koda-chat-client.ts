...
/**
 * HTTP client for the koda-chat Cloudflare Worker.
 *
 * Minimal inline port of `@koda/koda-chat/client` (cloud monorepo)
 * tailored to what the TUI needs. The koda-chat worker validates payloads
...
export type kodaChatClientConfig = {
  baseUrl: string
  getToken: () => Promise<string>
  onUnauthorized?: () => void
}

export class kodaChatApiError extends Error {
  constructor(
    public readonly status: number,
    public readonly body: unknown,
  ) {
    super(`kodaChat request failed: ${status}${formatBodyDetail(body)}`)
    this.name = "kodaChatApiError"
  }
}
...
export class kodaChatClient {
  ...
  constructor(config: kodaChatClientConfig) {
  ...
  private async request<T>(path: string, opts: HttpOpts = {}): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new kodaChatApiError(res.status, body)
    }
