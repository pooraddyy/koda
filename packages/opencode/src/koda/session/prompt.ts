...
import { PlanFollowup } from "@/koda/plan-followup"
import { PlanFile } from "@/koda/plan-file"
import { kodaSession } from "@/koda/session"
import { kodaSessionMessageOrder } from "@/koda/session/message-order"
import { kodaSessionPromptQueue } from "@/koda/session/prompt-queue"
import { Permission } from "@/permission"
import { PermissionProvenance } from "@/koda/permission/provenance"
import { Question } from "@/question"
import { environmentDetails } from "@/koda/editor-context"
import { Identifier } from "@/id/id"
import { Filesystem } from "@/util/filesystem"
import NATIVE_PLAN_PROMPT from "@/koda/session/native-plan-prompt.txt"
...
import { consumeAutoTitle, markAutoTitle } from "@/koda-sessions/rename-adoptions"

export namespace kodaSessionPrompt {
  ...
  export const abortIntakes = Effect.fn("kodaSessionPrompt.abortIntakes")(function* (sessionID: SessionID) {
    const entries = [...(intakes.get(sessionID) ?? [])]
  ...
  export const cancelTree = Effect.fn("kodaSessionPrompt.cancelTree")(function* (input: {
    sessionID: SessionID
    ...
    yield* Effect.forEach(
      ...
      (sessionID) =>
        Effect.gen(function* () {
          yield* kodaSessionPromptQueue.cancel(sessionID)
  ...
  export const recoverDanglingAssistant = Effect.fn("kodaSessionPrompt.recoverDanglingAssistant")(function* (input: {
    sessionID: SessionID
  ...
  export const recoverProviderFinishError = Effect.fn("kodaSessionPrompt.recoverProviderFinishError")(
    function* (input: {
      sessionID: SessionID
      status: Pick<SessionStatus.Interface, "get">
      sessions: Pick<Session.Interface, "messages" | "removeMessage">
  ...
  export const askPermission = Effect.fn("kodaSessionPrompt.askPermission")(function* (input: {
    ...
    // koda_change start - tag every rule with its true origin before merging, so the winning
  ...
  export function memoryToolEnabled(input: { ctx: MemoryPaths.Ctx }) {
    return kodaToolRegistry.memoryToolsEnabled({ ctx: input.ctx })
  }
  ...
  type PinnedEvolution = { blocks: string[] }
  const pinnedEvolution = new Map<string, PinnedEvolution>()
  const PINNED_EVOLUTION_MAX = 512

  function writePinnedEvolution(sessionID: string, value: PinnedEvolution) {
    pinnedEvolution.set(sessionID, value)
    if (pinnedEvolution.size > PINNED_EVOLUTION_MAX) {
      const oldest = pinnedEvolution.keys().next().value
      if (oldest !== undefined) pinnedEvolution.delete(oldest)
    }
  }

  /** Test-only: drop the per-session pinned evolution context cache. */
  export function clearPinnedEvolution() {
    pinnedEvolution.clear()
  }

  export const evolutionInject = Effect.fn("kodaSessionPrompt.evolutionInject")(function* (input: {
    ctx: MemoryPaths.Ctx
    sessionID: SessionID
  }) {
    const cached = pinnedEvolution.get(input.sessionID)
    if (cached) return cached.blocks
    const root = MemoryPaths.root({ ctx: input.ctx })
    const blocks = yield* Effect.tryPromise(() => EvolutionStore.list(root)).pipe(
      Effect.map((lessons) => EvolutionStore.prompt(lessons)),
      // Evolution is an optional enhancement; an unreadable store must never block a session.
      Effect.catch(() => Effect.succeed([] as string[])),
    )
    writePinnedEvolution(input.sessionID, { blocks })
    return blocks
  })
  ...
  export const memoryInject = Effect.fn("kodaSessionPrompt.memoryInject")(function* (input: {
    ctx: MemoryPaths.Ctx
    sessionID: SessionID
    record: boolean
    ...
    const verbose =
      input.cache.verbose ??
      (enabled
        ? yield* Effect.tryPromise(() => kodaMemory.status({ ctx: input.ctx })).pipe(
   ...
   * OneDrive ReparsePoint directories in some Node versions (koda#9755).
   */
  ...
  export function resolveCloseReason(input: {
    sessionID: string
    closeReasons: Map<string, kodaSession.CloseReason>
    exit: Exit.Exit<any, any>
  }): kodaSession.CloseReason {
  ...
  export function guardCompactionAttempt(input: {
    ...
    input.closeReasons.set(input.sessionID, "budget")
    if (input.message) {
  ...
  export function trimBeforeLastSummary(msgs: MessageV2.WithParts[]): MessageV2.WithParts[] {
    const summary = msgs.reduce<{ msg: MessageV2.WithParts; index: number } | undefined>((latest, msg, index) => {
      ...
      if (!latest || kodaSessionMessageOrder.compare(msg, latest.msg, index, latest.index) > 0) return { msg, index }
      return latest
    }, undefined)
