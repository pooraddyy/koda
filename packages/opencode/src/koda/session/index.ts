...
import { Bus } from "@/bus"
import { Instance, type InstanceContext } from "@/koda/instance"
...
import { ENV_FEATURE } from "@koda/koda-gateway"
import { existsSync } from "fs"
import path from "path"
import { iife } from "@/util/iife"
import { kodaSessionEvent, type kodaSessionCloseReason } from "./event"

export namespace kodaSession {
  const log = Log.create({ service: "session.koda" })

  // ---------------------------------------------------------------------------
  // Events
  // ---------------------------------------------------------------------------

  export const Event = kodaSessionEvent
  export type CloseReason = kodaSessionCloseReason
  ...
  // Emitted by kodaSessionPromptQueue on every transition that changes the set
  ...
  // context (e.g. kodaSessionPromptQueue transitions, which fire from inside
  ...
  export function featureForPlatform(platform: string | undefined): string | undefined {
    return platform === "cli" ? "cli" : undefined
  }
  ...
  export function attribution(id: string): { rootID: string; feature?: string } {
    ...
    const platform = resolvePlatform(rootID) ?? "cli"
  ...
  // ---------------------------------------------------------------------------
  // Provider-reported cost (koda / OpenRouter / Vercel AI Gateway)
  // ---------------------------------------------------------------------------
   ...
   *                                      (`costDetails.upstreamInferenceCost` for koda)
   ...
   *
   * koda does not charge end users a per-request fee, so for the koda provider the
  ...
  export function providerCost(input: {
    ...
    const iskoda = (input.provider?.id ?? input.providerID) === "koda"
    ...
    if (orUsage) {
      ...
      // koda doesn't charge a fee on top of the upstream inference cost, so for koda
      ...
      const cost = iskoda && upstream !== undefined ? upstream : regular
      if (cost !== undefined) return cost
    }

    // 2. Anthropic Messages or OpenAI Responses via OpenRouter. The koda Gateway wrapper
    //    restores the verbatim usage payload under the AI SDK's raw usage escape hatch.
    //    koda doesn't charge end users a per-request fee, so only upstream cost is relevant.
    ...
    //    gateway fee that koda would pass through, but koda doesn't charge end users a
  ...
  export function unshareSession(id: SessionID) {
    return EffectBridge.fromPromise(async () => {
      const { kodaSessions } = await import("@/koda-sessions/koda-sessions")
      await kodaSessions.unshare(id)
    }).pipe(Effect.catchCause((cause) => Effect.fail(Cause.squash(cause))))
  }

  export async function removeSession(id: string): Promise<void> {
    const { kodaSessions } = await import("@/koda-sessions/koda-sessions")
    await kodaSessions.remove(id).catch(() => {})
  }
    ...
    return Effect.gen(function* () {
      ...
      const worktree = (dir: string) => {
        for (const root of sorted) {
          ...
          if ((parts[0] === ".koda" || parts[0] === ".koda") && parts[1] === "worktrees" && parts[2]) {
...
export { kodaSessionFork } from "./fork-command"
