...
import { Telemetry, type ReviewCommand } from "@koda/koda-telemetry"
import { SessionNetwork } from "@/session/network"
import type { SessionID } from "@/session/schema"
import type { SessionStatus } from "@/session/status"
import { MessageV2 } from "@/session/message-v2"
import { isRecord } from "@/util/record"
import { parseReviewCommand, reviewCommandName } from "@/koda/review/command"
...
import { computeMetrics as computeMetricsHelper, type TokenRates } from "@/koda/session/metrics"
...
export namespace kodaSessionProcessor {
  const log = Log.create({ service: "session.processor.koda" })
  ...
  export const PROVIDER_FINISH_ERROR_MESSAGE =
    "The provider ended the response with an error before returning details. Start a new message to retry; koda will compact the oversized conversation first if needed."
  ...
  /**
   * Returns the koda-specific retry policy options (limit + offline handler).
  ...
  export function retryOpts(input: {
    ...
    const limit = Flag.koda_SESSION_RETRY_LIMIT
    return {
