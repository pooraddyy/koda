...
export namespace kodaRoutedModel {
  const ns = "koda"
  ...
  export function readAuto(
    ...
    if (input.providerID !== ProviderV2.ID.koda) return undefined
    if (!input.modelID.startsWith("koda-auto/") && !input.modelID.startsWith("openrouter/") && !input.modelID.includes("fable")) return undefined
