...
const log = Log.create({ service: "koda.cloud-session" })
 ...
 * Wraps the SDK's `.koda.cloud.session.import()` which returns `unknown` due to
...
export async function importCloudSession(
  ...
  const result = await client.koda.cloud.session.import({ sessionId })
