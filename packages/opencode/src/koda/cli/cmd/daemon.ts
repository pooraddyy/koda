...
import type { Daemon } from "@/koda/daemon/daemon"
...
function print(input: Daemon.Status, json?: boolean) {
  ...
  if (!input.running) {
    console.log(input.stale ? `koda daemon stale: ${input.reason}` : `koda daemon not running`)
    console.log(`state: ${input.file}`)
    if (input.state?.log) console.log(`log: ${input.state.log}`)
    return
  }
  console.log(`koda daemon running`)
  if (input.state?.urls) {
...
async function hold(enabled: boolean, json: boolean, run: (signal?: AbortSignal) => Promise<Daemon.State>) {
  ...
  const { Daemon } = await import("@/koda/daemon/daemon")
  await Daemon.foreground(async (signal) => {
    const state = await run(signal)
    if (!signal.aborted && !json) console.log("Press Ctrl+C to stop the koda daemon.")
    return state
  })
}

async function network(args: { [key: string]: unknown }) {
  const { warnedNetworkOptions } = await import("@/koda/cli/port-warning")
...
function start(command: string) {
  return cmd({
    command,
    describe: "start the local koda daemon",
    ...
    handler: async (args) => {
      await hold(Boolean(args.foreground), Boolean(args.json), async (signal) => {
        ...
        if (!state) throw new Error("koda daemon did not provide process state")
        if (signal?.aborted) return state
        if (args.json) print(result, true)
        if (!args.json) {
          console.log(
            result.reused
              ? "koda daemon already running"
              : daemon.restarted
                ? "koda daemon restarted"
                : "koda daemon started",
          )
          print(result)
        }
...
const StatusCommand = cmd({
  command: "status",
  describe: "show local koda daemon status",
...
export const StopCommand = cmd({
  command: "stop",
  describe: "stop the local koda daemon",
  builder: (yargs) => withJson(yargs),
  handler: async (args) => {
    const { Daemon } = await import("@/koda/daemon/daemon")
    ...
    console.log(result.stopped ? "koda daemon stopped" : "koda daemon not running")
  },
})

const RestartCommand = cmd({
  command: "restart",
  describe: "restart the local koda daemon",
  builder: (yargs) => withForeground(withJson(withNetworkOptions(yargs))),
  handler: async (args) => {
    await hold(Boolean(args.foreground), Boolean(args.json), async (signal) => {
      const opts = await network(args)
      ...
      if (args.json) print(result, true)
      if (!args.json) {
        console.log("koda daemon restarted")
        print(result)
      }
      return state
    })
...
export const DaemonCommand = cmd({
  command: "daemon",
  describe: "manage the local koda daemon",
  builder: (yargs: Argv) =>
    yargs
