/**
 * koda-specific TUI app customizations.
 *
...
import { iskodaError, showkodaErrorToast } from "@/koda/koda-errors"
import { registerkodaCommands } from "@/koda/koda-commands"
import { initializeTUIDependencies } from "@koda/koda-gateway/tui"
import { DialogProcessList } from "@/koda/cli/cmd/tui/component/dialog-process-list"
import { useIndexingWarnings } from "@/koda/cli/cmd/tui/indexing-warning"
import { kodaTerminalTitle } from "./terminal-title"
import type { kodaTitleIcon } from "./title-icon"
import { Session as SessionApi } from "@/session/session"

// Re-export so upstream can render the route without importing directly
export { kodaClawView } from "@/koda/claw/view"
export { kodaTerminalTitle } from "./terminal-title"

// Hot reload TUI-local settings when a supported local configuration update is applied.
// Called from the App body (below SDKProvider and the TuiConfig provider).
export { useTuiConfigHotReload } from "@/koda/cli/cmd/tui/context/tui-config-hot-reload"
export { kodaTuiConfig } from "@/koda/cli/cmd/tui/context/tui-config"
...
export function useSessionEffects(deps: {
  ...
  const pty = process.env.koda_PTY_ID
...
export function getTerminalTitle(input: {
  ...
  icon?: kodaTitleIcon.Value
}): kodaTerminalTitle.Result | undefined {
  if (input.route.data.type === "home") {
    return {
      title: kodaTerminalTitle.format({ base: input.base, indicator: "none", icon: input.icon }),
      active: false,
      indicator: "none",
    }
  }
  ...
  if (input.route.data.type === "session") {
    const state = kodaTerminalTitle.session({
      base: input.base,
    ...
    return {
      ...state,
      title: kodaTerminalTitle.format({ base: input.base, title, indicator: state.indicator, icon: input.icon }),
    }
  }

  if (input.route.data.type === "plugin") {
    return {
      title: kodaTerminalTitle.format({
        base: input.base,
  ...
  if (input.route.data.type === "kodaclaw") {
    return {
      title: kodaTerminalTitle.format({ base: input.base, title: "kodaClaw", indicator: "none", icon: input.icon }),
      active: false,
      indicator: "none",
    }
  }
...
/**
 * Intercepts koda-specific errors and shows a warning toast.
 * Returns `true` if the error was handled, `false` otherwise.
 */
export function handleSessionError(error: unknown, toast: ReturnType<typeof useToast>): boolean {
  if (error && typeof error === "object" && iskodaError(error as any)) {
    showkodaErrorToast(error as any, toast)
    return true
  }
  return false
}
 ...
 *
 * - Injects TUI dependencies into koda-gateway
 * - Registers koda Gateway commands (profile, teams, kodaclaw, etc.)
...
export function init() {
  ...
  // Inject TUI dependencies for koda-gateway
  ...
  // Register koda Gateway commands (profile, teams, kodaclaw, remote, etc.)
  registerkodaCommands(useSDK)
  ...
  useBindings(() => ({
    commands: [
      ...
      },
      {
        ...
        // The saved rule is server-side, so it also applies to headless and active
        // terminal sessions rather than prompting inconsistently.
        desc: "Toggle auto-approve for all permission prompts, saved to global configuration and shared with active sessions",
        category: "System",
