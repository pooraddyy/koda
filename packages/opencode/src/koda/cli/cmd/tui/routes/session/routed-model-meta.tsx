...
import type { AssistantMessage, Part, Provider, StepFinishPart } from "@koda/sdk/v2"
...
export namespace RoutedModelMeta {
  ...
  export function label(list: Providers, model: StepFinishPart["model"]) {
    ...
    const id = kodaRoutedModel.display(model.modelID)
    ...
    return kodaRoutedModel.displayName(text)
  }

  function routed(model: StepFinishPart["model"], message: Message) {
    if (!model) return undefined
    if (message.providerID !== "koda") return undefined
    if (!message.modelID.startsWith("koda-auto/")) return undefined
