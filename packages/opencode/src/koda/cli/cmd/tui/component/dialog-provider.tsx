/**
 * koda-specific overrides for the provider dialog.
 *
...
import type { ProviderAuthAuthorization } from "@koda/sdk/v2"
import { KodaAutoMethod } from "@/koda/components/dialog-koda-auto-method"
export { selectProvider } from "@/koda/anaconda-desktop/tui/setup"
...
/**
 * If the provider is koda Gateway, renders the custom `kodaAutoMethod`
...
export function renderAutoMethod(opts: {
  ...
  if (opts.providerID !== "koda") return undefined
  return () => (
    <KodaAutoMethod
 ...
 * provider is koda Gateway. Returns `undefined` otherwise.
 */
export function renderApiDescription(
  ...
  if (providerID !== "koda") return undefined
  return () => (
    <box gap={1}>
      <text fg={theme.textMuted}>
        koda Gateway gives you access to all the best coding models at the cheapest prices with a single API key.
      </text>
      <text fg={theme.text}>
        Go to <span style={{ fg: theme.primary }}>https://koda.ai/gateway</span> to get a key
      </text>
    </box>
  )
}
