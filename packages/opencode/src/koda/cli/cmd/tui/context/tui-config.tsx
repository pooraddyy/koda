 ...
 * changed by another supported local configuration writer. Fetched config stays serializable and is resolved into a fresh
...
export namespace kodaTuiConfig {
  ...
  export function makeStore(initial: TuiConfig.Resolved) {
    ...
    const set: SetTuiConfig = (next) => {
      ...
      const config: TuiConfig.Resolved = {
        ...next,
        title_icon: next.title_icon ?? kodaTitleIcon.Default,
        attention: {
          ...
          sound_pack: next.attention?.sound_pack ?? "koda.default",
