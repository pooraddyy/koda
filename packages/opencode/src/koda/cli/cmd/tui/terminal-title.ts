import { kodaTitleIcon } from "./title-icon"
...
export namespace kodaTerminalTitle {
  ...
  } satisfies Record<kodaTitleIcon.Value, Record<Indicator, string>>

  export function format(input: { base: string; title?: string; indicator: Indicator; icon?: kodaTitleIcon.Value }) {
    const text = input.title ? `${input.base} | ${truncate(input.title)}` : input.base
    const prefix = icons[input.icon ?? kodaTitleIcon.Default][input.indicator]
