...
import type { kodaBalance, kodaProfile } from "@koda/koda-gateway"
...
interface Args {
  ...
  getProfile?: (token: string) => Promise<kodaProfile>
  getBalance?: (token: string, organizationId?: string) => Promise<kodaBalance | null>
...
export const ProfileCommand = cmd({
  command: "profile",
  describe: "show koda account profile",
  builder: (yargs: Argv) =>
    yargs.option("json", {
...
export async function handle(args: Args) {
  ...
  const auth = await get("koda")
  const error = args.error ?? UI.error
  const exit = args.exit ?? ((code: number) => (process.exitCode = code))

  if (!auth || auth.type !== "oauth") {
    error("Not authenticated with koda Gateway")
    exit(1)
    return
  }

  const { fetchBalance, fetchProfile } = await import("@koda/koda-gateway")
