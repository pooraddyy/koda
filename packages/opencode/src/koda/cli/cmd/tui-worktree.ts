...
// Supports `koda --worktree <name>` (create/reuse a git worktree before the TUI
// starts, placed at `.koda/worktrees/<name>` to match Agent Manager's own
...
const log = Log.create({ service: "koda.tui-worktree" })

// Keeps `.koda/worktrees/` out of `git status` through the terminal worktree flow.
const koda_WORKTREE_DIR = ".koda/worktrees"
...
export async function ensureGitExclude(root: string) {
  ...
  if (current.includes(`${koda_WORKTREE_DIR}/`)) return
  const prefix = current ? `${current}\n\n` : ""
  await Filesystem.write(excludePath, `${prefix}# koda Code agent worktrees\n${koda_WORKTREE_DIR}/\n`).catch((err) =>
...
// Exported for `koda worktree create` (worktree.ts), which calls this and
...
export async function resolveWorktree(name: string, root: string, timeoutMs = 10 * 60_000) {
  ...
  const { primaryWorktree } = await import("@/koda/primary-worktree")
  ...
  return withInstance(root, async (run) => {
    ...
    // it unconditionally would nest `.koda/worktrees/<name>` inside another
    ...
    const directory = path.join(ctx.worktree, koda_WORKTREE_DIR, slug)
