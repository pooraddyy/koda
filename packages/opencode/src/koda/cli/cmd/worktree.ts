...
// `koda worktree list`/`remove`: CLI-side counterpart to `koda --worktree <name>`
...
import { slugify } from "@/koda/cli/cmd/tui-worktree"
...
// Lazy: this module is imported eagerly by kodaCli.register (see setup.ts), so
...
export const WorktreeCreateCommand = cmd({
  ...
  async handler(args) {
    ...
    // instance context (it's shared with `koda --worktree`'s pre-TUI-launch
    // path in tui-worktree.ts), so it can't run inside effectCmd's own.
    const { resolveWorktree } = await import("@/koda/cli/cmd/tui-worktree")
    ...
    // do e.g. `cd "$(koda worktree create foo)"`.
...
export const WorktreeListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.worktree.list")(function* () {
    ...
    // Data rows go to stdout (like `koda session list`'s table/JSON), not
    // UI.println's stderr, so `koda worktree list | ...` actually captures them.
