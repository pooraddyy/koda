...
const MARKER_START = "# >>> kodadev launcher >>>"
const MARKER_END = "# <<< kodadev launcher <<<"

export const DevSetupCommand = cmd({
  command: "dev-setup",
  describe: "install a `kodadev` shell alias for this checkout",
  builder: (y) =>
    y
      .option("shell", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    UI.empty()
    UI.println(`  ${B}koda CLI dev launcher setup${N}`)
    UI.empty()
    ...
    UI.println(
      `  ${action === "install" ? "This will add" : "This will replace"} one line so ${H}kodadev${N} runs this checkout from any directory:`,
    )
    UI.empty()
    UI.println(`      ${snippet}`)
    UI.empty()
...
// Deprecated: kept so `koda dev-alias` keeps working for scripts/docs.
...
function aliasLine(shell: Shell, repo: string): string {
  const sh = path.join(repo, "bin", "kodadev")
  const bat = path.join(repo, "bin", "kodadev.cmd")
  const q = (s: string) => `'${s.replaceAll("'", `'\\''`)}'`
  if (shell === "fish") return `alias kodadev ${q(sh)}`
  if (shell === "powershell") return `function kodadev { & '${bat.replaceAll("'", "''")}' @args }`
  return `alias kodadev=${q(sh)}`
}
...
async function applyBlock(rc: string, snippet: string): Promise<{ created: boolean; backup?: string }> {
  ...
  const backup = `${rc}.kodadev.bak.${stamp()}`
...
export async function detectRepo(): Promise<string> {
  const hint = process.env.koda_DEV_REPO
  if (hint) return hint
  ...
  // For a local build it's <repo>/packages/opencode/dist/<target>/bin/koda, so
  ...
  throw new Error(
    "cannot locate koda source checkout; set koda_DEV_REPO=/path/to/koda or run ./bin/kodadev dev-setup from the repo",
  )
}
