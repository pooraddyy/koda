...
async function getHelpText(name: string, cmd: Cmd): Promise<string> {
  const inst = yargs([])
    .scriptName(name ? `koda ${name}` : "koda")
    .wrap(null)
  if (cmd.builder) {
...
async function getSubcommands(
  ...
  const inst = yargs([]).scriptName(`koda ${name}`).wrap(null)
  builder(inst)
  ...
  try {
    ...
    for (const [sub, handler] of Object.entries(handlers as Record<string, any>)) {
      ...
      const full = `${name} ${sub}`
      const subInst = yargs([]).scriptName(`koda ${full}`).wrap(null)
...
function formatMarkdown(
  ...
  for (const section of sections) {
    parts.push(`## ${section.name ? `koda ${section.name}` : "koda"}`)
    parts.push("")
    if (section.hidden) {
...
function formatText(
  ...
  for (const section of sections) {
    parts.push(rule)
    const display = section.name ? `koda ${section.name}` : "koda"
    ...
    for (const sub of section.subs) {
      const sublabel = sub.hidden ? `--- koda ${sub.name} [internal] ---` : `--- koda ${sub.name} ---`
      parts.push(sublabel)
      parts.push("")
      parts.push(sub.help)
      parts.push("")
...
export async function generateCommandTable(options?: { commands?: Cmd[] }) {
  ...
  for (const cmd of cmds) {
    ...
    const display = raw.startsWith("$0") ? "koda" + raw.slice(2) : "koda " + raw
