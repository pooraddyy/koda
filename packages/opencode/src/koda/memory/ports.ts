...
import { MemoryConfig } from "@koda/koda-memory/effect/config"
import { MemoryError } from "@koda/koda-memory/effect/errors"
import type { MemoryPorts } from "@koda/koda-memory/effect/ports"
import { MemoryRedact } from "@koda/koda-memory/redact"
import { MemoryShared } from "@koda/koda-memory/shared"
...
function recalledMemory(turn: Turn) {
  return [turn.user, ...turn.assistants].flatMap((item) => item.parts).some((part) => {
    ...
    const marker = (part.metadata as { kodaMemory?: { type?: string; count?: number } } | undefined)?.kodaMemory
