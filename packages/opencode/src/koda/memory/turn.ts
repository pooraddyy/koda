...
import { MemoryTurn as TurnCore } from "@koda/koda-memory/effect/turn"
...
import { kodaSessionPrompt } from "@/koda/session/prompt"
...
export namespace MemoryLifecycle {
  export const subscribe = Effect.fn("MemoryLifecycle.subscribe")(function* (input: {
    ...
    yield* input.bus.subscribeCallback(kodaSession.Event.TurnOpen, (evt) =>
      bridge.fork(
    ...
    yield* input.bus.subscribeCallback(kodaSession.Event.TurnClose, (evt) =>
      bridge.fork(
        Effect.gen(function* () {
          const ctx = yield* InstanceState.context
          const enabled = yield* kodaSessionPrompt.memoryToolEnabled({ ctx })
          if (!enabled) return
