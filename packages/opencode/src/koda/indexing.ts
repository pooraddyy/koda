...
import { type IndexingTelemetryEvent, type VectorStoreSearchResult } from "@koda/koda-indexing/engine"
import { toIndexingConfigInput, type IndexingConfig } from "@koda/koda-indexing/config"
import { hasIndexingPlugin } from "@koda/koda-indexing/detect"
import { IndexingStatus, disabledIndexingStatus } from "@koda/koda-indexing/status"
import { Telemetry } from "@koda/koda-telemetry"
import { fetchkodaEmbeddingModelCatalog } from "@koda/koda-gateway"
...
import { indexingWithkodaDefault, resolvekodaIndexingAuth, type kodaIndexingAuth } from "./indexing-auth"
import { primaryWorktree } from "./primary-worktree"

const log = Log.create({ service: "koda-indexing" })
...
const baselineDirectory = Effect.fn("kodaIndexing.baselineDirectory")(function* (dir: string) {
...
async function kodaAuth(cfg: Config.Info): Promise<kodaIndexingAuth> {
  const info = await auth.runPromise((svc) => svc.get("koda"))
  return resolvekodaIndexingAuth({ config: cfg, auth: info })
}

function enrichkoda(input: ReturnType<typeof toIndexingConfigInput>, auth: kodaIndexingAuth) {
  if (input.embedderProvider !== "koda") return input

  return {
    ...input,
    kodaApiKey: input.kodaApiKey ?? auth.apiKey,
    kodaBaseUrl: input.kodaBaseUrl ?? auth.baseUrl,
    kodaOrganizationId: input.kodaOrganizationId ?? auth.organizationId,
  }
}

async function model(input: ReturnType<typeof toIndexingConfigInput>, auth: kodaIndexingAuth) {
  if (input.embedderProvider !== "koda" || !input.enabled) return input

  const catalog = await fetchkodaEmbeddingModelCatalog({ baseURL: auth.baseUrl, token: auth.apiKey })
  ...
  if (!found) {
    if (input.modelId || input.modelDimension) {
      log.warn("ignoring unsupported koda embedding model configuration", { model: input.modelId })
    }
...
export namespace kodaIndexing {
  ...
  export async function models() {
    try {
      ...
      const fallback = await fetchkodaEmbeddingModelCatalog()
      return fallback.models.length > 0 ? fallback : catalog
    } catch (err) {
      log.warn("falling back to public koda embedding model catalog", { err })
      return fetchkodaEmbeddingModelCatalog()
    }
  }
