...
import { appendTerminalOutput } from "@/koda/interactive-terminal/output"
import { model as modelEnv } from "@/koda/process/env"
import { Identifier } from "@/id/id"
import { capture, Instance, type InstanceContext } from "@/koda/instance"
...
export namespace InteractiveTerminal {
  ...
  class StateService extends Context.Service<StateService, State>()("@koda/InteractiveTerminal.State") {}

  class RuntimeService extends Context.Service<
    RuntimeService,
    { readonly get: (ctx: InstanceContext) => Effect.Effect<State, never, Scope.Scope> }
  >()("@koda/InteractiveTerminal.Runtime") {}
  ...
  function gate(shell: string, command: string) {
    ...
    return `stty -echo; IFS= read -r __koda_gate; stty echo; ${command}`
  }
  ...
  function environment(input: NodeJS.ProcessEnv) {
    ...
    env.koda_TERMINAL = "1"
    env.koda_INTERACTIVE_TERMINAL = "1"
