...
} from "@koda/koda-indexing/engine"
import type { IndexingStatus } from "@koda/koda-indexing/status"
...
declare global {
  const koda_INDEXING_WORKER_PATH: string
}
...
export namespace IndexingWorker {
  ...
  const channel = () => {
    ...
    const file =
      typeof koda_INDEXING_WORKER_PATH !== "undefined"
        ? koda_INDEXING_WORKER_PATH
        ...
        return call(
          state,
          {
            ...
            input: {
              ...
              config,
              baselineDirectory,
              lancedbPath: process.env.koda_LANCEDB_PATH,
            },
          },
          (message) => {
