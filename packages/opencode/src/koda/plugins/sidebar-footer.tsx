import type { TuiPlugin, TuiPluginApi, TuiPluginModule } from "@koda/plugin/tui"
import { createMemo, createSignal, onCleanup, onMount, Show } from "solid-js"
import { Global } from "@opencode-ai/core/global"
import * as Log from "@opencode-ai/core/util/log"
import type { kodaPassState } from "@koda/koda-gateway"
import type { Message } from "@koda/sdk/v2"
import { onBalanceRefresh } from "../balance-refresh"
import { REDACTED_BALANCE } from "../pii"

const id = "internal:koda-sidebar-footer"
...
export function passLine(pass: kodaPassState) {
...
// A billed turn: a completed koda assistant message with a non-zero cost.
export function billable(info: Message) {
  if (info.role !== "assistant") return false
  return info.providerID === "koda" && info.time.completed !== undefined && info.cost > 0
}
              ...
              {(pass) => (
                <box gap={0}>
                  <box flexDirection="row" justifyContent="space-between" gap={1}>
                    <text fg={theme().textMuted}>{" └ koda Pass"}</text>
      ...
      <Show when={show()}>
        ...
        >
          ...
          <box flexGrow={1} gap={1}>
            ...
            <text fg={theme().textMuted}>koda includes free models so you can start immediately.</text>
