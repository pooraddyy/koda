...
import { serverUrls } from "@/koda/cli/server-urls"

export namespace Daemon {
  const username = "koda"
  const lock = "koda-daemon"
  ...
  function root() {
    return process.env.koda_TEST_DAEMON_STATE_DIR ?? Global.Path.state
  }

  function logs() {
    return process.env.koda_TEST_DAEMON_LOG_DIR ?? Global.Path.log
  }
  ...
  export function matches(state: State, input: Options, explicit: readonly NetworkOption[]) {
    if (state.password === "koda") return false
  ...
  async function port(input: Options) {
    ...
    if (input.env?.koda_TEST_DAEMON_EPHEMERAL_PORT) return 0
  ...
  async function line(out: string) {
    ...
    const match = text.match(/koda server listening on http:\/\/([^:\s]+):(\d+)/)
