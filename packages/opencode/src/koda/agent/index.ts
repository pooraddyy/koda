...
import { kodaConfigSources } from "../config/sources"
...
// Exported so kodaTask.inherited carries the same set into delegated sessions; a tool
...
function planEditRules(worktree: string) {
  return {
    "*": "deny" as const,
    [path.join(".koda", "plans", "*.md")]: "allow" as const,
...
export interface kodaData {
  mcpRules: Record<string, "allow" | "ask" | "deny">
  defaultsPatch: Permission.Ruleset
}

// Prepare koda-specific data derived from config. Call once per state initialization.
export function prepare(cfg: Config.Info): kodaData {
  const mcpRules = getMcpRules(cfg)
  const defaultsPatch = Permission.fromConfig({
    bash,
    recall: "ask",
    koda_memory_recall: "ask",
    koda_memory_save: "ask",
  })
...
// Lift koda-internal metadata onto typed agent fields and remove it from `options`.
...
// Patch the base agents map in-place with all koda-specific changes:
// - Rename build → code
// - Patch plan with readOnlyBash, mcpRules, .koda paths
...
export function patchAgents(
  ...
  // Patch plan mode
  if (agents.plan) {
    const guard = planGuard(worktree, koda.mcpRules)
    agents.plan = {
      ...agents.plan,
      ...
      permission: Permission.merge(
        ...
        guard,
        user,
        baseline(guard, user, koda.mcpRules),
  ...
  // Add ask agent
  const guard = askGuard(koda.mcpRules)
  agents.ask = {
    name: "ask",
 ...
 * config-backed agent entries, and/or removing it from legacy .kodamodes YAML files.
...
export async function remove(input: {
  ...
  let found = false
  const result = await kodaConfigSources.list({ directory: input.directory, worktree: input.worktree })
  ...
  // 2. Remove from legacy .kodamodes YAML files (read by ModesMigrator)
  const { ModesMigrator } = await import("@/koda/modes-migrator")
  ...
  const legacy = [
    {
      ...
      file: path.join(home, ".koda", "cli", "global", "settings", "custom_modes.yaml"),
    },
    { scope: "global" as const, file: path.join(home, ".kodamodes") },
    { scope: "project" as const, file: path.join(input.directory, ".kodamodes") },
  ]
...
async function removeConfigAgent(name: string, sources: kodaConfigSources.Source[]) {
  const files = sources
