...
export function hint(provider: ProviderV2.ID, error: APICallError) {
  ...
  return "Google Gemini rejected this API key. Check its type and status in Google AI Studio. Replace a Standard key with a new auth key; if it is already an auth key, check its Gemini API access or create a replacement. Restricted Standard keys work only until September 2026. See https://koda.ai/docs/ai-providers/gemini."
}
