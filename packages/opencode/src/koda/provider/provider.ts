...
//
// koda-specific provider logic extracted from packages/opencode/src/provider/provider.ts
...
// calls at well-defined injection points (each marked with koda_change).

import { createkoda, type kodaProvider, AI_SDK_PROVIDERS, PROMPTS } from "@koda/koda-gateway"
import { DEFAULT_HEADERS } from "@/koda/const"
...
export const koda_BUNDLED_PROVIDERS: Record<string, () => Promise<(options: any) => BundledSDK>> = {
  "@koda/koda-gateway": async () => createkoda as unknown as (options: any) => BundledSDK,
}
...
export const koda_MODEL_SCHEMA_EXTENSIONS = {
...
// ---------------------------------------------------------------------------
// fromModelsDevModel patch — returns koda-specific fields
// ---------------------------------------------------------------------------

export function patchModelsDevModel(providerID: string, source: any) {
  return {
    variants: providerID === "koda" ? (source.variants ?? {}) : {},
...
// ---------------------------------------------------------------------------
// Config model patch — merges koda-specific fields from config + existing
// ---------------------------------------------------------------------------
...
export function patchkodaProviderPrivacy(provider: { options?: Record<string, any> } | undefined, config: any) {
  if (!provider || config.hide_prompt_training_models !== true) return
  provider.options = { ...provider.options, dataCollection: "deny" }
}

export function kodaCustomLoaders(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    koda: Effect.fnUntraced(function* (input: any) {
      ...
      const hasKey = yield* Effect.gen(function* () {
        ...
        if (config.provider?.["koda"]?.options?.apiKey) return true
        return false
      })

      const options: Record<string, string> = {}
      if (env.koda_ORG_ID) {
        options.kodaOrganizationId = env.koda_ORG_ID
      }
      ...
      return {
        ...
        options,
        async getModel(sdk: kodaProvider, modelID: string) {
  ...
  switch (providerID) {
    ...
    case "cerebras":
      result.options.headers = {
        ...result.options.headers,
        "X-Cerebras-3rd-Party-Integration": "koda",
      }
      break
    case "azure": {
    ...
    // in provider.ts with single-line koda_change markers
  }
}
...
export function kodaSmallModelPriority(providerID: string): string[] | undefined {
  if (providerID.startsWith("koda")) return ["koda-auto/small"]
  return undefined
}
