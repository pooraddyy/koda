...
export const PlanFollowupRuntime = {
  ...
  handover(input: LLM.StreamInput, signal: AbortSignal) {
    return llm().runPromise((svc) => kodaLLM.text(svc.stream(input)).pipe(Effect.orDie), { signal })
  },
...
export namespace PlanFollowup {
  ...
  async function resolveCodeModel(input: Pick<MessageV2.User, "model">) {
    const state = Flag.koda_CLIENT === "cli" ? await kodaModelState.get().catch(() => undefined) : undefined
      ...
      questions: [
        {
          ...
          // The terminal prompt is hidden while this blocking question is active, so
          // keep a custom-answer row available for a free-text implementation response.
          custom: true,
          options: [
            {
  ...
  async function startNew(input: {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        // the SSE event that carries the session transition.
