...
import { resolvekodaIndexingAuth } from "@/koda/indexing-auth"
...
export async function getAuthOrgId(): Promise<OrgState> {
  try {
    const [cfg, info] = await Promise.all([
      ...
      auth.runPromise((svc) => svc.get("koda")),
    ])
    const id = resolvekodaIndexingAuth({ config: cfg, auth: info }).organizationId
