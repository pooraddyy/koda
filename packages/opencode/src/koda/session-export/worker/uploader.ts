...
async function headers(args: HeaderArgs): Promise<Headers> {
  ...
  const out = new Headers({
    ...
    "x-koda-export-api-version": "1",
    "x-koda-export-schema-version": "1",
    "x-koda-export-agent-version": args.agentVersion,
    "x-koda-export-surface": args.surface,
    "x-koda-export-root-session-id": first.rootSessionId,
    "x-koda-export-session-id": first.sessionId,
    "x-koda-export-batch-id": args.batchId,
    "x-koda-export-seq-start": String(Math.min(...seqs)),
    "x-koda-export-seq-end": String(Math.max(...seqs)),
    "x-koda-export-event-count": String(args.rows.length),
    "x-koda-export-payload-sha256": await sha256Hex(args.body),
    "x-koda-export-client-sent-at": new Date().toISOString(),
    "x-koda-export-content-encoding": "identity",
  })
  const token = process.env.koda_SESSION_EXPORT_AUTH_TOKEN
  if (token) out.set("authorization", `Bearer ${token}`)
  const anon = args.anonId ?? (await anonId(args.anonIdPath))
  if (!token && anon) out.set("x-koda-anon-id", anon)
  return out
}
