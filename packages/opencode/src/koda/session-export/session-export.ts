...
declare global {
  const koda_SESSION_EXPORT_WORKER_PATH: string
}
...
function target(): WorkerTarget {
  if (typeof koda_SESSION_EXPORT_WORKER_PATH !== "undefined") return koda_SESSION_EXPORT_WORKER_PATH
...
function currentSurface(): string {
  return process.env.koda_FEATURE?.trim() || "unknown"
}
