...
export namespace IgnoreMigrator {
  const log = Log.create({ service: "koda.ignore-migrator" })

  const kodaIGNORE_FILE = ".kodaignore"
  const GLOBAL_kodaIGNORE = path.join(os.homedir(), ".koda", kodaIGNORE_FILE)
  ...
  /**
   * Parse .kodaignore content into patterns.
  ...
  /**
   * Load patterns from a .kodaignore file
   */
  ...
  /**
   * Migrate .kodaignore to Opencode permission config
   */
  export async function migrate(options: { projectDir: string; skipGlobalPaths?: boolean }): Promise<MigrationResult> {
    ...
    // 1. Load global .kodaignore (lower priority)
    if (!options.skipGlobalPaths) {
      const globalPatterns = await loadIgnoreFile(GLOBAL_kodaIGNORE, "global")
      allPatterns.push(...globalPatterns)

      if (globalPatterns.length > 0) {
        log.debug("loaded global .kodaignore", { count: globalPatterns.length })
      }
    }

    // 2. Load project .kodaignore (higher priority - added last)
    const projectIgnorePath = path.join(options.projectDir, kodaIGNORE_FILE)
    const projectPatterns = await loadIgnoreFile(projectIgnorePath, "project")
    allPatterns.push(...projectPatterns)

    if (projectPatterns.length > 0) {
      log.debug("loaded project .kodaignore", { count: projectPatterns.length })
    }
  ...
  /**
   * Load .kodaignore and return permission config.
  ...
  export async function loadIgnoreConfig(
    ...
    try {
      ...
      if (result.patternCount > 0) {
        log.info("loaded .kodaignore patterns", {
      ...
      log.warn("failed to load .kodaignore", { error: err })
      return {}
    }
  }
}
