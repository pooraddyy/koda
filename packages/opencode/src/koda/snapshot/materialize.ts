...
export namespace kodaSnapshotMaterialize {
  ...
  export const ref = (gitdir: string) => `refs/koda/materialize/${Hash.fast(path.resolve(gitdir))}`
  const snapshotRef = (hash: string, time = Date.now()) => `refs/koda/snapshots/${time}/${hash}`
  ...
  export const prune = Effect.fnUntraced(function* (input: Input, before: number) {
    ...
    const refs = result.text
      ...
      .map((item) => item.trim())
      .filter((item) => {
        const match = item.match(/^refs\/koda\/snapshots\/(\d+)\/[0-9a-f]+$/)
