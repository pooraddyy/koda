...
//          persist `"snapshot": false` to `.koda/koda.json`, and skip. All
...
//     Instead we write the file directly via `kodaConfig.updateProjectConfig`
...
//
// All of this is koda-specific — the upstream snapshot module remains a thin
...
import { kodaPartLifecycle } from "@/koda/session/part-lifecycle"
import { kodaConfig } from "@/koda/config/config"
import { capture } from "@/koda/instance"
...
export namespace kodaSnapshotTrack {
  ...
  export const TIMEOUT_MS = duration("koda_SNAPSHOT_TRACK_TIMEOUT_MS", 10_000)
  export const TURN_TIMEOUT_MS = duration("koda_SNAPSHOT_TURN_TIMEOUT_MS", 120_000)
   ...
   * elsewhere, without needing a dedicated part renderer outside the TUI.
   *
  ...
  export const progressPart = (input: {
    ...
    text: input.text,
    synthetic: true,
    metadata: { [kodaPartLifecycle.key]: "transient" },
  })
