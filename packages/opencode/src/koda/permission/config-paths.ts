...
import { kodaPaths } from "@/koda/paths"

export namespace ConfigProtection {
  /**
   * Config directory prefixes (relative paths, forward-slash normalized).
   * Matches .koda/ and legacy .koda/ at any depth within the project.
   */
  const CONFIG_DIRS = [".koda/", ".koda/"]
   ...
   */
  const CONFIG_ROOT_FILES = new Set(["koda.json", "koda.jsonc", "opencode.json", "opencode.jsonc", "AGENTS.md"])
  ...
  export function isRelative(pattern: string): boolean {
    ...
    for (const dir of CONFIG_DIRS) {
      const bare = dir.slice(0, -1) // e.g. ".koda"
      // Match at root (e.g. ".koda/foo") or nested (e.g. "packages/sub/.koda/foo")
  ...
  function configs(): string[] {
    return Array.from(
      new Set([Global.Path.config, process.env.XDG_CONFIG_HOME ? path.join(process.env.XDG_CONFIG_HOME, "koda") : ""]),
    ).filter(Boolean)
  }
  ...
  function skillRoot(pattern: string): string | undefined {
    ...
    const roots = [...configs(), ...kodaPaths.globalDirs()]
  ...
  function fallback(p: string): boolean {
    ...
    return keys(p).some(
      (key) =>
        key.endsWith("/config/koda") ||
        key.includes("/config/koda/") ||
        key.endsWith("/.config/koda") ||
        key.includes("/.config/koda/"),
    )
  }
