...
import { kodaConfigSources } from "./sources"

export namespace kodaConfigOverlay {
  const log = Log.create({ service: "koda.config.overlay" })
  ...
  export const Result = z.object({
    ...
    sources: z.array(kodaConfigSources.Source),
  ...
  export type Input = {
    ...
    global: Config.Info
    sources: kodaConfigSources.Source[]
  }

  const files = ["koda.jsonc", "koda.json", "opencode.jsonc", "opencode.json"] as const
  const dirs = [".kilocode", ".kilo", ".koda"] as const
  ...
  const fieldPaths = [
    ...
    ["indexing", "koda", "apiKey"],
    ["indexing", "koda", "baseUrl"],
    ["indexing", "koda", "organizationId"],
  ...
  export async function project(input: { directory: string; worktree?: string }): Promise<Config.Info> {
    ...
    // koda_change - project config is untrusted; confine {file:} reads to the project root
    ...
    return configs.reduce((result, cfg) => kodaConfig.mergeConfig(result, cfg), {} as Config.Info)
  }
  ...
  export async function projectTarget(input: { directory: string; worktree?: string }) {
    ...
    return candidates.find((file) => existsSync(file)) ?? path.join(input.directory, ".koda", "koda.jsonc")
  }
  ...
  export async function resolve(input: Input): Promise<Result> {
    // koda_change start - project agents untrusted, {file:} confined to the project root; global agents trusted
    const root = input.worktree && input.worktree !== "/" ? input.worktree : input.directory
  ...
  function globalDirs() {
    return [
      Global.Path.config,
      path.join(Global.Path.home, ".koda"),
      path.join(Global.Path.home, ".kilocode"),
      path.join(Global.Path.home, ".kilo"),
    ]
  }

  // koda_change start - root confines untrusted agent {file:} reads
  async function withAgents(input: Config.Info, dirs: string[], trusted: boolean, root?: string): Promise<Config.Info> {
    ...
    const next = kodaConfig.mergeConfig(kodaConfig.mergeConfig(input, { agent }), { agent: mode })
  ...
  async function load(file: string, fileScope?: ConfigVariable.FileScope): Promise<Config.Info> {
    // koda_change start - a single unsafe/invalid project config file must not break the settings overlay;
  ...
  async function loadUnsafe(file: string, fileScope?: ConfigVariable.FileScope): Promise<Config.Info> {
    ...
    // koda_change start - remove variable-bearing MCP headers before resolving other project file references
