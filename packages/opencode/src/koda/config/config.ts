...
import { fetchOrganizationModes } from "@koda/koda-gateway"
...
export namespace kodaConfig {
  const log = Log.create({ service: "koda.config" })
  ...
  /** All config file names in precedence order (koda + opencode). */
  export const ALL_CONFIG_FILES = ["koda.jsonc", "koda.json", "opencode.jsonc", "opencode.json"] as const

  /** Config directory suffixes in update-target preference order. */
  export const koda_DIR_SUFFIXES = [".kilocode", ".kilo", ".koda"] as const
   ...
   * preference order.
   */
  export const projectConfigFiles = Effect.fn("kodaConfig.projectConfigFiles")(function* (input: {
    fs: FSUtil.Interface
    directory: string
    worktree?: string
  }) {
    const dirs = yield* input.fs
      .up({ targets: [...koda_DIR_SUFFIXES], start: input.directory, stop: input.worktree })
      .pipe(Effect.orDie)
  ...
  export const updateProjectConfig = Effect.fn("kodaConfig.updateProjectConfig")(function* (input: {
    fs: FSUtil.Interface
    directory: string
    worktree?: string
    config: Config.Info
    ...
    const file = files.find((item) => existsSync(item)) ?? path.join(input.directory, ".koda", "koda.jsonc")
   ...
   */
  export const propagateUnset = Effect.fn("kodaConfig.propagateUnset")(function* (input: {
    fs: FSUtil.Interface
  ...
  /**
   * Load all koda legacy configs (modes, workflows, rules, MCP, ignore).
  ...
  export async function loadLegacyConfigs(input: {
    ...
    try {
      ...
      if (Object.keys(migration.agents).length > 0) {
        ...
        log.debug("loaded koda custom modes", {
          count: Object.keys(migration.agents).length,
          modes: Object.keys(migration.agents),
        })
      }
      for (const skipped of migration.skipped) {
        log.debug("skipped koda mode", { slug: skipped.slug, reason: skipped.reason })
      }
    } catch (err) {
      log.warn("failed to load koda modes", { error: err })
    }

    // Load koda workflows as commands
    try {
      ...
      if (Object.keys(migration.commands).length > 0) {
        ...
        log.debug("loaded koda workflows as commands", {
          count: Object.keys(migration.commands).length,
          commands: Object.keys(migration.commands),
        })
      }
    } catch (err) {
      log.warn("failed to load koda workflows", { error: err })
    }
    ...
    try {
      ...
      if (migration.instructions.length > 0) {
        ...
        log.debug("loaded koda rules", {
          count: migration.instructions.length,
          files: migration.instructions,
        })
      }
      for (const warning of migration.warnings) {
        log.debug("koda rules warning", { warning })
      }
    } catch (err) {
      log.warn("failed to load koda rules", { error: err })
    }

    // Terminal-only runtime loads project-scoped Koda MCP configuration only.
    const mcp = await McpMigrator.loadMcpConfig(input.projectDir)
    if (Object.keys(mcp).length > 0) {
      result = input.merge(result, { mcp })
    }

    // Load .kodaignore patterns
    try {
      const permission = await IgnoreMigrator.loadIgnoreConfig(input.projectDir)
      if (Object.keys(permission).length > 0) {
        result = input.merge(result, { permission })
        log.debug("loaded koda ignore patterns", {
          hasRead: !!(permission as Record<string, unknown>).read,
          hasEdit: !!(permission as Record<string, unknown>).edit,
        })
      }
    } catch (err) {
      log.warn("failed to load koda ignore patterns", { error: err })
    }
  ...
  /**
   * Load organization custom modes from the koda Cloud API.
  ...
  export async function loadOrganizationModes(
    ...
    const warnings: Config.Warning[] = []
    try {
      const koda = auth["koda"]
      if (koda?.type === "oauth" && koda.access && koda.accountId) {
        const modes = await fetchOrganizationModes(koda.access, koda.accountId)
        if (modes.length > 0) {
  ...
  export const GLOBAL_CONFIG_FILES = ["config.json", "koda.json", "koda.jsonc", "opencode.json", "opencode.jsonc"]
  ...
  export function isConfigDir(dir: string, flagDir?: string): boolean {
    return (
      dir.endsWith(".koda") || dir.endsWith(".kilocode") || dir.endsWith(".kilo") || dir === flagDir
    )
  }

  /**
   * Agent-only compatibility roots. These roots deliberately do not enable
   * Koda config files, commands, plugins, MCP servers, or other side effects.
   */
  export function isAgentCompatibilityDir(dir: string): boolean {
    const base = path.basename(path.normalize(dir))
    return base === ".pi" || base === ".agents"
  }

  // ── Opencode config migration notice ─────────────────────────────────

  /** Client-neutral docs page describing where koda reads configuration from. */
  export const CONFIG_DOCS_URL = "https://koda.ai/docs/getting-started/settings"

  /** Stable id for the synthetic "move your opencode config" notification (used for client-side dismissal). */
  export const OPENCODE_NOTIFICATION_ID = "koda.local.opencode-config-detected"

  /**
   * Detect leftover opencode config directories. koda used to fall back to
  ...
  export function detectOpencodeConfig(input: {
    ...
    // Global opencode config dir (sibling of the koda global config dir, e.g. ~/.config/opencode).
  ...
  export function opencodeConfigNotification(input: { directory: string; worktree?: string; scanProject: boolean }) {
    ...
    return {
      ...
      message:
        `koda no longer falls back to opencode configuration. ` +
        `Found opencode config at ${found[0]}${suffix}. ` +
        `Move it into a .koda directory (project) or ${Global.Path.config} (global).`,
