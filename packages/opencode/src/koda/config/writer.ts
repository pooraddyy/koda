...
import { kodaConfigOverlay } from "./overlay"

export namespace kodaConfigWriter {
  export type Conflict = {
    ok: false
    code: "target-changed" | "revision-conflict" | "target-not-writable"
    message: string
    target: kodaConfigOverlay.Target
  }
  ...
  export async function write(input: {
    ...
    const target = await kodaConfigOverlay.target(input)
    ...
    const patch = kodaConfigOverlay.patch({ scope: input.scope, set: input.set, unset: input.unset })
    if (Object.keys(patch).length === 0) return { ok: true, target, changed: false, sandboxChanged: false }
    await mkdir(path.dirname(target.path), { recursive: true })
    await input.beforeWrite?.()
    const checked = await kodaConfigOverlay.target(input)
    ...
    if (
      expected &&
      kodaConfigOverlay.revision(checked.path, checked.exists, checked.exists ? before : "") !== expected.revision
    ) {
      return {
        ok: false,
    ...
    return {
      ok: true,
      target: await kodaConfigOverlay.target(input),
