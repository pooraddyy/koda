...
export namespace SessionResume {
  ...
  function markUnpaired(step: Step): Step {
    ...
    const resultIDs = new Set(step.parts.filter((p): p is ToolResult => p.type === "tool_result").map((r) => r.callID))
  ...
  const CLAUDE_META_TYPES = new Set(["ai-title", "last-prompt", "snapshot", "mode", "attachment"])
  ...
  function parseClaudeSteps(objects: unknown[]): { steps: Step[]; unsupported: number } {
    ...
    for (const obj of objects) {
      ...
      if (role === "assistant") {
        ...
        // role === "user"
        const hasToolResults = content.some((block) => isRecord(block) && block.type === "tool_result")
  ...
  function parseClaudeToolResults(blocks: unknown[]): { results: Part[]; unsupported: number } {
    const results: Part[] = []
    let unsupported = 0
  ...
  function mapUserParts(
    parts: Part[],
    msgID: string,
    sesID: string,
  ): { parts: Record<string, unknown>[]; dropped: string[] } {
  ...
  function mapAssistantParts(
    ...
    const toolParts = new Map<
      string,
      Array<{ part: Record<string, unknown>; index: number; status?: ToolCall["status"] }>
    >()
    const results: Part[] = []
    const out: Record<string, unknown>[] = []
    const dropped: string[] = []
    ...
    for (const item of results) {
      ...
      const entry = toolParts
        .get(result.callID)
        ?.find((candidate) => (candidate.part.state as Record<string, unknown>).status === "running")
      if (entry) {
        const tp = entry.part
        if (result.error) {
          tp.state = {
  ...
  export const resumeCodex = {
    name: "resume-codex",
    description: "import an OpenAI session transcript",
    source: "command" as const,
    template: "",
    hints: ["$ARGUMENTS"],
  }
