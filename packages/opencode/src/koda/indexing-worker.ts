import type { CodeIndexManager } from "@koda/koda-indexing/engine"
...
async function init(request: Extract<Request, { method: "init" }>) {
  ...
  if (request.input.lancedbPath) process.env.koda_LANCEDB_PATH = request.input.lancedbPath
  const [engine, status] = await Promise.all([
    import("@koda/koda-indexing/engine"),
    import("@koda/koda-indexing/status"),
  ])
