...
import { Struct } from "effect" // koda_change
...
import { EffectFlock } from "@opencode-ai/core/util/effect-flock" // koda_change
import { Config } from "@/config/config"
import { Global } from "@opencode-ai/core/global"
import { Info } from "@opencode-ai/schema/file-diff"
// koda_change start
import { Flag } from "@opencode-ai/core/flag/flag"
import { DiffFull } from "../koda/snapshot/diff-full"
import { kodaSnapshotTrack } from "../koda/snapshot/track"
import { kodaSnapshotSeed } from "../koda/snapshot/seed"
import { kodaSnapshotMaterialize } from "../koda/snapshot/materialize"
...
}).pipe(withStatics((s) => ({ zod: zod(s) }))) // koda_change
export type Patch = typeof Patch.Type

// koda_change - retain the legacy Zod facade while sharing the canonical schema
export const FileDiff = Info.pipe(withStatics((s) => ({ zod: zod(s) })))
export type FileDiff = typeof FileDiff.Type

// koda_change start - lightweight FileDiff without heavy content (patch/before/after) for session summaries
...
const prune = "7.days"
const retention = 7 * 24 * 60 * 60 * 1000 // koda_change
...
export const MAX_DIFF_SIZE = 256 * 1024 // koda_change
...
export interface Interface {
  ...
  // koda_change start - pass prompt context and managed initialization policy
  readonly track: (opts?: {
    sessionID?: SessionID
    messageID?: MessageID
    snapshotInitialization?: kodaSnapshotTrack.SnapshotInitialization
  ...
  readonly diffFile: (from: string, to: string, file: string) => Effect.Effect<FileDiff | undefined> // koda_change - authoritative full-content detail
}
...
export const layer: Layer.Layer<Service, never, Requirements> =
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const flock = yield* EffectFlock.Service // koda_change
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Snapshot.state")(function* (ctx) {
          ...
          // koda_change start - restoration must fail if deletion fails
          const remove = (file: string) => fs.remove(file, { force: true }).pipe(Effect.orDie)
          // koda_change end
          // koda_change start - serialize snapshot repositories across CLI and extension processes
          ...
          const enabled = Effect.fnUntraced(function* () {
            ...
            if (Flag.koda_CLIENT === "acp") return false // koda_change - ACP clients do not support snapshots
          ...
          const add = Effect.fnUntraced(function* (opts?: { env?: Record<string, string>; root?: boolean }) {
            ...
            // koda_change start - initial seeded writes stay protected by the source pin
            yield* stage(
          ...
          const materialize = Effect.fnUntraced(function* () {
            yield* locked(kodaSnapshotMaterialize.run({ gitdir: state.gitdir, git, fs }).pipe(Effect.orDie)).pipe(
          ...
          const cleanup = Effect.fnUntraced(function* () {
            return yield* locked(
              Effect.gen(function* () {
                ...
                // koda_change start - retain snapshots for the same seven-day window as object pruning
                yield* kodaSnapshotMaterialize.prune({ gitdir: state.gitdir, git, fs }, Date.now() - retention)
                // koda_change end
          ...
          const track = Effect.fnUntraced(function* (opts?: Parameters<Interface["track"]>[0]) {
            ...
            return yield* locked(
              Effect.gen(function* () {
                ...
                const seeded: { value?: kodaSnapshotSeed.Output } = {} // koda_change
                ...
                if (!existed) {
                  ...
                  // koda_change start - seed all eligible new snapshots from the worktree index
                  seeded.value = yield* kodaSnapshotSeed.seed({
                    dir: state.directory,
                ...
                // koda_change start - pin every snapshot before background materialization
                ...
                if (
                  seed &&
                  !(yield* kodaSnapshotMaterialize.localize({
                    gitdir: state.gitdir,
                    git,
                    fs,
                ...
                if (
                  seed &&
                  !(yield* kodaSnapshotMaterialize.localizeTrees(
                    { gitdir: state.gitdir, git, fs, staging: seed.staging },
                    hash,
                  ))
                )
                  return
                if (!(yield* kodaSnapshotMaterialize.pin({ gitdir: state.gitdir, git, fs }, hash))) return
            ...
            return yield* locked(
              Effect.gen(function* () {
                ...
                if (result.code === 0) {
                  ...
                  return yield* Effect.die(new Error(`Failed to restore snapshot ${snapshot}`)) // koda_change
                }
                yield* Effect.logError("failed to restore snapshot", {
                  snapshot,
                  exitCode: result.code,
          ...
          const revert = Effect.fnUntraced(function* (patches: Patch[]) {
            return yield* locked(
              Effect.gen(function* () {
                // koda_change start - validate every checkpoint before mutating workspace files
                ...
                const single = Effect.fnUntraced(function* (op: (typeof ops)[number]) {
                  ...
                  // koda_change start - never report success for a file that Git could not restore
                  if (tree.code !== 0) {
          ...
          const diffFull = Effect.fnUntraced(function* (from: string, to: string) {
            return yield* locked(
              Effect.gen(function* () {
                ...
                // koda_change start - use git patches to avoid blocking the event loop on large diffs
          ...
          yield* materialize() // koda_change - resume interrupted snapshot object materialization
          ...
          // koda_change start - authoritative full-content detail for editor diff tabs
          ...
          return { cleanup, track, patch, restore, revert, diff, diffFull, diffFile } // koda_change - diffFile
        }),
      )

      // koda_change start - service-local state and cache avoid leaking across Snapshot layer instances
      const trackState = kodaSnapshotTrack.makeStates()
      ...
      return Service.of({
        ...
        // koda_change start - isolate turn-facing snapshot work from poisoned locks
        track: Effect.fn("Snapshot.track")(function* (opts) {
          const ctx = yield* InstanceState.context
          const guard = trackState(ctx.worktree)
          return yield* kodaSnapshotTrack.protect({
            inner: kodaSnapshotTrack.wrap({
        ...
        diffFull: Effect.fn("Snapshot.diffFull")(function* (from: string, to: string) {
          // koda_change start - cache full diffs at the service boundary
          if (from === to) return []
...
export const node = LayerNode.make({
  service: Service,
  layer,
  deps: [FSUtil.node, AppProcess.node, Config.node, EffectFlock.node], // koda_change
})
