...
import { TsCheck } from "../koda/ts-check" // koda_change
import type { RuntimeFlags } from "@/effect/runtime-flags"

const log = Log.create({ service: "lsp.server" }) // koda_change
...
// koda_change start - tsgo native LSP or lightweight diagnostic client
// When koda_EXPERIMENTAL_LSP_TOOL is enabled, spawn tsgo --lsp --stdio as a
...
export const Typescript: Info = {
  ...
  async spawn(root) {
    if (!Flag.koda_EXPERIMENTAL_LSP_TOOL) return undefined
...
export const Clangd: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    // koda_change start - reject release metadata before it becomes an executable path
    ...
    // koda_change start - do not use remote metadata as a local path
  ...
  async spawn(root, _ctx, flags) {
    ...
    return {
      process: spawn(
        java,
        [
          "-Djava.import.generatesMetadataFilesAtProjectRoot=false", // koda_change
          "-jar",
          launcherJar,
          "-configuration",
          configFile,
...
export const LuaLS: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    if (!bin) {
      ...
      // koda_change start - use a fixed local archive name
