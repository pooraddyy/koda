...
import { IngestQueue } from "@/koda-sessions/ingest-queue"
import { IngestDrain } from "@/koda-sessions/ingest-drain"
import { clearInFlightCache, withInFlightCache } from "@/koda-sessions/inflight-cache"
import type * as SDK from "@koda/sdk/v2"
import z from "zod"
...
import { RemoteWS } from "@/koda-sessions/remote-ws"
import { RemoteSender } from "@/koda-sessions/remote-sender"
import { RemoteProtocol } from "@/koda-sessions/remote-protocol"
import { buildInstanceAdvertisement } from "@/koda-sessions/instance-advertisement"
import { detectPrLink, readPrLinkOverride } from "@/koda-sessions/pr-link"
import type { PrLink } from "@/koda-sessions/pr-link"
import { AttachedState } from "@/koda-sessions/attached-state"
import {
  ...
  markAutoTitle,
  markRenameAdopted,
} from "@/koda-sessions/rename-adoptions"
import { SessionStatus } from "@/session/status"
import { Telemetry } from "@koda/koda-telemetry"
import { Question } from "@/question"
import { Permission } from "@/permission"
import { withTimeout } from "@/util/timeout"
import { Snapshot } from "@/snapshot"
import { cumulativeSessionDiff } from "@/koda/session-portability/cumulative-diff"
...
async function provide<R>(input: { directory: string; fn: () => R }): Promise<R> {
  const { provide } = await import("@/koda/instance")
  return provide(input)
}

export namespace kodaSessions {
  export const Event = {
    RemoteStatusChanged: BusEvent.define(
      "koda-sessions.remote-status-changed",
      Schema.Struct({
  ...
  export class Service extends Context.Service<Service, Interface>()("@koda/kodaSessions") {}

  const log = Log.create({ service: "koda-sessions" })
  ...
  const tokenValidKeyTemplate = "koda-sessions:token-valid:"
  let tokenValidKey = tokenValidKeyTemplate + "unknown"

  const tokenKey = "koda-sessions:token"
  const orgKey = "koda-sessions:org"
  const clientKey = "koda-sessions:client"
  const gitUrlKeyPrefix = "koda-sessions:git-url:"
  ...
  function agentNotificationTimeoutMs(): number {
    const value = process.env["koda_AGENT_NOTIFICATION_TIMEOUT_MS"]
  ...
  async function authValid(token: string) {
    ...
    return withInFlightCache(tokenValidKey, 15 * 60_000, async () => {
      const response = await fetch(`${koda_API_BASE}/api/user`, {
        headers: {
  ...
  async function kodaToken() {
    return withInFlightCache(tokenKey, ttlMs, async () => {
      const auth = await runtime.runPromise((svc) => svc.get("koda"))
      ...
      const key = process.env["koda_API_KEY"]?.trim()
      if (key) return key
      return undefined
    })
  }
  ...
  async function getClient(): Promise<Client | undefined> {
    return withInFlightCache(clientKey, ttlMs, async () => {
      const token = await kodaToken()
      if (!token) return undefined

      const valid = await authValid(token)
      if (!valid) return undefined

      const base = process.env["koda_SESSION_INGEST_URL"] ?? "https://ingest.kodasessions.ai"
  ...
  const shareDisabled = process.env["koda_DISABLE_SHARE"] === "true" || process.env["koda_DISABLE_SHARE"] === "1"
  const ingestDisabled =
    process.env["koda_DISABLE_SESSION_INGEST"] === "true" || process.env["koda_DISABLE_SESSION_INGEST"] === "1"
  const debugIngest =
    process.env["koda_DEBUG_SESSION_INGEST"] === "true" || process.env["koda_DEBUG_SESSION_INGEST"] === "1"
  ...
  const remoteEnabled = process.env["koda_REMOTE"] === "1"
  let remote: { conn: RemoteWS.Connection; sender: RemoteSender.Sender } | undefined
  let enabling: Promise<void> | undefined
  let remoteSeq = 0
  // koda_change - K1 W1: module-level instance advertisement flag.
  // `enableRemote` can be triggered either by the explicit `koda remote` command
  // or by bootstrap auto-enable (`koda_REMOTE=1` / `remote_control` config); it
  ...
  // koda_change - PR link advertise (plan 8.2/8.4): resolve the worktree PR
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const reportSessionTitle = Effect.fn("kodaSessions.reportSessionTitle")(function* (
        sessionID: string,
        title: string,
        opts: { generated: boolean },
      ) {
      ...
      const state = yield* InstanceState.make(
        Effect.fn("kodaSessions.state")(function* (ctx) {
          ...
          watch(Session.Event.Updated, async (evt) => {
            ...
            try {
              await ingest.sync(sessionID, [
                { type: "koda_meta", data: await meta(sessionID, session) },
      ...
      const init = Effect.fn("kodaSessions.init")(function* () {
        yield* InstanceState.get(state)
      })

      const sendAgentNotification = Effect.fn("kodaSessions.sendAgentNotification")(function* (
        sessionID: string,
        input: { id: string; message: string },
      ) {
        if (ingestDisabled) {
  ...
  // koda_change - DEF-1: default advertisement for every successful
  ...
  export async function enableRemote() {
    ...
    enabling = (async () => {
      const token = await kodaToken()
      if (!token) {
        throw new Error("Unable to enable remote: no koda credentials found. Run `koda auth login`.")
      }

      const valid = await authValid(token)
      if (valid === false) {
        throw new Error("Unable to enable remote: invalid or expired koda credentials. Run `koda auth login`.")
      }
      if (valid === undefined) throw new Error("Unable to enable remote: failed to verify koda credentials.")

      const url = (process.env["koda_SESSION_INGEST_URL"] ?? "https://ingest.kodasessions.ai")
        .replace(/^https:\/\//, "wss://")
        .replace(/^http:\/\//, "ws://")

      const [{ RemoteWS }, { RemoteSender }] = await Promise.all([
        import("@/koda-sessions/remote-ws"),
        import("@/koda-sessions/remote-sender"),
      ])
      ...
      // koda_change - K1 W1: capture module-level advertisement so each
      ...
      const getSessions = async (): Promise<RemoteProtocol.Heartbeat> => {
              ...
              [...ids].map((id) =>
                svc.get(SessionID.make(id)).pipe(
                  Effect.map((session) => ({
                    ...
                    gitUrl,
                    gitBranch,
                    // koda_change - K1 W1: per-session platform, mirrors
                    // meta()'s resolution order so the live value always agrees
                    // with the session's stored created_on_platform.
                    platform: kodaSession.resolvePlatform(id) || process.env["koda_PLATFORM"] || "cli",
                  })),
                  Effect.orElseSucceed(() => undefined),
                ),
              ),
        ...
        // koda_change - PR link advertise (plan 8.2): resolve once
      ...
      const conn = RemoteWS.connect({
        ...
        onOpen: () => {
          ...
          // koda_change - K1 W1: on reconnect, a headless `koda remote` host
      ...
      const sender = RemoteSender.create({
        ...
        log,
        // koda_change - K1 W1: in-process attach/detach/ownership seams
        // back to kodaSessions. The sender does NOT spawn a process per
        // session — concurrent remote sessions share this CLI process with
        // per-directory InstanceRef isolation.
        attachSession: (id) => kodaSessions.attachRemoteSession(id),
        detachSession: (id) => kodaSessions.detachRemoteSession(id),
        hasSession: (id) => kodaSessions.hasRemoteSession(id),
        ownedCount: () => kodaSessions.ownedRemoteSessionCount(),
        cancelPrompt: async (id) => {
          // koda_change - K1 W1: dynamic import breaks the module-load cycle
          // (@/session/prompt reads kodaSessionPrompt at eval; a static edge here
  ...
  // koda_change - K1 W1: instance advertisement setter.
  ...
  // explicit `koda remote` command — `enableRemote` itself is coalescing), we
  ...
  // (once a process runs `koda remote`, it keeps advertising for its whole
  ...
  // koda_change - K1 W1: session-detach semantics. The exit_cli handler
  ...
  // koda_change - K1 W1: ownership probe used by the exit_cli handler
  ...
  // koda_change - K1 W1: count of "owned" sessions (presence ∪ pending).
  ...
  // zero remaining + no callback (koda remote) => keep host alive. Sessions
  ...
  export async function share(sessionId: string) {
    if (ingestDisabled) {
      throw new Error("Session ingest is disabled (koda_DISABLE_SESSION_INGEST=1)")
    }

    if (shareDisabled) {
      throw new Error("Sharing is disabled (koda_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to share session: no koda credentials found. Run `koda auth login`.")
    }
    ...
    const url = `https://app.koda.ai/s/${result.share_token}`
  ...
  export async function unshare(sessionId: string) {
    ...
    if (shareDisabled) {
      throw new Error("Unshare is disabled (koda_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to unshare session: no koda credentials found. Run `koda auth login`.")
    }
  ...
  async function meta(sessionId?: string, info?: Session.Info | null) {
    const override = sessionId ? kodaSession.resolvePlatform(sessionId) : undefined
    const platform = override || process.env["koda_PLATFORM"] || "cli"
  ...
  async function getOrgId(sessionId?: string, info?: Session.Info | null): Promise<Uuid | undefined> {
    ...
    const env = process.env["koda_ORG_ID"]
