// koda_change - extracted state machine that separates presence-owned
...
export namespace AttachedState {
  ...
  // koda_change - collision-safe union key. The historical "|" join
  ...
  export function create(options: Options): Interface {
    ...
    // koda_change - K1 W1: tombstones for ids that have been remotely
    ...
    // koda_change - in-flight dedup. Concurrent announce(id) callers
    ...
    // koda_change - in-flight detach dedup, mirrors `inflight` for the
    ...
    // koda_change end
    let lastSentKey = ""
    // koda_change - lifecycle generation. Incremented on reset() so a
    ...
    return {
      setPresence(ids) {
        ...
        // koda_change - K1 W1: suppression tombstone. Any id in `next`
      ...
      async announce(id) {
        ...
        // koda_change - join a same-kind in-flight announce so concurrent
        ...
        // koda_change - K1 W1: if a detach is in flight for this id, we
        ...
        const owned = (async () => {
          ...
          try {
            ...
            // koda_change - K1 W1: if reset() ran while this heartbeat was
      ...
      // koda_change - K1 W1: session-detach semantics.
      async detach(id) {
        // koda_change - join a same-kind in-flight detach so concurrent
        ...
        // koda_change - K1 W1: if an announce is in flight for this id we
      ...
      reset() {
        ...
        pending.clear()
        inflight.clear()
        // koda_change - K1 W1: also drop the in-flight detach map and
        ...
        suppressed.clear()
        lastSentKey = ""
        // koda_change - bump the lifecycle generation so any in-flight
