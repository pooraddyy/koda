import { RemoteCommand } from "@/koda-sessions/remote-command"
import { RemoteExit } from "@/koda-sessions/remote-exit"
import { RemoteModelCatalog } from "@/koda-sessions/remote-model-catalog"
import { RemoteProtocol } from "@/koda-sessions/remote-protocol"
import { consumeRenameAdoption, markRenameAdopted } from "@/koda-sessions/rename-adoptions"
import type { RemoteWS } from "@/koda-sessions/remote-ws"
import { GlobalBus } from "@/bus/global"
import { RemoteAttachments } from "@/koda/remote-attachments"
...
type Provide = typeof import("@/koda/instance").provide
...
// koda_change start - create_session: strict v1 request with optional inheritance fields
...
// koda_change start - redact anything but the error class so messages/credentials
...
// koda_change start — lazy init to avoid circular dependency
...
function normalizeModel(model: string | RemoteModelCatalog.ModelRef | undefined) {
  ...
  return {
    providerID: ProviderV2.ID.make("koda"),
    modelID: ModelV2.ID.make(model.startsWith("koda/") ? model.slice("koda/".length) : model),
  }
}
...
export namespace RemoteSender {
  export type Options = {
    ...
    session?: {
      ...
      // koda_change start - injectable create hook for create_session.
      ...
      // koda_change - injectable remove hook used to roll back an orphan
      ...
      // koda_change - injectable setTitle for system session.renamed handling
      readonly setTitle?: (input: { sessionID: SessionID; title: string }) => Promise<void>
      // koda_change end
    }
    // koda_change - K1 W1: in-process attach/detach/ownership/cancel
    // seams. All four are optional and default to a lazy import of
    // `kodaSessions` (production wires them in `enableRemote`, so the
  ...
  export function create(options: Options): Sender {
    ...
    // koda_change start - orphan rollback for create_session: when
    ...
    // koda_change end
    // koda_change - K1 W1: session create + in-process attach seams used by
    // create_session. Production wires `attachSession` to
    // `kodaSessions.attachRemoteSession` from inside `enableRemote` (see
    // koda-sessions.ts). Test fixtures inject stubs via the Options object.
    ...
    // koda_change end
    // koda_change start - injectable slash command discovery + execution
    ...
    async function replay(sessionId: string) {
      ...
      const queued = kodaSessionPromptQueue.snapshot(SessionID.make(sessionId))
      options.conn.send({
        type: "event",
        sessionId,
        ...(root ? { parentSessionId: root } : {}),
    ...
    function dispatch(msg: RemoteProtocol.Command) {
      // koda_change start - slash command discovery and execution
      ...
      if (msg.command === "exit_cli") {
        // koda_change - K1 W1: `exit_cli` now means "detach THIS remote
        // session and (if this is the last interactive session) close the
        // CLI." It is NOT "terminate the CLI." A headless `koda remote` host
        ...
        //      flush first. If zero remain + no callback (headless `koda
        ...
        void (async () => {
          try {
            ...
            //    (`koda remote`) never register a RemoteExit callback, so
      ...
      if (msg.command === "create_session") {
        // koda_change - K1 W1: in-process create_session. Optional wire
        // fields (agent/model/orgId) ride protocolVersion 1; orgId lands in
        // session metadata so the first koda_meta carries the claim. Handler
      ...
      // koda_change end
      // koda_change start - sessionless list_models for the pre-session instance picker
            ...
            try {
              ...
              await run({
                ...
                fn: async () => {
                  ...
                  // setTitle and the koda-sessions consumer is deferred, so a
