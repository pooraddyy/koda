...
export namespace RemoteCommand {
  ...
  export const Response = z
    .object({
      ...
      // koda_change - K1 W1: `canExitSession` is an INDEPENDENT producer
      // contract advertised by every CLI (interactive TUI and headless
      // `koda remote` alike) so the mobile client can rely on the
  ...
  // The only entry from BUILTIN_COMMANDS (koda/session/builtin-commands) exposed
  ...
  export function build(items: ReadonlyArray<CommandInfo>, exitAvailable = false): Response {
    ...
    // koda_change - K1 W1: always advertise `canExitSession: true`. This
    ...
    // command entry on RemoteExit.get()). A headless `koda remote` host has
  ...
  export function create(services: Services): Interface {
    return {
      ...
      execute: async (input) => {
        // koda_change - enforce membership in the supplied bounded
