// koda_change - koda compatibility layer. Upstream deleted this Bus (Effect PubSub) service in v1.16.2
// in favour of EventV2; koda keeps it ONLY for existing koda-owned callers (koda/* features) that rely on
// its eager-callback subscription + fork-atomicity semantics. Do NOT add new shared/upstream-shaped consumers.
// Full migration of koda callers onto core EventV2 is tracked as a dedicated follow-up.
import { Effect, Exit, Fiber, Layer, PubSub, Scope, Context, Stream, Schema } from "effect" // koda_change
...
import { context as instanceContext, type InstanceContext } from "@/project/instance-context" // koda_change
import { InstanceRef } from "@/effect/instance-ref"
import { LocalContext } from "@/util/local-context" // koda_change
import { LayerNode } from "@opencode-ai/core/effect/layer-node" // koda_change
...
export const node = LayerNode.make({ service: Service, layer, deps: [] }) // koda_change

const { runPromise } = makeRuntime(Service, layer) // koda_change
...
// koda_change start - legacy callback facade inherits the active instance context
function active() {
