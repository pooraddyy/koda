...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    // koda_change start - install local themes into supported koda config directories
    const base = path.basename(source_dir)
    const local_dir =
      base === ".koda" || base === ".koda"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".koda", "themes")
    // koda_change end
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".koda", "tui.json"), // koda_change
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.koda_PURE ? [] : pluginOrigins
    if (Flag.koda_PURE && pluginOrigins.length) {
    }
