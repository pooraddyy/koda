...
import { escapeHtml } from "@/util/html" // koda_change - retained OAuth error page uses escaped diagnostics
...
function authHeaders() {
  return {
    ...
    "User-Agent": `koda/${InstallationVersion}`, // koda_change
  }
}
...
export function buildAuthorizeUrl(
  ...
  // koda_change start
  // clients. `referrer=koda` lets xAI attribute koda-originated
  // koda_change end
...
<html>
  <head>
    <title>koda - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
...
<html>
  <head>
    <title>koda - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  const server = createServer((req, res) => {
    ...
    if (url.pathname === OAUTH_REDIRECT_PATH) {
      ...
      if (error) {
        ...
        res.end(HTML_ERROR(errorMsg)) // koda_change - shared callback page is currently OpenCode-branded
        return
      }
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // koda_change start
      // behavior and crash the entire koda process. Matches the silent-
      // koda_change end
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `koda/${InstallationVersion}`) // koda_change
