import type { Hooks, PluginInput } from "@koda/plugin"
...
import { refreshCodexAuth } from "@/koda/provider/codex-refresh" // koda_change
import { OpenAIWebSocketPool } from "./ws-pool"

const log = Log.create({ service: "plugin.codex" }) // koda_change
...
const ALLOWED_MODELS = new Set([
  ...
  "gpt-5.4",
  "gpt-5.4-mini",
  // koda_change start - additional codex models supported by koda
  "gpt-5.1-codex",
  "gpt-5.1-codex-max",
  "gpt-5.1-codex-mini",
  "gpt-5.2-codex",
...
async function refreshAccessToken(refreshToken: string, issuer = ISSUER, signal?: AbortSignal): Promise<TokenResponse> {
  const response = await fetch(`${issuer}/oauth/token`, {
    method: "POST",
    signal,
    headers: { "Content-Type": "application/x-www-form-urlencoded", "User-Agent": `koda/${InstallationVersion}` },
    // koda_change end
    body: new URLSearchParams({
...
// koda_change start - retain koda-branded OAuth callback until the shared page supports koda branding
const HTML_SUCCESS = `<!doctype html>
<html>
  <head>
    <!-- koda_change start -->
    <title>koda - OpenAI Authorization Successful</title>
    <!-- koda_change end -->
    <style>
      body {
        font-family:
  ...
  <body>
    <div class="container">
      ...
      <!-- koda_change start -->
      <p>You can close this window and return to koda.</p>
      <!-- koda_change end -->
    </div>
    <script>
...
<html>
  <head>
    <!-- koda_change start -->
    <title>koda - OpenAI Authorization Failed</title>
    <!-- koda_change end -->
    <style>
      body {
        font-family:
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  oauthServer = createServer((req, res) => {
    ...
    if (url.pathname === "/auth/callback") {
      ...
      res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" })
      res.end(HTML_SUCCESS) // koda_change - shared callback page is currently OpenCode-branded
      return
    }
      ...
      async models(provider, ctx) {
        ...
        return Object.fromEntries(
          Object.entries(provider.models)
            .filter(([, model]) => {
              if (model.options?.reasoningMode === "pro") return false // koda_change - tolerate catalog entries without options
              if (ALLOWED_MODELS.has(model.api.id)) return true
              if (DISALLOWED_MODELS.has(model.api.id)) return false // koda_change
              ...
              modelID,
              {
                ...
                limit: model.id.includes("gpt-5.5")
                  ...
                  : model.id.includes("gpt-5.6")
                    ? {
                        context: 1_050_000, // koda_change - use the current Codex limits for GPT-5.6 OAuth models
                        input: 922_000, // koda_change
                        output: 128_000,
                      }
                    : model.limit,
              },
