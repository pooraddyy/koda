...
import type { Model } from "@koda/sdk/v2"
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>koda - DigitalOcean Authorization</title>
    <style>
    ...
    <script>
      (async function() {
        ...
        try {
          ...
          msgEl.textContent = "You can close this window and return to koda."
...
async function startOAuthServer(): Promise<void> {
  ...
  oauthServer = createServer((req, res) => {
    ...
    if (req.method === "GET" && url.pathname === OAUTH_REDIRECT_PATH) {
      ...
      res.end(HTML_CALLBACK) // koda_change - shared callback page is currently OpenCode-branded
      return
    }
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              url,
              instructions:
                "Sign in to DigitalOcean in your browser. koda will use your DigitalOcean API token directly for inference and load your Inference Routers. Re-run /connect to refresh routers later.",
