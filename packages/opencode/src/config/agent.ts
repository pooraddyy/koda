...
import { ConfigVariable } from "./variable" // koda_change
import { normalizeExternalAgentFrontmatter } from "@/koda/agent/external-agent-compat"
// koda_change start
...
import { report } from "@/koda/config/report"
import type { Warning } from "./config"
// koda_change end

const log = Log.create({ service: "config" })

// koda_change start - trusted gates {env:}; fileScope confines untrusted agent prompt {file:} reads
export async function load(
  ...
  for (const item of await Glob.scan("{agent,agents}/**/*.md", {
    ...
    const md = await ConfigMarkdown.parse(item, { trusted, fileScope, sourceScope }).catch(async (err) => {
      // koda_change end
      const message = FrontmatterError.isInstance(err) ? err.data.message : `Failed to parse agent ${item}`
      // koda_change start
      if (warnings) warnings.push({ path: item, message })
      try {
        const { capture } = await import("@/koda/instance")
        const ctx = capture()
    ...
    // koda_change start - substitute agent prompt variables relative to the agent file. Project agents are
    ...
    const config = {
      name,
      ...normalizeExternalAgentFrontmatter(md.data),
      prompt,
    }
    // koda_change end
    // koda_change start - use Effect schema (propertyOrder: original) + non-fatal handleInvalid
    try {
      result[config.name] = ConfigParse.schema(ConfigAgentV1.Info, config, item)
    } catch (err) {
      if (ConfigError.InvalidError.isInstance(err)) {
        await kodaConfig.handleInvalid("agent", item, err.data.issues ?? [], err, warnings)
        continue
      }
      throw err
    }
...
export async function loadMode(
  ...
  for (const item of await Glob.scan("{mode,modes}/*.md", {
    ...
    const md = await ConfigMarkdown.parse(item, { trusted, fileScope, sourceScope }).catch(async (err) => {
      // koda_change end
      const message = FrontmatterError.isInstance(err) ? err.data.message : `Failed to parse mode ${item}`
      // koda_change start
      if (warnings) warnings.push({ path: item, message })
      try {
        const { capture } = await import("@/koda/instance")
