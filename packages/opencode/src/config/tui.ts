...
import { kodaDefaultPlugins } from "@/koda/config/default-plugins" // koda_change
...
export interface Interface {
  ...
  readonly info: () => Effect.Effect<Info> // koda_change - editable terminal configuration
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // koda_change start - trusted gates {env:}; fileScope confines untrusted {file:} reads
  const load = (
    text: string,
    ...
    // koda_change end
    Effect.gen(function* () {
      // koda_change start - only trusted tui config resolves {env:}; untrusted {file:} confined to fileScope
  ...
  // koda_change start - trusted + fileScope threaded to load
  const loadFile = (filepath: string, trusted: boolean, fileScope?: ConfigVariable.FileScope): Effect.Effect<Info> =>
    ...
    Effect.gen(function* () {
      ...
      return yield* load(text, filepath, trusted, fileScope) // koda_change
    })

  // koda_change start - trusted + fileScope threaded to loadFile
  const mergeFile = (acc: Acc, file: string, trusted: boolean, fileScope?: ConfigVariable.FileScope) =>
    // koda_change end
    Effect.gen(function* () {
      const data = yield* loadFile(file, trusted, fileScope) // koda_change
  ...
  // koda_change start - discover canonical and legacy koda config directories
  // Every config dir we may read from: global config, .koda and legacy .kilocode/.kilo
  // folders between cwd and home, and koda_CONFIG_DIR.
  // koda_change end
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.koda_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  for (const file of ConfigPaths.fileInDirectory(Global.Path.config, "tui")) {
    yield* mergeFile(acc, file, true) // koda_change - global config is trusted
  }

  // 2. Explicit koda_TUI_CONFIG override, if set.
  if (Flag.koda_TUI_CONFIG) {
    const configFile = Flag.koda_TUI_CONFIG
    yield* mergeFile(acc, configFile, true) // koda_change - explicit env-provided path is trusted
    yield* Effect.logDebug("loaded custom tui config", { path: configFile })
  }

  // 3. Project tui files, applied root-first so the closest file wins.
  for (const file of projectFiles) {
    yield* mergeFile(acc, file, false, { root: ctx.directory, source: file }) // koda_change - untrusted, {file:} confined to project
  }

  // koda_change start - load tui.json from supported koda config directories
  // 4. `.koda` and legacy `.kilocode`/`.kilo` directories (and koda_CONFIG_DIR)
  ...
  const dirs = unique(directories).filter(
    (dir) =>
      dir.endsWith(".koda") || dir.endsWith(".kilocode") || dir.endsWith(".kilo") || dir === Flag.koda_CONFIG_DIR,
  )
  // koda_change end

  for (const dir of dirs) {
    // koda_change start - trust global (home/koda_CONFIG_DIR) dirs like config.ts; in-repo .koda/.koda stay untrusted
  ...
  if (acc.result.attention?.sound_pack === undefined) result.attention.sound_pack = "koda.default" // koda_change - preserve koda's default sound pack

  // koda_change start - inject koda default plugins to keep TUI aligned with server config
  const defaults = kodaDefaultPlugins.apply(
    { plugin: result.plugin ? [...result.plugin] : undefined, plugin_origins: acc.plugin_origins },
    { disabled: Flag.koda_DISABLE_DEFAULT_PLUGINS },
  )
  ...
  return {
    config,
    info: { ...acc.result, plugin: defaults.plugin }, // koda_change - include applied koda defaults
    pluginOrigins: defaults.plugin_origins ?? [], // koda_change - exclude builtins from dependency installation
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const info = Effect.fn("TuiConfig.info")(() => Effect.succeed(data.info)) // koda_change
    const pluginOrigins = Effect.fn("TuiConfig.pluginOrigins")(() => Effect.succeed(data.pluginOrigins))

    const waitForDependencies = Effect.fn("TuiConfig.waitForDependencies")(() =>
      Effect.forEach(deps, Fiber.join, { concurrency: "unbounded" }).pipe(Effect.ignore(), Effect.asVoid),
    )
    return Service.of({ get, info, pluginOrigins, waitForDependencies }) // koda_change
