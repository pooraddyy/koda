...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/koda" // koda_change
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "koda") // koda_change
    default:
      return "/etc/koda" // koda_change
  }
}

export function managedConfigDir() {
  return process.env.koda_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
