...
import { access, constants } from "fs/promises" // koda_change
...
const TUI_SCHEMA_URL = "https://app.koda.ai/tui.json" // koda_change
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  // koda_change start: use koda directory everywhere
  const project = Flag.koda_DISABLE_PROJECT_CONFIG
    ? []
    : await Filesystem.findUp(["koda.json", "koda.jsonc"], input.cwd, undefined, { rootFirst: true })
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "koda")]
  // koda_change end
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "koda"))
  }
  if (Flag.koda_CONFIG) files.push(Flag.koda_CONFIG)
