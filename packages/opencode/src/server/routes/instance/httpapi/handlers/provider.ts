...
import { mapValues, pickBy } from "remeda" // koda_change
import { ModelCache } from "@/provider/model-cache" // koda_change
import {
  disposeAllInstancesAfterProviderAuthCallback,
  invalidatePresence,
} from "@/koda/server/provider-auth-lifecycle" // koda_change
import { providerMetadata } from "@/koda/provider/metadata" // koda_change
import { filterPromptTrainingModels } from "@/koda/provider/model-filter" // koda_change
import { overlay as overlayAnacondaDesktop } from "@/koda/anaconda-desktop/provider" // koda_change
...
function mapProviderAuthError<A, R>(self: Effect.Effect<A, ProviderAuth.Error, R>) {
  return self.pipe(
    Effect.mapError((error) => {
      ...
      return new ProviderAuthApiError({ name: "BadRequest", data: { message: error.message } }) // koda_change
    }),
  )
}
...
export const providerHandlers = HttpApiBuilder.group(InstanceHttpApi, "provider", (handlers) =>
  Effect.gen(function* () {
    ...
    const cache = yield* ModelCache.Service // koda_change

    const list = Effect.fn("ProviderHttpApi.list")(function* () {
      const config = yield* cfg.get()
      const all = overlayAnacondaDesktop(yield* ModelsDev.Service.use((s) => s.get())) // koda_change
    ...
    const callback = Effect.fn("ProviderHttpApi.callback")(function* (ctx: {
      ...
      // koda_change start - drop old-user presence before instance disposal on koda OAuth callback
      if (ctx.params.providerID === "koda") yield* invalidatePresence()
      // koda_change end
      yield* disposeAllInstancesAfterProviderAuthCallback() // koda_change
      return true
    })
