import { Image } from "@/image/image" // koda_change - classify user image validation defects
import { busyMessage, isBusy } from "@/koda/database/sqlite-error" // koda_change
import { kodaSessionHttpApi } from "@/koda/server/httpapi/session-fork" // koda_change
...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const viewers = yield* kodaViewers.Service // koda_change
    ...
    const diff = Effect.fn("SessionHttpApi.diff")(function* (ctx: {
      ...
      // koda_change start - pass full-content detail query fields through to the summary service
    ...
    const forkRaw = kodaSessionHttpApi.forkRaw(fork) // koda_change - carry upstream bodyless full-session fork support
    ...
    const prompt = Effect.fn("SessionHttpApi.prompt")(function* (ctx: {
      ...
      const message = yield* promptSvc
        .prompt({ ...ctx.payload, sessionID: ctx.params.sessionID } as unknown as SessionPrompt.PromptInput) // koda_change
        .pipe(
          // koda_change start - reject only typed user image validation defects as request errors
    ...
    const promptAsync = Effect.fn("SessionHttpApi.promptAsync")(function* (ctx: {
      ...
      yield* promptSvc
        ...
        .pipe(
          Effect.catchCause((cause) => {
            if (Cause.hasInterruptsOnly(cause)) return Effect.void // koda_change - Stop is not an error
            return Effect.gen(function* () {
              const error = Cause.squash(cause)
              // koda_change start - keep SQLite lock failures out of local CLI logs
              ...
              yield* events.publish(Session.Event.Error, {
                sessionID: ctx.params.sessionID,
                error: busy // koda_change
                    ? new NamedError.Unknown({ message: busyMessage }).toObject() // koda_change
                    : new NamedError.Unknown({ message: Cause.pretty(cause) }).toObject(), // koda_change
              })
            })
          }),
    ...
    const deleteMessage = Effect.fn("SessionHttpApi.deleteMessage")(function* (ctx: {
      ...
      yield* requireSession(ctx.params.sessionID)
      // koda_change start - allow deleting prompts that are queued behind the active turn
      const remove = yield* runState.assertNotBusy(ctx.params.sessionID).pipe(
        Effect.as(true),
        Effect.catchTag("SessionBusyError", () =>
          kodaSessionPromptQueue.drop(ctx.params.sessionID, ctx.params.messageID),
        ),
      )
    ...
    return handlers
      ...
      .handleRaw("fork", forkRaw) // koda_change - carry upstream bodyless full-session fork support
      .handle("abort", abort)
      .handle("init", init)
      .handle("share", share)
      ...
      .handle("viewed", viewed) // koda_change
  }),
)
