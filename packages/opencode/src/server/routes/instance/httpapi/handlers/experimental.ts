...
import { EffectBridge } from "@/effect/bridge" // koda_change
...
import { Filesystem } from "@/util/filesystem" // koda_change
import { Review } from "@/koda/review/review" // koda_change
import { WorktreeDiff } from "@/koda/review/worktree-diff" // koda_change
import { WorktreeFamily } from "@/koda/worktree-family" // koda_change
import { Worktree } from "@/worktree"
import { Effect, Option } from "effect"
import * as HttpServerResponse from "effect/unstable/http/HttpServerResponse"
import { HttpApiBuilder, HttpApiError } from "effect/unstable/httpapi"
import * as Log from "@opencode-ai/core/util/log" // koda_change
import path from "path" // koda_change
  ...
  Effect.gen(function* () {
    ...
    const tool = Effect.fn("ExperimentalHttpApi.tool")(function* (ctx: { query: typeof ToolListQuery.Type }) {
      ...
      const list = yield* registry.tools({
        ...
        modelID: model ? ModelV2.ID.make(model.api.id) : ctx.query.model, // koda_change
        family: model?.family, // koda_change
    ...
    // koda_change start - discover Agent Manager and external git worktrees
    ...
    // koda_change start - worktree diff endpoints for agent manager
    ...
    const session = Effect.fn("ExperimentalHttpApi.session")(function* (ctx: { query: typeof SessionListQuery.Type }) {
      ...
      // koda_change end
      if (roots && directory && !current) return HttpServerResponse.jsonUnsafe([]) // koda_change
      const all = yield* sessions.listGlobal({
        projectID, // koda_change
        directory: ctx.query.worktrees ? undefined : ctx.query.directory, // koda_change
        directories: roots, // koda_change
        currentDirectory: directory, // koda_change
      ...
      // koda_change start - resolve worktree folder name for each session
      const result = sorted
        ? all.map((session) => {
      ...
      return HttpServerResponse.jsonUnsafe(list, {
        headers:
          result.length > limit && list.length > 0 // koda_change
