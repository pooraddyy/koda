...
import { Credential } from "@opencode-ai/core/credential" // koda_change
import { Skill } from "@/skill"
import { Discovery } from "@/skill/discovery"
import { Snapshot } from "@/snapshot"
import { Storage } from "@/storage/storage"
import { SyncEvent } from "@/sync" // koda_change
...
import { Pty } from "@opencode-ai/core/pty" // koda_change
...
} from "@/koda/server/reference-reconciler" // koda_change
...
// koda_change start
import {
  provide as providekodaHttpApiHandlers,
  provideListener as providekodaListenerRoutes,
} from "@/koda/server/httpapi/server"
// koda_change end
...
const instanceApiRoutes = HttpApiBuilder.layer(InstanceHttpApi).pipe(
  ...
  providekodaHttpApiHandlers, // koda_change
)
...
const serverRoutes = HttpApiBuilder.layer(ServerApi).pipe(
  // koda_change start - effective references must be ready before any V2 location consumer runs
...
// The terminal-only server exposes declared API routes only; no browser fallback is mounted.
const uiRoute = Layer.empty
...
const app = LayerNode.group([
  ...
  Plugin.node,
  ModelsDev.node,
  ModelCache.node, // koda_change - export the shared koda model cache to route handlers
  Provider.node,
  ProviderAuth.node,
  Agent.node,
  Skill.node,
...
export function createRoutes(
  ...
  return Layer.mergeAll(
    ...
    Layer.provide([
      ...
      fenceLayer,
      cors(corsOptions),
      MemoryService.layer, // koda_change
      // koda_change start
      AgentManager.defaultLayer,
      Notebook.defaultLayer,
      kodaViewers.defaultLayer,
      SyncEvent.defaultLayer,
      // koda_change end
      AppNodeBuilderV1.build(MoveSession.node, [[LocationServiceMap.node, locationServiceMapV2]]),
      AppNodeBuilderV1.build(EffectFlock.node), // koda_change
...
// koda_change start - keep listener routes local while application services come from AppRuntime
export function createListenerRoutes(corsOptions?: CorsOptions) {
  ...
  return Layer.mergeAll(
    ...
    providekodaListenerRoutes(corsOptions),
    ...
    // createRoutes uses.
    //
    // These builds sit inside kodaListener's Layer.fresh boundary, so each one self-provides its own
