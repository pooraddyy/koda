...
// koda_change start
import { matchLegacykodaOpenApi } from "@/koda/server/httpapi/public"
import * as kodaServer from "@/koda/server/server"
// koda_change end
...
const QueryParameterSchemas: Record<string, OpenApiSchema> = {
  ...
  "GET /koda/cloud-sessions cursor": { type: "string" },
  "GET /koda/cloud-sessions limit": { type: "number" },
  // koda_change end
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for the koda serve command", // koda_change
...
function matchLegacyOpenApi(input: Record<string, unknown>) {
  ...
  restoreAgentManagerNulls(spec) // koda_change
  ...
  matchLegacykodaOpenApi(input) // koda_change
  return input
}
...
function applyLegacySchemaOverrides(spec: OpenApiSpec) {
  ...
  // koda_change start - preserve model reset sentinels in indexing config patches
...
// koda_change start - preserve nullable Agent Manager move targets in generated SDK schemas
...
function normalizeParameter(param: OpenApiParameter, route: string) {
  ...
  if (param.in === "path") {
    param.schema = pathParameterSchema(route, param.name) ?? stripOptionalNull(param.schema) // koda_change
    return
  }
  if (param.in === "query") {
...
export const PublicApi = OpenCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: kodaServer.DOC_TITLE, // koda_change
    version: "1.0.0",
    description: kodaServer.DOC_DESCRIPTION, // koda_change
