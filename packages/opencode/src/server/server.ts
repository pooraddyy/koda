...
import { serverUrls } from "@/koda/cli/server-urls" // koda_change
...
import * as kodaListener from "@/koda/server/listener" // koda_change
...
export async function listen(opts: ListenOptions): Promise<Listener> {
  ...
  return {
    ...
    port: listener.port,
    url: listener.url,
    urls: listener.urls, // koda_change
...
const listenEffect: (opts: ListenOptions) => Effect.Effect<EffectListener, unknown> = Effect.fn("Server.listen")(
  function* (opts: ListenOptions) {
    ...
    return {
      ...
      port: address.port,
      url: listenerUrl,
      urls: serverUrls(opts.hostname, address.port), // koda_change
...
function startListener(opts: ListenOptions, port: number) {
  ...
  return kodaListener.build(listenerLayer(opts, port), scope).pipe(
    // koda_change
  ...
  return Effect.gen(function* () {
    ...
    return (close?: boolean) =>
      Effect.gen(function* () {
        ...
        if (close) yield* Effect.promise(() => Pty.shutdown()) // koda_change
