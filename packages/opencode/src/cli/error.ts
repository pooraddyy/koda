...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed.` // koda_change
  }
  ...
  // ProviderModelNotFoundError: { providerID: string, modelID: string, suggestions?: string[], modelsEmpty?: boolean } // koda_change
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      ...(providerModelNotFound.modelsEmpty === true ? ["No models are currently available."] : []), // koda_change
      `Try: \`koda models\` to list available models`, // koda_change
      `Or check your koda.json provider/model names`, // koda_change
    ].join("\n")
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`koda auth login ${url}\` to re-authenticate.`] : []), // koda_change
    ].join("\n")
  }
