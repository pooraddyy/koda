...
import { existsSync } from "node:fs" // koda_change
import { detectPrLink, parsePrUrl, readPrLinkOverride, writePrLinkOverride } from "@/koda-sessions/pr-link" // koda_change

const subcommand = "pr" // koda_change

// koda_change start - resolve the currently running CLI instead of hardcoding opencode
export function cliCommand(
  ...
  if (script === subcommand) return [input.execPath] // koda_change
...
export const PrCommand = cmd({
  command: subcommand,
  describe: "manage pull requests", // koda_change
  builder: (yargs: Argv) =>
    yargs
...
export const PrCheckoutCommand = effectCmd({
  ...
  describe: "fetch and checkout a GitHub PR branch, then run koda", // koda_change
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr.checkout")(function* (args) {
    ...
    const cli = cliCommand() // koda_change
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        const sessionMatch = prInfo.body.match(/https:\/\/app\.koda\.ai\/s\/([a-zA-Z0-9_-]+)/) // koda_change
        if (sessionMatch) {
    ...
    UI.println()
    UI.println("Starting koda...") // koda_change
    UI.println()

    const run = sessionId ? [...cli, "-s", sessionId] : cli // koda_change
    ...
    if (code !== 0) return yield* Effect.die(new Error(`koda exited with code ${code}`)) // koda_change
  }),
})

// koda_change start - link/unlink/status write and read the manual PR override in Storage
