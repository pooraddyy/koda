import type { Session as SDKSession, Message, Part } from "@koda/sdk/v2"
...
const log = Log.create({ service: "import" }) // koda_change
...
// koda_change start
/** Extract share token from a koda share URL like https://app.koda.ai/s/<jwt> */
export function parseShareUrl(url: string): string | null {
  const match = url.match(/^https?:\/\/app\.koda\.ai\/s\/([A-Za-z0-9_.-]+)$/)
...
async function ingestBootstrap(sessionId: string) {
  ...
  return kodaSessions.bootstrap(sessionId)
}
...
const runImport = Effect.fn("Cli.import.body")(function* (file: string, ctx: InstanceContext) {
  ...
  if (isUrl) {
    // koda_change start - Migrate to upstream ShareNext architecture #10281
    const slug = parseShareUrl(file)
    if (!slug) {
      process.stdout.write(`Invalid URL format. Expected: https://app.koda.ai/s/<id>`)
