...
// koda_change - @/koda/auth/remove is dynamically imported in the logout handler to keep startup fast
...
export const ProvidersCommand = cmd({
  // koda_change start - keep "auth" as primary command name
  command: "auth",
  aliases: ["providers"],
  // koda_change end
...
export const ProvidersLoginCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.provider) {
      ...
      // koda_change start - accept codex as an alias for OpenAI ChatGPT auth
    ...
    if (provider === "other") {
      ...
      yield* Prompt.log.warn(
        `This only stores a credential for ${provider} - you will need configure it in koda.json, check the docs for examples.`, // koda_change
      )
    }
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          ...
          "Configure via koda.json options (profile, region, endpoint) or\n" + // koda_change
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://koda.ai/docs/ai-providers/cloudflare", // koda_change
      )
    }
...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const { remove: removeAuth } = yield* Effect.promise(() => import("@/koda/auth/remove"))
