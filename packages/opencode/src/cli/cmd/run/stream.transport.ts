...
import type { Event, GlobalEvent, kodaClient } from "@koda/sdk/v2" // koda_change - revert to upstream native Event type
...
function sid(event: Event): string | undefined {
  ...
  if (event.type === "interactive_terminal.updated") return event.properties.info.sessionID // koda_change
  ...
  if (
    ...
    event.type === "interactive_terminal.data" || // koda_change
    event.type === "interactive_terminal.deleted" || // koda_change
...
function globalPayloadEvent(value: unknown): Event | undefined {
  ...
  // koda_change start - revert to upstream: ignore sync compatibility copies
