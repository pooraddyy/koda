...
import type { kodaClient, PermissionRequest, QuestionRequest, ToolPart } from "@koda/sdk/v2"
import type { RunInteractiveTerminalSnapshot } from "@/koda/cli/cmd/run/types" // koda_change
...
type PromptModel = Parameters<kodaClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<kodaClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<kodaClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<kodaClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<kodaClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<kodaClient["experimental"]["resource"]["list"]>>["data"]>
...
export type FooterView =
  ...
  | { type: "interactive_terminal"; terminal: RunInteractiveTerminalSnapshot } // koda_change
...
export type PermissionReply = Parameters<kodaClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<kodaClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<kodaClient["question"]["reject"]>[0]
