...
//
// Renders the full koda entry logo and a compact [O] exit badge, plus // koda_change
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "koda", right, undefined, TextAttributes.BOLD) // koda_change
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `koda run -i -s ${meta.session_id}`, // koda_change
      right,
      undefined,
      TextAttributes.BOLD,
    )
