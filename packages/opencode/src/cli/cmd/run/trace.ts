...
//
// Enable with koda_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/koda/log/direct/<timestamp>-<pid>.jsonl. Also writes // koda_change
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.koda_DIRECT_TRACE) {
    state = false
    return undefined
  }
