...
import type { Event, Part, PermissionRequest, QuestionRequest, ToolPart } from "@koda/sdk/v2" // koda_change - revert to upstream native Event type
...
export type SessionData = {
  ...
  terminal?: RunInteractiveTerminalSnapshot // koda_change
...
function pickSessionView(data: SessionData): FooterView {
  return pickBlockerView({
    terminal: data.terminal, // koda_change
...
export function reduceSessionData(input: SessionDataInput): SessionDataOutput {
  ...
  // koda_change start - direct interactive mode terminal footer
