...
import { kodaRunTerminal } from "@/koda/cli/cmd/run-terminal" // koda_change
import { createRunDemo } from "./demo"
import { resolveModelInfo, resolveRunTuiConfig, resolveSessionInfo } from "./runtime.boot"
import { createRuntimeLifecycle } from "./runtime.lifecycle"
import { trace } from "./trace"
import { cycleVariant, formatModelLabel, resolveSavedVariant, resolveVariant, saveVariant } from "./variant.shared"
// koda_change - preserve compatible variants when switching models
import { resolvePreservedVariant } from "@/koda/cli/cmd/run/variant" // koda_change
...
async function runInteractiveRuntime(input: RunRuntimeInput, deps: RunRuntimeDeps = {}): Promise<void> {
  ...
  const terminal = kodaRunTerminal.create(ctx.sdk, () => state.sessionID) // koda_change
  ...
  const shell = await (deps.createRuntimeLifecycle ?? createRuntimeLifecycle)({
    ...
    // koda_change start - human-driven terminal in direct interactive mode
    ...
    onModelSelect: async (model) => {
      ...
      // koda_change start - preserve the active effort across model switches
      ...
      const switching = resolveSavedVariant(model).then((saved) => {
        ...
        // koda_change - prefer the active effort over a model-specific saved preference
        state.activeVariant =
  ...
  void modelTask.then((info) => {
    ...
    // koda_change start - preserve the active effort when the model catalog arrives asynchronously
    const next =
