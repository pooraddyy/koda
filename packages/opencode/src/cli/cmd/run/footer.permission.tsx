...
export function RejectField(props: {
  ...
  return (
    <textarea
      ...
      maxHeight={3}
      wrapMode="word"
      placeholder={"Tell koda what to do differently" /* koda_change */}
...
export function RunPermissionBody(props: {
  ...
  const temporary = createMemo(() => temporaryPermission(props.request)) // koda_change
  const opts = createMemo(() => permissionOptions(state().stage, temporary())) // koda_change
  ...
  const shift = (dir: -1 | 1) => {
    setState((prev) => permissionShift(prev, dir, temporary())) // koda_change
  }
      ...
      >
        ...
        <Switch>
          ...
          <Match when={state().stage === "reject"}>
            <box paddingLeft={1}>
              <text fg={props.theme.muted}>{"Tell koda what to do differently" /* koda_change */}</text>
            </box>
          </Match>
        </Switch>
      </box>
