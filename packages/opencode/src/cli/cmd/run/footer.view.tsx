...
import { RunInteractiveTerminalBody } from "@/koda/cli/cmd/run/interactive-terminal" // koda_change
import {
...
import { chatActivityLabel, nextChatActivityIndex } from "./chat-activity"
...
type RunFooterViewProps = {
  ...
  onTerminalWrite: (input: { terminalID: string; data: string }) => Promise<void> // koda_change
  onTerminalResize: (input: { terminalID: string; cols: number; rows: number }) => Promise<void> // koda_change
  onTerminalClose: (terminalID: string) => Promise<void> // koda_change
  onCycle: () => void
  onInterrupt: () => boolean
  onBackground?: () => void
...
export function RunFooterView(props: RunFooterViewProps) {
  ...
  createEffect(() => {
    ...
    activityTimer = setInterval(() => setActivityIndex(nextChatActivityIndex(activityIndex())), 1600)
  })
  onCleanup(() => {
                ...
                >
                  ...
                  <text fg={statusColor()} wrapMode="none" truncate flexGrow={1} flexShrink={1}>
                    <Show when={busy() && !exiting()} fallback={statusText()}>
                      <span style={{ fg: statusColor(), bold: true }}>{chatActivityLabel(activityIndex())}</span>
                      <Show
                        when={interruptLabel()}
                        fallback={
                      ...
                      >
                        {(label) => (
                          <span style={{ fg: armed() ? statusColor() : theme().muted }}> · {label()} to interrupt</span>
                        )}
                      </Show>
                    </Show>
                  </text>
