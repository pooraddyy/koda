...
// koda_change start - keep equivalent up keybindings aligned at panel boundaries
...
function handleKey(input: {
  ...
  // koda_change start - treat ctrl+p as the up binding
  if (up(input.event)) {
    input.event.preventDefault()
    input.menu.move(-1)
    return
...
export function RunSubagentSelectBody(props: {
  ...
  useKeyboard((event) => {
    ...
    // koda_change start - close on either up binding at the first row
