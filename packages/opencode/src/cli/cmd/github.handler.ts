...
import { GitHubSecurity } from "@/koda/security/github" // koda_change
...
const AGENT_USERNAME = "kodaconnect[bot]" // koda_change
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/koda.yml" // koda_change
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            step2,
            "",
            "    3. Go to a GitHub issue and comment `/koda summarize` to see the agent in action", // koda_change
            "",
            "   Learn more about the GitHub agent - https://koda.ai/docs/code-with-ai/platforms/github", // koda_change
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/kodaconnect" // koda_change
        const command =
        ...
        async function getInstallation() {
          // koda_change start - updated to new endpoint
          return await fetch(`https://api.koda.ai/api/integrations/github/check-installation?owner=${app.owner}`)
            .then((res) => res.json())
      ...
      async function addWorkflowFiles() {
        // koda_change start - updated workflow template with koda branding and gateway secrets
        const providerEnvStr =
          provider === "amazon-bedrock"
            ? ""
            : providers[provider].env.map((e) => `\n          ${e}: \${{ secrets.${e} }}`).join("")

        const kodaGatewayEnv =
          provider === "koda"
            ? `\n          koda_API_KEY: \${{ secrets.koda_API_KEY }}\n          koda_ORG_ID: \${{ secrets.koda_ORG_ID }}`
            : ""

        const envStr = providerEnvStr || kodaGatewayEnv ? `\n        env:${providerEnvStr}${kodaGatewayEnv}` : ""
...
jobs:
  koda:
    ...
    steps:
      ...
      - name: Run koda
        uses: koda-Org/koda/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.koda.ai" : "https://koda.ai" // koda_change
    ...
    try {
      ...
      // Setup koda session // koda_change
      ...
      console.log("koda session", session.id) // koda_change
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.koda.ai" // koda_change
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/koda,/kc") // koda_change
        .split(",")
      ...
      for (const m of matches) {
        const tag = m[0]
        // koda_change start - only fetch canonical GitHub attachment routes
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to koda...") // koda_change
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("koda-github-action") // koda_change
      } catch (error) {
    ...
    async function exchangeForAppToken(token: string) {
      // koda_change start - updated endpoint URLs per new API structure
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `koda/${type}-${hex}-${timestamp}` // koda_change
      }
      return `koda/${type}${issueId}-${timestamp}` // koda_change
    }
    ...
    function footer(opts?: { image?: boolean }) {
      // koda_change start - simplified footer with text branding (no image backend yet)
      const share = shareId ? `[koda session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
      return `\n\n---\n*Powered by [koda](https://koda.ai)*&nbsp;&nbsp;|&nbsp;&nbsp;${share}[github run](${runUrl})`
      // koda_change end
    }
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the koda infrastructure after your response", // koda_change
