...
// koda_change - kodaMcpConfig is dynamically imported in addMcpToConfig to keep startup fast
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: koda mcp add") // koda_change
      return
    }
    ...
    for (const [name, serverConfig] of servers) {
      ...
      if (!status) {
        ...
        statusIcon = "·"
        statusText = `connecting (attempt ${status.attempt + 1})`
      } else if (status.status === "reconnecting") {
        statusIcon = "·"
        statusText = `reconnecting (attempt ${status.attempt + 1})`
        hint = "\n    " + status.message
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in koda.json:") // koda_change
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // koda_change start - prefer supported koda config directories over root files
  const roots = [
    path.join(baseDir, "koda.jsonc"),
    path.join(baseDir, "koda.json"),
  ...
  const candidates = global
    ? roots
    : [
        path.join(baseDir, ".koda", "koda.jsonc"),
        path.join(baseDir, ".koda", "koda.json"),
        path.join(baseDir, ".koda", "opencode.jsonc"),
        path.join(baseDir, ".koda", "opencode.json"),
        path.join(baseDir, ".koda", "koda.jsonc"),
        path.join(baseDir, ".koda", "koda.json"),
        path.join(baseDir, ".koda", "opencode.jsonc"),
        path.join(baseDir, ".koda", "opencode.json"),
  ...
  // Default to koda.json if none exist
  return path.join(baseDir, "koda.json")
  // koda_change end
}
...
async function addMcpToConfig(name: string, mcpConfig: ConfigMCPV1.Info, configPath: string) {
  ...
  // koda_change start - lazy import keeps the CLI startup graph light
  const { kodaMcpConfig } = await import("@/koda/cli/cmd/mcp")
  const result = kodaMcpConfig.format(configPath, applyEdits(text, edits))
  // koda_change end
...
export const McpAddCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.add")(function* (args) {
    ...
    const ctx = maybeCtx
    const global = Flag.koda_CONFIG_DIR ?? Global.Path.config // koda_change - honor the active koda config profile
    yield* Effect.promise(async () => {
      ...
      if (args.name) {
        ...
        const configPath = await resolveConfigPath(global, true) // koda_change
      ...
      const [projectConfigPath, globalConfigPath] = await Promise.all([
        ...
        resolveConfigPath(global, true), // koda_change
      ])
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., koda x @modelcontextprotocol/server-filesystem", // koda_change
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "koda-debug", version: InstallationVersion }, // koda_change
            },
            id: 1,
          }),
        })
