...
import type { GlobalEvent } from "@koda/sdk/v2"
import type { EventSource } from "@opencode-ai/tui/context/sdk"
import { writeHeapSnapshot } from "v8"
import type { StartInput } from "@/koda/cli/cmd/tui/thread" // koda_change - runtime imports deferred into handlers
import { win32InstallCtrlCGuard } from "@opencode-ai/tui/terminal-win32"
import { validateSession } from "../tui/validate-session"
// koda_change start - correlate the TUI worker with its parent process
import {
  koda_PROCESS_ROLE,
  koda_RUN_ID,
  ensureRunID,
...
// koda_change end
import type { RemoteExitBridgeClient } from "@/koda/cli/cmd/tui/remote-exit-bridge" // koda_change - runtime import deferred
import type { Exit } from "@opencode-ai/tui/context/exit" // koda_change

declare global {
  const koda_WORKER_PATH: string
}

type RpcClient = ReturnType<typeof Rpc.client<typeof rpc>>

// koda_change start - bridge remote exit only for the embedded worker transport
...
export async function runEmbeddedRemoteExitBridge(input: {
  ...
  const { createParentRemoteExitBridge } = await import("@/koda/cli/cmd/tui/remote-exit-bridge")
...
// koda_change start - share the extracted TUI runner between daemon and worker paths
...
async function target() {
  if (typeof koda_WORKER_PATH !== "undefined") return koda_WORKER_PATH
...
export function resolveThreadDirectory(project?: string, envPWD = process.env.PWD, cwd = process.cwd()) {
  // koda_change start - ignore stale PWD from wrappers such as `bun --cwd`, except koda-dev's caller cwd
  const dev = process.env.koda_DEV_CWD
  ...
  return dev ? root : real // koda_change
}

export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start koda tui", // koda_change
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start koda in", // koda_change
      })
      .option("model", {
        type: "string",
        alias: ["m"],
      ...
      // koda_change start - create/reuse a git worktree before starting
      .option("worktree", {
        type: "string",
        describe: "create (or reuse) a git worktree with this name and start koda there",
      })
      // koda_change end
      .option("prompt", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    // koda_change start - lazy koda implementations so other CLI commands
    // don't pay their module cost at startup
    const { importCloudSession, localSessionID, validateCloudFork, reportCloudImportError } = await import("@/koda/cloud-session")
    const { kodaTuiThreadDaemon } = await import("@/koda/cli/cmd/tui/thread")
    const { preload } = await import("@/koda/cli/cmd/tui")
    const { resolveTuiDirectory } = await import("@/koda/cli/cmd/tui-worktree")
    // koda_change end
    ...
    try {
      ...
      // koda_change start - `--worktree <name>` creates/reuses a worktree; resuming
      ...
      // koda_change start
      const preloads = preload(typeof koda_WORKER_PATH !== "undefined", () =>
      ...
      // koda_change start - default TUI sessions attach to the daemon unless explicitly disabled
      if (await kodaTuiThreadDaemon.attach({ args, cwd, input: () => input(args.prompt), start })) return
      // koda_change end
      const auth = kodaTuiThreadDaemon.workerAuth() // koda_change - protect TUI-owned HTTP routes from unauthenticated local callers
      // koda_change start - propagate stable run metadata and an explicit worker role
      const env = sanitizedProcessEnv({
        [koda_PROCESS_ROLE]: "worker",
        [koda_RUN_ID]: ensureRunID(),
        ...auth.env,
        koda_BACKGROUND_PROCESS_PORTS: "true",
      })
      // koda_change end
      const worker = new Worker(file, {
        preload: preloads, // koda_change
        env, // koda_change
      })
      worker.onerror = (e) => {
      ...
      // koda_change start - graceful shutdown on external signals
      ...
      // koda_change - external kill -INT takes the same graceful path as SIGHUP/SIGTERM.
      ...
      const transport = external
        ? {
            ...
            fetch: undefined,
            headers: auth.headers, // koda_change
            events: undefined,
          }
        : {
            url: "http://koda.internal",
            fetch: createWorkerFetch(client),
            headers: auth.headers, // koda_change
            events: createEventSource(client),
          }

      // koda_change - upstream validates here, but --cloud-fork's session id is only local after
      ...
      try {
        // koda_change start - import cloud session before TUI renders
        if (args.cloudFork && args.session) {
          UI.println("Importing session from cloud...")
          const { createkodaClient } = await import("@koda/sdk/v2")
          const sdk = createkodaClient({
            baseUrl: transport.url,
            fetch: transport.fetch,
            headers: transport.headers, // koda_change
            directory: cwd,
          })
          try {
        ...
        try {
          await validateSession({
            url: transport.url, // koda_change
            sessionID: localSessionID(args), // koda_change
            directory: cwd,
            fetch: transport.fetch,
            headers: transport.headers, // koda_change
          })
        ...
        await start(
          {
            // koda_change - shared lazy loader also supports daemon attach
            url: transport.url,
            async onSnapshot() {
