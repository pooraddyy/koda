...
export const RemoteCommand = cmd({
  ...
  handler: async () => {
    ...
    const { Instance } = await import("@/koda/instance")
    await bootstrap(process.cwd(), async () => {
      // koda_change - K1 W1: advertise this instance on the relay
      // heartbeat so the cloud side can show it as a spawn-capable instance.
      // The process-wide `koda_REMOTE_ATTACH_SESSION` guard was removed in K1
      // (in-process sessions only; no spawned children), so this is always
      // advertised for the explicit `koda remote` command path.
      ...
      kodaSessions.setInstanceAdvertisement(buildInstanceAdvertisement(Instance.directory))
