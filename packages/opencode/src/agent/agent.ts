...
import PROMPT_SCOUT from "@/koda/agent/scout.txt" // koda_change
import PROMPT_SUMMARY from "./prompt/summary.txt"
import PROMPT_TITLE from "./prompt/title.txt"
import { Permission } from "@/permission"
import { mergeDeep, pipe, sortBy, values } from "remeda"
import { Global } from "@opencode-ai/core/global"
import { kodaPaths } from "@/koda/paths" // koda_change
...
export const Info = Schema.Struct({
  ...
  deprecated: Schema.optional(Schema.Boolean), // koda_change
  ...
  budget: Schema.optional(ConfigBudget.Info),
...
type State = Omit<Interface, "generate"> & { version: string } // koda_change
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Agent.state")(function* (ctx) {
        ...
        // koda_change start - include global config dirs so agents can read them without prompting
        ...
        const whitelistedDirs = [
          ...
          ...kodaPaths.globalDirs().map((dir) => path.join(dir, "*")),
        ...
        const baseDefaults = Permission.fromConfig({
          ...
          suggest: "deny", // koda_change
          question: "deny",
          interactive_terminal: "deny", // koda_change - human-driven tools are primary-agent only
          plan_enter: "deny",
          plan_exit: "deny",
          // koda_change start
          repo_clone: "deny",
        ...
        // koda_change start - patch defaults with bash allowlist and recall permission
        const koda = kodaAgent.prepare(cfg)
        const defaults = Permission.merge(baseDefaults, koda.defaultsPatch)
        // koda_change end
        ...
        const agents: Record<string, Info> = {
          ...
          // koda_change start - retain koda's opt-in repository research agent
        ...
        // koda_change start - rename build→code, add debug/orchestrator/ask, patch plan/explore
        kodaAgent.patchAgents(agents, defaults, user, cfg, koda, ctx.worktree, whitelistedDirs)

        const agentConfigs = kodaAgent.preprocessConfig(cfg.agent ?? {})
        for (const [key, value] of Object.entries(agentConfigs)) {
          ...
          // koda_change start - carry metadata as typed fields, never as provider options
          ...
          // koda_change start
          kodaAgent.processConfigItem(item)
          kodaAgent.hardenPlan(key, item, ctx.worktree, user, Permission.fromConfig(value.permission ?? {}))
          kodaAgent.hardenExplore(key, item, user, Permission.fromConfig(value.permission ?? {}))
        }

        function referencePrompt(reference: kodaReference.Resolved) {
          ...
          return [
            ...
            `koda materializes this configured repository before use. Do not call repo_clone for this reference.`,
        ...
        function referenceDescription(reference: kodaReference.Resolved) {
        ...
        if (flags.experimentalScout) {
          ...
          const resolvedReferences = kodaReference.resolveAll({
            references,
            directory: ctx.directory,
            worktree: ctx.worktree,
          })
        ...
        kodaAgent.hardenSystemAgents(agents) // koda_change - keep system utility agents deny-only after config merges

        const get = Effect.fnUntraced(function* (agent: string) {
          return agents[kodaAgent.resolveKey(agent)] // koda_change - treat "build" as "code"
        })
        ...
        const list = Effect.fnUntraced(function* () {
          ...
          return pipe(
            ...
            values(),
            sortBy(
              [(x) => (cfg.default_agent ? x.name === cfg.default_agent : x.name === "code"), "desc"], // koda_change - renamed from "build" to "code"
              [(x) => x.name, "asc"],
            ),
          )
        })
        ...
        const defaultInfo = Effect.fnUntraced(function* () {
          ...
          if (c.default_agent) {
            // koda_change start
            const effective = kodaAgent.resolveKey(c.default_agent)
          ...
          // koda_change start - prefer "code" as default agent (key order changes after rename from "build")
        ...
        return {
          version: kodaAgent.cacheKey(cfg), // koda_change
          get,
          list,
          defaultInfo,
          defaultAgent,
    ...
    // koda_change start - rebuild cached agents when permission-relevant config changes
    const current = Effect.fnUntraced(function* <A>(select: (s: State) => Effect.Effect<A>) {
      const cfg = yield* config.get()
      const s = yield* InstanceState.get(state)
      if (s.version === kodaAgent.cacheKey(cfg)) return yield* select(s)
    ...
    return Service.of({
      get: Effect.fn("Agent.get")(function* (agent: string) {
        return yield* current((s) => s.get(agent)) // koda_change
      }),
      list: Effect.fn("Agent.list")(function* () {
        return yield* current((s) => s.list()) // koda_change
      }),
      defaultInfo: Effect.fn("Agent.defaultInfo")(function* () {
        return yield* current((s) => s.defaultInfo()) // koda_change
      }),
      defaultAgent: Effect.fn("Agent.defaultAgent")(function* () {
        return yield* current((s) => s.defaultAgent()) // koda_change
      }),
      generate: Effect.fn("Agent.generate")(function* (input: {
        ...
        const params = {
          // koda_change start - enable telemetry with custom PostHog tracer
          experimental_telemetry: kodaAgent.telemetryOptions(cfg),
          // koda_change end
          temperature: 0.3,
          messages: [
            ...(isOpenaiOauth
...
export const node = LayerNode.make({
  service: Service,
  layer: layer,
  deps: [Config.node, Auth.node, Plugin.node, Skill.node, Provider.node, RuntimeFlags.node, locationServiceMapNode], // koda_change
})
