...
import * as kodaOAuthCallback from "../koda/mcp-oauth-callback" // koda_change

const log = Log.create({ service: "mcp.oauth-callback" }) // koda_change
...
<head>
  <!-- koda_change start -->
  <title>koda - Authorization Successful</title>
  <!-- koda_change end -->
  <style>
...
<head>
  <!-- koda_change start -->
  <title>koda - Authorization Failed</title>
  <!-- koda_change end -->
  <style>
    body { font-family: system-ui, -apple-system, sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #1a1a2e; color: #eee; }
    .container { text-align: center; padding: 2rem; }
...
export async function ensureRunning(redirectUri?: string): Promise<void> {
  // koda_change start - delegate koda-specific callback binding from here because OAuth state lives in this module
  await kodaOAuthCallback.ensureRunning({
    redirectUri,
