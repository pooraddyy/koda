...
if (process.platform === "win32" && !("type" in process)) {
  Object.defineProperty(process, "type", { value: "koda-bun", configurable: true })
}
// koda_change end
...
import { model as modelEnv } from "@/koda/process/env" // koda_change
import { InstanceState } from "@/effect/instance-state"
import { ChildProcess, ChildProcessSpawner } from "effect/unstable/process"
import { CrossSpawnSpawner } from "@opencode-ai/core/cross-spawn-spawner"
import * as SandboxNetwork from "@/koda/sandbox/network" // koda_change
...
const CLIENT_OPTIONS = {
  capabilities: {
    // Upstream issue anomalyco/opencode#11948 // koda_change
    // sampling: {},
    // Upstream issue anomalyco/opencode#23066 // koda_change
    // elicitation: {},
    // Upstream issue anomalyco/opencode#2308 // koda_change
    roots: {},
    // Upstream issue anomalyco/opencode#28567 // koda_change
    // tasks: {},
  },
} satisfies ClientOptions

// koda_change start - inject --rm for Docker containers to prevent stopped container accumulation
...
function createClient(directory: string) {
  const client = new Client({ name: "koda", version: InstallationVersion }, CLIENT_OPTIONS) // koda_change
...
const StatusReconnecting = Schema.Struct({
  status: Schema.Literal("reconnecting"),
  attempt: NonNegativeInt,
  message: Schema.String,
}).annotate({ identifier: "MCPStatusReconnecting" })
...
export interface McpTool {
  ...
  // koda_change start - identifies the owning server for MCP Apps routing
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: koda mcp auth ${key}`, // koda_change
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const finalArgs = ensureDockerRm(cmd, args) // koda_change
      const baseDir = yield* InstanceState.directory
      const cwd = mcp.cwd ? path.resolve(baseDir, mcp.cwd) : baseDir
      const bridge = yield* EffectBridge.make() // koda_change - drain child stderr without writing over the TUI
      const transport = new StdioClientTransport({
        stderr: "pipe",
        command: cmd,
        args: finalArgs, // koda_change
        cwd,
        // koda_change start - local MCPs must not inherit backend credentials
        env: modelEnv({
      ...
      // koda_change start - a piped stderr stream must be consumed or verbose MCP servers can block
    ...
    const connect = Effect.fn("MCP.connect")(function* (name: string) {
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        if (attempt === 0) {
          s.status[name] = { status: "connecting", attempt }
        } else {
          s.status[name] = {
            status: "reconnecting",
            attempt,
            message: `Retrying MCP connection (${attempt + 1}/${maxAttempts})`,
          }
          yield* Effect.sleep(`${Math.min(500, 100 * 2 ** (attempt - 1))} millis`)
        }
        const result = yield* createAndStore(name, { ...mcp, enabled: true })
        if (result.status === "connected") return
        if (result.status === "needs_auth" || result.status === "needs_client_registration") return
      }
    })
    ...
    const tools = Effect.fn("MCP.tools")(function* () {
      ...
      for (const [clientName, client] of Object.entries(s.clients)) {
        ...
        for (const def of listed) {
          const tool = { def, client, clientName, timeout } // koda_change - clientName for MCP Apps routing
          // koda_change start - preserve remote MCP authority on the native entry for every execution path
    ...
    function collectFromConnected<T extends { name: string }>(
      ...
      return Effect.gen(function* () {
        ...
        return yield* Effect.forEach(
          ...
          ([clientName, client]) =>
            McpCatalog.collect(
              // koda_change - distinguish collection from direct network fetch
              clientName,
              client,
    ...
    // koda_change start - `opts?: { callback?: boolean }` parameter is koda-specific
    const startAuth = Effect.fn("MCP.startAuth")(function* (mcpName: string, opts?: { callback?: boolean }) {
      ...
      // koda_change start - authenticate() defers binding the callback port until a redirect is needed
    ...
    const authenticate = Effect.fn("MCP.authenticate")(function* (
      ...
      const result = yield* startAuth(mcpName, { callback: false }) // koda_change
      ...
      // koda_change start - bind only after redirect exists, and clean up if binding fails
      const mcpConfig = yield* getMcpConfig(mcpName)
