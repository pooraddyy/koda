...
import { EventSequenceTable, EventTable } from "@opencode-ai/core/event/sql" // koda_change - upstream moved the event tables to core
import { EventID } from "./schema"
import { Context, Effect, Layer, Schema as EffectSchema } from "effect"
import type { DeepMutable } from "@opencode-ai/core/schema"
import { EventV2 } from "@opencode-ai/core/event"
import { EventManifest } from "@/event-manifest" // koda_change
import { serviceUse } from "@opencode-ai/core/effect/service-use"
import { InstanceState } from "@/effect/instance-state"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { EffectBridge } from "@/effect/bridge"
import * as EventWire from "@/koda/event-wire" // koda_change
...
> = {
  ...
  wire?: boolean // koda_change - EventV2 rows cross persistence and bus boundaries as encoded data
}
...
// koda_change start - serialized rows carry the schema's encoded representation
...
export const layer = Layer.effect(Service)(
  Effect.gen(function* () {
    ...
    const replay: Interface["replay"] = Effect.fn("SyncEvent.replay")(function* (event, options) {
      ...
      // koda_change start - decode only EventV2 rows
...
export const defaultLayer = layer.pipe(
  Layer.provide([ProjectBus.defaultLayer, AppNodeBuilder.build(RuntimeFlags.node)]), // koda_change
)
...
export function init(input: { projectors: Array<[Definition, ProjectorFunc]>; convertEvent?: ConvertEvent }) {
  ...
  for (const entry of EventManifest.Latest.values()) {
    if (!entry.durable) continue // koda_change - mirror current durable events into legacy sync
    register({
      type: entry.type,
      version: entry.durable.version, // koda_change
      aggregate: entry.durable.aggregate, // koda_change
      properties: entry.data,
      schema: entry.data,
      wire: true, // koda_change
    })
  }
...
function process<Def extends Definition>(
  ...
  Database.transaction((tx) => {
    ...
    const data = def.wire ? EventWire.encode(def.schema, event.data) : event.data // koda_change
    ...
    if (options.experimentalWorkspaces) {
      ...
      tx.insert(EventTable)
        .values({
          id: EventV2.ID.make(event.id), // koda_change - core event table uses the branded EventV2 ID
          seq: event.seq,
          aggregate_id: event.aggregateID,
          type: versionedType(def.type, def.version),
          data: data as Record<string, unknown>, // koda_change
        })
        .run()
    }
    ...
    Database.effect(() => {
      ...
      const publish = (value: unknown) =>
        ...
        options.bridge.fork(
          Effect.gen(function* () {
            // koda_change start - encode EventV2 properties before crossing the legacy boundary
            if (def.wire) {
...
export function effectPayloads() {
  return [
    ...
    ...EventManifest.Latest.values()
      .filter(
        (definition) =>
          definition.durable !== undefined && // koda_change
          !registry.has(versionedType(definition.type, definition.durable.version)), // koda_change
      )
      .map((definition) =>
        EffectSchema.Struct({
          type: EffectSchema.Literal("sync"),
          name: EffectSchema.Literal(versionedType(definition.type, definition.durable!.version)), // koda_change
          id: EffectSchema.String,
          seq: EffectSchema.Finite,
          aggregateID: EffectSchema.Literal(definition.durable!.aggregate), // koda_change
