...
import type { AssistantMessage, Message, kodaClient, SessionMessageResponse } from "@koda/sdk/v2"
...
export const AuthMethodID = "koda-login" // koda_change
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `koda auth login` in the terminal",
      name: "Login with koda",
      id: AuthMethodID,
    }
    // koda_change end
...
function makeDirectoryService(sdk: kodaClient) {
...
function makeUsageService(sdk: kodaClient) {
...
async function loadDirectorySnapshot(sdk: kodaClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  const kodaProvider = providers[ProviderV2.ID.make("koda")] // koda_change
  const kodaModel = kodaProvider ? Provider.sort(Object.values(kodaProvider.models))[0] : undefined // koda_change
  if (kodaProvider && kodaModel) return { providerID: kodaProvider.id, modelID: kodaModel.id } // koda_change
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "koda prompt failed" // koda_change - user-visible ACP error
}
