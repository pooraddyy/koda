...
import * as launch from "../../src/lsp/launch" // koda_change - spy on spawn
import { CrossSpawnSpawner } from "@opencode-ai/core/cross-spawn-spawner"
import { provideTestInstance, provideTmpdirInstance, TestInstance, tmpdir } from "../fixture/fixture" // koda_change
import { awaitWithTimeout, testEffect } from "../lib/effect"
import { type InstanceContext } from "../../src/project/instance-context"
import { Flag } from "@opencode-ai/core/flag/flag" // koda_change
import { TsCheck } from "../../src/koda/ts-check" // koda_change

// koda_change - Typescript.spawn ignores ctx, so a cast is fine here.
...
describe("lsp.spawn", () => {
  ...
  // koda_change start - provide the runtime flag so spawn() is reached past the TsClient short-circuit
  ...
  // koda_change start - Typescript spawn is gated behind koda_EXPERIMENTAL_LSP_TOOL.
  test("spawns tsgo LSP when koda_EXPERIMENTAL_LSP_TOOL is enabled", async () => {
    const saved = Flag.koda_EXPERIMENTAL_LSP_TOOL
    Flag.koda_EXPERIMENTAL_LSP_TOOL = true
    ...
    try {
      ...
      Flag.koda_EXPERIMENTAL_LSP_TOOL = saved
      spawnSpy.mockRestore()
      tsgoSpy.mockRestore()
    }
  })

  test("Typescript.spawn returns undefined when koda_EXPERIMENTAL_LSP_TOOL is off", async () => {
    const saved = Flag.koda_EXPERIMENTAL_LSP_TOOL
    Flag.koda_EXPERIMENTAL_LSP_TOOL = false
    try {
