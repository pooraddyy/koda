...
import { capture } from "../../src/koda/instance" // koda_change
...
describe("InstanceStore", () => {
  it.live("loads instance context", () =>
    Effect.gen(function* () {
      ...
      // koda_change start - capture prefers legacy ALS, then falls back to the Effect fiber reference
  ...
  it.live("runs bootstrap with InstanceRef provided", () =>
    Effect.gen(function* () {
      ...
      expect(capture()).toBeUndefined() // koda_change - bootstrap legacy ALS does not leak into the caller
    }),
  )
  ...
  // koda_change start - reload disposers retain legacy instance context
  ...
  // koda_change - InstanceStore.boot provides InstanceRef to bootstrap.run so
  // kodaBootstrap (and anything it forkDetaches, e.g. kodaIndexing.init) can read
  // the current directory. This regression test pins the koda contract.
  it.live("provides InstanceRef during bootstrap for koda bootstrap compatibility", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const store = yield* InstanceStore.Service
