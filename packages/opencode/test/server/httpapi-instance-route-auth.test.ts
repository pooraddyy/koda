    ...
    HttpApiApp.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          // koda_change start - keep the filewatcher-disable flag visible so the
          ...
          ConfigProvider.fromUnknown({
            koda_SERVER_PASSWORD: input.password,
            koda_SERVER_USERNAME: input.username,
...
describe("HttpApi instance route authorization", () => {
  test("requires configured auth before opening the instance event stream", async () => {
    ...
    const authed = await server.request(EventPaths.event, {
      headers: { ...headers, authorization: basic("koda", "secret") }, // koda_change - koda username default
    })
