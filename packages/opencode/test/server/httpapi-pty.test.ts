...
// koda_change start - route PTY tests must not compete with per-project indexing workers.
...
describe("pty HttpApi bridge", () => {
  test("serves available shell list through experimental Effect routes", async () => {
    ...
    const response = await app().request(PtyPaths.shells, { headers: { "x-koda-directory": tmp.path } })
  ...
  testPty("serves PTY JSON routes through experimental Effect routes", async () => {
    await using tmp = await tmpdir({ git: true, config: { formatter: false, lsp: false } })
    const headers = { "x-koda-directory": tmp.path }
    const list = await app().request(PtyPaths.list, { headers })
    expect(list.status).toBe(200)
    expect(await list.json()).toEqual([])

    // koda_change start - test initial spawn dimensions
  ...
  testPty("hides exited sessions on the legacy surface", async () => {
    ...
    // koda_change start - exit propagation can exceed 5s on a loaded CI shard; the loop
  ...
  // koda_change start - location disposal must preserve the process-wide PTY registry.
  ...
  // koda_change start - portable coverage for the exact legacy routes used by regular Agent Manager terminals
