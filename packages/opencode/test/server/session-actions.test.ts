...
import { Effect, Fiber, Layer } from "effect" // koda_change
import { BackgroundJob } from "@/background/job" // koda_change
import { Session as SessionNs } from "@/session/session"
import { disposeAllInstances, TestInstance } from "../fixture/fixture"
import { pollWithTimeout, testEffect } from "../lib/effect" // koda_change
import { httpApiLayer, requestInDirectory } from "./httpapi-layer"

// koda_change start - provide the background-job service for promotion coverage
const it = testEffect(
  Layer.mergeAll(LayerNode.compile(SessionNs.node), LayerNode.compile(BackgroundJob.node), httpApiLayer), // koda_change
)
// koda_change end
...
describe("session action routes", () => {
  // koda_change start - background subagents are enabled by default
  it.instance(
  ...
  // koda_change start - verify HTTP promotion of a running task
  it.instance(
