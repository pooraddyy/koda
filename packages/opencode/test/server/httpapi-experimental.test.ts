...
import { realpath } from "node:fs/promises" // koda_change
      ...
      Effect.gen(function* () {
        ...
        yield* withCreatedWorktree(tmp.directory, (info) =>
          Effect.gen(function* () {
            ...
            const directory = yield* Effect.promise(() => realpath(info.directory)) // koda_change
            expect(yield* json(listed)).toContainEqual({ directory, managed: true }) // koda_change
