...
const testMcpHandlers = HttpApiBuilder.group(TestHttpApi, "mcp", (handlers) =>
  Effect.succeed(
    handlers
      ...
      .handle("disconnect", () => Effect.die("unexpected MCP disconnect")) // koda_change
      .handle("readResource", () => Effect.die("unexpected MCP readResource")) // koda_change
      .handle("callTool", () => Effect.die("unexpected MCP callTool")), // koda_change
  ),
)
