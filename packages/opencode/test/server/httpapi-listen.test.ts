...
const original = {
  ...
  envPassword: process.env.koda_SERVER_PASSWORD,
  envUsername: process.env.koda_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.koda_SERVER_PASSWORD
  else process.env.koda_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.koda_SERVER_USERNAME
  else process.env.koda_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.koda_SERVER_PASSWORD = auth.password
  Flag.koda_SERVER_USERNAME = auth.username
  process.env.koda_SERVER_PASSWORD = auth.password
  process.env.koda_SERVER_USERNAME = auth.username
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-koda-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-koda-ticket": "1" }),
...
async function createCat(listener: Awaited<ReturnType<typeof startListener>>, dir: string) {
  ...
  expect(response.status).toBe(200)
  return (await response.json()) as { id: string; pid: number } // koda_change
}
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-koda-directory": tmp.path },
      })
      expect(response.status).toBe(200)
      ...
      // koda_change start - true server shutdown must terminate retained PTY processes.
  ...
  test("default in-process handler does not emit Effect HTTP response logs", async () => {
    ...
    try {
      // koda_change start - use an authenticated local route instead of proxy-dependent status
      Flag.koda_SERVER_PASSWORD = auth.password
      Flag.koda_SERVER_USERNAME = auth.username
      process.env.koda_SERVER_PASSWORD = auth.password
      process.env.koda_SERVER_USERNAME = auth.username
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.koda_DISABLE_DEFAULT_PLUGINS
    process.env.koda_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.koda_DISABLE_DEFAULT_PLUGINS
      else process.env.koda_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-koda-ticket": "1" },
      })
