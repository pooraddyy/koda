...
import { pollWithTimeout, testEffect } from "../lib/effect" // koda_change

const originalWorkspaces = Flag.koda_EXPERIMENTAL_WORKSPACES
...
// koda_change start - released V2 clients persisted media-shaped tool content
...
afterEach(async () => {
  Flag.koda_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-koda-directory": test.directory }
  ...
  // koda_change start - protect mixed-version session database compatibility
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const response = yield* request(`/api/session/${session.id}/message`, {
          headers: { "x-koda-directory": test.directory },
        })
        expect(response.status).toBe(200)
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-koda-directory": test.directory, "content-type": "application/json" }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.koda_EXPERIMENTAL_WORKSPACES = true
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-koda-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-koda-directory": hint, "content-type": "application/json" }
        const created = yield* requestJson<Session.Info>(SessionPaths.create, {
          method: "POST",
          headers,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-koda-directory": globalWorktreeSentinel, "content-type": "application/json" }
  ...
  // koda_change start - deleting a prompt that already started is a successful no-op
  it.live(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const headers = { "x-koda-directory": dir, "content-type": "application/json" }
    ...
    30_000, // koda_change - windows CI needs headroom beyond 10s
  )
  // koda_change end
