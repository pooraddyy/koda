...
import { awaitWithTimeout, pollWithTimeout, testEffect } from "../lib/effect" // koda_change
...
type Sdk = ReturnType<typeof createkodaClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.koda_SERVER_PASSWORD = input?.password
      Flag.koda_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".koda", "skills", "project-rest-skill", "SKILL.md"), // koda_change
      `---
...
describe("HttpApi SDK", () => {
  ...
  httpapiInstance(
    ...
    ({ sdk, directory }) =>
      Effect.gen(function* () {
        ...
        const raw = yield* call(() => sdk.v2.fs.read({ path: "hello.txt" })) // koda_change
        const session = yield* call(() => sdk.session.create({ title: "sdk" }))
        const v2session = yield* call(() => sdk.v2.session.create({ agent: "build" })) // koda_change
        ...
        expect({ status: v2session.response.status, error: v2session.error }).toEqual({ status: 200, error: undefined }) // koda_change
        expect(v2session.data).toMatchObject({ data: { location: { directory } } }) // koda_change
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-koda-directory")).toBe(false)
        expect(request!.headers.has("x-koda-workspace")).toBe(false)

        // koda_change start - encoded legacy directory headers still route on payload requests
        const legacy = yield* client("raw", undefined, {
          headers: { "x-koda-directory": encodeURIComponent(directory) },
        })
  ...
  httpapiInstance(
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        // koda_change start - match Hono AuthMiddleware username default ("koda")
        const badSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("koda", "wrong") },
        })
        const bad = yield* capture(() => badSdk.file.read({ path: "hello.txt" }))
        const goodSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("koda", "secret") },
        })
  ...
  // koda_change start - verify invalid user images fail at the real SDK boundary
  ...
  // koda_change start - verify provider errors remain in successful assistant messages
