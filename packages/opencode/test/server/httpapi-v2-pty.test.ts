...
// koda_change start - PTY route tests do not need an indexing worker per temp project;
// detached indexing startup races PTY setup and can exhaust the Darwin test deadline.
process.env.koda_DISABLE_CODEBASE_INDEXING = "vscode-no-workspace"
// koda_change end
...
function request(route: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-koda-directory", directory)
...
const directoryHeader = (dir: string) => HttpClientRequest.setHeader("x-koda-directory", dir)
...
describe("v2 pty HttpApi", () => {
  ...
  // koda_change start - portable live PTY coverage on Linux, macOS, and Windows CI
  ...
  ;(process.platform === "win32" ? effectIt.live.skip : effectIt.live)(
    ...
    () =>
      Effect.gen(function* () {
        const dir = yield* tmpdirScoped({ git: true, config: { formatter: false, lsp: false } })
        // koda_change start - verify child env precedence and credential stripping through the canonical PTY route
        const previous = {
        ...
        yield* Effect.acquireRelease(
          Effect.sync(() => {
            process.env.koda_SERVER_PASSWORD = "host-password"
            process.env.koda_SERVER_USERNAME = "host-username"
          }),
          () =>
            Effect.sync(() => {
              if (previous.password === undefined) delete process.env.koda_SERVER_PASSWORD
              else process.env.koda_SERVER_PASSWORD = previous.password
              if (previous.username === undefined) delete process.env.koda_SERVER_USERNAME
              else process.env.koda_SERVER_USERNAME = previous.username
            }),
        )
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              ...
              '    output.env.koda_TERMINAL = "plugin"',
              '    output.env.koda_PTY_ID = "plugin"',
              '    output.env.koda_SERVER_PASSWORD = "plugin-password"',
              '    output.env.koda_SERVER_USERNAME = "plugin-username"',
        ...
        const created = yield* HttpClientRequest.post("/api/pty").pipe(
          ...
          HttpClientRequest.bodyJson({
            ...
            args: [
              "-c",
              'printf "%s|%s|%s|%s|%s|%s|%s|%s|%s\\n" "$CALLER" "$SHARED" "$PLUGIN" "$TERM" "$koda_TERMINAL" "$koda_PTY_ID" "${koda_SERVER_PASSWORD-unset}" "${koda_SERVER_USERNAME-unset}" "$HOOK_CWD"; sleep 5',
            ],
            cwd,
            env: {
              CALLER: "caller",
              SHARED: "caller",
              TERM: "caller",
              koda_TERMINAL: "caller",
              koda_PTY_ID: "caller",
              koda_SERVER_PASSWORD: "caller-password",
              koda_SERVER_USERNAME: "caller-username",
            },
          }),
          Effect.flatMap(HttpClient.execute),
        )
    ...
    30_000, // koda_change - external plugin loading and websocket setup can exceed Bun's 5s default
  )
})
