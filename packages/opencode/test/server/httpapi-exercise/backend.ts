...
import type { Method, RequestSpec } from "./types" // koda_change
...
export async function disposeApps() {
  // koda_change start - an in-flight SSE fiber can leave the in-process router scope unable
...
function app(modules: Runtime, options: CallOptions) {
  ...
  const web = HttpRouter.toWebHandler(
    modules.HttpApiApp.routes.pipe(
      Layer.provide(
        // koda_change start - keep the filewatcher-disable flag visible (see httpapi-instance-route-auth.test.ts)
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            koda_SERVER_PASSWORD: password,
            koda_SERVER_USERNAME: username,
...
function toAuthProbeRequest(scenario: ActiveScenario, credentials: "missing" | "valid", signal: AbortSignal) {
  ...
  const headers = {
    ...
    ...spec.headers,
    ...(credentials === "valid" ? { authorization: basic("koda", "secret") } : {}), // koda_change
  }
  return new Request(new URL(spec.path, "http://localhost"), {
    method: scenario.method,
    headers,
