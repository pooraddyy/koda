...
process.env.koda_DISABLE_SESSION_INGEST = "true" // koda_change - isolate the exerciser from async koda session sync
process.env.koda_DISABLE_PRESENCE = "1" // koda_change - presence now has a default Event Service URL; never open real sockets from the exerciser
process.env.koda_DISABLE_CODEBASE_INDEXING = "vscode-no-workspace" // koda_change - route scenarios do not need an indexing worker per temp project
export const exerciseConfigDirectory = path.join(exerciseGlobalRoot, "config", "opencode")
export const exerciseDataDirectory = path.join(exerciseGlobalRoot, "data", "koda") // koda_change
