...
function runActive(options: Options, scenario: ActiveScenario) {
  ...
  return withContext(options, scenario, "shared", (ctx) =>
    Effect.gen(function* () {
      ...
      // koda_change start - append the actual response to assertion failures so CI logs
...
function runAuth(scenario: ActiveScenario) {
  return Effect.gen(function* () {
    ...
    if (scenario.auth === "protected") {
      ...
      if (!scenario.validAuthProbe) return // koda_change - blocking routes skip the valid probe; a leaked valid request hangs final app disposal
  ...
  return Effect.acquireRelease(
    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
        ...
        // koda_change start - headers closure extracted so scenarios can build their own requests
        const headers = (extra?: Record<string, string>) => ({
          ...(context.dir?.path ? { "x-koda-directory": context.dir.path } : {}),
          ...extra,
        })
        // koda_change end
...
const resetState = Effect.promise(async () => {
  ...
  // koda_change - each exerciser process already owns an isolated DB; unlinking it between scenarios races async koda callbacks
  await Bun.sleep(25)
})
