 ...
 *
 * The script intentionally isolates `koda_DB` before importing modules that touch
 ...
 */
import { Effect, Layer } from "effect" // koda_change
...
import { coverageResult, parseOptions, routeKey, routeKeys, selectedScenarios, shardScenarios } from "./routing" // koda_change
import { runScenario } from "./runner"
import { disposeApps } from "./backend"
import { publicApi, runtime } from "./runtime" // koda_change
import { type Scenario, type Result } from "./types"
import { kodaScenarios } from "../../koda/server/httpapi-exercise-scenarios" // koda_change
import { sessionAfterDefaultAgent } from "../../koda/server/httpapi-exercise-ready" // koda_change
...
const scenarios: Scenario[] = [
  ...
  // koda_change start - opt into git init; the worktree assertion requires a git-backed project
  http.protected.get("/path", "path.get").inProject({ git: true }).json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-koda-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-koda-directory")
  }),
  // koda_change end
  ...
  http.protected
    ...
    .mutating()
    // koda_change start - isolated git project: a non-git directory resolves to the
  ...
  // koda_change start
  http.protected.get("/koda/notebook", "koda.notebook.list").json(200, array),
  http.protected
    .post("/koda/notebook/{requestID}/reply", "koda.notebook.reply")
    .at((ctx) => ({
      path: route("/koda/notebook/{requestID}/reply", { requestID: "nbr_httpapi_reply" }),
      headers: ctx.headers(),
      body: {
        result: {
          operation: "read",
    ...
    .json(404, object, "status"),
  http.protected
    .post("/koda/notebook/{requestID}/reject", "koda.notebook.reject")
    .at((ctx) => ({
      path: route("/koda/notebook/{requestID}/reject", { requestID: "nbr_httpapi_reject" }),
      headers: ctx.headers(),
      body: { error: { code: "not_found", message: "Notebook not found" } },
    }))
    .json(404, object, "status"),
  http.protected.get("/koda/agent-manager", "koda.agentManager.list").json(200, array),
  http.protected
    .post("/koda/agent-manager/{requestID}/reply", "koda.agentManager.reply")
    .at((ctx) => ({
      path: route("/koda/agent-manager/{requestID}/reply", { requestID: "amr_httpapi_reply" }),
      headers: ctx.headers(),
      body: { result: { operation: "overview", overview: { sections: [], ungrouped: [] } } },
    }))
    .json(404, object, "status"),
  http.protected
    .post("/koda/agent-manager/{requestID}/reject", "koda.agentManager.reject")
    .at((ctx) => ({
      path: route("/koda/agent-manager/{requestID}/reject", { requestID: "amr_httpapi_reject" }),
      headers: ctx.headers(),
  ...
  http.protected.get("/experimental/worktree", "worktree.list").inProject({ git: true }).json(200, array), // koda_change
  http.protected
  ...
  http.protected.get("/experimental/capabilities", "experimental.capabilities.get").json(200, (body) => {
    // koda_change start - background subagents are always available
    check(
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "x-koda-ticket": "1" },
    }))
    .json(404, object, "status"),
  http.protected
  ...
  http.protected
    ...
    .seeded((ctx) => sessionAfterDefaultAgent(ctx, "Permission create owner")) // koda_change
    .at((ctx) => ({
      path: route("/api/session/{sessionID}/permission", { sessionID: ctx.state.id }),
      headers: ctx.headers(),
      body: { action: "read", resources: [".env"] },
    }))
    // koda_change start - opt into git init; the permission resolver consults VCS state for unknown resources
    .inProject({ git: true })
    // koda_change end
    .json(200, (body) => {
      object(body)
      ...
      check(body.data.effect === "ask", `permission create should create a pending request, got ${JSON.stringify(body.data.effect)}`) // koda_change
    }),
  http.protected
  ...
  http.protected
    ...
    .status(500, undefined, "status"), // koda_change
  http.protected
  ...
  http.protected
    ...
    .skipValidAuthProbe() // koda_change - valid requests intentionally block waiting for queued TUI input
    .mutating()
...
const main = Effect.gen(function* () {
  ...
  // koda_change start - coverage only needs the OpenAPI group; bypass runtime + LLM + DB allocation
  ...
  // koda_change end
  // koda_change start - dispose final non-mutating instances so shared test scopes can close; skip when coverage returned early so runtime() is never imported in static mode
  ...
  const results: Result[] = yield* Effect.forEach(
    ...
    (scenario) =>
      Effect.gen(function* () {
        ...
        // koda_change start - retry a failed scenario before failing the pass, mirroring
...
// koda_change start - route-only coverage must not acquire a listening fake LLM server; effect/auth runs alone
