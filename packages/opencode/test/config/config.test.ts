...
import { ConfigAgentV1 } from "@opencode-ai/core/v1/config/agent" // koda_change
...
import { Git } from "../../src/git" // koda_change
import {
...
import { isIndexingPlugin } from "@koda/koda-indexing/detect" // koda_change
import { isAtomicChatPlugin } from "@/koda/atomic-chat-feature" // koda_change
...
const schemaConfig = (config: object) => ({ $schema: "https://app.koda.ai/config.json", ...config }) // koda_change
...
const managedConfigDir = process.env.koda_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.koda_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.koda_CONSOLE_TOKEN
  else process.env.koda_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})

const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(path.join(managedConfigDir, filename ?? "koda.json"), JSON.stringify(settings)) // koda_change

// koda_change start
async function writeConfig(dir: string, config: object, name = "koda.json") {
  // koda_change end
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".koda"), schemaConfig(input.local)) : undefined, // koda_change
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(dir, "koda.jsonc")) // koda_change
      expect(content).toContain('"$schema": "https://app.koda.ai/config.json"') // koda_change
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)

it.effect("does not create global config when koda_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
// koda_change start
it.instance("preserves koda provider free model metadata", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.koda.ai/config.json",
    ...
    const config = yield* Config.use.get()
    const model = config.provider?.koda?.models?.["free-e2e"]
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // koda_change - upstream hardcodes project config to config.json; koda writes to koda.json
    ...
    const writtenConfig = yield* FSUtil.use.readJson(path.join(test.directory, "koda.json")) // koda_change
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* FSUtil.use.readJson(path.join(dir, "koda.json")) // koda_change
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.writeWithDirs(
      // koda_change start
      path.join(test.directory, "koda.jsonc"),
      `{
        // This is a comment
        "$schema": "https://app.koda.ai/config.json",
        "model": "test/model",
...
it.instance("jsonc overrides json in the same directory", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      {
        $schema: "https://app.koda.ai/config.json", // koda_change
        model: "base",
        username: "base",
      },
      "koda.jsonc", // koda_change
    )
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.koda.ai/config.json", // koda_change
      model: "override",
...
// koda_change start
it.instance("prefers .koda directory config over legacy .koda", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(path.join(test.directory, ".koda"), {
      $schema: "https://app.koda.ai/config.json",
      model: "legacy/model",
    })
    yield* writeConfigEffect(path.join(test.directory, ".koda"), {
...
// koda_change start - project config is untrusted: {env:} rejected; {file:} confined to the project root
...
it.instance("injects $schema into config without existing schema", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "koda.json"), JSON.stringify({ username: "test-user" }))
    const config = yield* Config.use.get()
    expect(config.username).toBe("test-user")
    expect(config.$schema).toBe("https://app.koda.ai/config.json")

    // Read the file to verify $schema was injected
    const content = yield* FSUtil.use.readFileString(path.join(test.directory, "koda.json"))
    expect(content).toContain('"$schema": "https://app.koda.ai/config.json"')
...
it.instance("injects $schema into comment-first JSONC config", () =>
  Effect.gen(function* () {
    ...
    const content = yield* FSUtil.use.readFileString(path.join(test.directory, "koda.jsonc"))
    expect(content).toContain('"$schema": "https://app.koda.ai/config.json"')
...
it.instance("does not write config when $schema already present", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    const filepath = path.join(test.directory, "koda.json")
    // Config already has $schema - should not rewrite file
    yield* FSUtil.use.writeWithDirs(
      filepath,
      JSON.stringify({ $schema: "https://app.koda.ai/config.json", username: "test-user" }),
    )
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:koda_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.instance("reports warning for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "koda.json"), "{ invalid json }")
    yield* Config.use.get()
    const issues = yield* Config.Service.use((svc) => svc.warnings())
    expect(issues.length).toBeGreaterThan(0)
  }),
...
// koda_change start
it.instance("loads config from .koda directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".koda", "agent", "test.md"), // koda_change
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".koda", "agent", "ordered.md"), // koda_change
      `---
permission:
  bash: allow
  "*": deny
...
// koda_change start
it.instance("loads agents from .koda/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".koda", "agents", "helper.md"), // koda_change
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".koda", "agents", "nested", "child.md"), // koda_change
      `---
model: test/model
mode: subagent
---
...
// koda_change start
it.instance("loads commands from .koda/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".koda", "command", "hello.md"), // koda_change
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".koda", "command", "nested", "child.md"), // koda_change
      `---
...
// koda_change start
it.instance("loads commands from .koda/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".koda", "commands", "hello.md"), // koda_change
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".koda", "commands", "nested", "child.md"), // koda_change
      `---
description: Nested command
---
Nested command template`,
...
// koda_change start
it.instance("prefers .koda commands over legacy .koda commands", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".koda", "command", "hello.md"),
      `---
...
it.instance("updates config and writes to file", () =>
  Effect.gen(function* () {
    ...
    const writtenConfig = yield* FSUtil.use.readJson(
      path.join(test.directory, ".koda", "koda.jsonc"), // koda_change
    )
...
it.effect("does not try to install dependencies in read-only koda_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("koda_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("ignores an inaccessible koda_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("koda_CONFIG_DIR", configDir, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("creates a missing koda_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("installs dependencies in writable koda_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const configDir = path.join(dir, "configdir")
    yield* FSUtil.use.ensureDir(configDir)
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".koda", "agent", "helper.md"), // koda_change
      `---
model: test/model
mode: subagent
---
...
it.effect("keeps plugin origins aligned with merged plugin list", () =>
  withConfigTree(
    ...
    Effect.gen(function* () {
      ...
      // koda_change start - bundled plugins intentionally have no external plugin origins
...
// koda_change - Note: preload.ts sets koda_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("permission config preserves user key order", () =>
  ...
  Effect.gen(function* () {
    const test = yield* TestInstance
    // koda_change start — isolate from global config to prevent cross-test contamination
...
// koda_change start - preserve legacy agent requirement declarations without forwarding them
...
// koda_change start - regression for `env` alias on local MCP entries
...
it.instance("project config can override MCP server enabled status", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // koda_change - base config in .json, override in .jsonc (jsonc loads second and wins)
...
it.instance("MCP config deep merges preserving base config properties", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // koda_change - base config in .json, override in .jsonc (jsonc loads second and wins)
    // koda_change - Base config with full MCP definition
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.koda.ai/config.json", // koda_change
      mcp: {
        myserver: {
    ...
    // koda_change - Override just enables it, should preserve other properties
    yield* writeConfigEffect(
      test.directory,
      {
        $schema: "https://app.koda.ai/config.json", // koda_change
...
// koda_change start
it.instance("local .koda config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.koda.ai/config.json", // koda_change
    ...
    yield* writeConfigEffect(
      path.join(test.directory, ".koda"), // koda_change
      {
        $schema: "https://app.koda.ai/config.json", // koda_change
        mcp: {
          docs: {
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "koda.json") // koda_change
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.koda/plugin/oh-my-opencode.js"] // koda_change
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.koda/plugin/demo.ts", "file:///project/.koda/plugin/demo.ts"] // koda_change

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.koda/plugin/demo.ts"]) // koda_change
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".koda", "plugin", "my-plugin.js"), // koda_change
          "export default {}",
        )
...
describe("koda_DISABLE_PROJECT_CONFIG", () => {
  // koda_change start
  ...
  it.instance("skips project .koda directory when flag is set", () =>
    withProcessEnv(
      "koda_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".koda", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { koda_CONFIG_DIR: undefined, koda_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "koda_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        ...
        yield* withProcessEnvs(
          { koda_DISABLE_PROJECT_CONFIG: "true", koda_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed koda_PERMISSION JSON used to crash
...
describe("koda_PERMISSION env var", () => {
  it.instance("does not crash when koda_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "koda_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("koda_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in koda_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
  ...
  it.instance("substitutes {file:} tokens in koda_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
