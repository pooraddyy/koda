...
import { mkdir } from "node:fs/promises" // koda_change
...
describe("opencode mcp add (non-interactive subprocess)", () => {
    ...
    ({ home, opencode }) =>
      Effect.gen(function* () {
        ...
        const config = yield* Effect.promise(() =>
          Bun.file(path.join(home, ".config", "koda", "koda.json")).json(), // koda_change
        )
  ...
  // koda_change start
  cliIt.concurrent(
    "writes to koda_CONFIG_DIR without touching the default profile",
    ({ home, opencode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* opencode.spawn(
          ...
          { env: { koda_CONFIG_DIR: profile } },
        )
        opencode.expectExit(result, 0)

        const config = yield* Effect.promise(() => Bun.file(path.join(profile, "koda.json")).json())
        expect(config.mcp.profile).toEqual({ type: "remote", url: "https://example.com/profile" })
        expect(yield* Effect.promise(() => Bun.file(path.join(home, ".config", "koda", "koda.json")).exists())).toBe(
          false,
        )
      }),
    60_000,
