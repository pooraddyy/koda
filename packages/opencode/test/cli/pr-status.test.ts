...
// Mock @/koda-sessions/pr-link before importing the command so the status
// handler reads the override/detection from these stubs instead of spawning
// `gh` or touching real Storage.
const realPrLink = await import("@/koda-sessions/pr-link")
...
void mock.module("@/koda-sessions/pr-link", () => ({
  ...realPrLink,
