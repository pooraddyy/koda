...
import { kodaClient, type GlobalEvent } from "@koda/sdk/v2"
...
type GlobalEventStream = Awaited<ReturnType<kodaClient["global"]["event"]>>["stream"]
type SessionMessage = NonNullable<Awaited<ReturnType<kodaClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<kodaClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<kodaClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    globalEvent?: kodaClient["global"]["event"]
    promptAsync?: kodaClient["session"]["promptAsync"]
    status?: kodaClient["session"]["status"]
    messages?: kodaClient["session"]["messages"]
    children?: kodaClient["session"]["children"]
    permissions?: kodaClient["permission"]["list"]
    questions?: kodaClient["question"]["list"]
  } = {},
) {
  const client = new kodaClient()

  const globalEvent: kodaClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: kodaClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: kodaClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: kodaClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: kodaClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: kodaClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: kodaClient["question"]["list"] = input.questions ?? (() => ok([]))
