...
// `koda_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { createkodaClient } from "@koda/sdk/v2" // koda_change
...
describe("opencode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, opencode }) =>
      Effect.gen(function* () {
        ...
        // koda_change - settle bash up front so the run exercises the stream finish rather than
        // koda's auto-reject exit contract, which a plain headless run would trip first.
  ...
  // koda_change start - koda headless runs must signal an unsuccessful session to automation
  cliIt.concurrent(
  ...
  cliIt.concurrent(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(result.exitCode).not.toBe(0)
        const events = opencode.parseJsonEvents(result.stdout)
        // koda_change - upstream expects a single record. koda emits two, and has since before
  ...
  cliIt.concurrent(
    ...
    ({ llm, opencode }) =>
      Effect.gen(function* () {
        ...
        // koda_change - settle bash up front; see the note on the reason assertion below
        ...
        expect(events.map((event) => event.type)).toEqual([
          "step_start",
          "tool_use",
          "text", // koda_change - a pre-denied tool settles before the SDK closes its preceding text part
          "step_finish",
          "step_start",
          "step_finish",
        ])
        // koda_change start - the pre-denied tool completes before text-end
        expect(events.find((event) => event.type === "text")?.part).toEqual(
          expect.objectContaining({ type: "text", text: "partial json" }),
        )
        // koda_change end
        // koda_change - upstream asserts reason "unknown" here. Reaching that requires the bash call
        // to proceed without permission friction, which a koda headless run never does: left alone the ask
  ...
  cliIt.concurrent(
    "auto-rejects requested permissions by default and allows them with the dangerous flag", // koda_change
    ({ home, llm, opencode }) =>
      Effect.gen(function* () {
        ...
        opencode.expectExit(denied, 1) // koda_change
        expect(denied.stderr).toContain("permission requested: bash")
        expect(denied.stderr).toContain("run ended with an auto-rejected permission; pass --auto for autonomous use") // koda_change
  ...
  cliIt.live(
    ...
    ({ llm, opencode }) =>
      Effect.gen(function* () {
        ...
        yield* llm.wait(1)
        const interrupted = Date.now() // koda_change - assert signal handling, independent of contended CLI startup
        run.interrupt()
        const result = yield* run.result

        expect(result.exitCode).not.toBe(0)
        expect(Date.now() - interrupted).toBeLessThan(10_000) // koda_change
      }),
    60_000, // koda_change
  )

  // koda_change start - non-interactive runs exclude human-driven tools like suggest
  cliIt.concurrent(
    "koda run --auto excludes suggest tool from LLM request",
    ({ llm, opencode }) =>
      Effect.gen(function* () {
        yield* llm.text("done")
  ...
  cliIt.live(
    "koda run auto-dismisses suggestion and exits cleanly if suggest tool is invoked in attached session",
    ({ llm, opencode }) =>
      Effect.gen(function* () {
        const server = yield* opencode.serve()
        const client = createkodaClient({ baseUrl: server.url })
