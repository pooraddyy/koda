...
describe("createTuiAttention", () => {
  ...
  test("supports focused-only requests", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "koda", message: "focused" }]) // koda_change
  })
  ...
  test("notification can deliver while focused when requested", async () => {
    ...
    expect(audio.playCalls).toBe(1)
    expect(renderer.notifications).toEqual([{ title: "koda", message: "hello" }]) // koda_change
  })
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "koda", message: "hello", sound: false })).toEqual({
      // koda_change
      ok: true,
      notification: true,
      sound: false,
  ...
  test("can play sound always while notification is blurred-only", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "koda", message: "hello again" }]) // koda_change
  })
  ...
  test("plays named sounds from the active sound pack", async () => {
    ...
    dispose()
    expect(attention.soundboard.current()).toBe("koda.default") // koda_change
  })
