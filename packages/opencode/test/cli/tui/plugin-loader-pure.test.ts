...
`,
  ...
  const pure = process.env.koda_PURE
  const meta = process.env.koda_PLUGIN_META_FILE
  process.env.koda_PURE = "1"
  process.env.koda_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.koda_PURE
    } else {
      process.env.koda_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.koda_PLUGIN_META_FILE
    } else {
      process.env.koda_PLUGIN_META_FILE = meta
    }
  }
})
