...
describe("tui thread", () => {
  ...
  // koda_change start - preserve the sanitized koda worker environment
  ...
  test("ignores stale PWD when resolving a relative mini project", async () => { // koda_change
    await using pwd = await tmpdir({ git: true })
    await using cwd = await tmpdir({ git: true })

    expect(resolveThreadDirectory(".", pwd.path, cwd.path)).toBe(cwd.path) // koda_change
