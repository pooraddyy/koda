...
import type { TuiAttentionSoundPack } from "@koda/plugin/tui"
...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".koda", "themes", localThemeFile) // koda_change
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".koda", "themes", preloadedThemeFile) // koda_change
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".koda", "themes", tmp.extra.globalThemeFile)) // koda_change
      .then(() => true)
      .catch(() => false)
...
`,
  ...
  process.env.koda_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
  ...
  try {
    ...
    cwd.mockRestore()
    wait.mockRestore()
    delete process.env.koda_PLUGIN_META_FILE
  }
})
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ".koda", "themes", themeFile) // koda_change
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.koda_PLUGIN_META_FILE!,
    )
