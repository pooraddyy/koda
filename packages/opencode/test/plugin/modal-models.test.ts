...
import type { Model, Provider } from "@koda/sdk/v2"
...
test("discovers Modal workspace models", async () => {
        ...
        data: [
          {
            ...
            interleaved: {
              field: "reasoning_text", // koda_change - accept Modal's supported alternate field
            },
          },
          {
  ...
  expect(model.capabilities.interleaved).toEqual({ field: "reasoning_text" }) // koda_change
...
// koda_change start - discovery failures must retain the static provider catalog
