...
import { MCP } from "../../src/mcp" // koda_change
...
const agentLayer = (flags: Partial<RuntimeFlags.Info> = {}) =>
  AppNodeBuilder.build(Agent.node, [
    [MCP.node, Layer.mock(MCP.Service)({})], // koda_change
...
const scout = testEffect(agentLayer({ experimentalScout: true })) // koda_change
...
it.instance("returns default native agents when no config", () =>
  Effect.gen(function* () {
    ...
    expect(names).not.toContain("scout") // koda_change
...
it.instance("build agent has correct default properties", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(build, "interactive_terminal")).toBe("allow") // koda_change
  }),
)
...
it.instance("plan agent denies edits except .opencode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(plan, "interactive_terminal")).toBe("deny") // koda_change
...
it.instance("explore agent denies edit and write", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(explore, "interactive_terminal")).toBe("deny") // koda_change
  }),
)
...
// koda_change start - Scout is opt-in and owns repository research permissions
...
scout.instance(
  ...
  {
    config: {
      // koda_change - Scout-backed koda agents use the supported references config
      references: {
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".koda", "skill", "perm-skill") // koda_change
