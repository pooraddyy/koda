...
import { kodaTask } from "../../src/koda/tool/task" // koda_change
...
// koda_change start - preserve Plan edit/notebook ceilings across koda task delegation,
...
it.instance("Plan delegation preserves notebook ceilings without projecting bash denies", () =>
  Effect.gen(function* () {
    ...
    const rules = kodaTask.inherited({
      caller: caller!,
      session: { permission: [] } as unknown as Parameters<typeof kodaTask.inherited>[0]["session"],
      mcp: {},
    })
  ...
  () =>
    Effect.gen(function* () {
      ...
      const effective = (caller: Agent.Info) =>
        Permission.merge(explore!.permission, kodaTask.permissions(inherited(caller)))
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const effective = Permission.merge(worker!.permission, kodaTask.permissions(rules))
      expect(Permission.evaluate("bash", "git status", effective).action).toBe("allow")
