...
describe("Database.getChannelPath", () => {
  it.effect("returns database path for the current channel", () =>
    Effect.gen(function* () {
      ...
      const expected = ["latest", "beta", "prod"].includes(InstallationChannel)
        ? path.join(Global.Path.data, "koda.db")
        : path.join(Global.Path.data, `koda-${InstallationChannel.replace(/[^a-zA-Z0-9._-]/g, "-")}.db`) // koda_change
  ...
  it.effect("uses the shared database path when channel databases are disabled", () =>
    Effect.gen(function* () {
      ...
      expect(Database.getChannelPath(flags)).toBe(path.join(Global.Path.data, "koda.db"))
