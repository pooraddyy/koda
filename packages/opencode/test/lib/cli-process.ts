...
//   - koda_CONFIG_CONTENT      : provider config inline, no files to find
//   - koda_TEST_HOME           : pins os.homedir() → tmpdir
//   - koda_DISABLE_PROJECT_CONFIG : skip walking up for opencode.json
//   - koda_PURE                : skip external plugin discovery + install
//   - koda_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
import { TestCli } from "../../script/koda/test-cli" // koda_change

const opencodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(opencodeRoot, "src/index.ts")
// koda_change start - reuse the runner's once-built CLI graph instead of transpiling it in every child
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    koda_CONFIG_CONTENT: configJson,
    koda_DISABLE_PROJECT_CONFIG: "1",
    koda_PURE: "1",
    koda_DISABLE_AUTOUPDATE: "1",
    koda_DISABLE_AUTOCOMPACT: "1",
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("opencode.spawn")(function* (args: string[], opts?: SpawnOpts) {
      ...
      const timeoutMs = opts?.timeoutMs ?? 45_000 // koda_change - current full CLI startup leaves less than 30s for multi-step runs
      ...
      const command = ChildProcess.make("bun", [...cliArgs, ...args], {
        ...
        extendEnv: true,
        stdin: "ignore",
        detached: false, // koda_change - keep test children in the runner's process lifecycle
      })
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          koda_CONFIG_CONTENT: JSON.stringify({
      ...
      const proc = yield* Effect.acquireRelease(
        Effect.sync(() =>
          Bun.spawn(["bun", ...cliArgs, ...runArgs(message, opts)], {
            // koda_change - cliArgs carries the solid preload
            cwd: home,
...
export const cliIt = {
  ...
  concurrent: <A, E>(
    ...
    // koda_change start - full CLI processes contend heavily during startup after the Effect graph migration
    test.serial(
      name,
