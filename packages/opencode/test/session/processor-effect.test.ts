...
import { SessionNetwork } from "../../src/session/network" // koda_change
import { Bus } from "../../src/bus" // koda_change
...
// koda_change start - exercise non-default output token ceilings in the processor
...
// koda_change start - configured output ceiling must reach finish-step overflow accounting
...
it.live("session.processor effect tests reset reasoning state across retries", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
        // koda_change start — auto-reply to network reconnection prompts triggered by reset()
        ...
        offAsk() // koda_change — cleanup subscriber
      }),
    { config: (url) => providerCfg(url) },
  ),
)
...
// koda_change start — send_file delivery attachments must skip image normalization.
