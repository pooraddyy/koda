...
import { USER_AGENT } from "../../src/installation" // koda_change
...
type ConfigModel = NonNullable<NonNullable<NonNullable<ConfigV1.Info["provider"]>[string]>["models"]>[string] // koda_change
...
describe("session.llm.ai-sdk adapter", () => {
  ...
  // koda_change start - preserve AI SDK raw usage for koda provider billing
  ...
  test("preserves providerMetadata on step-finish so Anthropic cache writes survive getUsage", async () => {
    ...
    expect(stepFinish.providerMetadata).toEqual({
      ...
      koda: { routedModelID: "claude-3-5-sonnet" },
    })
    // koda_change end
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(headers.get("User-Agent") ?? "").toMatch(/^koda-Code\//) // koda_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const capture = yield* Effect.promise(() => request)
        const body = capture.body
        const headers = capture.headers // koda_change

        expect(capture.url.pathname.endsWith("/messages")).toBe(true)
        expect(headers.get("User-Agent")?.split(" ")[0]).toBe(USER_AGENT) // koda_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // koda_change start - auth keys use the same Google API key header as Standard keys
