...
import { Snapshot } from "@/snapshot" // koda_change
...
function fake(
  ...
  return {
    ...
    metadata: Effect.fn("TestSessionProcessor.metadata")(() => Effect.void), // koda_change
...
type CompactionProcessOptions = {
  ...
  flags?: Partial<RuntimeFlags.Info> // koda_change
  snapshot?: Layer.Layer<Snapshot.Service> // koda_change
}
...
function compactionProcessLayer(options?: CompactionProcessOptions) {
  const replacements: LayerNode.Replacements = [
    ...
    [RuntimeFlags.node, RuntimeFlags.layer({ experimentalEventSystem: true, ...options?.flags })], // koda_change
    [SessionSummary.node, summary],
    ...(options?.snapshot ? ([[Snapshot.node, options.snapshot]] as const) : []), // koda_change
  ]
  if (!options?.llm) {
...
// koda_change start - keep retry-backoff cancellation tests independent of git snapshot cleanup latency
...
describe("session.compaction.process", () => {
  ...
  // koda_change start - configured output ceiling controls automatic tail budgeting
  itCompaction.instance(
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      }).pipe(withCompaction({ llm: stub.llmLayer, snapshot: snap })) // koda_change
    },
    { timeout: 10_000 }, // koda_change - snapshot is isolated above
  )
    ...
    () =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          // koda_change start - Effect 4.x timeout throws TimeoutError on the error
        ...
        }).pipe(withCompaction({ plugin: plugin(ready), snapshot: snap })) // koda_change - avoid git snapshot startup
      }),
    {}, // koda_change - isolate cancellation from git setup
  )
...
describe("SessionNs.getUsage", () => {
  ...
  // koda_change start - Test for OpenRouter provider cost
  ...
  test("uses upstreamInferenceCost for koda provider", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    const provider = { id: "koda" } as Provider.Info
    const result = SessionNs.getUsage({
      model,
      provider,
      usage: usage({ inputTokens: 1_000_000, outputTokens: 100_000, totalTokens: 1_100_000 }),
    ...
    // Should use upstreamInferenceCost for koda provider (BYOK)
  ...
  test("uses regular cost when upstreamInferenceCost is missing for koda", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    // When upstream cost is missing for koda, fall back to regular cost field
    expect(result.cost).toBe(0.01)
  })

  // Tests for Anthropic Messages / OpenAI Responses / Vercel AI Gateway cost extraction
  // live in test/koda/provider-cost.test.ts (koda_change).
  // koda_change end
