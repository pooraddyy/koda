...
describe("session.message-v2.toModelMessage", () => {
  ...
  // koda_change start — send_file delivery attachments must not be replayed to the model
  test("strips send_file delivery attachments from model context", async () => {
        ...
        parts: [
          {
            ...
            state: {
              ...
              output: "File example.log (50 bytes, text/plain) delivered to the user's koda app.",
    ...
    expect(toolContent.output).toStrictEqual({
      type: "text",
      value: "File example.log (50 bytes, text/plain) delivered to the user's koda app.",
    })
  })
  // koda_change end
