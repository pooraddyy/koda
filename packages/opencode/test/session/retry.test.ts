...
const it = testEffect(LayerNode.compile(LayerNode.group([SessionStatus.node, CrossSpawnSpawner.node, Database.node])))
...
describe("session.retry.delay", () => {
  ...
  it.instance("policy updates retry status and increments attempts", () =>
    Effect.gen(function* () {
      ...
      const retryEvents: Array<{ attempt: number; error?: unknown }> = []
      ...
      const step = yield* Schedule.toStepWithMetadata(
        SessionRetry.policy({
          ...
          set: (info) => {
            retryEvents.push({ attempt: info.attempt, error: info.error })
      ...
      expect(retryEvents[0]).toMatchObject({ attempt: 1, error })
      expect(retryEvents[1]).toMatchObject({ attempt: 2, error })
    }),
  )
})
...
describe("session.retry.retryable", () => {
  ...
  // koda_change start - koda does not support OpenCode Go upsells
  test("does not retry free usage limits", () => {
    ...
    expect(SessionRetry.retryable(error, "koda")).toBeUndefined()
  })
  // koda_change end
})
