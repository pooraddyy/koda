...
import { legacyReviewMessage } from "../../src/koda/review/command"
...
import { SessionProjector } from "@opencode-ai/core/session/projector" // koda_change
import { SessionPrompt } from "../../src/session/prompt"
import { SessionRevert } from "../../src/session/revert"
import { SessionRunState } from "../../src/session/run-state"
import { kodaSession } from "../../src/koda/session"
// koda_change start - Item 14 cancel→reprompt proof helpers
...
// koda_change start - one compiled graph per env. Effect v4 does not memoize nested layers, so
...
// Mirrors upstream's harness, with koda's kodaSessions/MemoryService/fastAgents deltas.
...
function makePrompt(input?: { processor?: "blocking" }) {
  const replacements = [
    ...
    [kodaSessions.node, kodaSessions.testLayer],
  ] as const
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://app.koda.ai/config.json", ...config }),
  )
})
...
// koda_change start - guard provider-compatible max-step request shape
it.instance(
...
noLLMServer.instance.skip(
  ...
  () =>
    Effect.gen(function* () {
      ...
      // koda_change start - compile the v2 reader against this test's database graph
...
it.instance(
  "budget exhaustion stops the prompt loop and persists its terminal reason",
  () =>
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig((url) => ({
        ...providerCfg(url),
        agent: { code: { budget: { tokens: 1 } } },
      }))
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      const session = yield* sessions.create({ title: "Budgeted prompt" })
      expect((yield* agents.get("build"))?.budget).toEqual({ tokens: 1 })
      ...
      yield* prompt.prompt({
        ...
        agent: "build",
        noReply: true,
        parts: [{ type: "text", text: "stop after the first provider response" }],
      })
      yield* llm.text("bounded", { usage: { input: 2, output: 1 } })
...
// koda_change start - Item 14 CLI prove-it: cancel settles to Idle promptly,
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* pollWithTimeout(
        Effect.sync(() => (kodaSessionPromptQueue.hasFollowup(chat.id) ? (true as const) : undefined)),
      ...
      expect(kodaSessionPromptQueue.hasFollowup(chat.id)).toBe(false)
      expect((yield* status.get(chat.id)).type).toBe("idle")
      const free = yield* run.assertNotBusy(chat.id).pipe(Effect.exit)
      expect(Exit.isSuccess(free)).toBe(true)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const intake = yield* kodaSessionPrompt.intake(
        sessionID,
        Effect.gen(function* () {
...
it.instance(
  ...
  { git: true },
  30_000, // koda_change - isolated suite load can delay queued live-loop cancellation
)
...
it.instance("prompt submitted during an active run is included in the next LLM input", () =>
  Effect.gen(function* () {
    ...
    const messages = inputs.at(-1)?.messages
    if (!Array.isArray(messages)) throw new Error("expected LLM messages")
    // koda_change start - koda appends environment details to queued user prompts
    expect(messages.at(-1)).toMatchObject({
      role: "user",
...
unixNoLLMServer(
  ...
  () =>
    Effect.gen(function* () {
      ...
      // koda_change start - bind v2 execution and location services in the consolidated test graph
      const messages = yield* SessionV2.Service.use((session) => session.messages({ sessionID: chat.id })).pipe(
        Effect.provide(
          LayerNode.compile(SessionV2.node, [
            [SessionExecution.node, SessionExecution.noopLayer],
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() =>
          Bus.subscribe(kodaSession.Event.TurnOpen, (event) => {
