...
// Regression coverage for https://github.com/koda-Org/koda/issues/8656
//
...
// `config` hook (see ../fixture/stall-plugin.ts), so the SDK, koda's fetch
...
function session(dir: string) {
  ...
  const headers = { "Content-Type": "application/json", "x-koda-directory": dir }
