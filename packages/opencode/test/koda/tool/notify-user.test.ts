...
import { NotifyUserTool } from "@/koda/tool/notify-user"
...
function runNotifyTool(params: { readonly message: string }, sessions: kodaSessions.Interface) {
  const layer = Layer.mergeAll(
    Layer.succeed(kodaSessions.Service, kodaSessions.Service.of(sessions)),
...
describe("notify_user tool", () => {
  ...
  test("registers with id and description", async () => {
    const layer = Layer.mergeAll(
      Layer.succeed(kodaSessions.Service, kodaSessions.Service.of({
        init: () => Effect.void,
  ...
  test("trims message before sending", async () => {
    ...
    const sessions = kodaSessions.Service.of({
      init: () => Effect.void,
  ...
  test("returns failure text when not connected", async () => {
    ...
    expect(result.output).toContain("not connected to koda cloud")
  })
