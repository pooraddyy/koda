...
import { SendFileTool } from "@/koda/tool/send-file"
...
const status = spyOn(kodaSessions, "remoteStatus")
...
describe("send_file tool", () => {
  test("is only available while remote is connected", () => {
    ...
    expect(kodaToolRegistry.available(tool, agentInfo)).toBe(false)

    status.mockReturnValue({ enabled: true, connected: false })
    expect(kodaToolRegistry.available(tool, agentInfo)).toBe(false)

    status.mockReturnValue({ enabled: true, connected: true })
    expect(kodaToolRegistry.available(tool, agentInfo)).toBe(true)
  })
  ...
  test("sniffs correct MIME for a file with wrong extension", async () => {
    const dir = await tmpdir()
    try {
      // Actually just test a real PNG — kodaReadObject opens by path,
  ...
  // koda_change start — send_file now authorizes missing files with external_directory + read
  ...
  // koda_change start — send_file now authorizes directories before returning a
...
// koda_change start — fsService mock now includes stat + realPath for the
// missing/directory authorization sequence that runs before kodaReadObject.file().
