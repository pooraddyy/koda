...
import { MemorySaveTool } from "@/koda/tool/memory-save"
...
const watch = process.env.koda_EXPERIMENTAL_DISABLE_FILEWATCHER
...
beforeEach(() => {
  process.env.koda_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  spyOn(RemoteSender, "create").mockReturnValue({ handle() {}, dispose() {} })
})

afterEach(async () => {
  mock.restore()
  if (watch === undefined) delete process.env.koda_EXPERIMENTAL_DISABLE_FILEWATCHER
  if (watch !== undefined) process.env.koda_EXPERIMENTAL_DISABLE_FILEWATCHER = watch
  await resetDatabase()
})
...
describe("koda_memory_save", () => {
  ...
  test("defaults mutating memory tool permission to ask", () => {
    const koda = kodaAgent.prepare({})

    expect(Permission.evaluate("koda_memory_recall", "typed", koda.defaultsPatch).action).toBe("ask")
    expect(Permission.evaluate("koda_memory_save", "remember", koda.defaultsPatch).action).toBe("ask")
  })

  test("remembers, corrects, and forgets explicit project memory", async () => {
    await using dir = await tmpdir({ git: true })
    await withConfig(path.join(dir.path, "global", ".koda"), async () => {
      ...
      const corrected = await execute(
        dir.path,
        {
          action: "correct",
          key: "koda_cli_tui",
          text: "koda CLI should be treated as a terminal UI.",
        },
        gate,
      )
      const shown = await kodaMemory.show({ ctx: memory })

      expect(saved.title).toBe("koda memory saved: 1 op")
      expect(corrected.title).toBe("koda memory correction saved: 1 op")
      expect(shown.sources.project).toContain("- koda_cli_tui :: koda CLI is a TUI.")
      expect(shown.sources.corrections).toContain("- koda_cli_tui :: koda CLI should be treated as a terminal UI.")

      const forgotten = await execute(dir.path, { action: "forget", query: "koda_cli_tui" }, gate)
      const next = await kodaMemory.show({ ctx: memory })

      expect(forgotten.title).toBe("koda memory updated: 2 removed")
      expect(next.sources.project).not.toContain("koda_cli_tui")
      expect(next.sources.corrections).not.toContain("koda_cli_tui")
      expect(asks.map((req) => req.permission)).toEqual(["koda_memory_save", "koda_memory_save", "koda_memory_save"])
  ...
  test("does not save when project memory is disabled", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".koda"), async () => {
      ...
      expect(result.title).toBe("koda memory: disabled")
  ...
  test("declines user-level memory but saves project conventions", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".koda"), async () => {
      ...
      expect(skipped.title).toBe("koda memory skipped: out of scope")
      expect(skipped.output).toContain("user-level memory is not supported yet")
      expect(blocked.title).toBe("koda memory unchanged")
  ...
  test("handles malformed save inputs without corrupting memory files", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".koda"), async () => {
      ...
      expect(missing.title).toBe("koda memory remember: no text")
      expect(forget.title).toBe("koda memory forget: no query")
      expect(secret.title).toBe("koda memory: error")
      expect(secret.output).toContain("rejected secret-like content")
      expect(noisy.title).toBe("koda memory saved: 1 op")
