...
import {
  ...
  IMAGE_MODELS,
  DEFAULT_MODEL,
} from "../../../src/koda/tool/generate-image"
...
describe("generate-image provider resolver", () => {
  test("uses koda cloud when koda auth is present", () => {
    const result = resolveProvider({ type: "oauth", access: "koda-token", accountId: "org-123" }, undefined)
    expect(result).not.toBeNull()
    expect(result!.token).toBe("koda-token")
    expect(result!.organizationId).toBe("org-123")
    expect(result!.provider).toBe("koda")
    expect(result!.url).toContain("openrouter")
  })

  test("uses koda cloud with API key auth", () => {
    const result = resolveProvider({ type: "api", key: "koda-api-key" }, undefined)
    expect(result!.token).toBe("koda-api-key")
    expect(result!.provider).toBe("koda")
  })

  test("falls back to OpenRouter with BYO key when no koda auth", () => {
  ...
  test("prefers koda auth over OpenRouter key", () => {
    const result = resolveProvider({ type: "oauth", access: "koda-token" }, "or-key")
    expect(result!.provider).toBe("koda")
    expect(result!.token).toBe("koda-token")
  })
})
