...
import { MemoryRecallTool } from "@/koda/tool/memory-recall"
...
describe("koda_memory_recall description", () => {
...
describe("koda_memory_recall", () => {
  ...
  test("shows typed memory hits separately from session digests", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".koda"), async () => {
      const memory = { directory: dir.path, worktree: dir.path }
      const enabled = await kodaMemory.enable({ ctx: memory })
        ...
        ops: [
          {
            ...
            section: "Commands",
            key: "terminal_tests",
            text: "Run memory unit tests from packages/koda-memory with bun test test/core.test.ts.",
          },
          {
            action: "add",
            file: "project.md",
      ...
      const typed = await execute(dir.path, { mode: "typed", query: "terminal unit tests" })
      ...
      expect(typed.title).toContain("koda memory typed")
      expect(typed.output).toContain("targeted_context_not_instruction")
      expect(typed.output).toContain("terminal_tests")
      ...
      expect(digest.title).toContain("koda memory digest")
  ...
  test("catalog mode lists all stored keys with optional filter", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".koda"), async () => {
        ...
        ops: [
          {
            ...
            file: "project.md",
            section: "Facts",
            key: "koda_was_originally_a_fork",
            text: "koda was originally a fork of roo and has a koda-legacy repo",
          },
          {
            action: "add",
            file: "environment.md",
      ...
      expect(all.output).toContain("koda_was_originally_a_fork")
      expect(all.output).toContain("terminal_tests")
      expect(filtered.output).toContain("koda_was_originally_a_fork")
      expect(filtered.output).not.toContain("terminal_tests")
    })
  })
  ...
  test("search mode renders typed and digest memory without catalog mode", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".koda"), async () => {
      ...
      expect(result.title).toContain("koda memory search")
  ...
  test("handles large unmatched recall queries without returning stored memory", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".koda"), async () => {
      ...
      expect(result.title).toBe("koda memory search: no results")
