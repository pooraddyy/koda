...
import {
  koda_EXA_URL,
  MAX_koda_EXA_RESULTS,
  type kodaExaParams,
  callkodaExa,
} from "../../../src/koda/tool/websearch-koda-exa"
...
const runCall = async (
  ...
  recorded?: Recorded,
  kodaToken = "koda-test-token",
) =>
  Effect.runPromiseExit(
    Effect.gen(function* () {
      const http = fakeHttp(respond, recorded)
      return yield* callkodaExa(http, params, kodaToken)
    }),
  )

describe("callkodaExa request shape", () => {
  test("posts to koda_EXA_URL with bearer token and highlights-only contents", async () => {
    ...
    expect(recorded.authorization).toBe("Bearer koda-test-token")
    const parsed = JSON.parse(recorded.body!)
    expect(parsed.query).toBe("drone")
    expect(parsed.type).toBe("auto")
    expect(parsed.numResults).toBe(MAX_koda_EXA_RESULTS)
  ...
  test("clamps numResults at MAX_koda_EXA_RESULTS", async () => {
    const recorded: Recorded = {}
    await runCall({ query: "x", numResults: 25 }, () => okJson({ results: [] }), recorded)
    expect(JSON.parse(recorded.body!).numResults).toBe(MAX_koda_EXA_RESULTS)
  })
  ...
  test("koda_EXA_URL is built from koda_API_BASE", () => {
    expect(koda_EXA_URL).toMatch(/\/api\/exa\/search$/)
  })
})

describe("callkodaExa response formatting", () => {
...
describe("callkodaExa error handling", () => {
  test("dies with auth-required message on 401", async () => {
    ...
    expect(String((exit as Extract<typeof exit, { _tag: "Failure" }>).cause)).toContain("koda auth login")
  })
