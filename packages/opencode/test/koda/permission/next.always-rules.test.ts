...
afterAll(async () => {
  ...
  for (const file of ["koda.jsonc", "koda.json", "config.json", "opencode.json", "opencode.jsonc"]) {
    ...
    withDir({ git: true }, () =>
      Effect.gen(function* () {
        const fiber = yield* ask({
          ...
          permission: "bash",
          patterns: ["koda-permission-8353 test"],
          metadata: { rules: ["koda-permission-8353 *", "koda-permission-8353 test"] },
          always: ["koda-permission-8353 *", "koda-permission-8353 test"],
          ruleset: [],
        }).pipe(Effect.forkScoped)
        ...
        yield* saveAlwaysRules({
          ...
          approvedAlways: ["koda-permission-8353 test"],
        })
        ...
        expect(cfg.permission?.bash).toMatchObject({ "koda-permission-8353 test": "allow" })
        expect(cfg.permission?.bash).not.toMatchObject({ "koda-permission-8353 *": "allow" })

        const broad = yield* ask({
          id: PermissionV1.ID.make("permission_saved_always_broad"),
          sessionID: SessionID.make("session_saved_always"),
          permission: "bash",
          patterns: ["koda-permission-8353 install"],
          metadata: {},
          always: [],
          ruleset: [],
        }).pipe(Effect.forkScoped)
