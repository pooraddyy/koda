...
test("project config permission keys are attributed to the local scope", async () => {
  ...
  await Bun.write(path.join(koda, "koda.json"), JSON.stringify({ permission: { bash: { "echo *": "allow" } } }))
...
test("a scalar project bash permission maps to the '*' pattern under the local scope", async () => {
  ...
  const koda = path.join(dir, ".koda")
  await fs.mkdir(koda, { recursive: true })
  await Bun.write(path.join(koda, "koda.json"), JSON.stringify({ permission: { bash: "allow" } }))
