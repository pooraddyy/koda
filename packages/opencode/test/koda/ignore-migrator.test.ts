...
import { IgnoreMigrator } from "../../src/koda/ignore-migrator"
...
# Comment 2`
  ...
  *.env  `
  ...
  describe("migrate", () => {
    test("returns empty permission for project without .kodaignore", async () => {
    ...
    test("loads project .kodaignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
    ...
    test("handles complex .kodaignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".kodaignore"),
            `# Secrets
secrets/
*.env
!.env.example
...
`,
  ...
  describe("loadIgnoreConfig", () => {
    test("returns empty object for project without .kodaignore", async () => {
      await using tmp = await tmpdir()
    ...
    test("returns permission config for project with .kodaignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".kodaignore"), "secrets/")
        },
