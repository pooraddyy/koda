...
const wrapper = path.join(root, "bin", "koda")
...
describe("npm install artifact behavior", () => {
  test("keeps the CLI wrapper contract", async () => {
    ...
    expect(text).toContain("const envPath = process.env.koda_BIN_PATH")
    expect(text).toContain('const base = "@koda-code/cli-" + platform + "-" + arch')
  ...
  test("copies cached binary runtime resources during postinstall", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "koda-postinstall-artifact-"))
    try {
      const pkg = path.join(tmp, "node_modules", "@koda-code", "cli")
      const native = path.join(tmp, "node_modules", "@koda-code", `cli-${process.platform}-${process.arch}`)
      ...
      await Bun.write(
        ...
        JSON.stringify({
          optionalDependencies: {
            [`@koda-code/cli-${process.platform}-${process.arch}`]: "1.0.0",
          },
        }),
      )
      await Bun.write(
        path.join(native, "package.json"),
        JSON.stringify({ name: `@koda-code/cli-${process.platform}-${process.arch}` }),
      )
      const binary = "#!/bin/sh\n# binary\nexit 0\n"
      await Bun.write(path.join(bin, "koda"), binary)
      await Bun.write(path.join(bin, "koda-sandbox-mutation-worker.js"), "worker")
      ...
      expect(await Bun.file(path.join(pkg, "bin", ".koda")).text()).toBe(binary)
      expect(await Bun.file(path.join(pkg, "bin", "koda-sandbox-mutation-worker.js")).text()).toBe("worker")
  ...
  test("links npm bin commands to the wrapper during local install", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "koda-install-artifact-"))
    try {
      ...
      await fs.copyFile(wrapper, path.join(bin, "koda"))
      await Bun.write(
        path.join(pkg, "package.json"),
        JSON.stringify(
          {
            name: "koda-install-artifact-repro",
            version: "1.0.0",
            bin: {
              koda: "./bin/koda",
              koda: "./bin/koda",
      ...
      const hidden = path.join(prefix, "node_modules", ".bin", ".koda")
