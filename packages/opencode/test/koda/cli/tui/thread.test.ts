...
import { preload } from "../../../../src/koda/cli/cmd/tui"
import { kodaTuiThreadDaemon } from "../../../../src/koda/cli/cmd/tui/thread"
...
describe("koda tui thread", () => {
  ...
  test(
    ...
    async () => {
      ...
      const data = proc.onData((chunk) => {
        ...
        if (state.text.includes("KODA CLI")) ready.resolve()
      })
      ...
      try {
        await ready.promise
        expect(state.text).toContain("KODA CLI")
      } finally {
        clearTimeout(timer)
        data.dispose()
  ...
  test("uses koda-dev caller directory when running through package cwd", async () => {
    await using root = await tmpdir()
    const pkg = path.join(root.path, "packages", "opencode")
    await fs.mkdir(pkg, { recursive: true })

    const prev = process.env.koda_DEV_CWD
    process.env.koda_DEV_CWD = root.path
    try {
      ...
      if (prev === undefined) delete process.env.koda_DEV_CWD
      else process.env.koda_DEV_CWD = prev
    }
  })
  ...
  test("validates imported daemon session over HTTP after importing from cloud", async () => {
    ...
    using server = Bun.serve({
      ...
      fetch(request) {
        ...
        calls.push(route)
        if (route === "POST /koda/cloud/session/import") return Response.json({ id: local })
    ...
    const args = { port: 0, hostname: "127.0.0.1", mdns: false, "mdns-domain": "koda.local", cors: [] }
    const start: Parameters<typeof kodaTuiThreadDaemon.attach>[0]["start"] = async (input) => {
      opened.push(input.args.sessionID)
    }

    try {
      await kodaTuiThreadDaemon.attach({
      ...
      expect(calls).toEqual(["POST /koda/cloud/session/import", `GET /session/${local}`])
  ...
  test("imports cloud fork before validating daemon session", async () => {
    ...
    mock.module("@/koda/daemon/client", () => ({
      DaemonClient: {
    ...
    const mod = await import(`../../../../src/koda/cli/cmd/tui/thread?${key}`)

    const handled = await mod.kodaTuiThreadDaemon.attach({
