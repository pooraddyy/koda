...
import { detectRepo, findRepoFrom } from "../../../src/koda/cli/dev-setup"
...
describe("findRepoFrom", () => {
  test("returns the repo root when started from a nested path", async () => {
    ...
    const nested = path.join(tmp.path, "packages", "opencode", "src", "koda", "cli")
  ...
  test("returns undefined when no sentinel is found up to filesystem root", async () => {
    ...
    // above tmp does not contain a koda repo at its own root.
    const found = await findRepoFrom(tmp.path)
...
describe("detectRepo", () => {
  test("honours koda_DEV_REPO without touching the filesystem", async () => {
    // Pointing at a path that does not exist — the hint must be returned as-is
    // (this proves no walk happens when koda_DEV_REPO is set).
    const hint = "/definitely/does/not/exist/koda-repo"
    const got = await withEnv("koda_DEV_REPO", hint, () => detectRepo())
    expect(got).toBe(hint)
  })

  test("koda_DEV_REPO wins over the filesystem walk", async () => {
    // Build a synthetic repo, point koda_DEV_REPO at it, and run from an
    // unrelated cwd. Even though the walk from cwd (inside the real koda
    // checkout) would succeed, the hint must take priority.
    await using tmp = await tmpdir()
    await makeSynthRepo(tmp.path)

    const got = await withEnv("koda_DEV_REPO", tmp.path, () => detectRepo())
