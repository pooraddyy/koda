...
describe("MCP errors", () => {
  test("uses koda-neutral capability messaging", () => {
...
describe("model not found errors", () => {
  test("indicates when no models are available", () => {
    ...
    expect(named).toContain("koda.json")
...
describe("remote config authentication errors", () => {
  test("uses the koda login command", () => {
    const error = FormatError({
    ...
    expect(error).toContain("koda auth login https://example.com/config.json")
