...
import { kodaTuiConfig } from "@/koda/cli/cmd/tui/context/tui-config"
import { kodaTerminalTitle } from "@/koda/cli/cmd/tui/terminal-title"
...
function resolve(input: TuiConfig.Info): TuiConfig.Resolved {
  ...
  return {
    ...
    attention: {
      ...
      sound_pack: input.attention?.sound_pack ?? "koda.default",
...
describe("kodaTuiConfig.makeStore", () => {
  test("reactive reads update when set() reconciles a new config", () => {
    const store = kodaTuiConfig.makeStore(resolve(cfg({ keybinds: { app_exit: "ctrl+c" }, theme: "koda" })))
    ...
    createRoot((d) => {
      ...
      createEffect(() =>
        titles.push(
          kodaTerminalTitle.format({ base: "KODA CLI", indicator: "working", icon: store.config.title_icon }),
        ),
      )
    })
    ...
    expect(themes).toEqual(["koda"])
    expect(icons).toEqual(["none"])
    expect(titles).toEqual(["KODA CLI"])
    ...
    expect(themes).toEqual(["koda", "nord"])
    expect(icons).toEqual(["none", "emojis"])
    expect(titles).toEqual(["KODA CLI", "💭 KODA CLI"])

    dispose()
  })

  test("set() restores the default title icon when the setting is removed", () => {
    const store = kodaTuiConfig.makeStore(resolve(cfg({ title_icon: "unicode" })))
