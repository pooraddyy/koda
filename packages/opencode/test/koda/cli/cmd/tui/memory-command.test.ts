...
import type { kodaClient } from "@koda/sdk/v2"
import { memoryRow } from "@/koda/cli/cmd/tui/component/memory-status"
import { runMemoryCommand } from "@/koda/cli/cmd/tui/memory-command"
import { MemoryTuiEvents } from "@/koda/cli/cmd/tui/memory-events"
import { MemoryTuiState } from "@/koda/cli/cmd/tui/memory-state"
...
describe("memory TUI command parser", () => {
  ...
  test("status opens overview dialog", async () => {
    const shown: string[] = []
    const opened: string[] = []
    const client = { memory: {} } as unknown as kodaClient
  ...
  test("inspect reveals the memory folder", async () => {
    ...
    const client = {
      memory: {
        status: async () => ({ data: { root: "/tmp/koda-memory", state: { enabled: true } } }),
      },
    } as unknown as kodaClient
    ...
    expect(opened).toEqual(["/tmp/koda-memory"])
    expect(shown).toEqual(["Memory folder: /tmp/koda-memory"])
  })
  ...
  test("on and off call enable and disable endpoints", async () => {
    ...
    const client = {
      memory: {
        enable: async () => {
          calls.push("enable")
          return { data: { root: "/tmp/koda-data/memory/repo-abc123", index: { tokens: 42 } } }
        },
        disable: async () => {
          calls.push("disable")
