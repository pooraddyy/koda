...
import { kodaRun } from "../../../../src/koda/cli/cmd/run"
import { buildRunMessage } from "../../../../src/koda/cli/cmd/run-message"
...
describe("kodaRun", () => {
  test("prefers a configured command over an endpoint-backed built-in", async () => {
    ...
    expect(await kodaRun.resolveBuiltin(sdk as never, "compact", "/tmp/project")).toBeUndefined()
  })
  ...
  test("resolves an endpoint-backed built-in when no configured command matches", async () => {
    ...
    expect(await kodaRun.resolveBuiltin(sdk as never, "compact", "/tmp/project")).toBe("compact")
  })
...
describe("buildRunMessage", () => {
  ...
  test("passes args['--'] through verbatim without wrap-quote (#9622)", () => {
    // `koda run -- "- Who are you?"` - yargs+populate-- captures the leading-dash
