...
import { kodaRunTerminal } from "@/koda/cli/cmd/run-terminal"
...
test("routes direct interactive terminal requests through the session workspace", async () => {
  ...
  const terminal = kodaRunTerminal.create(sdk, () => "ses_terminal")
...
test("retries workspace lookup after a failed request", async () => {
  ...
  const sdk = createkodaClient({ baseUrl: "http://test", fetch })
  const terminal = kodaRunTerminal.create(sdk, () => "ses_terminal")
