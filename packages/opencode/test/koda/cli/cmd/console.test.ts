...
import { getNetworkIPs, serverUrls } from "../../../../src/koda/cli/server-urls"
...
describe("console daemon startup", () => {
  test("detects every explicit network option form", () => {
    ...
    expect(explicitNetworkOptions(["koda", "console", "--", "--port=4321"])).toStrictEqual([])
  })
  ...
  test("rejects legacy fixed-password daemon state", () => {
    expect(Daemon.matches({ ...state(), password: "koda" }, opts(), [])).toBe(false)
  })
