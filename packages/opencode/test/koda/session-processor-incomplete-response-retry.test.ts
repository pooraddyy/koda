...
function empty(vercelID?: string) {
  ...
  const providerMetadata = vercelID ? { koda: { vercelID } } : undefined
  return [
...
const env = (event = false) =>
  ...
  ]).pipe(Layer.provideMerge(Layer.mergeAll(NodeFileSystem.layer, Bus.layer, SyncEvent.defaultLayer, reference)))
...
describe("session processor incomplete response retry", () => {
    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          expect(error.data.message).toBe(kodaSessionProcessor.INCOMPLETE_RESPONSE_MESSAGE)
    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          yield* ctx.test.reply(
            LLMEvent.stepStart({ index: 0 }),
            LLMEvent.toolCall({ id: "call", name: "web_search", input: { query: "koda" }, providerExecuted: true }),
            LLMEvent.toolResult({
              id: "call",
              name: "web_search",
  ...
  it.effect("keeps provider retries independent after an empty response", () =>
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          process.env.koda_SESSION_RETRY_LIMIT = "2"
          const ctx = yield* setup(dir)
          yield* ctx.test.reply(...empty())
          ...
          try {
            ...
            delay.mockRestore()
            delete process.env.koda_SESSION_RETRY_LIMIT
          }
  ...
  it.effect("does not retry a provider error after final output", () =>
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          process.env.koda_SESSION_RETRY_LIMIT = "1"
          const ctx = yield* setup(dir)
          yield* ctx.test.push(
            Stream.make(
              LLMEvent.stepStart({ index: 0 }),
      ...
      (dir) =>
        Effect.gen(function* () {
          ...
          const rows = yield* database.db
            ...
            .from(EventTable)
            .where(eq(EventTable.aggregate_id, ctx.msg.sessionID))
            .all()
          const retried = rows.filter((row) => row.type.startsWith("session.next.retried"))
          expect(retried).toHaveLength(1)
          expect(retried[0]?.data).toMatchObject({
            attempt: 1,
            error: { message: expect.any(String), isRetryable: true },
          })
        }),
      { git: true },
    ),
