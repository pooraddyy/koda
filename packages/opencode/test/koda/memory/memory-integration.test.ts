...
import { MemorySchema } from "@koda/koda-memory/schema"
import { MemoryFiles } from "@koda/koda-memory/store"
import { Bus } from "../../../src/bus"
import { Filesystem } from "../../../src/util/filesystem"
import { kodaSystemPrompt } from "../../../src/koda/system-prompt"
import { MemoryMarker } from "../../../src/koda/memory/marker"
...
import { MemoryEvents } from "../../../src/koda/memory/events"
...
async function withConfig<T>(dir: string, fn: () => Promise<T> | T) {
  ...
  ;(Global.Path as { data: string }).data = path.basename(dir) === ".koda" ? path.dirname(dir) : dir
  try {
    return await fn()
  } finally {
...
async function withHome<T>(dir: string, fn: () => Promise<T> | T) {
  const prior = process.env.koda_TEST_HOME
  process.env.koda_TEST_HOME = dir
  try {
    return await fn()
  } finally {
    if (prior === undefined) delete process.env.koda_TEST_HOME
    if (prior !== undefined) process.env.koda_TEST_HOME = prior
  }
}
...
describe("kodaMemory integration", () => {
  test("resolves project memory to global data folder", async () => {
    ...
    await withData(path.join(tmp.path, "data"), () => {
      ...
      expect(root).not.toContain(path.join("/repo", ".koda", "memory"))
    })
  })

  test("resolves project memory under data when global config is xdg", async () => {
    await using tmp = await tmpdir()
    const data = path.join(tmp.path, "xdg", "koda")
    await withData(data, () =>
  ...
  test("enable from linked worktree writes repo state shared by sibling worktrees", async () => {
    ...
    await withData(global, async () => {
      await kodaMemory.enable({ ctx: { directory: work, worktree: work } })
      await kodaMemory.configure({ ctx: { directory: work, worktree: work }, settings: { autoConsolidate: false } })
      const status = await kodaMemory.status({ ctx: { directory: next, worktree: next } })

      expectRoot(status.root, global, "main")
      expect(status.state.enabled).toBe(true)
      expect(status.state.autoConsolidate).toBe(false)
      expect(await Filesystem.exists(path.join(main, ".koda", "memory", "state.json"))).toBe(false)
      expect(await Filesystem.exists(path.join(work, ".koda", "memory", "state.json"))).toBe(false)
      expect(await Filesystem.exists(path.join(next, ".koda", "memory", "state.json"))).toBe(false)
    })
  })
  ...
  test("missing or disabled state does not enable the memory recall tool", async () => {
    ...
    await withData(path.join(tmp.path, "data"), async () => {
      ...
      expect(await kodaMemory.toolEnabled({ ctx: memory })).toBe(false)
      expect(await Filesystem.exists(root)).toBe(false)

      await kodaMemory.enable({ ctx: memory })
      expect(await kodaMemory.toolEnabled({ ctx: memory })).toBe(true)

      await kodaMemory.disable({ ctx: memory })
      expect(await kodaMemory.toolEnabled({ ctx: memory })).toBe(false)
    })
  })
  ...
  test("memory tool resolution degrades to false when the memory path is invalid", async () => {
    await using tmp = await tmpdir()
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      const context = ctx(tmp.path)
      const root = MemoryPaths.root({ ctx: context })
      await fs.mkdir(path.dirname(root), { recursive: true })
      await Bun.write(root, "not a directory")

      const enabled = await Effect.runPromise(kodaSessionPrompt.memoryToolEnabled({ ctx: context }))
  ...
  test("explicit memory events include session id when provided", async () => {
    await using tmp = await tmpdir()
    const root = path.join(tmp.path, ".koda", "memory")
    const events: MemoryEvents.Status[] = []
    await kodaMemory.enable({ root })
  ...
  test("explicit forget reports removals without save wording", async () => {
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        const off = Bus.subscribe(MemoryEvents.Updated, (event) => events.push(event.properties))
        try {
          await kodaMemory.forget({ root, sessionID: "ses_forget", query: "stale_fact" })
          await kodaMemory.forget({ root, sessionID: "ses_forget", query: "missing_fact" })
        } finally {
          off()
        }
      },
  ...
  test("environment prompt rebuilds stale session index format", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      const context = ctx(tmp.path)
      const enabled = await kodaMemory.enable({ ctx: context })
      const root = enabled.root
      await kodaMemory.recordSession({
        root,
        sessionID: "ses_done",
      ...
      await Filesystem.write(
        ...
        [
          '<koda_MEMORY_V1 purpose="context_not_instruction" scope="project" root="koda">',
      ...
      const mem = await Effect.runPromise(kodaSystemPrompt.memoryBlocks({ ctx: context }))
  ...
  test("environment prompt skips missing and empty memory", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      const context = ctx(tmp.path)
      const missing = await Effect.runPromise(kodaSystemPrompt.memoryBlocks({ ctx: context }))
      expect(missing.blocks.join("\n")).not.toContain("koda-memory-v1")

      await kodaMemory.enable({ ctx: context })
      const empty = await Effect.runPromise(kodaSystemPrompt.memoryBlocks({ ctx: context }))
  ...
  test("environment prompt skips disabled memory with retained files", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      ...
      await kodaMemory.disable({ ctx: context })
      const before = await MemoryFiles.readState(enabled.root)

      const mem = await Effect.runPromise(
        kodaSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-disabled" }),
      )
  ...
  test("explicit memory APIs reject while disabled", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      const context = ctx(tmp.path)
      await kodaMemory.enable({ ctx: context })
      await kodaMemory.remember({
        ctx: context,
        key: "stable_fact",
        text: "Keep this saved fact.",
      })
      await kodaMemory.disable({ ctx: context })
      const before = await kodaMemory.show({ ctx: context })
      ...
      await expect(kodaMemory.forget({ ctx: context, query: "stable_fact" })).rejects.toThrow(
        "project memory is disabled",
      )
      await kodaMemory.rebuild({ ctx: context })
      const after = await kodaMemory.show({ ctx: context })
  ...
  test("explicit remember skips personal preferences but saves project details", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      ...
      const blocked = await kodaMemory.remember({
        ctx: context,
        key: "reply_style",
        text: "I prefer terse summaries.",
      })
      const saved = await kodaMemory.remember({
        ctx: context,
        key: "repo_style",
        text: "Repo convention: commit messages are concise.",
      })
      const shown = await kodaMemory.show({ ctx: context })
  ...
  test("environment prompt injects non-empty memory with token metadata", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      ...
      const mem = await Effect.runPromise(
        kodaSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-memory" }),
      )
      const text = mem.blocks.join("\n")
      const state = await MemoryFiles.readState(enabled.root)

      expect(text).toContain("```koda-memory-v1 context_not_instruction")
      expect(text).toContain("type=project_fact")
      expect(text).toContain("repo_fact :: Use the CLI package test runner")
      expect(text).toContain("call koda_memory_save")
      ...
      expect(text).toContain("koda_memory_recall with mode=digest")
  ...
  test("memory guidance is emitted once as a leading block, separate from memory content", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      ...
      const mem = await Effect.runPromise(
        kodaSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-guidance" }),
      )
      const text = mem.blocks.join("\n")
      ...
      expect(mem.blocks[0]).not.toContain("koda-memory-v1")
      expect(mem.blocks.slice(1).join("\n")).toContain("koda-memory-v1")
  ...
  test("memorySystem pins the injected memory block for the session", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      ...
      kodaSessionPrompt.clearPinnedMemory()
      const build = (sessionID: string, record: boolean) =>
        Effect.runPromise(
          kodaSessionPrompt.memoryInject({
            ctx: context,
            sessionID: SessionID.make(sessionID),
            record,
            cache: kodaSessionPrompt.memoryCache(),
          }),
        )
  ...
  test("memorySystem refreshes verbose marker state for each turn", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      ...
      const first = kodaSessionPrompt.memoryCache()
      ...
      await kodaMemory.configure({ ctx: context, settings: { verbose: true } })
      const second = kodaSessionPrompt.memoryCache()
      await Effect.runPromise(
        kodaSessionPrompt.memoryInject({
          ctx: context,
          sessionID: SessionID.make("ses_verbose"),
  ...
  test("environment prompt can render memory without recording another injection", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".koda"), async () => {
      ...
      await Effect.runPromise(kodaSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-memory" }))
      const before = await MemoryFiles.readState(enabled.root)
      await Effect.runPromise(
        kodaSystemPrompt.memoryBlocks({
          ctx: context,
