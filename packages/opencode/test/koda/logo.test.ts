...
import { plain, session, supports, tui } from "../../src/koda/cli/logo"
...
describe("koda logo", () => {
  ...
  test("allows an override", () => {
    expect(supports({ koda_UNICODE_LOGO: "1", SSH_TTY: "/dev/pts/0" }, "linux")).toBe(true)
    expect(supports({ koda_UNICODE_LOGO: "0" }, "linux")).toBe(false)
  })

  test("uses modern and fallback logo variants", () => {
    expect(tui({ koda_UNICODE_LOGO: "1" }, "linux").join("\n")).toContain("🬺🬏")
