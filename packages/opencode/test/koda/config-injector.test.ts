...
import { kodaConfigInjector } from "../../src/koda/config-injector"
import { tmpdir } from "../fixture/fixture"
import path from "path"
import fs from "fs/promises"

describe("kodaConfigInjector", () => {
  describe("buildConfig", () => {
    test("returns empty config when no modes exist", async () => {
      await using tmp = await tmpdir()

      const result = await kodaConfigInjector.buildConfig({ projectDir: tmp.path, skipGlobalPaths: true })
    ...
    test("includes rules in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Bun.write(path.join(dir, ".koda", "rules", "main.md"), "# Rules")
        },
      })

      const result = await kodaConfigInjector.buildConfig({
        projectDir: tmp.path,
  ...
  - slug: translate
    ...
    test("includes ignore patterns in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".kodaignore"), "secrets/\n*.env")
        },
      })
  ...
  - slug: translate
    ...
    test("handles negation patterns in ignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".kodaignore"), "*.env\n!.env.example")
        },
      })
  ...
  describe("getEnvVars", () => {
    test("returns empty object for empty config", () => {
      const envVars = kodaConfigInjector.getEnvVars("{}")
    ...
    test("returns koda_CONFIG_CONTENT for non-empty config", () => {
      const config = JSON.stringify({ agent: { test: {} } })
      const envVars = kodaConfigInjector.getEnvVars(config)

      expect(envVars).toEqual({
        koda_CONFIG_CONTENT: config,
      })
    })
  })
})
