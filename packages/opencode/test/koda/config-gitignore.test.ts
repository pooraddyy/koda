...
//
// koda uses Npm.Service (arborist) for dependency installation and may write
// a .gitignore inside the .koda config dir. Users may have pnpm or yarn as
// their system package manager, which can produce lockfiles in the .koda/
...
test(".gitignore in .koda config dir includes pnpm and yarn lockfile patterns", async () => {
  await using tmp = await tmpdir()
  const dir = path.join(tmp.path, "a")
  const koda = path.join(dir, ".koda")
  await fs.mkdir(koda, { recursive: true })
  ...
  const ignore = await Filesystem.readText(path.join(koda, ".gitignore"))
