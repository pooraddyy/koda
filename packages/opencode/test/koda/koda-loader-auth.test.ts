...
// Tests that unauthenticated koda models are assembled with paid models and autoloaded anonymously.
...
import { kodaCustomLoaders, patchkodaProviderPrivacy } from "../../src/koda/provider/provider"
...
it.live("assembles paid koda models without auth", () =>
  Effect.gen(function* () {
    const providers = yield* ModelsDev.Service.use((models) => models.get()).pipe(
      Effect.provide(layer()),
      provideInstance(process.cwd()),
    )
    const koda = Provider.fromModelsDevProvider(providers.koda)

    expect(koda.models["paid-model"]).toMatchObject({
      id: "paid-model",
      providerID: "koda",
      cost: { input: 1, output: 2 },
      isFree: false,
...
it.live("does not infer free status from zero catalog prices", () =>
  Effect.gen(function* () {
    ...
    expect(koda.models["free-model"].isFree).toBeUndefined()
  }),
)
...
it.effect("enables a paid catalog when config apiKey is present", () =>
  Effect.gen(function* () {
    const result = yield* load({ config: { provider: { koda: { options: { apiKey: "test-key" } } } } })
    expect(result.autoload).toBe(true)
...
it.effect("keeps data collection denied after configured options are applied", () =>
  Effect.sync(() => {
    const provider = { options: { dataCollection: "allow", baseURL: "https://api.koda.ai" } }
    patchkodaProviderPrivacy(provider, { hide_prompt_training_models: true })
    expect(provider.options).toEqual({ dataCollection: "deny", baseURL: "https://api.koda.ai" })
  }),
)
