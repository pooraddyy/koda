...
import { kodaCompactionChunks } from "../../src/koda/session/compaction-chunks"
import { kodaSessionCompaction } from "../../src/koda/session/compaction"
...
const dbfile = path.join(os.tmpdir(), `koda-compaction-chunks-${process.pid}-${crypto.randomUUID()}.db`)
...
describe("kodaCompactionChunks", () => {
  test("splits oversized history into chronological chunks", async () => {
    ...
    const chunks = await Effect.runPromise(kodaCompactionChunks.split({ messages, model, size: 2_000 }))
  ...
  test("uses runtime output cap for fallback selection and chunk budget", () => {
    ...
    expect(kodaCompactionChunks.needed({ cfg, model, tokens: 5_000, outputTokenMax })).toBe(false)
    expect(kodaCompactionChunks.budget({ cfg, model, outputTokenMax })).toBe(5_692)
  })
