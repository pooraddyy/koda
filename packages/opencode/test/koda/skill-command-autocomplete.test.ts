...
describe("skill slash commands", () => {
  it.live("registers the checkpointed long-run command", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const command = yield* Command.Service
          const longRun = yield* command.get("long-run")

          expect(longRun?.source).toBe("command")
          expect(longRun?.description).toContain("checkpointed")
          expect(longRun?.hints).toEqual(["$ARGUMENTS"])
          expect(yield* Effect.promise(async () => longRun?.template)).toContain("durable checkpoint")
        }),
      { git: true },
    ),
  )
      ...
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".koda", "skill", "review", "SKILL.md"),
              `---
name: review
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".koda", "skill", "proj", "SKILL.md"),
              `---\nname: proj\ndescription: proj.\n---\n\nRun: !\`printf hi\`\n`,
            ),
          )
