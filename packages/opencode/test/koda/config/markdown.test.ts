  ...
  try {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        const item = path.join(project, ".koda", "agents", "unsafe.md")
    ...
    const file = await kodaMarkdown.substitute(tmp.extra.file, tmp.extra.item, {
      trusted: false,
    ...
    const env = await kodaMarkdown.substitute(tmp.extra.env, tmp.extra.item, {
      trusted: false,
      fileScope: { root: tmp.extra.project, source: tmp.extra.item },
    }).then(
      () => false,
    ...
    expect(
      await kodaMarkdown.substitute("{file:../../allowed.txt}", tmp.extra.item, {
        trusted: false,
        fileScope: { root: tmp.extra.project, source: tmp.extra.item },
      }),
    ).toBe("project content")

    const trusted = await kodaMarkdown.substitute(tmp.extra.text, tmp.extra.global, { trusted: true })
