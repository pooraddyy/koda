...
import { kodaConfig } from "../../../src/koda/config/config"
...
afterEach(async () => {
  delete process.env.koda_MD_TEST
  await clear()
...
describe("markdown substitutions", () => {
  test("applies file and env substitutions to parsed markdown body", async () => {
    process.env.koda_MD_TEST = "env content"
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, "body.md"), "file content")
        await Filesystem.write(
          path.join(dir, "SKILL.md"),
          ["---", "name: test", "description: Test", "---", "{file:body.md}", "{env:koda_MD_TEST}"].join("\n"),
        )
      },
    })
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const config = await Bun.file(path.join(globalTmp.path, "koda.jsonc")).json()
...
describe("project MCP trust boundaries", () => {
  ...
  test("drops env-backed project MCP headers without dropping static siblings", async () => {
    ...
    await using tmp = await tmpdir({ git: true })
    const prev = Global.Path.config
    const secret = process.env.koda_PROJECT_MCP_SECRET
    ;(Global.Path as { config: string }).config = globalTmp.path
    process.env.koda_PROJECT_MCP_SECRET = "process-secret"
    await clear()
    await disposeAllInstances()
    ...
    try {
      await writeConfig(tmp.path, {
        mcp: {
          unsafe: {
            type: "remote",
            url: "https://project.example.com/unsafe",
            headers: { Authorization: "Bearer {env:koda_PROJECT_MCP_SECRET}" },
          },
          sibling: {
            type: "remote",
      ...
      if (secret === undefined) delete process.env.koda_PROJECT_MCP_SECRET
      else process.env.koda_PROJECT_MCP_SECRET = secret
      ;(Global.Path as { config: string }).config = prev
      await clear()
      await disposeAllInstances()
    }
  ...
  test("does not carry global credentials through remote-local-remote project layers", async () => {
    ...
    try {
      ...
      await writeConfig(path.join(tmp.path, ".koda"), {
...
describe("koda web search config", () => {
...
describe("koda indexing config", () => {
  ...
  test("updates a project JSON config containing retired experimental flags", async () => {
    await using tmp = await tmpdir({ git: true })
    const file = path.join(tmp.path, ".koda", "koda.json")
    await Filesystem.write(
      file,
      JSON.stringify({
        username: "keep",
  ...
  test("updates a global JSONC config containing retired experimental flags", async () => {
    await using globalTmp = await tmpdir()
    await using tmp = await tmpdir()
    const file = path.join(globalTmp.path, "koda.jsonc")
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
    ...
    try {
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const config = await load()
          const input = kodaIndexing.input(config.indexing, global.indexing)
  ...
  test("project indexing enabled overrides global enablement", async () => {
    const input = kodaIndexing.input({ enabled: false }, { enabled: true })
    expect(input.enabled).toBe(false)
    expect(kodaIndexing.input(undefined, { enabled: true }).enabled).toBe(true)
    expect(kodaIndexing.input({ enabled: true }, { enabled: false }).enabled).toBe(true)
  })

  test("creates missing project config as .koda/koda.jsonc", async () => {
    await using tmp = await tmpdir({ git: true })
    ...
    expect(await Bun.file(path.join(tmp.path, ".koda", "koda.jsonc")).exists()).toBe(true)
    expect(await Bun.file(path.join(tmp.path, ".koda", "koda.json")).exists()).toBe(false)
  })

  test("accepts delete sentinels for indexing model overrides", () => {
    const patch = decode({ indexing: { model: null, dimension: null } })
    const merged = kodaConfig.mergeConfig(
      {
        indexing: {
          provider: "openai",
    ...
    const input = kodaIndexing.input(patch.indexing)
...
describe("custom provider model config", () => {
  test("persists and removes reasoning across a global config reload", async () => {
    await using globalTmp = await tmpdir()
    const file = path.join(globalTmp.path, "koda.json")
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
...
describe("unset propagation across layered config files", () => {
  ...
  test("removes subagent_model from every global config file when unset", async () => {
    await using globalTmp = await tmpdir()
    const json = path.join(globalTmp.path, "koda.json")
    const jsonc = path.join(globalTmp.path, "koda.jsonc")
    ...
    try {
      await writeConfig(globalTmp.path, { subagent_model: "koda/openai/gpt-5" }, "koda.json")
      await Filesystem.write(jsonc, jsoncText)

      await saveGlobal(decode({ subagent_model: null }))

      // The key must be gone from the lower-precedence koda.json as well, or
  ...
  test("removes nested sentinels from jsonc siblings while preserving comments", async () => {
    ...
    try {
      await Filesystem.write(path.join(globalTmp.path, "koda.jsonc"), '{ "username": "marius" }\n')
      await Filesystem.write(
        opencode,
        [
          "{",
          ...
          '  "agent": {',
          '    "explore": {',
          '      "model": "koda/anthropic/claude-sonnet-4-6",',
          '      "description": "Keep me"',
          "    }",
          "  }",
          "}",
  ...
  test("does not rewrite sibling files on set or when the key is absent", async () => {
    await using globalTmp = await tmpdir()
    const json = path.join(globalTmp.path, "koda.json")
    const jsonText = JSON.stringify({ subagent_model: "koda/old-model", username: "marius" }, null, 2)
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
    ...
    try {
      ...
      await saveGlobal(decode({ subagent_model: "koda/new-model" }))
  ...
  test("removes subagent_model from every project config file when unset", async () => {
    await using tmp = await tmpdir({ git: true })
    await Filesystem.write(
      path.join(tmp.path, ".koda", "koda.json"),
      JSON.stringify({ subagent_model: "koda/openai/gpt-5" }),
    )
    await Filesystem.write(path.join(tmp.path, ".koda", "koda.jsonc"), '{\n  "username": "keep"\n}\n')
    ...
    const json = JSON.parse(await Bun.file(path.join(tmp.path, ".koda", "koda.json")).text())
    expect(json).not.toHaveProperty("subagent_model")
    // The primary target only gained nothing; the delete was a no-op there.
    const jsonc = JSON.parse(await Bun.file(path.join(tmp.path, ".koda", "koda.jsonc")).text())
...
describe("project plugin dependencies", () => {
  ...
  test("does not install dependencies for an ordinary project config directory", async () => {
    await sandbox(async (dir) => {
      await writeConfig(path.join(dir, ".koda"), { username: "koda" })
  ...
  test("installs dependencies for an auto-discovered file plugin and waits for completion", async () => {
    await sandbox(async (dir) => {
      const config = path.join(dir, ".koda")
      ...
      expect(calls).toEqual([{ dir: config, name: "@koda/plugin" }])
    })
  })
  ...
  test("does not install dependencies for built-in or package plugins", async () => {
    await sandbox(async (dir) => {
      await writeConfig(path.join(dir, ".koda"), {
        plugin: ["@koda/koda-indexing", "opencode-gitlab-auth"],
      })
      const calls: string[] = []
      const npm = Layer.mock(Npm.Service)({
...
describe("agent config", () => {
  ...
  test("removes an agent variant override without removing its model", () => {
    ...
    expect(patch.agent?.explore?.variant).toBeNull()
    expect(merged.agent?.explore).toEqual({ model: "koda/anthropic/claude-sonnet-4-6" })
  })
...
describe("project config directory precedence", () => {
  test("prefers .koda over legacy .kilo and ignores .opencode", async () => {
    await using tmp = await tmpdir()
    const entries = [
      {
        root: ".opencode",
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(config.username).toBe("koda")
        expect(config.model).toBe("test/koda")
        expect(config.small_model).toBeUndefined()

        expect(config.command?.shared).toMatchObject({
          description: "koda command",
          template: "koda command template",
        })
        ...
        expect(config.agent?.shared).toMatchObject({
          description: "koda agent",
          prompt: "koda agent prompt",
        })
        ...
        expect(plugins).toContain("koda.ts")
        expect(plugins).toContain("legacy.ts")
...
describe("linked worktree config", () => {
  test("uses primary config directories as local fallbacks", async () => {
    ...
    await Bun.write(path.join(primary.path, "koda.json"), JSON.stringify({ model: "test/primary" }))
    await $`git add koda.json`.cwd(primary.path).quiet()
    ...
    try {
      await Bun.write(path.join(worktree, "koda.json"), JSON.stringify({ model: "test/worktree" }))
      await Bun.write(
        path.join(primary.path, ".koda", "koda.jsonc"),
        JSON.stringify({ username: "primary-dir", indexing: { enabled: true } }),
      )
      await Bun.write(path.join(worktree, ".koda", "koda.jsonc"), JSON.stringify({ username: "worktree-dir" }))
  ...
  test("uses nested primary config directories as local fallbacks", async () => {
    ...
    try {
      ...
      await Bun.write(
        path.join(primary.path, "packages", ".opencode", "koda.jsonc"),
        JSON.stringify({ snapshot: true, autoupdate: false, share: "auto", default_agent: "opencode-only" }),
      )
      await Bun.write(
        path.join(primary.path, "packages", ".koda", "koda.jsonc"),
        JSON.stringify({ snapshot: false, autoupdate: "notify", share: "disabled" }),
      )
      await Bun.write(path.join(primary.path, "packages", ".kilo", "koda.jsonc"), JSON.stringify({ snapshot: true }))
      await Bun.write(path.join(directory, ".koda", "koda.jsonc"), JSON.stringify({ share: "manual" }))
  ...
  test("keeps koda_CONFIG_DIR above the primary fallback", async () => {
    await using primary = await tmpdir({ git: true })
    ...
    await Bun.write(path.join(primary.path, ".koda", "koda.jsonc"), JSON.stringify({ username: "primary-dir" }))
    await Bun.write(path.join(explicit.path, "koda.jsonc"), JSON.stringify({ username: "explicit-dir" }))
    const previous = process.env["koda_CONFIG_DIR"]
    process.env["koda_CONFIG_DIR"] = explicit.path

    try {
      const config = await provideTestInstance({ directory: worktree, fn: load })
      expect(config.username).toBe("explicit-dir")
    } finally {
      if (previous === undefined) delete process.env["koda_CONFIG_DIR"]
      else process.env["koda_CONFIG_DIR"] = previous
      await $`git worktree remove --force ${worktree}`.cwd(primary.path).quiet().nothrow()
    }
  })
})
...
describe("opencode config migration notice", () => {
  ...
  test("detects a project .opencode directory", async () => {
    ...
    await withGlobalConfig(path.join(globalTmp.path, "koda"), () => {
      const found = kodaConfig.detectOpencodeConfig({ directory: tmp.path, scanProject: true })
  ...
  test("skips the project scan when disabled", async () => {
    ...
    await withGlobalConfig(path.join(globalTmp.path, "koda"), () => {
      const found = kodaConfig.detectOpencodeConfig({ directory: tmp.path, scanProject: false })
  ...
  test("builds a dismissible notification when opencode config exists", async () => {
    ...
    await withGlobalConfig(path.join(globalTmp.path, "koda"), () => {
      const notice = kodaConfig.opencodeConfigNotification({ directory: tmp.path, scanProject: true })
      expect(notice?.id).toBe(kodaConfig.OPENCODE_NOTIFICATION_ID)
      expect(notice?.message).toContain(path.join(tmp.path, ".opencode"))
      expect(notice?.action?.actionURL).toBe(kodaConfig.CONFIG_DOCS_URL)
...
describe("bash permission migration", () => {
  ...
  "permission": "${action}"
      ...
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Filesystem.write(path.join(dir, "koda.jsonc"), input)
        },
      })
      ...
      try {
        await kodaConfig.migrateBashPermission()

        const file = path.join(tmp.path, "koda.jsonc")
    ...
    test(`preserves string-form ${action} permission in json`, async () => {
      ...
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Filesystem.write(path.join(dir, "koda.json"), input)
        },
      })
      ...
      try {
        ...
        const file = path.join(tmp.path, "koda.json")
        const text = await Filesystem.readText(file)
        const parsed = ConfigParse.schema(Config.Info, ConfigParse.jsonc(text, file), file)
        expect(text).toBe(input)
        expect(parsed.permission?.["*"]).toBe(action)
