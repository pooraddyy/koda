...
import { kodaDefaultPlugins } from "../../../src/koda/config/default-plugins"
import { INDEXING_PLUGIN } from "../../../src/koda/indexing-feature"
...
describe("koda default indexing plugin", () => {
  afterEach(async () => {
  ...
  test("injects indexing without registering an external plugin origin", () => {
    ...
    kodaDefaultPlugins.apply(config, { disabled: false })
  ...
  test("removes a persisted indexing marker from external plugin origins", () => {
    ...
    kodaDefaultPlugins.apply(config, { disabled: true })
  ...
  test("does not hard-enable indexing plugin when default plugins are disabled", async () => {
    const original = Flag.koda_DISABLE_DEFAULT_PLUGINS
    ...
    try {
      ...
      Flag.koda_DISABLE_DEFAULT_PLUGINS = original
    }
  })
})
