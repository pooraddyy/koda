...
import { hasAtomicChatPlugin } from "@/koda/atomic-chat-feature"
import { kodaDefaultPlugins } from "@/koda/config/default-plugins"
import { PluginLoader } from "@/plugin/loader"

const atomic = "@koda/plugin-atomic-chat"

describe("koda default atomic chat plugin", () => {
  test("injects atomic chat without registering an external plugin origin", () => {
    ...
    kodaDefaultPlugins.apply(cfg, { disabled: false })
  ...
  test("does not add atomic chat plugin when default plugins are disabled", () => {
    ...
    kodaDefaultPlugins.apply(cfg, { disabled: true })
