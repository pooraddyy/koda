...
import { ConfigVariableGuard } from "@/koda/config/variable"
...
test("rejects untrusted file references that escape the scope root", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "koda-config-variable-root-"))
  const outside = await fs.mkdtemp(path.join(os.tmpdir(), "koda-config-variable-outside-"))
...
test("allows untrusted file references that stay inside the scope root", async () => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "koda-config-variable-inside-")))
  ...
  try {
    expect(
      await ConfigVariable.substitute({
        ...
        dir: root,
        text: "{file:value}",
        fileScope: { root, source: path.join(root, "koda.json") },
      }),
    ).toBe("allowed")
  } finally {
    await fs.rm(root, { recursive: true, force: true })
...
test("allows untrusted absolute file references that resolve inside the scope root", async () => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "koda-config-variable-abs-inside-")))
  const file = path.join(root, "value")
  await fs.writeFile(file, "allowed")
  try {
...
test("rejects server credential environment substitutions", async () => {
  await expect(
    ConfigVariable.substitute({
      ...trusted,
      text: "password={env:koda_SERVER_PASSWORD}",
      env: { koda_SERVER_PASSWORD: "secret" },
    }),
  ).rejects.toBeInstanceOf(InvalidError)
})
...
test("reads ordinary file substitutions on every platform when trusted", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "koda-config-variable-file-"))
...
test.skipIf(process.platform !== "linux")("does not substitute an environment file through a symlink", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "koda-config-variable-"))
...
test("scope-blocked file reference still rejects under missing:empty", async () => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "koda-config-variable-empty-root-")))
  const outside = await fs.mkdtemp(path.join(os.tmpdir(), "koda-config-variable-empty-out-"))
  const file = path.join(outside, "secret")
  await fs.writeFile(file, "top-secret")
  try {
    await expect(
...
test("missing (non-blocked) file reference is emptied under missing:empty", async () => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "koda-config-variable-missing-")))
  try {
...
test("guard read rejects an out-of-scope file with a BlockedError", async () => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "koda-guard-root-")))
  const outside = await fs.mkdtemp(path.join(os.tmpdir(), "koda-guard-out-"))
  const file = path.join(outside, "secret")
  await fs.writeFile(file, "top-secret")
  try {
    const err = await ConfigVariableGuard.read(file, { root, source: "koda.json", token: "{file:...}" }).then(
      () => undefined,
      (e) => e,
    )
