...
it.live("nvidia provider includes koda billing origin header", () =>
    ...
    withNvidiaKey(
      Provider.Service.use((provider) =>
        Effect.gen(function* () {
          ...
          expect(headers["HTTP-Referer"]).toBe("https://koda.ai/")
          expect(headers["X-Title"]).toBe("koda Code")
          expect(headers["X-BILLING-INVOKE-ORIGIN"]).toBe("koda")
        }),
      ),
    ),
  ),
