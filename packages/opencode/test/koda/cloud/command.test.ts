...
import type { AgentSendRequest, AgentStartRequest, MessageResult } from "@/koda/cloud/contracts"
import { CloudCommands } from "@/koda/cloud/commands"
import { CloudError } from "@/koda/cloud/errors"
import { Git } from "@/git"
...
it.instance(
  "assembles the default start request from koda state and the current repository",
  () =>
    Effect.acquireUseRelease(
      ...
      (server) =>
        Effect.gen(function* () {
          ...
          yield* run(test.directory, "remote", "add", "origin", "git@github.com:koda-Org/koda.git")
          ...
          const response = {
            ...
            kodaSessionId: "ses_command_test",
            messageId: MESSAGE,
          ...
          const result = yield* CloudCommands.start(
            ...
            {
              env: { koda_API_URL: server.url.origin },
              make: (options) => {
          ...
          expect(requests[0]).toEqual({
            ...
            repository: { type: "github", repo: "koda-Org/koda" },
            options: {
              createdOnPlatform: "koda-cli",
              kodaOrganizationId: ORG,
            },
          })
    ...
    ),
  {
    ...
    config: {
      default_agent: "plan",
      agent: { plan: { model: "koda/anthropic/command-model" } },
    },
  },
)
...
it.instance("sends follow-ups, prints status without assistant content, and applies the result exit code", () =>
  Effect.gen(function* () {
    ...
    const deps = {
      env: { koda_ORG_ID: "not-relevant-to-existing-sessions" },
      make: () => ({
        async start() {
