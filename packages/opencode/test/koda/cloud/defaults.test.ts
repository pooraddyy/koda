...
import { CloudAuth } from "@/koda/cloud/auth"
import { CloudCatalog } from "@/koda/cloud/catalog"
import { CloudDefaults } from "@/koda/cloud/defaults"
import { MAX_CLOUD_AGENT_RESPONSE_BYTES } from "@/koda/cloud/response-json"
...
const authLayer = (info: Auth.Info) =>
  Layer.mock(Auth.Service)({
    get: (id) => Effect.succeed(id === "koda" ? info : undefined),
  })
      ...
      Bun.serve({
        ...
        fetch(request) {
          ...
          requests.push({
            ...
            feature: request.headers.get("x-koda-feature"),
            organization: request.headers.get("x-koda-organizationid"),
            path: url.pathname,
          })
...
it.instance(
    ...
    withCatalog(["anthropic/explicit", "anthropic/default"], "anthropic/default", (url, requests) =>
      Effect.gen(function* () {
        ...
        const auth = Layer.mock(Auth.Service)({
          get: (id) => (id === "koda" ? Ref.get(current) : Effect.succeed(undefined)),
          set: (id, info) => (id === "koda" ? Ref.set(current, info) : Effect.void),
        })

        const resolved = yield* Effect.gen(function* () {
          const result = yield* CloudDefaults.resolve({
            mode: "debug",
            model: "koda/anthropic/explicit",
            orgID: explicitID,
            env: {
              koda_API_KEY: "ignored-env-token",
              koda_ORG_ID: envID,
            },
          })
          const service = yield* Auth.Service
          expect(yield* service.get("koda")).toEqual(stored)
          return result
        }).pipe(
          Effect.provide(
            Layer.mergeAll(
              ...
              stateLayer(
                state({
                  model: { debug: { providerID: "koda", modelID: "anthropic/saved" } },
                }),
              ),
              CloudCatalog.layer({ env: { koda_API_URL: url.origin } }),
            ),
          ),
        )
    ...
    ),
  {
    config: {
      default_agent: "plan",
      model: "koda/anthropic/repository",
      agent: { plan: { model: "koda/anthropic/mode" } },
    },
  },
)
        ...
        Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.mergeAll(
              ...
              stateLayer(
                state({
                  model: { code: { providerID: "koda", modelID: "anthropic/stale" } },
                  recent: [{ providerID: "koda", modelID: "anthropic/recent" }],
                }),
              ),
              CloudCatalog.layer({ env: { koda_API_URL: url.origin } }),
            ),
      ...
      CloudDefaults.resolve().pipe(
        ...
        Effect.provide(
          Layer.mergeAll(
            ...
            stateLayer(
              state({
                model: { code: { providerID: "koda", modelID: "anthropic/saved" } },
              }),
            ),
            CloudCatalog.layer({ env: { koda_API_URL: url.origin } }),
          ),
...
it.instance(
    ...
    ),
  {
    config: {
      ...
      agent: {
        code: { model: "koda/anthropic/code" },
        custom: { mode: "primary", model: "koda/anthropic/custom" },
      },
    },
  },
)
...
it.instance("rejects invalid persisted organization state and insecure catalog origins", () =>
  Effect.gen(function* () {
    ...
    const catalog = yield* CloudDefaults.resolve({ env: { koda_API_URL: "http://example.com" } }).pipe(
      Effect.provide(
        Layer.mergeAll(
          authLayer(new Auth.Api({ type: "api", key: "stored-api-token" })),
          stateLayer(state()),
          CloudCatalog.layer({
            env: { koda_API_URL: "http://example.com" },
    ...
    withCatalog(
      ...
      (url, requests) =>
        Effect.gen(function* () {
          ...
          const resolved = yield* CloudDefaults.resolve({
            env: { koda_API_KEY: "ignored-env-token" },
          }).pipe(
            Effect.provide(
              Layer.mergeAll(
                ...
                stateLayer(
                  state({
                    model: { plan: { providerID: "koda", modelID: "anthropic/saved" } },
                    recent: [{ providerID: "koda", modelID: "anthropic/recent" }],
                  }),
                ),
                CloudCatalog.layer({ env: { koda_API_URL: url.origin } }),
