...
import {
  ...
  MessageIdSchema,
} from "../../../src/koda/cloud/contracts"
import { parseServiceOrigin } from "../../../src/koda/cloud/origin"
import { MAX_CLOUD_AGENT_RESPONSE_BYTES } from "../../../src/koda/cloud/response-json"
import { createCloudAgentClient } from "../../../src/koda/cloud/trpc"
  ...
  test("places bearer auth only in headers and correlates generated message identities", async () => {
    ...
    try {
      ...
      const start = await agent.start({
        ...
        options: { createdOnPlatform: "koda-cli" },
      })
