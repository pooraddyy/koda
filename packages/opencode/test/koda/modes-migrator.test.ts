...
import { ModesMigrator } from "../../src/koda/modes-migrator"
...
describe("ModesMigrator", () => {
  ...
  describe("convertPermissions", () => {
    ...
    test("converts fileRegex groups to restricted permissions", () => {
      const groups: ModesMigrator.kodaMode["groups"] = [
        "read",
    ...
    test("handles tuple without fileRegex", () => {
      const groups: ModesMigrator.kodaMode["groups"] = [["edit", {}]]
      const permissions = ModesMigrator.convertPermissions(groups)
  ...
  describe("convertMode", () => {
    test("converts full mode to agent config", () => {
      const mode: ModesMigrator.kodaMode = {
        slug: "translate",
        name: "Translate",
  ...
  - slug: translate
    ...
    groups:
      - read`,
          ...
          // Create project .kodamodes (should win)
          await Bun.write(
            path.join(dir, ".kodamodes"),
            `customModes:
  - slug: translate
