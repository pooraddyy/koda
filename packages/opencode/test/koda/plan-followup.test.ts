...
import { formatTodos, generateHandover, PlanFollowup, PlanFollowupRuntime } from "../../src/koda/plan-followup"
import { Instance } from "../../src/koda/instance"
import * as kodaInstance from "../../src/koda/instance"
...
async function withInstance(fn: () => Promise<void>) {
  ...
  const provide = spyOn(kodaInstance, "provide").mockImplementation((input) =>
...
describe("plan follow-up", () => {
  ...
  test("ask - retargets prompt queue so injected message is visible in scope", () =>
    withInstance(async () => {
      const { kodaSessionPromptQueue } = await import("../../src/koda/session/prompt-queue")
      ...
      kodaSessionPromptQueue.retarget(seeded.sessionID, original)
      ...
      const scoped = kodaSessionPromptQueue.scope(seeded.sessionID, all)
