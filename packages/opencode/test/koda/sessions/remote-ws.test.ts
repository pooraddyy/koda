...
describe("RemoteWS", () => {
  ...
  test("ignores callbacks from a stale WebSocket generation", async () => {
    ...
    try {
      ...
      expect(conn.connected).toBe(true)
      // koda_change - K1 W1: with the immediate heartbeat on FIRST open,
  ...
  // koda_change - K1 W1: instance advertisement flows through the
  ...
  // koda-sessions.ts (see `setInstanceAdvertisement`) still fires one
  // immediate heartbeat when the flag is flipped, which is the practical
  // point at which a user runs `koda remote` and wants the cloud picker
  ...
  // is the practical "advertise on `koda remote` command" path — the
  ...
  test("setInstanceAdvertisement triggers an immediate heartbeat (out-of-band path)", async () => {
    await withFakeWebSocket(async (clock) => {
      ...
      // out-of-band path in koda-sessions.ts that calls
