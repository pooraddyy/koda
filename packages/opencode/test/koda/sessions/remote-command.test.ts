...
import { RemoteExit } from "../../../src/koda-sessions/remote-exit"
...
describe("RemoteCommand", () => {
  ...
  test("validates structured command requests without duplicating session identity", () => {
    const valid = {
      ...
      model: { providerID: "koda", modelID: "anthropic/claude-sonnet-4" },
      variant: "high",
    }
  ...
  test("advertises only canonical exit while the embedded worker callback is registered", () => {
    ...
    // koda_change - K1 W1: canExitSession is always true, independent of
  ...
  test("catalog omits exit until the embedded worker TUI is ready", async () => {
    ...
    // koda_change - K1 W1: canExitSession is true even when the synthetic
    // `/exit` entry is absent (e.g. a headless `koda remote` host has no
