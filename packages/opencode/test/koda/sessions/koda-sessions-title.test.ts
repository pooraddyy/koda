...
import {
  ...
  markAutoTitle,
  markRenameAdopted,
} from "../../../src/koda-sessions/rename-adoptions"
...
const kodaSessions = (await import("../../../src/koda-sessions/koda-sessions")).kodaSessions

// Session must be provideMerged so yield* Session.Service and the
// kodaSessions Updated handler share one store (otherwise get() misses creates).
...
function layer(overrides: Partial<Config.Interface> = {}) {
  return AppNodeBuilder.build(
    LayerNode.group([kodaSessions.node, Session.node, SessionProjector.node]),
...
function reset(...tokens: string[]) {
  ...
  clearInFlightCache("koda-sessions:org")
...
const ENV_KEYS = [
  "koda_API_KEY",
  "koda_SESSION_INGEST_URL",
  "koda_ORG_ID",
  "koda_AGENT_NOTIFICATION_TIMEOUT_MS",
] as const
...
beforeEach(() => {
  delete process.env.koda_ORG_ID
  clearRenameMarks()
})
...
function metaItems(requests: Req[]) {
  ...
  for (const r of requests) {
    ...
    for (const item of data) {
      if (item?.type === "koda_meta") items.push(item.data)
    }
  }
  return items
}
...
const drainIngest = Effect.gen(function* () {
  ...
  yield* Effect.promise(() => kodaSessions.drainIngestForShutdown())
})

it.instance("meta org precedence: session metadata > koda_ORG_ID > auth accountId", () => {
  const requests: Req[] = []
  installFetch(mockFetch(requests))
  patchEnv({
    koda_API_KEY: "test-token",
    koda_SESSION_INGEST_URL: "https://ingest.kodasessions.ai",
    koda_ORG_ID: ORG_ENV,
  })
  reset("test-token")
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      yield* Effect.promise(() => kodaSessions.bootstrap(withMeta.id))
      requests.length = 0
      ...
      yield* Effect.promise(() => kodaSessions.bootstrap(plain.id))
      requests.length = 0
...
it.instance("meta falls through invalid metadata orgId to env", () => {
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.promise(() => kodaSessions.bootstrap(bad.id))
    requests.length = 0
...
it.instance("meta falls back to env when session row has no resolvable org", () => {
  // Precedence only: real Updated → koda_meta with a live session that has no
...
it.instance("meta falls back to env when Session.Service.get fails", () => {
  ...
  // global runtime; unknown id → get rejects → .catch(() => null) → koda_ORG_ID.
  ...
  patchEnv({ koda_ORG_ID: ORG_ENV })
  clearInFlightCache("koda-sessions:org")

  return Effect.gen(function* () {
    const result = yield* Effect.promise(() => kodaSessions._metaForTests("ses_missing_for_meta_get_fail"))
...
it.instance("title broadcast: auto-title posts generated true; custom posts generated false; adoption skips", () => {
  ...
  patchEnv({
    koda_API_KEY: "test-token",
    koda_SESSION_INGEST_URL: "https://ingest.kodasessions.ai",
    koda_AGENT_NOTIFICATION_TIMEOUT_MS: "5000",
  })
  reset("test-token")
  ...
  return Effect.gen(function* () {
    ...
    const id = created.id
    yield* Effect.promise(() => kodaSessions.bootstrap(id))
...
it.instance("reportSessionTitle goes through readiness and tolerates POST failure", () => {
  ...
  return Effect.gen(function* () {
    ...
    const sessions = yield* Session.Service
    const created = yield* sessions.create({})
    yield* Effect.promise(() => kodaSessions.bootstrap(created.id))

    const failed = yield* koda.reportSessionTitle(created.id, "X", { generated: false })
    expect(failed).toEqual({ ok: false, reason: "http_500" })
    expect(requests.some((r) => r.path.endsWith("/title"))).toBe(true)

    titleStatus = 200
    const ok = yield* koda.reportSessionTitle(created.id, "Y", { generated: true })
    expect(ok).toEqual({ ok: true })
  }).pipe(Effect.provide(layer()))
})

it.instance("reportSessionTitle reports not_connected when unauthenticated", () => {
  patchEnv({ koda_API_KEY: undefined })
  delete process.env.koda_API_KEY
  reset()

  return Effect.gen(function* () {
    const koda = yield* kodaSessions.Service
    const result = yield* koda.reportSessionTitle("ses_x", "T", { generated: false })
...
it.instance(
  ...
  () => {
    ...
    const customLayer = kodaSessions.layer.pipe(
