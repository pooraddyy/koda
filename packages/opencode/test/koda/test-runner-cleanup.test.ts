...
function env(marker: string) {
  const vars: NodeJS.ProcessEnv = { ...process.env, koda_TEST_RUNNER_PID_FILE: marker }
  delete vars.koda_TEST_PROFILE
  delete vars.koda_TEST_SHARD
  delete vars.koda_TEST_CLI_PATH
  return vars
}

function spawn(name: string, marker: string, args: string[] = []) {
  return Bun.spawn(
    ["bun", "run", "script/test-runner.ts", "--concurrency", "1", "--retries", "-1", ...args, `koda/${name}`],
    {
      cwd: root,
      env: env(marker),
      stdout: "pipe",
...
describe("test runner cleanup", () => {
  test(
    ...
    async () => {
      ...
      const state = { pid: 0 }
      const src = [
        "const marker = process.env.koda_TEST_RUNNER_PID_FILE",
        'if (!marker) throw new Error("koda_TEST_RUNNER_PID_FILE is required")',
