...
import { generateHelp, generateCommandTable } from "../../src/koda/help"
...
import { ProvidersCommand } from "../../src/cli/cmd/providers" // koda_change — upstream renamed auth → providers
...
import { HelpCommand } from "../../src/koda/help-command"
import { ProfileCommand } from "../../src/koda/cli/cmd/profile"
import { DaemonCommand } from "../../src/koda/cli/cmd/daemon"
import { CloudCommand } from "../../src/koda/cli/cmd/cloud"
...
const AttachStub = {
  command: "attach <url>",
  describe: "attach to a running koda server",
  handler() {},
}
...
describe("koda help --all (markdown)", () => {
  test("contains ## heading for each known top-level command", async () => {
    ...
    for (const cmd of ["run", "auth", "debug", "mcp", "session", "agent", "profile"]) {
      expect(output).toContain(`## koda ${cmd}`)
    }
  })

  test("contains headings for nested subcommands", async () => {
    const output = await generateHelp({ all: true, format: "md", commands })
    expect(output).toContain("koda auth login")
    expect(output).toContain("koda auth logout")
    expect(output).toContain("koda debug config")
  })
})

describe("koda help --all (text)", () => {
...
describe("koda help <command>", () => {
  test("koda help auth contains auth subcommand headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).toContain("koda auth login")
    expect(output).toContain("koda auth logout")
    expect(output).toContain("koda auth list")
  })

  test("koda help auth does NOT contain run or debug headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).not.toContain("## koda run")
    expect(output).not.toContain("## koda debug")
  })

  test("documents pr subcommands", async () => {
    const output = await generateHelp({ command: "pr", format: "md", commands })
    expect(output).toContain("koda pr checkout")
    expect(output).toContain("koda pr link")
    expect(output).toContain("koda pr unlink")
    expect(output).toContain("koda pr status")
  })

  test("documents daemon foreground mode", async () => {
    const output = await generateHelp({ command: "daemon", format: "md", commands })
    expect(output).toContain("koda daemon start")
    expect(output).toContain("--foreground")
    expect(output).toContain("-f")
  })
})

describe("koda cloud help", () => {
  async function parser() {
    const cli = yargs([])
      .scriptName("koda cloud")
      .exitProcess(false)
      .help()
      .fail((msg, err) => {
  ...
  test("requires a subcommand and exposes only the public Cloud Agent operations", async () => {
    ...
    const names = [...help.matchAll(/^\s*koda cloud ([a-z][a-z-]*)\b/gm)].map((match) => match[1])
...
describe("edge cases", () => {
  ...
  test("koda help nonexistent throws unknown command error", async () => {
...
describe("generateCommandTable", () => {
  ...
  test("contains rows for known commands", async () => {
    ...
    for (const name of ["run", "auth", "debug", "mcp"]) {
      expect(output).toContain(`koda ${name}`)
    }
  })

  test("default command appears as koda [project], not $0", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`koda [project]`")
  ...
  test("skips commands with no describe", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).not.toContain("`koda generate`")
  })

  test("contains koda completion row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`koda completion`")
  })

  test("contains koda help row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`koda help")
  })
})

describe("koda CLI customizations are wired into index.ts", () => {
  const file = (rel: string) => Bun.file(path.resolve(import.meta.dir, rel)).text()
  const INDEX = "../../src/index.ts"
  const SETUP = "../../src/koda/cli/setup.ts"
  const BARREL = "../../src/koda/commands.ts"

  test("CLI is branded `koda`, not `opencode`", async () => {
    const index = await file(INDEX)
    expect(index).toContain('.scriptName("koda")')
    expect(index).not.toContain('.scriptName("opencode")')
  })

  test("index.ts invokes the kodaCli integration points", async () => {
    // These thin call-sites are the only wiring between upstream index.ts and the koda
    // customizations in setup.ts. If a future upstream merge drops them, every koda command
    // and the telemetry/lifecycle hooks silently disappear, exactly the regression this guards.
    const index = await file(INDEX)
    expect(index).toContain("kodaCli.register(")
    expect(index).toContain("kodaCli.bootstrap(")
    expect(index).toContain("kodaCli.shutdown(")
  })

  test("does not register the removed browser command", async () => {
    const index = await file(INDEX)
    const setup = await file(SETUP)
    const barrel = await file(BARREL)
    expect(setup).not.toContain('command("console")')
