...
import { assertNetwork, assertWrite, enabled as sandboxed } from "@koda/sandbox"
import { Bus } from "@/bus"
import { GlobalBus } from "@/bus/global"
import { Config } from "@/config/config"
import * as Network from "@/koda/sandbox/network"
import * as SandboxPolicy from "@/koda/sandbox/policy"
import { SandboxStore } from "@/koda/sandbox/store"
...
test("refreshes the session snapshot after a backend restart", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "koda-sandbox-restart-"))
  ...
  const env = {
    ...process.env,
    koda_TEST_HOME: path.join(root, "home"),
...
posix("canonicalizes a symlinked policy state root", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "koda-sandbox-state-link-"))
  const target = path.join(root, "real-state")
  const link = path.join(root, "state")
  await fs.mkdir(target)
  await fs.symlink(target, link)
  const script = 'import { SandboxStore } from "@/koda/sandbox/store"; console.log(SandboxStore.root)'
  ...
  try {
    ...
    expect(result.stdout.toString().trim().split("\n").at(-1)).toBe(
      path.join(await fs.realpath(target), "koda-sandbox-policy"),
    )
  } finally {
    await fs.rm(root, { recursive: true, force: true })
  }
...
linux("reports configured network namespace availability", async () => {
  const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "koda-sandbox-status-"))
  ...
  try {
    const result = Bun.spawnSync([process.execPath, "-e", script], {
      cwd: import.meta.dir,
      env: { ...process.env, koda_BWRAP_PATH: helper, koda_SERVER_PASSWORD: "sandbox-test" },
      stdout: "pipe",
      stderr: "pipe",
      windowsHide: true,
    })
...
it.instance("does not let project config weaken an initialized policy", () =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const password = Flag.koda_SERVER_PASSWORD
      Flag.koda_SERVER_PASSWORD = "sandbox-test"
      return password
    }),
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const file = path.join(test.directory, "koda.json")
    ...
    (password) => Effect.sync(() => (Flag.koda_SERVER_PASSWORD = password)),
  ),
)
