...
import { assertWrite, run as runSandbox } from "@koda/sandbox"
import { Effect, Exit } from "effect"
import { profile } from "@/koda/sandbox/policy"
import { SandboxPreference } from "@/koda/sandbox/preference"
...
function fixture() {
  return tmpdir<Dirs>({
    init: async (root) => {
      ...
      const a = path.join(main, ".koda", "worktrees", "a")
      const b = path.join(main, ".koda", "worktrees", "b")
...
function expected(...directories: string[]) {
  return new Set([...directories, ...koda])
}
...
describe("sandbox policy", () => {
  ...
  test("keeps koda state writable without exposing sandbox policy state", async () => {
    await using tmp = await fixture()
    const dirs = tmp.extra
    ...
    expect(policy.environment.deny).toEqual([
      ...
      "koda_CONFIG_CONTENT",
      "koda_CONFIG_DIR",
      "koda_SERVER_PASSWORD",
      "koda_SERVER_USERNAME",
    ])
