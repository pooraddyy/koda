...
import * as SandboxActivation from "@/koda/sandbox/activation"
...
const it = testEffect(
  Layer.mergeAll(
    Session.layer.pipe(
      ...
      Layer.provide(AppNodeBuilder.build(SessionV2.node, [[SessionExecution.node, SessionExecution.noopLayer]])), // koda_change
    ),
...
describe("sandbox session cleanup", () => {
  ...
  it.live("creates honor the koda.sandbox metadata over the config default", () =>
    Effect.gen(function* () {
      const sessions = yield* Session.Service
      ...
      const session = yield* provideInstance(dir)(
        sessions.create({ title: "sandbox-explicit", metadata: { "koda.sandbox": { enabled: true, version: 0 } } }),
      )
