...
import * as ToolNetwork from "@/koda/sandbox/network"
import { MCP } from "@/mcp"
...
const sessions = Layer.mock(Session.Service)({
  get: () => Effect.succeed(session("/workspace/project/.koda/worktrees/a")),
})
...
const mcp = Layer.mock(MCP.Service)({
  ...
  clients: () => Effect.succeed({}), // koda_change - upstream's MCP resource tools probe the clients
})
...
mac("confines model-originated file mutations to the active worktree", () =>
  Effect.gen(function* () {
    ...
    const koda = yield* call(write, { filePath: state, content: "state" }, "call-state").pipe(Effect.exit)
    ...
    expect(Exit.isSuccess(koda)).toBe(true)
