...
import { kodaCompactionPayloadRecovery } from "../../src/koda/session/compaction-payload-recovery"
...
describe("kodaCompactionPayloadRecovery", () => {
  test("detects koda gateway payload-size errors", () => {
    const error = new MessageV2.ContextOverflowError({
      message: "Request Entity Too Large",
      responseBody: "Request Entity Too Large\n\nFUNCTION_PAYLOAD_TOO_LARGE",
    }).toObject()

    expect(kodaCompactionPayloadRecovery.matches(error)).toBe(true)
  })
  ...
  test("strips media and marks completed tool outputs compacted", async () => {
    ...
    await Effect.runPromise(
      kodaCompactionPayloadRecovery.strip({
    ...
    async () => {
      ...
      stub.push((input) => {
        ...
        return Stream.fail(
          new APICallError({
            message: "Request Entity Too Large",
            url: "https://api.koda.ai/api/openrouter/responses",
            requestBodyValues: {},
            statusCode: 413,
