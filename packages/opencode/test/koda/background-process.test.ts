...
`,
      ...
      const client = process.env["koda_CLIENT"]
      ...
      try {
        ...
        yield* Effect.promise(() => BackgroundProcess.stopSession(sessionID))
        if (client === undefined) delete process.env["koda_CLIENT"]
        else process.env["koda_CLIENT"] = client
      }
    }),
  )
...
`,
      ...
      const client = process.env["koda_CLIENT"]
      const scans = process.env["koda_BACKGROUND_PROCESS_PORTS"]
      process.env["koda_CLIENT"] = "cli"
      process.env["koda_BACKGROUND_PROCESS_PORTS"] = "true"
      ...
      try {
        ...
        if (scans === undefined) delete process.env["koda_BACKGROUND_PROCESS_PORTS"]
        else process.env["koda_BACKGROUND_PROCESS_PORTS"] = scans
      }
    }),
  )
...
`,
      ...
      const scans = process.env["koda_BACKGROUND_PROCESS_PORTS"]
      process.env["koda_CLIENT"] = "cli"
      delete process.env["koda_BACKGROUND_PROCESS_PORTS"]
