...
import { McpMigrator } from "../../src/koda/mcp-migrator"
...
describe("McpMigrator", () => {
  describe("convertServer", () => {
    test("converts local server with command and args", () => {
      const server: McpMigrator.kodaMcpServer = {
        command: "npx",
  ...
  describe("migrate", () => {
    ...
    test("migrates servers from project .koda/mcp.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".koda")
          await Bun.write(
    ...
    test("reads from legacy .koda/mcp.json when .koda/mcp.json is absent", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".koda")
          await Bun.write(
    ...
    // Regression: malformed .koda/mcp.json must not prevent .koda/mcp.json from loading
    test("loads .koda/mcp.json even when .koda/mcp.json is malformed", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".koda", "mcp.json"), "{ corrupt json !!!")
          await Bun.write(
            path.join(dir, ".koda", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test(".koda/mcp.json overrides .koda/mcp.json for same server name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".koda", "mcp.json"),
    ...
    test("does not read project-level .koda/mcp_settings.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".koda")
          await Bun.write(
