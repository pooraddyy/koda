...
import { kodaTask } from "../../src/koda/tool/task"
...
function expectPlan(item: Agent.Info | undefined, action: Permission.Action = "allow") {
  ...
  expect(Permission.evaluate("edit", ".koda/plans/fix.md", item!.permission).action).toBe(action)
}
...
// own ruleset from the same global config. kodaTask.inherited has to carry the guarded set
...
test("plan carries guarded denies into delegated sessions under a global catch-all", async () => {
  ...
  await provideTestInstance({
    ...
    fn: async () => {
      ...
      const child = kodaTask.merge(
        deriveSubagentSessionPermission({ parentSessionPermission: [], subagent: explore! }),
        kodaTask.permissions(kodaTask.inherited({ caller: plan!, session: { permission: [] }, mcp: undefined })),
      )
...
test("plan agent still hard-denies non-plan edits after user edit allow", async () => {
  ...
  await provideTestInstance({
    ...
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".koda/plans/fix.md", plan!.permission).action).toBe("allow")
...
test("plan agent preserves explicit per-agent edit denies", async () => {
  ...
  expectPlan(plan)
  expect(Permission.evaluate("edit", ".koda/plans/private.md", plan!.permission).action).toBe("deny")
})
