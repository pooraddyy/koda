...
import { appendSessionDiffs, mergeSessionDiffs } from "../../src/koda/session-portability/cumulative-diff"
import { extractSessionDiffs, restoreSessionDiffs } from "../../src/koda/session-portability/session-diff-restore"

const dirs: string[] = []

function tmp() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "koda-session-diff-"))
  dirs.push(dir)
  return dir
}
...
function patch(dir: string) {
  ...
  return git(dir, ["-c", "color.ui=false", "diff", "--src-prefix=a/", "--dst-prefix=b/"])
}
