...
import { koda_ERROR_CODES, iskodaError, parsekodaErrorCode } from "../../src/koda/koda-errors"
...
describe("parsekodaErrorCode", () => {
  ...
  it("extracts PROMOTION_MODEL_LIMIT_REACHED from { code } (top-level)", () => {
    ...
    expect(parsekodaErrorCode(error)).toBe("PROMOTION_MODEL_LIMIT_REACHED")
  })
  ...
  it("returns undefined for non-koda error codes", () => {
    const error = makeAPIError({
      statusCode: 429,
      responseBody: JSON.stringify({ error: { code: "SOME_OTHER_ERROR" } }),
    })
    expect(parsekodaErrorCode(error)).toBeUndefined()
  })
...
describe("iskodaError", () => {
  it("returns true for PAID_MODEL_AUTH_REQUIRED", () => {
    const error = makeAPIError({
      statusCode: 401,
      responseBody: JSON.stringify({ error: { code: "PAID_MODEL_AUTH_REQUIRED" } }),
    })
    expect(iskodaError(error)).toBe(true)
  })
  ...
  it("returns false for regular 429 errors without koda code", () => {
    const error = makeAPIError({
      statusCode: 429,
      isRetryable: true,
      message: "Too Many Requests",
    })
    expect(iskodaError(error)).toBe(false)
  })
...
describe("SessionRetry.retryable with koda errors", () => {
