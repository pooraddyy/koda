// Verifies kodaCostPropagation.propagate() serializes concurrent writes to
...
import { kodaCostPropagation } from "../../src/koda/session/cost-propagation"
...
describe("kodaCostPropagation.propagate", () => {
  it.live("sums deltas correctly under parallel execution", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        yield* Effect.all(
          deltas.map((d) => kodaCostPropagation.propagate(sessions, chat.id, assistant.id, d)),
  ...
  it.live("is a no-op when amount is non-positive", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const sessions = yield* Session.Service
        const { chat, assistant } = yield* seed()
        yield* kodaCostPropagation.propagate(sessions, chat.id, assistant.id, 0)
        yield* kodaCostPropagation.propagate(sessions, chat.id, assistant.id, -1.5)
        const parent = yield* MessageV2.get({ sessionID: chat.id, messageID: assistant.id })
        if (parent.info.role !== "assistant") return
