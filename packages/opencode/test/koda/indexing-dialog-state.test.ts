...
import type { Config, IndexingConfig } from "@koda/sdk/v2"
import {
  ...
  mergeIndexingConfig,
  type IndexingScope,
} from "../../src/koda/components/indexing-dialog-state"

describe("indexing dialog state", () => {
  test.serial("loads koda models directly from the public catalog", async () => {
    ...
    global.fetch = (async (input) => {
      ...
      return new Response(
        JSON.stringify({
          defaultModel: "koda/default",
          models: [{ id: "koda/default", name: "Default", dimension: 1024, scoreThreshold: 0.35 }],
          aliases: {},
        }),
    ...
    try {
      const catalog = await loadkodaEmbeddingModels()

      expect(catalog.models).toHaveLength(1)
      expect(catalog.defaultModel).toBe("koda/default")
  ...
  test("builds stable loading, empty, and loaded model options", () => {
    expect(kodaModelOptions()).toEqual([{ value: "", title: "Loading supported models..." }])
    expect(kodaModelOptions({ defaultModel: "", models: [], aliases: {} })).toEqual([
    ...
    expect(kodaModelOptions(catalog)).toEqual([
      { value: "provider/default", title: "Default (1024d)" },
      { value: "provider/code", title: "Code (code, 1536d)" },
    ])
    expect(currentkodaModel(catalog, "code")).toBe("provider/code")
    expect(currentkodaModel(catalog, "missing")).toBe("provider/default")
  })
  ...
  test("isolates global auth config from project indexing values", () => {
    const project: IndexingConfig = { koda: { apiKey: "project-key", baseUrl: "https://project.test" } }
    const inherited: IndexingConfig = { enabled: true }
    const effective: Config = {
      provider: { koda: { options: { apiKey: "provider-key" } } },
      indexing: project,
    }
