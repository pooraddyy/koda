...
import { RemoveError, remove } from "../../src/koda/agent"
import type { Info as AgentInfo } from "../../src/agent/agent"
import { tmpdir } from "../fixture/fixture"

describe("koda agent remove", () => {
  test("removes config-backed imported agents", async () => {
    await using tmp = await tmpdir()
    const dir = path.join(tmp.path, ".koda")
    const file = path.join(dir, "koda.jsonc")
  ...
  "agent": {
    ...
    expect(cfg.agent.code.model).toBe("koda/gpt-5")
  })

  test("removes duplicate agents from every editable config source", async () => {
    await using tmp = await tmpdir()
    const dir = path.join(tmp.path, ".koda")
    const files = [path.join(dir, "koda.jsonc"), path.join(dir, "opencode.jsonc")]
    await mkdir(dir, { recursive: true })
  ...
  test("limits removal to the selected scope", async () => {
    ...
    const dir = path.join(tmp.path, ".koda")
    const local = path.join(dir, "koda.jsonc")
    const global = path.join(Global.Path.config, "koda.jsonc")
  ...
  test("preserves organization-managed agents", async () => {
    await using tmp = await tmpdir()
    const dir = path.join(tmp.path, ".koda", "agents")
