...
import { BackgroundProcess } from "../../src/koda/background-process"
...
import * as SandboxPolicy from "../../src/koda/sandbox/policy"
...
describe("koda task nesting", () => {
  ...
  test("preserves inherited restrictions while refreshing prompt tool toggles", () => {
    const permission = kodaSessionPrompt.mergeToolPermissions({
      existing: [
      ...
      () =>
        Effect.gen(function* () {
          ...
          const effective = Permission.merge(
            ...
            kodaSessionPrompt.guardPermissions({ agent: validator, session: child }),
          )
