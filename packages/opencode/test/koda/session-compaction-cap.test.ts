...
describe("session compaction cap", () => {
  it.live(
    "closes the turn with reason=budget after MAX_COMPACTION_ATTEMPTS compactions",
    () =>
      provideTmpdirServer(
        Effect.fnUntraced(function* ({ llm }) {
          ...
          const turnClose = yield* Deferred.make<kodaSession.CloseReason>()
          const unsub = Bus.subscribe(kodaSession.Event.TurnClose, (evt) => {
          ...
          expect(yield* llm.calls).toBe(kodaSessionPrompt.MAX_COMPACTION_ATTEMPTS * 2 + 1)
...
describe("kodaSessionPrompt.guardCompactionAttempt", () => {
  it.effect("returns { exhausted: false } and does not mutate state below the cap", () =>
    Effect.sync(() => {
      ...
      const result = kodaSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_under",
        attempts: kodaSessionPrompt.MAX_COMPACTION_ATTEMPTS - 1,
        closeReasons,
        message: msg,
      })
  ...
  it.effect("sets close reason and attaches error once attempts reach the cap", () =>
    Effect.sync(() => {
      ...
      const result = kodaSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_cap",
        attempts: kodaSessionPrompt.MAX_COMPACTION_ATTEMPTS,
        closeReasons,
        message: msg,
      })
      expect(result.exhausted).toBe(true)
      if (!result.exhausted) return
      expect(closeReasons.get("ses_cap")).toBe("budget")
  ...
  it.effect("works without a message and still sets the close reason", () =>
    Effect.sync(() => {
      ...
      expect(result.exhausted).toBe(true)
      expect(closeReasons.get("ses_no_msg")).toBe("budget")
    }),
  )
})
