...
import { filterPromptTrainingModels, nonEmptyProviders } from "../../../src/koda/provider/model-filter"

function model(id: string, training?: boolean): Provider.Model {
  return {
    id: ModelV2.ID.make(id),
    providerID: ProviderV2.ID.koda,
    api: { id: "koda", url: "https://api.koda.ai", npm: "@koda/koda-gateway" },
    name: id,
    capabilities: {
      temperature: true,
      reasoning: false,
...
describe("prompt-training model filter", () => {
  test("hides only explicitly marked koda Gateway models", () => {
    const providers = {
      koda: provider("koda", {
    ...
    expect(Object.keys(result.koda.models)).toEqual(["private", "unknown"])
    expect(Object.keys(result.other.models)).toEqual(["training"])
    expect(Object.keys(providers.koda.models)).toEqual(["training", "private", "unknown"])
  })

  test("preserves the catalog when disabled", () => {
    const providers = { koda: provider("koda", { training: model("training", true) }) }
