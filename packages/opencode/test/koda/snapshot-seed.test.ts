...
import { Instance } from "../../src/koda/instance"
import { Filesystem } from "../../src/util/filesystem"
import { kodaSnapshotMaterialize } from "../../src/koda/snapshot/materialize"
import { kodaSnapshotSeed } from "../../src/koda/snapshot/seed"
...
test(
  ...
  async () => {
    ...
    expect(
      (
        await $`git --git-dir=${common} rev-parse --verify --quiet ${kodaSnapshotMaterialize.ref(fast.gitdir)}`
          .nothrow()
          .text()
      ).trim(),
    ).toBe("")
...
test(
  ...
  async () => {
    ...
    const sourceRef = kodaSnapshotMaterialize.ref(result.gitdir)
    ...
    const expired = `refs/koda/snapshots/1/${result.value!}`
    ...
    expect((await $`git --git-dir=${result.gitdir} for-each-ref refs/koda/snapshots`.text()).trim()).toContain(
      result.value!,
    )

    const locked = `refs/koda/snapshots/2/${result.value!}`
...
test("interrupted seed removes borrowed state after source gc", async () => {
  ...
  const original = await fs.readFile(index)
  const ref = kodaSnapshotMaterialize.ref(gitdir)
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const fiber = yield* kodaSnapshotSeed.seed({
        dir: source.path,
