...
import { CodexAuthExpiredError, refreshCodexAuth } from "../../src/koda/provider/codex-refresh"
...
async function race(input: { reuse: "early" | "late"; delay: number; lock?: Lock }) {
  ...
  try {
    const data = path.join(dir, "share", "koda")
  ...
  test("serializes expired Codex auth as ProviderAuthError", () => {
    ...
    expect(result).toStrictEqual({
      ...
      data: {
        providerID: "openai",
        message:
          "Your ChatGPT sign-in expired or was revoked. Sign in with ChatGPT again to continue using OpenAI models.",
      },
    })
  })
