...
import { CodeIndexManager } from "@koda/koda-indexing/engine"
import { normalizeIndexingStatus } from "@koda/koda-indexing/status"
import type { Config } from "../../src/config/config"
import { GlobalBus } from "../../src/bus/global"
import { WorkspaceV2 } from "@opencode-ai/core/workspace"
import { WorkspaceContext } from "../../src/control-plane/workspace-context"
import { kodaIndexing, IndexingModelError } from "../../src/koda/indexing"
import { indexingWarningKey } from "../../src/koda/indexing-warning"
import { IndexingWorker } from "../../src/koda/indexing-worker-client"
...
const koda: Partial<Config.Info> = {
  plugin: ["@koda/koda-indexing"],
  indexing: {
    enabled: true,
    vectorStore: "qdrant",
...
const stalekoda: Partial<Config.Info> = {
  plugin: ["@koda/koda-indexing"],
  indexing: {
    enabled: true,
    provider: "koda",
...
const configDir = process.env["koda_CONFIG_DIR"]
const disabled = process.env["koda_DISABLE_CODEBASE_INDEXING"]
...
async function wait(read: () => Promise<kodaIndexing.Status>, state: kodaIndexing.Status["state"]) {
...
afterEach(async () => {
  ...
  if (configDir === undefined) delete process.env["koda_CONFIG_DIR"]
  else process.env["koda_CONFIG_DIR"] = configDir
  if (disabled === undefined) delete process.env["koda_DISABLE_CODEBASE_INDEXING"]
  else process.env["koda_DISABLE_CODEBASE_INDEXING"] = disabled
...
describe("indexing model catalog", () => {
  test("ignores a project-scoped koda origin", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        const global = path.join(dir, "global")
        const project = path.join(dir, "project")
        await fs.mkdir(path.join(project, ".koda"), { recursive: true })
        await fs.mkdir(global, { recursive: true })
        await Bun.write(path.join(global, "koda.jsonc"), "{}")
        await Bun.write(
          path.join(project, ".koda", "koda.jsonc"),
          JSON.stringify({ indexing: { koda: { baseUrl: "http://127.0.0.1:4567" } } }),
        )
        return { global, project }
      },
    })
    process.env["koda_CONFIG_DIR"] = tmp.extra.global
    const calls: string[] = []
    ...
    const response = await Server.Default().app.request("/indexing/models", {
      headers: { "x-koda-directory": tmp.extra.project },
    })
...
describe("indexing startup degradation", () => {
  test("keeps server routes alive when indexing initialization fails", async () => {
    ...
    process.env["koda_CONFIG_DIR"] = tmp.path
  ...
  test("retains and deduplicates indexing warnings for TUI replay", async () => {
    ...
    const on = (data: {
      ...
      if (data.payload?.type !== kodaIndexing.Warning.type) return
  ...
  test("coalesces burst indexing progress publications", async () => {
    const complete: kodaIndexing.Status = {
      state: "Complete",
    ...
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["koda_CONFIG_DIR"] = tmp.path
    const events: kodaIndexing.Status[] = []
    const on = (data: {
      directory?: string
      payload?: { type?: string; properties?: { status?: kodaIndexing.Status } }
    }) => {
      if (data.directory !== tmp.path) return
      if (data.payload?.type !== kodaIndexing.Event.type) return
    ...
    try {
      await withTestInstance({
        directory: tmp.path,
        fn: async () => expect(await wait(() => kodaIndexing.current(), "Complete")).toEqual(complete),
      })
  ...
  test("reports routes as in progress while initialization is in flight", async () => {
    ...
    try {
      await provideTestInstance({
        directory: tmp.path,
        init: Effect.promise(() => kodaIndexing.init()),
        fn: async () => {
          await called(init)
          expect(await kodaIndexing.current()).toMatchObject({
            state: "In Progress",
  ...
  test("does not publish initialized status after in-flight startup is disposed", async () => {
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          await called(init)
          expect((await kodaIndexing.current()).state).toBe("In Progress")
        },
      })
  ...
  test("keeps degraded indexing queryable but releases its failed engine", async () => {
    ...
    try {
      await provideTestInstance({
        ...
        init: Effect.promise(() => kodaIndexing.init()),
        fn: async () => {
          const status = await wait(() => kodaIndexing.current(), "Error")

          expect(status.state).toBe("Error")
          expect(status.message).toContain("Failed to initialize: test indexing initialization failed")
          expect(await kodaIndexing.available()).toBe(false)
          expect(kodaIndexing.ready()).toBe(false)
          expect(await kodaIndexing.search("boot failure")).toEqual([])
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(kodaIndexing.ready()).toBe(false)
          expect(await kodaIndexing.available()).toBe(false)
          const search = kodaIndexing.search("boot failure")
  ...
  test("stays disabled when indexing enablement is unset", async () => {
    ...
    await provideTestInstance({
      directory: tmp.path,
      fn: async () => {
        const status = await wait(() => kodaIndexing.current(), "Disabled")
        ...
        expect(await kodaIndexing.available()).toBe(false)
        expect(kodaIndexing.ready()).toBe(false)
        expect(await kodaIndexing.search("disabled")).toEqual([])
  ...
  test("does not validate the indexing model when indexing is disabled", async () => {
    ...
    const logger = Log.create({ service: "koda-indexing" })
    const warn = spyOn(logger, "warn")
    const key = process.env.koda_API_KEY

    const config: Partial<Config.Info> = {
      ...stalekoda,
      indexing: { ...stalekoda.indexing, enabled: false, model: "removed/model" },
    }
    await using tmp = await tmpdir({ git: true, config })
    process.env["koda_CONFIG_DIR"] = tmp.path
    process.env.koda_API_KEY = "koda-token"
    ...
    try {
      ...
      if (key === undefined) delete process.env.koda_API_KEY
      else process.env.koda_API_KEY = key
      warn.mockRestore()
    }
  })

  test("disabled indexing does not resolve the koda model catalog", async () => {
  ...
  test("enriches koda provider config from env auth", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    const key = process.env.koda_API_KEY
    const org = process.env.koda_ORG_ID

    await using tmp = await tmpdir({ git: true, config: koda })
    process.env["koda_CONFIG_DIR"] = tmp.path
    process.env.koda_API_KEY = "koda-token"
    process.env.koda_ORG_ID = "org_123"
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(init.mock.calls[0]?.[0]).toMatchObject({
            embedderProvider: "koda",
            kodaApiKey: "koda-token",
            kodaOrganizationId: "org_123",
      ...
      if (org === undefined) delete process.env.koda_ORG_ID
      else process.env.koda_ORG_ID = org
      init.mockRestore()
    }
  })

  test("reports an error for an unsupported explicit koda model instead of falling back", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    await using tmp = await tmpdir({ git: true, config: stalekoda })
    process.env["koda_CONFIG_DIR"] = tmp.path
    process.env.koda_API_KEY = "koda-token"
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(await kodaIndexing.available()).toBe(false)
          expect(kodaIndexing.ready()).toBe(false)
          expect(await kodaIndexing.search("unsupported model")).toEqual([])
  ...
  test("passes a valid explicit koda model through without error", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    const config: Partial<Config.Info> = {
      ...stalekoda,
      indexing: { ...stalekoda.indexing, model: "mistralai/mistral-embed-2312" },
    }
    await using tmp = await tmpdir({ git: true, config })
    process.env["koda_CONFIG_DIR"] = tmp.path
    process.env.koda_API_KEY = "koda-token"
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const modelErr = warn.mock.calls.find((call) => IndexingModelError.isInstance(call[1]?.err))?.[1]?.err
          expect(modelErr).toBeUndefined()
          expect((await kodaIndexing.current()).state).not.toBe("Error")
        },
      })
    } finally {
      if (key === undefined) delete process.env.koda_API_KEY
  ...
  test("uses hosted dimensions for supported koda models", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
  ...
  test("leaves koda model metadata unset when the hosted catalog is unavailable", async () => {
  ...
  test("does not default to koda when an existing provider config is present", async () => {
    const init = spyOn(CodeIndexManager.prototype, "initialize").mockResolvedValue({ requiresRestart: false })
    const key = process.env.koda_API_KEY
