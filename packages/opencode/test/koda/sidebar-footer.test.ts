...
import { billable, creditLabel, format, passLine, resetLabel, scope } from "../../src/koda/plugins/sidebar-footer"
...
describe("koda sidebar footer", () => {
  ...
  test("shows pass period usage and reset date", () => {
    expect(passLine(kodaPass)).toBe("$73 / $199")
    expect(resetLabel(kodaPass.nextBillingAt)).toBe("Jul 1")
    expect(resetLabel(null)).toBeUndefined()
    expect(resetLabel("not-a-date")).toBeUndefined()
  })

  test("refreshes only after completed billed koda turns", () => {
