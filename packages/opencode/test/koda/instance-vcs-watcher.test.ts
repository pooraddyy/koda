...
const disableFilewatcher = process.env.koda_EXPERIMENTAL_DISABLE_FILEWATCHER
beforeAll(() => {
  delete process.env.koda_EXPERIMENTAL_DISABLE_FILEWATCHER
})
afterAll(() => {
  if (disableFilewatcher !== undefined) process.env.koda_EXPERIMENTAL_DISABLE_FILEWATCHER = disableFilewatcher
})
  ...
  async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const watcher = kodaWatcher.layer.pipe(Layer.provide(Layer.succeed(LocationServiceMap.Service, map)))
        const services = yield* Layer.build(watcher)
        const init = (directory: string) =>
          kodaWatcher.Service.use((service) => service.init()).pipe(
