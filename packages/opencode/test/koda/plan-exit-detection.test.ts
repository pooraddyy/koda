...
import { PlanFollowup } from "../../src/koda/plan-followup"
...
describe("plan_exit detection", () => {
        ...
        tools: [
          {
            tool: "plan_exit",
            input: {},
            output: "Plan is ready at .koda/plans/plan.md. Ending planning turn.",
          },
        ],
      })
  ...
  test("kodaSessionPrompt resolves plan follow-up through the supplied question service", () =>
    withInstance(async () => {
      const seeded = await seed({
        text: "Here is the plan",
        tools: [
      ...
      const result = await Effect.runPromise(
        Effect.gen(function* () {
          ...
          const pending = kodaSessionPrompt.askPlanFollowup({
            sessionID: seeded.sessionID,
            messages: seeded.messages,
            abort: AbortSignal.any([]),
            question,
  ...
  test("kodaSessionPrompt cleans listener-local plan follow-up when aborted outside instance context", () => {
  ...
  test("JetBrains client enables plan follow-up with custom answer", () =>
    withInstance(async () => {
      ...
      try {
        process.env.koda_CLIENT = "jetbrains"
        const seeded = await seed({
          text: "Here is the plan",
          tools: [
            {
        ...
        else process.env.koda_CLIENT = prev
      }
    }))
  ...
  test("plan reminder reuses custom plan_exit path when refining", () =>
    withInstance(async () => {
      ...
      await kodaSessionPrompt.insertPlanReminders({
  ...
  test("native plan reminder keeps in-chat approval for clients without follow-up support", () =>
    withInstance(async () => {
      ...
      try {
        ...
        process.env.koda_CLIENT = "vscode"
