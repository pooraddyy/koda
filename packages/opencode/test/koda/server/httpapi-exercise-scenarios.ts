...
const skill = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".koda/skill/httpapi-remove/SKILL.md"),
    "---\nname: httpapi-remove\ndescription: HTTP API removal fixture.\n---\n# HTTP API remove\n",
  )
  await Bun.write(path.join(dir, ".koda/skill/httpapi-remove/KEEP.txt"), "synthetic sentinel\n")
}

const agent = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".koda/agent/httpapi-remove.md"),
    "---\ndescription: HTTP API remove\n---\nRemove me.\n",
  )
}

const duplicates = async (dir: string) => {
  for (const name of ["koda.jsonc", "opencode.jsonc"]) {
    await Bun.write(
      path.join(dir, ".koda", name),
      JSON.stringify({
...
const command = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".koda/command/httpapi-remove.md"),
...
function enable(ctx: ScenarioContext) {
  const dir = directory(ctx)
  return Effect.promise(() => kodaMemory.enable({ ctx: { directory: dir, worktree: dir } }))
}
...
export const kodaScenarios: Scenario[] = [
  ...
  http.protected.get("/koda/profile", "koda.profile").probe({ path: "/path" }).status(401),
  http.protected.get("/koda/auth-status", "koda.authStatus").json(200, (body) => {
    object(body)
    check(body.authenticated === false, "koda auth status should report signed out")
    check(body.type === undefined, "koda auth status should not expose a credential type while signed out")
  }),
  http.protected.get("/koda/modes", "koda.modes").json(200, (body) => {
    object(body)
    array(body.modes)
  }),
  http.protected
    .post("/koda/fim", "koda.fim")
    .at((ctx) => ({ path: "/koda/fim", headers: ctx.headers(), body: { prefix: "const value = ", suffix: "\n" } }))
    .status(401),
  http.protected
    .post("/koda/edit", "koda.edit")
    .at((ctx) => ({ path: "/koda/edit", headers: ctx.headers(), body: edit }))
    .status(401),
  http.protected
    .post("/koda/audio/transcriptions", "koda.audio.transcriptions")
    .at((ctx) => ({
      path: "/koda/audio/transcriptions",
      headers: ctx.headers(),
      body: { model: "whisper-1", input_audio: { data: "", format: "wav" } },
    }))
    .status(401),
  http.protected.get("/koda/notifications", "koda.notifications").json(200, array),
  http.protected.get("/koda/models/images", "koda.models.images").probe({ path: "/path" }).status(401),
  http.protected.get("/koda/models/transcriptions", "koda.models.transcriptions").probe({ path: "/path" }).status(401),
  http.protected
    .post("/koda/organization", "koda.organization.set")
    .at((ctx) => ({ path: "/koda/organization", headers: ctx.headers(), body: { organizationId: null } }))
    .status(401),
  http.protected.get("/koda/claw/status", "koda.claw.status").probe({ path: "/path" }).status(401),
  http.protected.get("/koda/claw/chat-credentials", "koda.claw.chatCredentials").probe({ path: "/path" }).status(401),
  http.protected.get("/koda/cloud-sessions", "koda.cloudSessions").probe({ path: "/path" }).status(401),
  http.protected
    .get("/koda/cloud/session/{id}", "koda.cloud.session.get")
    .probe({ path: "/path" })
    .at((ctx) => ({ path: route("/koda/cloud/session/{id}", { id: "httpapi-missing" }), headers: ctx.headers() }))
    .status(401),
  http.protected
    .post("/koda/cloud/session/import", "koda.cloud.session.import")
    .at((ctx) => ({ path: "/koda/cloud/session/import", headers: ctx.headers(), body: { sessionId: "missing" } }))
    .status(401),
  ...
  http.protected
    ...
    .status(400),
  http.protected
    .get("/session/{sessionID}/model-usage", "koda.sessionModelUsage")
  ...
  http.protected
    .get("/koda/background-jobs", "koda.backgroundJobs")
    .at((ctx) => ({
      path: "/koda/background-jobs?sessionID=ses_httpapi_missing",
      headers: ctx.headers(),
    }))
    .json(200, (body) => {
      array(body)
  ...
  http.protected
    .post("/koda/background-jobs/{jobID}/cancel", "koda.backgroundJob.cancel")
    .at((ctx) => ({
      path: route("/koda/background-jobs/{jobID}/cancel", { jobID: "job_httpapi_missing" }),
      headers: ctx.headers(),
    }))
    .status(404),
  http.protected
    .get("/koda/collaboration", "koda.collaboration.list")
    .inProject({ git: true })
    .json(200, (body) => {
      array(body)
      for (const graph of body) {
        object(graph)
        check(typeof graph.id === "string", "collaboration graph should include an id")
        check(typeof graph.state === "string", "collaboration graph should include a state")
        array(graph.nodes)
      }
    }),
  http.protected
    .get("/koda/collaboration/{graphID}", "koda.collaboration.summary")
    .at((ctx) => ({
      path: route("/koda/collaboration/{graphID}", { graphID: "collab_httpapi_missing" }),
      headers: ctx.headers(),
    }))
    .status(404),
  http.protected
    .post("/koda/collaboration/{graphID}/cancel", "koda.collaboration.cancel")
    .at((ctx) => ({
      path: route("/koda/collaboration/{graphID}/cancel", { graphID: "collab_httpapi_missing" }),
      headers: ctx.headers(),
    }))
    .status(404),
  http.protected
    .post("/koda/collaboration/recover", "koda.collaboration.recover")
    .inProject({ git: true })
    .json(200, (body) => {
      array(body)
      for (const graph of body) {
  ...
  http.protected
    .get("/koda/hooks", "koda.hooks.list")
    .inProject({ git: true })
    .json(200, (body) => {
      array(body)
      for (const hook of body) {
        object(hook)
        check(typeof hook.id === "string", "hook listing should include an id")
        check(!("command" in hook), "hook listing must not expose command bodies")
      }
    }),
  http.protected
    .post("/koda/heap/snapshot", "koda.heap.snapshot")
    .mutating()
    .jsonEffect(200, (body) =>
      Effect.gen(function* () {
  ...
  http.protected
    .get("/koda/command/files", "koda.commandFiles")
    ...
    .json(200, (body, ctx) => {
      ...
      check(
        item.location === path.join(directory(ctx), ".koda/command/httpapi-remove.md"),
  ...
  http.protected
    .post("/koda/command/remove", "koda.removeCommand")
    .inProject({ git: true, init: command })
    .mutating()
    .preserveDatabase()
    .at((ctx) => ({
      path: "/koda/command/remove",
      headers: ctx.headers(),
      body: { location: path.join(directory(ctx), ".koda/command/httpapi-remove.md") },
    }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "command removal should return true")
        const location = path.join(directory(ctx), ".koda/command/httpapi-remove.md")
        check(!(yield* Effect.promise(() => Bun.file(location).exists())), "removed command should not remain on disk")
      }),
    ),
  http.protected
    .post("/koda/skill/remove", "koda.removeSkill")
    .inProject({ git: true, init: skill })
    .mutating()
    .preserveDatabase()
    .at((ctx) => ({
      path: "/koda/skill/remove",
      headers: ctx.headers(),
      body: { location: path.join(directory(ctx), ".koda/skill/httpapi-remove/SKILL.md") },
    }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "skill removal should return true")
        const location = path.join(directory(ctx), ".koda/skill/httpapi-remove/SKILL.md")
        const sentinel = path.join(directory(ctx), ".koda/skill/httpapi-remove/KEEP.txt")
  ...
  http.protected
    .post("/koda/agent/remove", "koda.removeAgent")
    .inProject({ git: true, init: agent })
    .mutating()
    .at((ctx) => ({ path: "/koda/agent/remove", headers: ctx.headers(), body: { name: "httpapi-remove" } }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "agent removal should return true")
        const location = path.join(directory(ctx), ".koda/agent/httpapi-remove.md")
        check(!(yield* Effect.promise(() => Bun.file(location).exists())), "removed agent should not remain on disk")
      }),
    ),
  http.protected
    .get("/koda/provider-usage", "koda.providerUsage.get")
    .inProject({ git: true })
    .json(200, (body) => {
      object(body)
      array(body.items)
  ...
  http.protected
    .post("/koda/provider-usage/refresh", "koda.providerUsage.refresh")
    .inProject({ git: true })
    .json(200, (body) => {
      object(body)
      array(body.items)
  ...
  http.protected
    .post("/koda/agent/remove", "koda.removeAgent.duplicates")
    .inProject({ git: true, init: duplicates })
    .mutating()
    .at((ctx) => ({ path: "/koda/agent/remove", headers: ctx.headers(), body: { name: "httpapi-duplicate" } }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "duplicate agent removal should return true")
        for (const name of ["koda.jsonc", "opencode.jsonc"]) {
          const cfg = yield* Effect.promise(() => Bun.file(path.join(directory(ctx), ".koda", name)).json())
  ...
  http.protected
    .post("/koda/agent/remove", "koda.removeAgent")
    .at((ctx) => ({ path: "/koda/agent/remove", headers: ctx.headers(), body: { name: "httpapi-missing" } }))
    .json(400, (body) => {
      object(body)
      check(body.message === "agent not found", "agent removal should preserve the backend error message")
    }),
  http.protected
    .post("/koda/session-import/project", "koda.sessionImport.project")
    .mutating()
    .at((ctx) => ({
      path: "/koda/session-import/project",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/koda/session-import/session", "koda.sessionImport.session")
    .mutating()
    .seeded((ctx) => ctx.project())
    .at((ctx) => ({
      path: "/koda/session-import/session",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/koda/session-import/message", "koda.sessionImport.message")
    .mutating()
    .seeded((ctx) => ctx.session({ title: "Import message" }))
    .at((ctx) => ({
      path: "/koda/session-import/message",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/koda/session-import/part", "koda.sessionImport.part")
    .mutating()
    .seeded((ctx) =>
      Effect.gen(function* () {
