...
import { invalidateAfterProviderAuthChange } from "../../../src/koda/server/provider-auth-lifecycle"
...
it.effect("clears provider models before disposing instances after auth changes", () =>
  Effect.gen(function* () {
    ...
    yield* invalidateAfterProviderAuthChange("koda").pipe(Effect.provide(layer(events)))

    expect(yield* Ref.get(events)).toEqual(["clear:koda", "dispose"])
  }),
)
