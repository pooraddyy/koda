...
import { kodaConfigOverlay } from "../../../src/koda/config/overlay"
import { kodaConfigWriter } from "../../../src/koda/config/writer"
import { Permission } from "../../../src/permission"
import { PtyPaths } from "../../../src/server/routes/instance/httpapi/groups/pty"
import { SessionPaths } from "../../../src/server/routes/instance/httpapi/groups/session"
import { SandboxStore } from "../../../src/koda/sandbox/store"
...
async function config(dir: string, value: unknown) {
  await Bun.write(path.join(dir, "koda.json"), JSON.stringify(value, null, 2))
}
...
describe("config overlay routes", () => {
  test("writes a missing project target atomically", async () => {
    ...
    const target = await kodaConfigOverlay.target({ scope: "project", directory: project.path })

    const result = await kodaConfigWriter.write({
      scope: "project",
  ...
  test("ignores a nested unset path when the project target is missing", async () => {
    ...
    expect(await Bun.file(path.join(project.path, ".koda", "koda.jsonc")).exists()).toBe(false)
  })

  test("removes an existing nested unset path", async () => {
    await using project = await tmpdir()
    const file = path.join(project.path, ".koda", "koda.jsonc")
  ...
  test("returns exact raw target data and a stable missing-file revision", async () => {
    await using project = await tmpdir()
    const first = await kodaConfigOverlay.target({ scope: "project", directory: project.path })
    const second = await kodaConfigOverlay.target({ scope: "project", directory: project.path })
    ...
    const saved = await kodaConfigOverlay.target({ scope: "project", directory: project.path })
  ...
  test("rejects a comment-only external edit with a typed revision conflict", async () => {
    ...
    const response = await Server.Default().app.request("/config/overlay", {
      method: "PATCH",
      headers: { "content-type": "application/json", "x-koda-directory": project.path },
      body: JSON.stringify({
        scope: "project",
        expected: {
  ...
  test("rejects a newly created higher-priority target", async () => {
    await using project = await tmpdir()
    const before = await json<Overlay>(await req(project.path, "/config/overlay?scope=project"))
    await Filesystem.write(path.join(project.path, "koda.json"), "{}")
  ...
  test("rejects a project config target that escapes through a symlink", async () => {
    ...
    await Filesystem.write(path.join(outside.path, "koda.jsonc"), "{}")
    await symlink(outside.path, path.join(project.path, ".koda"), "dir")
    const before = await json<Overlay>(await req(project.path, "/config/overlay?scope=project"))
    ...
    expect(await Bun.file(path.join(outside.path, "koda.jsonc")).text()).not.toContain('"model"')
  })

  test("does not expose partial content when an atomic replacement fails", async () => {
    await using project = await tmpdir()
    const file = path.join(project.path, "koda.jsonc")
    await Filesystem.write(file, '{\n  "model": "test/before"\n}\n')
    const target = await kodaConfigOverlay.target({ scope: "project", directory: project.path })

    await expect(
      kodaConfigWriter.write({
        scope: "project",
        directory: project.path,
        expected: target,
  ...
  test("rechecks missing target parents before replacement", async () => {
    ...
    expect(await Bun.file(path.join(outside.path, "koda.jsonc")).exists()).toBe(false)
  })
  ...
  test("ignores unsafe patch paths", () => {
    const patched = kodaConfigOverlay.patch({
      scope: "project",
      unset: [
        ["__proto__", "polluted"],
  ...
  test("prefers .koda over legacy .kilocode and ignores .opencode in project overlays", async () => {
    await using project = await tmpdir()
    const entries = [
      {
        root: ".opencode",
      ...
      },
      {
        root: ".koda",
        source: "koda",
        value: { username: "koda", model: "test/koda" },
      },
      {
        root: ".kilocode",
        source: "legacy",
        value: { username: "legacy" },
      },
    ] as const

    for (const item of entries) {
      const dir = path.join(project.path, item.root)
      await Filesystem.write(path.join(dir, "koda.json"), JSON.stringify(item.value))
      await Filesystem.write(
    ...
    const body = await kodaConfigOverlay.resolve({
      directory: project.path,
      scope: "project",
      effective: {},
      global: {},
    ...
    expect(body.project.username).toBe("koda")
    expect(body.project.model).toBe("test/koda")
    ...
    expect(body.targets.project.path).toBe(path.join(project.path, ".koda", "koda.json"))
  })
  ...
  test.serial("tolerates unsafe project config instead of failing the overlay", async () => {
    ...
    await Filesystem.write(
      path.join(project.path, ".koda", "koda.json"),
  ...
  test.serial("writes project indexing overrides to .koda/koda.jsonc", async () => {
    await using global = await tmpdir()
    await using project = await tmpdir()
    ...
    expect(await Bun.file(path.join(project.path, ".koda", "koda.json")).exists()).toBe(false)
  ...
  test.serial("removes local scalar override and falls back to global", async () => {
    await using global = await tmpdir()
    await using project = await tmpdir({ config: { model: "koda/project-model", username: "alice" } })
    await setGlobal(global.path, { model: "koda/global-model" })
  ...
  test.serial("writes project mcp overrides without copying inherited servers", async () => {
    ...
    const saved = (await Bun.file(path.join(project.path, ".koda", "koda.jsonc")).json()) as {
  ...
  test.serial("writes partial global workflow overrides when both JSON and JSONC exist", async () => {
    ...
    await Filesystem.write(path.join(global.path, "koda.json"), JSON.stringify({ username: "legacy" }))
    await Filesystem.write(path.join(global.path, "koda.jsonc"), "{\n  // Keep JSONC as the active target.\n}\n")
    ...
    const saved = ConfigParse.jsonc(await Bun.file(path.join(global.path, "koda.jsonc")).text(), "koda.jsonc")
    expect(saved).toMatchObject({
      command: { review: { model: "anthropic/claude-sonnet-4-6", variant: "high" } },
    })
    expect(await Bun.file(path.join(global.path, "koda.json")).json()).toMatchObject({ username: "legacy" })
  })
  ...
  test.serial("merges workflow overrides with a command body in the lower-precedence global file", async () => {
    ...
    ;(Global.Path as { config: string }).config = global.path
    await Filesystem.write(
      path.join(global.path, "koda.json"),
  ...
  terminal("preserves active terminals after updating global console preferences", async () => {
    ...
    await using project = await tmpdir()
    ;(Global.Path as { config: string }).config = global.path
    const headers = { "x-koda-directory": project.path }
