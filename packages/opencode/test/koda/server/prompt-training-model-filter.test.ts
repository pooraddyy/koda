...
function models(input: unknown, key: "all" | "providers") {
  ...
  const koda = input[key].find((provider) => record(provider) && provider.id === "koda")
  if (!record(koda) || !record(koda.models)) return []
  return Object.keys(koda.models)
}

function request(path: string, dir: string) {
  return Effect.promise(async () => {
    const result = await Server.Default().app.request(path, { headers: { "x-koda-directory": dir } })
      ...
      Effect.promise(() =>
        tmpdir({
          config: {
            formatter: false,
            lsp: false,
            enabled_providers: ["koda"],
            hide_prompt_training_models: true,
            provider: { koda: { options: { baseURL } } },
          },
        }),
      ),
