...
describe("TUI config routes", () => {
  ...
  test("loads legacy .koda TUI config and ignores .opencode", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, ".koda", "tui.json"), JSON.stringify({ theme: "nord" }))
      },
    })
  ...
  test("patches project TUI config", async () => {
    ...
    const saved = await Bun.file(path.join(tmp.path, ".koda", "tui.json")).json()
