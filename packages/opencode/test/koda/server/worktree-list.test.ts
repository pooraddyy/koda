...
const state = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = Flag.koda_EXPERIMENTAL_WORKSPACES
    Flag.koda_EXPERIMENTAL_WORKSPACES = true
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.koda_EXPERIMENTAL_WORKSPACES = original
        await resetDatabase()
      }),
    )
  }),
...
describe("koda Console worktree listing", () => {
  run(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const directory = path.join(test.directory, ".koda", "worktrees", "console-list")
