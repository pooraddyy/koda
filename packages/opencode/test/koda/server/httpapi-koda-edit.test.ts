...
import { HEADER_FEATURE, HEADER_ORGANIZATIONID } from "@koda/koda-gateway"
...
const env = {
  koda_AUTH_CONTENT: process.env.koda_AUTH_CONTENT,
...
function app() {
  const handler = HttpRouter.toWebHandler(
    // koda_change - keep the filewatcher-disable flag visible (see httpapi-instance-route-auth.test.ts)
    HttpApiServer.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            koda_EXPERIMENTAL_DISABLE_FILEWATCHER: process.env.koda_EXPERIMENTAL_DISABLE_FILEWATCHER ?? "true",
          }),
        ),
      ),
    ),
...
async function send(body: Record<string, unknown> = edit, signal?: AbortSignal) {
  ...
  return app().request(kodaGatewayPaths.edit, {
    method: "POST",
...
describe("HttpApi koda next edit", () => {
  test("requires koda Gateway authentication for the koda-backed model", async () => {
    process.env.koda_AUTH_CONTENT = "{}"
  ...
  test("proxies koda-backed edits with gateway auth and autocomplete headers", async () => {
    authenticate()
