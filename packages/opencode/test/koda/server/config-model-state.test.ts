...
describe("config model state routes", () => {
  test("reads TUI model favorites", async () => {
    ...
    await Bun.write(
      ...
      JSON.stringify({
        recent: [{ providerID: "koda", modelID: "gpt-5.5" }],
        favorite: [
          { providerID: "koda", modelID: "gpt-5.5" },
          { providerID: "koda", modelID: "qwen/qwen3-8b" },
        ],
        model: {},
        variant: {},
      }),
  ...
  test("updates favorites while preserving recents", async () => {
    ...
    await Bun.write(
      path.join(tmp.path, "model.json"),
      JSON.stringify({
        recent: [{ providerID: "koda", modelID: "recent" }],
        favorite: [],
        model: {},
        variant: {},
      }),
    ...
    const body = await json<{ recent: unknown[]; favorite: unknown[] }>(
      await req("/config/model-state", {
        ...
        body: JSON.stringify({ favorite: [{ providerID: "koda", modelID: "gpt-5.5" }] }),
      }),
    )

    expect(body.recent).toEqual([{ providerID: "koda", modelID: "recent" }])
    expect(body.favorite).toEqual([{ providerID: "koda", modelID: "gpt-5.5" }])
  })
})
