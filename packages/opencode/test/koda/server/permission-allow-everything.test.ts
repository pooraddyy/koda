...
import { AllowEverythingPermission } from "../../../src/koda/permission/allow-everything"
...
const original = {
  password: Flag.koda_SERVER_PASSWORD,
  username: Flag.koda_SERVER_USERNAME,
  envPassword: process.env.koda_SERVER_PASSWORD,
  envUsername: process.env.koda_SERVER_USERNAME,
}

afterEach(() => {
  Flag.koda_SERVER_PASSWORD = original.password
  Flag.koda_SERVER_USERNAME = original.username
...
const auth = () => `Basic ${Buffer.from("koda:secret").toString("base64")}`
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        const blocked = await Server.Default().app.request("/permission/allow-everything", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-koda-directory": tmp.path },
        ...
        const enable = await Server.Default().app.request("/permission/allow-everything", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-koda-directory": tmp.path, authorization: auth() },
          body: JSON.stringify({ enable: true }),
        })
