...
import { kodaGatewayApi, kodaGatewayPaths } from "../../../src/koda/server/httpapi/groups/koda-gateway"
import { kodaGatewayHandlers } from "../../../src/koda/server/httpapi/handlers/koda-gateway"
...
const TestHttpApi = HttpApi.make("opencode-instance").addHttpApi(kodaGatewayApi)
...
const layer = HttpRouter.serve(
  HttpApiBuilder.layer(TestHttpApi).pipe(
    Layer.provide(kodaGatewayHandlers),
...
describe("koda gateway HttpApi statuses", () => {
  it.live("reports locally stored API authentication without a Gateway request", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(kodaGatewayPaths.authStatus)
  ...
  it.live("preserves cloud session list rate limits", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(kodaGatewayPaths.cloudSessions)
  ...
  it.live("preserves missing cloud session previews", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(kodaGatewayPaths.cloudSession.replace(":id", "missing"))
  ...
  it.live("preserves cloud session preview server failures", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(kodaGatewayPaths.cloudSession.replace(":id", "failed"))
  ...
  it.live("preserves cloud session import authentication failures", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(kodaGatewayPaths.cloudSessionImport, { sessionId: "unauthorized" })
  ...
  it.live("maps cloud session import transport failures to internal errors", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(kodaGatewayPaths.cloudSessionImport, { sessionId: "failed" })
  ...
  it.live("preserves kodaClaw worker failures", () =>
    Effect.gen(function* () {
      yield* stub(() => new Response("worker failed", { status: 500 }))

      const response = yield* HttpClient.get(kodaGatewayPaths.clawStatus)

      expect(response.status).toBe(500)
      expect(yield* response.json).toEqual({ error: "kodaClaw request failed: 500 worker failed" })
    }),
  )

  it.live("normalizes numeric kodaClaw timestamps", () =>
    Effect.gen(function* () {
  ...
  it.live("maps kodaClaw transport failures to bad gateway", () =>
    Effect.gen(function* () {
      yield* stub(() => Promise.reject(new TypeError("network error")))
      ...
      expect(yield* response.json).toEqual({ error: "Failed to reach kodaClaw" })
    }),
  )
})
