...
import { AgentBuilderPaths } from "../../../src/koda/server/httpapi/groups/agent-builder"
import { BackgroundProcessPaths } from "../../../src/koda/server/httpapi/groups/background-process"
import { BranchNamePaths } from "../../../src/koda/server/httpapi/groups/branch-name"
import { ConfigConsolePaths } from "../../../src/koda/server/httpapi/groups/config-console"
import { IndexingPaths, kodaEmbeddingModel } from "../../../src/koda/server/httpapi/groups/indexing"
import { kodaGatewayPaths } from "../../../src/koda/server/httpapi/groups/koda-gateway"
import { kodaPaths } from "../../../src/koda/server/httpapi/groups/koda"
import { MemoryPaths } from "../../../src/koda/server/httpapi/groups/memory"
import { NetworkPaths } from "../../../src/koda/server/httpapi/groups/network"
import { TelemetryPaths } from "../../../src/koda/server/httpapi/groups/telemetry"
...
describe("koda PublicApi OpenAPI contract", () => {
  test("uses koda branding", () => {
    const spec = OpenApi.fromApi(PublicApi)
    expect(spec.info.title).toBe("koda")
    expect(spec.info.description).toBe("koda api")
  })

  test("includes legacy koda events in the generated SDK contract", () => {
  ...
  test("constrains embedding model metadata", () => {
    const accepts = (dimension: number, scoreThreshold: number) =>
      Result.isSuccess(
        EffectSchema.decodeUnknownResult(kodaEmbeddingModel)({
          id: "provider/model",
          name: "Model",
          dimension,
          scoreThreshold,
  ...
  test("keeps directory routing queries on koda Console routes", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const routes = [
  ...
  test("keeps workspace routing queries on all koda-owned routed endpoints", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const routes = [
      ...
      { method: "get", path: kodaPaths.providerUsage },
      { method: "post", path: kodaPaths.providerUsageRefresh },
      { method: "get", path: kodaPaths.sessionModelUsage },
  ...
  test("keeps personal organization resets nullable", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const body = spec.paths[kodaGatewayPaths.organization]?.post?.requestBody as Body | undefined
  ...
  test("keeps koda gateway responses nullable", () => {
    const spec = OpenApi.fromApi(PublicApi)
    ...
    const profile = response(kodaGatewayPaths.profile)?.properties
    expect(profile?.balance).toEqual({ anyOf: [expect.objectContaining({ type: "object" }), { type: "null" }] })
    expect(profile?.kodaPass).toEqual({ anyOf: [expect.objectContaining({ type: "object" }), { type: "null" }] })
    ...
    const pass = profile?.kodaPass?.anyOf?.find((item) => item.type === "object")?.properties
    ...
    const auth = response(kodaGatewayPaths.authStatus)?.properties
    expect(auth).toEqual({
      authenticated: { type: "boolean" },
      type: { type: "string", enum: ["api", "oauth"] },
    })

    const sessions = response(kodaGatewayPaths.cloudSessions)?.properties
    ...
    const claw = response(kodaGatewayPaths.clawStatus)?.properties
    ...
    expect(response(kodaGatewayPaths.clawChatCredentials)).toEqual({
  ...
  test("keeps transcription prompts in the public contract", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const body = spec.paths[kodaGatewayPaths.audioTranscriptions]?.post?.requestBody as Body | undefined
    const schema = body?.content?.["application/json"]?.schema
  ...
  test("keeps provider usage flat and credential-free", () => {
    ...
    expect(spec.paths[kodaPaths.providerUsage]?.get?.responses?.["200"]).toBeDefined()
    expect(spec.paths[kodaPaths.providerUsageRefresh]?.post?.responses?.["200"]).toBeDefined()
  ...
  test("documents the transcription model catalog route", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const route = spec.paths[kodaGatewayPaths.transcriptionModels]?.get
