...
import { TestCli } from "../../script/koda/test-cli"
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), "koda-test-cli-cwd-"))
        ...
        const output = await (async () => {
          ...
          let text = ""
          while (!text.includes("koda server listening on")) {
        ...
        expect(output, `stdout:\n${output}\nstderr:\n${err}`).toContain("koda server listening on")
        expect(code, `stdout:\n${output}\nstderr:\n${err}`).not.toBe(1)
      } finally {
