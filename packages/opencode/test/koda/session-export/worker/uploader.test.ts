...
import { Storage } from "@/koda/session-export/worker/storage"
import { Uploader, backoffFor } from "@/koda/session-export/worker/uploader"
import { Config } from "@/koda/session-export/config"
...
describe("Uploader", () => {
  ...
  beforeEach(() => {
    token = process.env.koda_SESSION_EXPORT_AUTH_TOKEN
    delete process.env.koda_SESSION_EXPORT_AUTH_TOKEN
  ...
  afterEach(() => {
    ...
    if (token === undefined) delete process.env.koda_SESSION_EXPORT_AUTH_TOKEN
    else process.env.koda_SESSION_EXPORT_AUTH_TOKEN = token
  })

  test("2xx response marks rows uploaded and deletes them", async () => {
    const telemetry: unknown[] = []
    const calls: Array<{ input: string; init: RequestInit }> = []
    process.env.koda_SESSION_EXPORT_AUTH_TOKEN = "local-token"
    ...
    expect(headers.get("x-koda-export-api-version")).toBe("1")
    expect(headers.get("x-koda-export-schema-version")).toBe("1")
    expect(headers.get("x-koda-export-agent-version")).toBe("v0")
    expect(headers.get("x-koda-export-root-session-id")).toBe("s1")
    expect(headers.get("x-koda-export-session-id")).toBe("s1")
    expect(headers.get("x-koda-export-seq-start")).toBe("0")
    expect(headers.get("x-koda-export-seq-end")).toBe("0")
    expect(headers.get("x-koda-export-event-count")).toBe("1")
    expect(headers.get("x-koda-export-content-encoding")).toBe("identity")
    expect(headers.get("x-koda-export-client-sent-at")).toMatch(/^\d{4}-\d{2}-\d{2}T/)
    expect(headers.get("x-koda-export-payload-sha256")).toBe(await sha256(body))
  ...
  test("sends anonymous id with all export headers when auth token is absent", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    const names = [
      "x-koda-export-api-version",
      "x-koda-export-schema-version",
      "x-koda-export-agent-version",
      "x-koda-export-surface",
      "x-koda-export-root-session-id",
      "x-koda-export-session-id",
      "x-koda-export-batch-id",
      "x-koda-export-seq-start",
      "x-koda-export-seq-end",
      "x-koda-export-event-count",
      "x-koda-export-payload-sha256",
      "x-koda-export-client-sent-at",
      "x-koda-export-content-encoding",
    ]

    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-koda-anon-id")).toBe("install_abc")
  ...
  test("falls back to telemetry id when anon id is not provided", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-koda-anon-id")).toBe("install_from_disk")
  })
  ...
  test("includes surface in batch metadata and upload headers", async () => {
    ...
    expect(headers.get("x-koda-export-surface")).toBe("vscode-extension")
