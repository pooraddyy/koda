...
import { Chunker } from "@/koda/session-export/worker/chunks"
import { Scrubber } from "@/koda/session-export/worker/scrub"
import { handleEvent } from "@/koda/session-export/worker/handlers"
import type {
  CompactionCaptured,
  LlmRequestCompleted,
  LlmRequestStarted,
...
} from "@/koda/session-export/events"
...
function started(id: string, input: Partial<LlmRequestStarted["input"]>): LlmRequestStarted {
  return {
    ...
    agent: "claude",
    modeId: "build",
    model: { providerId: "koda", modelId: "free-1", isFree: true },
