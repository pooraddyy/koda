...
import { getKillSwitchReason, resetEligibility } from "@/koda/session-export/eligibility"
...
describe("SessionExport worker respawn", () => {
  ...
  beforeEach(async () => {
    ...
    feature = process.env.koda_FEATURE
    resetEligibility()
  })
  ...
  afterEach(async () => {
    await SessionExport.shutdown()
    resetEligibility()
    if (feature === undefined) delete process.env.koda_FEATURE
    else process.env.koda_FEATURE = feature
  })

  test("passes surface to worker init", () => {
    const workers: FakeWorker[] = []
    process.env.koda_FEATURE = "cli"
    SessionExport.init({
      agentVersion: "v0",
      dbPath: ":memory:",
      subscribeAll: () => () => {},
