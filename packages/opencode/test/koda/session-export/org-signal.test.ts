...
import { getActiveOrg, resetOrgSource, setOrgSource } from "@/koda/session-export/eligibility"
...
describe("getActiveOrg", () => {
  ...
  afterEach(() => {
    if (env === undefined) delete process.env.koda_ORG_ID
    else process.env.koda_ORG_ID = env
    resetOrgSource()
  })
  ...
  test("returns env value when koda_ORG_ID is set", async () => {
    setOrgSource(async () => ({ type: "org", id: "org_auth" }))
    process.env.koda_ORG_ID = "org_envvar"
