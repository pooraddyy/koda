...
// path when its layer is built, so koda_TEST_HOME must be set before the runtime
...
// koda_DISABLE_CLAUDE_CODE flag the test env sets); project .koda skills are untrusted.
...
function writeProjectSkill(dir: string, name: string, body: string) {
  return Effect.promise(() =>
    Bun.write(
      path.join(dir, ".koda", "skill", name, "SKILL.md"),
      `---\nname: ${name}\ndescription: ${name} test skill.\n---\n\n${body}\n`,
    ),
  )
}
...
describe("skill shell injection", () => {
  ...
  unix("does not ask or run anything for a skill with only an inline code example", () =>
    Effect.gen(function* () {
      // This is the real-world trigger: koda-config.md documents the placeholder syntax
