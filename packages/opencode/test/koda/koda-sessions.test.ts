...
import { clearInFlightCache } from "../../src/koda-sessions/inflight-cache"
import { kodaSessions } from "../../src/koda-sessions/koda-sessions"
import { provide, Instance } from "../../src/koda/instance"
import { writePrLinkOverride } from "../../src/koda-sessions/pr-link"
import * as PrLink from "../../src/koda-sessions/pr-link"
import { RemoteWS } from "../../src/koda-sessions/remote-ws"
import { RemoteSender } from "../../src/koda-sessions/remote-sender"
...
import { RemoteProtocol } from "../../src/koda-sessions/remote-protocol"
...
function reset(...tokens: string[]) {
  clearInFlightCache("koda-sessions:token")
  clearInFlightCache("koda-sessions:client")
  for (const token of tokens) clearInFlightCache(`koda-sessions:token-valid:${token}`)
}
...
it.instance("bootstraps session ingest from koda_API_KEY without stored auth", () => {
  const original = process.env.koda_API_KEY
  const calls: string[] = []
  ...
  process.env.koda_API_KEY = "env-token"
  reset("env-token")

  return Effect.promise(() => kodaSessions.bootstrap("session-env")).pipe(
    Effect.andThen(() => Effect.sync(() => expect(calls).toEqual(["Bearer env-token", "Bearer env-token"]))),
    Effect.ensuring(
      Effect.sync(() => {
        if (original === undefined) delete process.env.koda_API_KEY
        else process.env.koda_API_KEY = original
        reset("env-token")
        request.mockRestore()
      }),
    ),
...
it.instance("prefers stored auth over koda_API_KEY for session ingest", () => {
  const original = process.env.koda_API_KEY
  const calls: string[] = []
  const fetch: typeof globalThis.fetch = Object.assign(
    async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  return Effect.gen(function* () {
    ...
    yield* auth.set("koda", { type: "api", key: "stored-token" })
    yield* Effect.promise(() => kodaSessions.bootstrap("session-auth"))
    expect(calls).toEqual(["Bearer stored-token", "Bearer stored-token"])
  }).pipe(
    Effect.ensuring(
      Effect.gen(function* () {
        const auth = yield* Auth.Service
        yield* auth.remove("koda").pipe(Effect.orDie)
        if (original === undefined) delete process.env.koda_API_KEY
        else process.env.koda_API_KEY = original
        reset("env-token", "stored-token")
        request.mockRestore()
...
it.instance("does not duplicate created-session subscribers when init is repeated", () => {
  ...
  return Effect.gen(function* () {
    ...
    const sessions = yield* kodaSessions.Service
    yield* auth.set("koda", { type: "api", key: "test-token" })
    yield* sessions.init()
    yield* sessions.init()
...
// koda_change start - K1 W1 / DEF-1: instance advertisement + per-session platform.
//
// `enableRemote` is idempotent/coalescing and is called from `/remote`, the
// explicit `koda remote` command, and bootstrap auto-enable (`koda_REMOTE=1` /
...
describe("kodaSessions.setInstanceAdvertisement (K1 W1 / DEF-1)", () => {
  ...
  beforeEach(() => {
    heartbeatCalls = 0
    outOfBand = undefined
    process.env["koda_DISABLE_SESSION_INGEST"] = "0"
    delete process.env["koda_SESSION_INGEST_URL"]
    process.env["koda_API_KEY"] = "tok"
    reset("tok")
    kodaSessions.resetInstanceAdvertisementForTests()
    ...
    clearInFlightCache("koda-sessions:token")
    clearInFlightCache("koda-sessions:token-valid:tok")

    // koda_change - only mock the specific endpoint authValid() calls
    // (${koda_API_BASE}/api/user). A blanket mock that returned 200 for
  ...
  afterEach(async () => {
    ...
    await provide({
      directory: tmp.path,
      fn: async () => {
        kodaSessions.disableRemote()
      },
    })
    pub.mockRestore()
    mock.restore()
    delete process.env["koda_DISABLE_SESSION_INGEST"]
    delete process.env["koda_SESSION_INGEST_URL"]
    delete process.env["koda_PLATFORM"]
    delete process.env["koda_API_KEY"]
    reset("tok")
  })

  // Reads the `getSessions` closure that koda-sessions.ts passed to
  ...
  test("enableRemote alone advertises the instance (covers /remote and auto-enable)", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        await kodaSessions.enableRemote()
  ...
  test("explicit set after enable replaces the payload (koda remote race)", async () => {
    await using tmp = await tmpdir({ git: true })
    await provide({
      directory: tmp.path,
      fn: async () => {
        ...
        kodaSessions.setInstanceAdvertisement({
          name: "mbp-igor",
          projectName: "cloud",
          version: "1.2.3",
        })
  ...
  test("setter triggers an out-of-band heartbeat when a connection is already established", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        kodaSessions.setInstanceAdvertisement({ name: "h", projectName: "p" })
        await outOfBand
  ...
  test("setter replaces payload and fires one out-of-band heartbeat per call", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        await kodaSessions.enableRemote()
        kodaSessions.setInstanceAdvertisement({ name: "first", projectName: "p" })
        await outOfBand
        const before = heartbeatCalls
        kodaSessions.setInstanceAdvertisement({ name: "second", projectName: "p" })
        await outOfBand
  ...
  test("explicit set before enableRemote is preserved (no re-set on enable)", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        kodaSessions.setInstanceAdvertisement({ name: "pre-set", projectName: "proj", version: "9.9.9" })
        await kodaSessions.enableRemote()
        const payload = await capturedGetSessions()()
  ...
  test("per-session platform resolution matches meta() order — env var fallback", async () => {
    ...
    //   kodaSession.resolvePlatform(id) || process.env["koda_PLATFORM"] || "cli"
    // For an id with no override, the env var (when set) wins over the default.
    process.env["koda_PLATFORM"] = "vscode"
    await using tmp = await tmpdir({ git: true })
    await provide({
      directory: tmp.path,
      fn: async () => {
        ...
        const expectedPlatform = process.env["koda_PLATFORM"] || "cli"
...
// koda_change start - K1 W1: real integration between SessionStatus,
...
// real kodaSessions seams and proves that a non-idle status is cleared
// deterministically, which is exactly what lets the fence resolve and the
// exit_cli handler ACK.
describe("kodaSessions.detachRemoteSession heartbeat fence (K1 W1)", () => {
  let heartbeatCalls = 0
  let outOfBand: Promise<void> | undefined
    ...
    test(`clears ${label} SessionStatus so the detach heartbeat fence resolves`, async () => {
      ...
      await provide({
        ...
        fn: async () => {
          ...
          await kodaSessions.attachRemoteSession(id)
          ...
          await kodaSessions.detachRemoteSession(id)
...
describe("kodaSessions heartbeat attention status (DEF-3)", () => {
  beforeEach(() => {
    process.env["koda_DISABLE_SESSION_INGEST"] = "0"
    delete process.env["koda_SESSION_INGEST_URL"]
    process.env["koda_API_KEY"] = "tok"
  ...
  test("idle/busy/retry unchanged when no attention is pending", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        await kodaSessions.attachRemoteSession(idleId)
        await kodaSessions.attachRemoteSession(busyId)
        await kodaSessions.attachRemoteSession(retryId)
...
// koda_change - PR link advertise (plan 8.2): the heartbeat resolves the
...
describe("kodaSessions PR link advertise (plan 8.2)", () => {
  ...
  test("stored override advertises prLink and ingests the set triple", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        const id = await setupSession()
        await kodaSessions.bootstrap(id)
