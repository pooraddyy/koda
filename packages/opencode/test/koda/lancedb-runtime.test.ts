...
const entry = "file:///tmp/koda-cache/node_modules/@lancedb/lancedb/dist/index.js"
const add = mock(async () => ({ directory: "/tmp/koda-cache", entrypoint: entry }))
...
const env = "koda_LANCEDB_PATH"
const prev = process.env[env]

describe("LanceDBRuntime", () => {
  beforeEach(async () => {
    const { LanceDBRuntime } = await import("../../src/koda/lancedb")
    LanceDBRuntime.clear()
    add.mockClear()
    add.mockImplementation(async () => ({ directory: "/tmp/koda-cache", entrypoint: entry }))
  })
  ...
  test("retries after a failed install", async () => {
    ...
    add
      ...
      .mockImplementationOnce(async () => ({ directory: "/tmp/koda-cache", entrypoint: entry }))
