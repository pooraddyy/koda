...
import { getkodaProjectId } from "../../src/koda/project-id"
...
describe("project-id", () => {
  describe("normalization", () => {
    test("extracts repo name from HTTPS git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin https://github.com/koda-Org/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from SSH git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin git@github.com:koda-Org/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from HTTPS URL without .git extension", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/koda-Org/handbook`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from ssh:// URL", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin ssh://git@github.com/koda-Org/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("truncates long repo names to 100 characters", async () => {
      ...
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/koda-Org/${longName}.git`.cwd(dir).quiet()
        },
      })
  ...
  describe("config file priority", () => {
    test("uses project.id from .koda/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          ...
          await Bun.write(
            path.join(dir, ".koda", "config.json"),
            JSON.stringify({
              project: {
    ...
    test("falls back to .koda/config.json when .koda/config.json is absent", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".koda"), { recursive: true })
    ...
    test("prefers .koda/config.json over .koda/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".koda"), { recursive: true })
          await Bun.write(path.join(dir, ".koda", "config.json"), JSON.stringify({ project: { id: "new-project" } }))
          await fs.mkdir(path.join(dir, ".koda"), { recursive: true })
          await Bun.write(
            path.join(dir, ".koda", "config.json"),
      ...
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.write(
            ...
            JSON.stringify({
              project: {
                id: "https://github.com/koda-Org/another-repo.git",
              },
            }),
          )
        },
  ...
  describe("fallback behavior", () => {
    ...
    test("handles malformed JSON in config file gracefully", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".koda"), { recursive: true })
          await Bun.write(path.join(dir, ".koda", "config.json"), "{ invalid json")
  ...
  describe("caching", () => {
    test("keeps project IDs isolated across active project contexts", async () => {
      await using first = await tmpdir({
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".koda"), { recursive: true })
          await Bun.write(path.join(dir, ".koda", "config.json"), JSON.stringify({ project: { id: "first" } }))
        },
      })
      await using second = await tmpdir({
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".koda"), { recursive: true })
          await Bun.write(path.join(dir, ".koda", "config.json"), JSON.stringify({ project: { id: "second" } }))
        },
      })

      const ids = await Promise.all([
        withTestInstance({ directory: first.path, fn: () => getkodaProjectId() }),
        withTestInstance({ directory: second.path, fn: () => getkodaProjectId() }),
      ])
    ...
    test("invalidates the cached project ID when the instance is disposed", async () => {
      ...
      const ids = await provideTestInstance({
        directory: tmp.path,
        fn: async (ctx) => {
          const first = await getkodaProjectId()
          await Bun.write(path.join(tmp.path, ".koda", "config.json"), JSON.stringify({ project: { id: "second" } }))
          const cached = await getkodaProjectId()
          await disposeInstance(ctx.directory)
          const refreshed = await getkodaProjectId()
  ...
  describe("edge cases", () => {
    test("handles git URLs with port numbers", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com:443/koda-Org/handbook.git`.cwd(dir).quiet()
        },
      })
