...
const koda = { id: "koda" } as Provider.Info

// Calculated cost for the `model()` + `baseUsage` pair: 1M input * $3 + 100k output * $15 = 3 + 1.5
const fallback = 3 + 1.5

describe("kodaSession.providerCost — Anthropic Messages / OpenAI Responses", () => {
...
describe("kodaSession.providerCost — Vercel AI Gateway", () => {
    ...
    const result = SessionNs.getUsage({
      ...
      metadata: {
        gateway: {
          ...
          // which koda doesn't pass on to end users — must be ignored.
          cost: "0",
...
describe("kodaSession.providerCost — fallback", () => {
