...
import { kitty, sequences } from "../../src/koda/cli/cmd/tui/util/terminal"

const keys = ["TERM_PROGRAM", "MSYSTEM", "koda_DISABLE_KITTY_KEYBOARD", "koda_ENABLE_KITTY_KEYBOARD"] as const
...
test("allows explicitly enabling Kitty keyboard", () => {
  env({ koda_ENABLE_KITTY_KEYBOARD: "1", MSYSTEM: "MINGW64" })
...
test("allows explicitly disabling Kitty keyboard", () => {
  env({ koda_DISABLE_KITTY_KEYBOARD: "1", koda_ENABLE_KITTY_KEYBOARD: "1" })
...
test("resets common terminal input modes", () => {
  env({ koda_DISABLE_KITTY_KEYBOARD: "1" })
