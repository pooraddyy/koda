// Unit tests for kodaSessionPrompt history-trim / media-strip helpers.
...
import { kodaSessionMessageOrder } from "../../src/koda/session/message-order"
...
describe("kodaSessionPrompt.hasCompletedSummary", () => {
  test("returns false for empty array", () => {
    expect(kodaSessionPrompt.hasCompletedSummary([])).toBe(false)
  })
  ...
  test("returns false when only summary has an error", () => {
    ...
    expect(kodaSessionPrompt.hasCompletedSummary(msgs)).toBe(false)
  })
  ...
  test("returns true when summary has finish and no error", () => {
    ...
    expect(kodaSessionPrompt.hasCompletedSummary(msgs)).toBe(true)
  })
})
...
describe("MessageV2.latest", () => {
  test("selects chronological state after retained pre-compaction tail", () => {
    ...
    const state = kodaSessionMessageOrder.latest(msgs)
  ...
  test("keeps queued subtasks moved after a chronologically later assistant", () => {
    ...
    kodaSessionMessageOrder.annotate([active, queued, done])

    const state = kodaSessionMessageOrder.latest([active, done, queued])
...
describe("kodaSessionPrompt.trimBeforeLastSummary", () => {
  ...
  test("trims to summary's text-user parent (reference-session bug)", () => {
    ...
    const result = kodaSessionPrompt.trimBeforeLastSummary(filtered)
  ...
  test("keeps the newest summary when retained history contains an older summary", () => {
    ...
    expect(kodaSessionPrompt.trimBeforeLastSummary(filtered)).toBe(filtered)
  })
  ...
  test("is idempotent", () => {
    ...
    const first = kodaSessionPrompt.trimBeforeLastSummary(msgs)
    const second = kodaSessionPrompt.trimBeforeLastSummary(first)
...
describe("kodaSessionPrompt.stripHistoricalMedia", () => {
  test("replaces image file part in historical user message with placeholder text", () => {
    ...
    const result = kodaSessionPrompt.stripHistoricalMedia(msgs)
  ...
  test("does not mutate the input", () => {
    ...
    const result = kodaSessionPrompt.stripHistoricalMedia(input)
...
describe("kodaSessionPrompt.maybeStripHistoricalMedia", () => {
  test("returns input unchanged when no completed summary exists", () => {
    const msgs = [
      user("msg_hist", [filePart("msg_hist", "image/png", "hist.png")]),
      user("msg_last", [textPart("msg_last", "follow-up")]),
    ]
    const result = kodaSessionPrompt.maybeStripHistoricalMedia(msgs)
    expect(result).toBe(msgs)
  })
