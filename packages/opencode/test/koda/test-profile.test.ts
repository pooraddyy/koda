...
import { TestProfile } from "../../script/koda/test-profile"
...
describe("test profiles", () => {
  test("darwin profile contains valid test files", () => {
    ...
    expect(result.files).toContain("koda/cli/install-artifact.test.ts")
    expect(result.files).toContain("koda/cli/tui/thread.test.ts")
    expect(result.files).toContain("koda/sandbox/macos-confinement.test.ts")
    expect(result.files).toContain("koda/core-watcher.test.ts")
    expect(result.files).toContain("koda/background-process.test.ts")
    expect(result.files).toContain("filesystem/filesystem.test.ts")
    expect(result.files).toContain("koda/interactive-terminal.test.ts")
    const sandbox = all.filter((file) => file.startsWith("koda/sandbox/"))
    expect(result.files.filter((file) => file.startsWith("koda/sandbox/"))).toEqual(sandbox)
    ...
    expect(result.files).not.toContain("koda/sessions/remote-ws.test.ts")
    ...
    expect(result.files).not.toContain("koda/daemon.test.ts")
    ...
    expect(result.files).not.toContain("koda/server/config-overlay.test.ts")
