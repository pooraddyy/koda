...
describe("indexing worktrees", () => {
  test("shares the primary checkout index with a linked worktree", async () => {
    ...
    const worktree = path.join(tmp.path, ".koda", "worktrees", "feature")
    ...
    await provideTestInstance({
      directory: worktree,
      fn: async () => {
        await kodaIndexing.search("worktree")
        expect((await kodaIndexing.current()).state).toBe("Complete")
        expect(await kodaIndexing.available()).toBe(true)
      },
    })
  ...
  test("does not classify an ordinary directory from its pathname", async () => {
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["koda_CONFIG_DIR"] = tmp.path
    const directory = path.join(tmp.path, ".koda", "worktrees", "feature")
    ...
    await provideTestInstance({
      directory,
      fn: async () => {
        await kodaIndexing.search("ordinary directory")
        expect((await kodaIndexing.current()).state).toBe("Complete")
        expect(await kodaIndexing.available()).toBe(true)
      },
    })
