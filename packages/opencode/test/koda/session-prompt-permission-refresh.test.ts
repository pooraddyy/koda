...
import { kodaHeadless } from "../../src/koda/permission/headless"
import { kodaSessionPrompt } from "../../src/koda/session/prompt"
import { kodaReadObject } from "../../src/koda/tool/read-object"
...
it.live("recognizes Windows named-pipe paths before filesystem inspection", () =>
  Effect.sync(() => {
    expect(kodaReadObject.namedPipe("\\\\.\\pipe\\secret")).toBe(true)
    expect(kodaReadObject.namedPipe("\\\\server\\pipe\\secret")).toBe(true)
    expect(kodaReadObject.namedPipe("C:\\project\\secret.txt")).toBe(false)
  }),
)
...
it.live(
  "blocks @file content denied by .kodaignore",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const sentinel = "koda_12133_MENTION_SENTINEL"
        yield* Effect.promise(() =>
          Promise.all([
            Bun.write(path.join(dir, "my_file.txt"), sentinel),
            Bun.write(path.join(dir, ".kodaignore"), "my_file.txt\n"),
          ]),
        )
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const sentinel = "koda_12133_ASK_SENTINEL"
        const denied = "koda_12133_REPLACEMENT_SENTINEL"
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const sentinel = "koda_12133_APPROVED_SENTINEL"
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir, llm }) {
        const sentinel = "koda_12133_ABORT_SENTINEL"
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const sentinel = "koda_12133_COMMAND_ABORT_SENTINEL"
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const allowed = "koda_12133_ALLOWED_BINARY_SENTINEL"
        const denied = "koda_12133_REPLACEMENT_BINARY_SENTINEL"
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const allowed = "koda_12133_ALLOWED_FILE_SENTINEL"
        const denied = "koda_12133_DENIED_INSTRUCTION_SENTINEL"
        ...
        yield* Effect.promise(() =>
          Promise.all([
            ...
            Bun.write(path.join(dir, ".kodaignore"), "nested/AGENTS.md\n"),
          ]),
        )
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        ...
        const nested = "koda_12133_DIRECTORY_SENTINEL"
        const direct = "koda_12133_BINARY_SENTINEL"
        const fs = yield* FSUtil.Service
...
symlinkIt(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const text = "koda_12133_SYMLINK_TEXT_SENTINEL"
        const binary = "koda_12133_SYMLINK_BINARY_SENTINEL"
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        ...
        const outside = path.join(os.tmpdir(), `koda-12133-${crypto.randomUUID()}.txt`)
        const sentinel = "koda_12133_REFERENCE_SYMLINK_SENTINEL"
        const fs = yield* FSUtil.Service
...
symlinkIt(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const outside = path.join(os.tmpdir(), `koda-12133-missing-${crypto.randomUUID()}`)
        const fs = yield* FSUtil.Service
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        ...
        const outside = path.join(os.tmpdir(), `koda-12133-directory-${crypto.randomUUID()}`)
        const fs = yield* FSUtil.Service
        yield* fs.ensureDir(folder)
        yield* fs.ensureDir(outside)
        yield* Effect.addFinalizer(() => Effect.promise(() => rm(outside, { recursive: true, force: true })))
  ...
  provideTmpdirServer(
    Effect.fnUntraced(function* () {
      ...
      const err = yield* awaitWithTimeout(
        kodaSessionPrompt.askPermission({
          permission,
      ...
      expect(yield* permission.list()).toEqual([])
      expect(yield* kodaHeadless.denies(child.id)).toBe(true)
      expect(yield* kodaHeadless.denies(root.id)).toBe(false)

      kodaHeadless.clear(root.id)
    }),
    { git: true },
  ),
)
...
it.live("interactive run: subagent permission asks still queue for a human reply", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* () {
      ...
      const fiber = yield* kodaSessionPrompt.askPermission({
        permission,
        agents: { get: () => Effect.succeed(agent) },
        sessions,
        agent,
