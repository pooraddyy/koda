...
//
// Unit tests for kodaSnapshotTrack.wrap — the slow-repo guard that sits
...
import { kodaSnapshotTrack } from "../../src/koda/snapshot/track"
import { kodaPartLifecycle } from "../../src/koda/session/part-lifecycle"
...
const makeHooks = (
  answer: kodaSnapshotTrack.Answer | Promise<kodaSnapshotTrack.Answer>,
): { hooks: kodaSnapshotTrack.Hooks; calls: Calls } => {
  const calls: Calls = { ask: 0, persist: 0, progress: [] }
  const hooks: kodaSnapshotTrack.Hooks = {
    async ask() {
      calls.ask += 1
      return answer
    },
...
describe("kodaSnapshotTrack.protect", () => {
  it.effect("returns at the availability deadline without waiting for cancellation", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* kodaSnapshotTrack.protect({
        inner,
        state,
        fallback,
        operation: "patch",
  ...
  it.effect("bypasses later operations after a deadline opens the directory circuit", () =>
    Effect.gen(function* () {
      ...
      const started = yield* Deferred.make<void>()
      let calls = 0
      const first = yield* kodaSnapshotTrack.protect({
        inner: Effect.sync(() => {
          calls += 1
      ...
      const second = yield* kodaSnapshotTrack.protect({
        inner: Effect.sync(() => {
          calls += 1
          return "unexpected"
        }),
  ...
  test("keeps circuit state isolated by directory", () => {
    const states = kodaSnapshotTrack.makeStates()
...
describe("kodaSnapshotTrack.wrap", () => {
  ...
  test("concurrent timeout does not override an active continue choice", async () => {
    const state = kodaSnapshotTrack.makeState()
    const answer = Promise.withResolvers<kodaSnapshotTrack.Answer>()
    const asked = Promise.withResolvers<void>()
    const { hooks, calls } = makeHooks(answer.promise)
    const firstHooks: kodaSnapshotTrack.Hooks = {
      ...hooks,
      async ask(input) {
        asked.resolve()
...
describe("kodaSnapshotTrack progress indicator", () => {
  test("classifies persisted progress as transient", () => {
    const part = kodaSnapshotTrack.progressPart({
      sessionID: SESSION,
      messageID: MESSAGE,
    ...
    expect(kodaPartLifecycle.transient(part)).toBe(true)
  })
  ...
  test("slow-but-succeeding path (under timeout) starts then ends the indicator", async () => {
    ...
    expect(withoutFrame(firstText.text)).toBe(
      kodaSnapshotTrack.formatProgress(kodaSnapshotTrack.PROGRESS_INITIALIZING, "").trim(),
    )
  ...
  test("timed-out path keeps the initializing label (no text escalation)", async () => {
    ...
    const base = kodaSnapshotTrack.formatProgress(kodaSnapshotTrack.PROGRESS_INITIALIZING, "").trim()
    for (const evt of events) {
...
describe("kodaSnapshotTrack persistDisable", () => {
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const state = kodaSnapshotTrack.makeState()
        const hooks: kodaSnapshotTrack.Hooks = {
          ...kodaSnapshotTrack.defaultHooks,
          async ask() {
            return "disable"
          },
          async startProgress() {},
        ...
        const file = path.join(test.directory, ".koda", "koda.jsonc")
  ...
  test("persistDisable without instance context does not throw", async () => {
    await kodaSnapshotTrack.defaultHooks.persistDisable()
  })
})

describe("kodaSnapshotTrack constants", () => {
  test("TIMEOUT_MS defaults to 10s and respects env override", () => {
    ...
    // with koda_SNAPSHOT_TRACK_TIMEOUT_MS, which this test suite does not set.
    expect(kodaSnapshotTrack.TIMEOUT_MS).toBe(10_000)
  })

  test("exposes stable answer labels", () => {
    expect(kodaSnapshotTrack.ANSWER_CONTINUE).toBe("Continue with snapshots")
    expect(kodaSnapshotTrack.ANSWER_DISABLE).toBe("Disable for this project")
  })
})
