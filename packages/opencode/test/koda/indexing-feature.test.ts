...
} from "../../src/koda/indexing-feature"
...
describe("indexing plugin helpers", () => {
  test("detects plugin-enabled configs", () => {
    ...
    expect(indexingEnabled({ plugin: ["@koda/koda-indexing@1.0.0"] })).toBe(true)
  })
  ...
  test("does not add duplicate indexing plugin", () => {
    const list = ensureIndexingPlugin(["@koda/koda-indexing@1.0.0"], INDEXING_PLUGIN)
    expect(list).toEqual(["@koda/koda-indexing@1.0.0"])
  })
