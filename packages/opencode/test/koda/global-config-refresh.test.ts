...
async function update(target: ReturnType<typeof app>, provider: "koda" | "openrouter") {
...
async function provider(target: ReturnType<typeof app>, directory: string) {
  const response = await target.request("/config", { headers: { "x-koda-directory": directory } })
  return (await response.json()).indexing?.provider as string | undefined
}

async function config(dir: string, value: object) {
  await Bun.write(path.join(dir, "koda.json"), JSON.stringify(value))
}
...
describe("global config refresh", () => {
  ...
  test("update ignores disposal notification failures", async () => {
    ...
    try {
      expect((await update(target, "koda")).status).toBe(200)
    } finally {
