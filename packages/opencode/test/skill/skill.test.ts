...
const itWithoutClaudeCodeSkills = testEffect(Layer.mergeAll(skills(false, true), node, testInstanceStoreLayer)) // koda_change
...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.koda_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.koda_TEST_HOME = prev
      }),
  )

const discovered = <T extends { location: string }>(list: readonly T[]) =>
  list.filter((skill) => ![Skill.BUILTIN_LOCATION, "<built-in>"].includes(skill.location)) // koda_change
...
describe("skill", () => {
  ...
  // koda_change start
  it.live("discovers skills from .koda/skill/ directory", () =>
    // koda_change end
      ...
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".koda", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
...
`,
          ...
          const list = discovered(yield* skill.all()) // koda_change
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".koda", "skill", "dir-skill", "SKILL.md"), // koda_change: .koda is primary
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".koda", "skill", "dir-skill")) // koda_change: .koda is primary
  ...
  // koda_change start
  it.live("discovers multiple skills from .koda/skill/ directory", () =>
    // koda_change end
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".koda", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".koda", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".koda", "skill", "no-frontmatter", "SKILL.md"), // koda_change: .koda is primary
              `# No Frontmatter
...
`,
          ...
          const skill = yield* Skill.Service
          expect(discovered(yield* skill.all())).toEqual([]) // koda_change
        }),
      { git: true },
    ),
  )
  ...
  it.live("discovers skills without descriptions", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          // koda_change start - load .koda skills without falling back to .opencode
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".koda", "skill", "manual-skill", "SKILL.md"),
                `---
name: manual-skill
---
...
`,
              ...
              // koda_change start
              Bun.write(
                path.join(dir, ".koda", "skill", "opencode-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .koda/skill directory.
---
...
`,
              ...
              // koda_change start
              Bun.write(
                path.join(dir, ".koda", "skill", "agent-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .koda/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".koda", "skills", "agent-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .koda/skills directory.
---
