...
import { disposeAllInstances, provideTmpdirInstance, TestInstance } from "../fixture/fixture" // koda_change
...
// koda_change - skip on windows: address windows ci failures #9496
...
describe("tool.skill", () => {
  unix("execute returns skill content block with files", () =>
    Effect.gen(function* () {
      ...
      const skill = path.join(dir, ".koda", "skill", "tool-skill") // koda_change
...
`,
      ...
      const home = process.env.koda_TEST_HOME
  ...
  // koda_change start
  it.live("built-in koda-config keeps rendered shell examples inert", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          ...
          const result = yield* tool.execute({ name: "koda-config" }, ctx)

          expect(result.metadata.dir).toBe("builtin")
          expect(result.output).toContain("Finding a named command")
          expect(result.output).toContain("~/.config/koda/")
          expect(result.output).toContain("~/.koda/")
