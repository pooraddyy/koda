...
import { Effect, Exit, Layer, Result, Schema } from "effect" // koda_change
...
import { run as runSandbox, type Profile } from "@koda/sandbox" // koda_change
...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".koda")])), // koda_change
})

type RegistryLayerOptions = {
  flags?: Partial<RuntimeFlags.Info>
  plugin?: Layer.Layer<Plugin.Service>
  config?: Parameters<typeof TestConfig.layer>[0] // koda_change
...
const registryLayer = (opts: RegistryLayerOptions = {}) => {
  const replacements = [
    [Config.node, opts.config ? TestConfig.layer(opts.config) : configLayer], // koda_change
...
const scout = testEffect(registryLayer({ flags: { experimentalScout: true } })) // koda_change
      ...
      tools: () =>
        Effect.succeed({
          weather_current: {
            ...
            clientName: "weather", // koda_change
          },
        }),
...
// koda_change start - verify the execute catalog is suppressed in restricted sessions
...
describe("tool.registry", () => {
  ...
  // koda_change start - background task parameters are available by default
  ...
  it.instance("loads tools from .koda/tool (singular)" /* koda_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const opencode = path.join(test.directory, ".koda") // koda_change
  ...
  it.instance("ignores non-tool exports in .koda/tool files" /* koda_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".koda", "tool") // koda_change
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools from .koda/tools (plural)" /* koda_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const opencode = path.join(test.directory, ".koda") // koda_change
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".koda", "tools") // koda_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const plugin = path.join(opencode, "node_modules", "@koda", "plugin") // koda_change
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@koda/plugin", type: "module", exports: { ".": "./dist/index.js" } }), // koda_change
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@koda/plugin"', // koda_change
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".koda", "tools") // koda_change
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
