...
import { Cause, Deferred, Effect, Exit, Fiber, Layer } from "effect" // koda_change - Cause for resume-hint coverage
...
import { MessageV2 } from "@/session/message-v2" // koda_change
import type { SessionPrompt } from "../../src/session/prompt"
import { MessageID, PartID, SessionID } from "../../src/session/schema" // koda_change - SessionID used by cost propagation tests
import { SessionRunState } from "@/session/run-state"
import { SessionStatus } from "@/session/status"
import { Provider } from "../../src/provider/provider" // koda_change
import { kodaSession } from "../../src/koda/session" // koda_change
...
const background = it // koda_change - background subagents are enabled by default
const disabled = testEffect(layer({ experimentalBackgroundSubagents: false })) // koda_change
...
// koda_change start - stub signature + prompt body extended to persist assistant cost for propagation tests
...
describe("tool.task", () => {
  ...
  // koda_change start - verify forked task children remain resumable
  ...
  // koda_change start - resumed children rebuild parent platform attribution after restart
  it.instance("execute preserves platform attribution when resuming a task", () =>
    Effect.gen(function* () {
      const sessions = yield* Session.Service
      const { chat, assistant } = yield* seed()
      kodaSession.setPlatformOverride(chat.id, "agent-manager")
      const child = yield* sessions.create({ parentID: chat.id, title: "Existing child" })
      kodaSession.clearPlatformOverride(child.id)
      const tool = yield* TaskTool
      const def = yield* tool.init()
      ...
      expect(kodaSession.resolvePlatform(child.id)).toBe("agent-manager")
      expect(kodaSession.resolveRoot(child.id)).toBe(chat.id)
    }),
  )
  // koda_change end
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // koda_change start — use arrayContaining: koda appends inherited caller restrictions
        ...
        expect(seen?.tools).toEqual({
          ...
          todowrite: false,
          task: false, // koda_change - koda disallows nested subagents
          bash: false,
          read: false,
        })
      }),
    {
      config: {
        ...
        experimental: {
          ...
          openTelemetry: true, // koda_change
        },
      },
    },
  )

  // koda_change start - terminal child assistant errors fail the task tool boundary
  ...
  // koda_change start - background subagent failures also surface the resumable task_id (#11620)
  ...
  // koda_change end
  // koda_change start - preserve the disabled-background regression test
  ...
  // koda_change start - completed background tasks propagate their invocation cost delta
  ...
  // koda_change start - the background.extend() path must also propagate its run's cost delta (regression)
...
// koda_change start - subagent cost propagation coverage (#6321)
