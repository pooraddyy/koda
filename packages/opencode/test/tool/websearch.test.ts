...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    // koda_change start - the override is resolved via Env.Service by the tool and passed in
  ...
  test("is only enabled for koda or explicit websearch provider flags", () => {
    // koda_change
    expect(webSearchEnabled(ProviderV2.ID.koda, { exa: false, parallel: false })).toBe(true) // koda_change
    expect(webSearchEnabled(ProviderV2.ID.opencode, { exa: false, parallel: false })).toBe(false) // koda_change
