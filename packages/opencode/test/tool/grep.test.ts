...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.koda_REPO_CLONE_GITHUB_BASE_URL
      process.env.koda_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.koda_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.koda_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
...
describe("tool.grep", () => {
  ...
  it.instance("does not report an unknown total when results are truncated", () =>
    Effect.gen(function* () {
      ...
      expect(result.output).toContain("100 matches limit reached. Use limit=200 for more, or refine pattern.") // koda_change
  ...
  // koda_change start - exact-file searches must not widen to siblings
