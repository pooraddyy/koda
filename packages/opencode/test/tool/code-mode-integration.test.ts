import { afterAll, beforeAll, describe, expect, test } from "bun:test" // koda_change
...
import { Cause, Effect, Exit, Layer, ManagedRuntime } from "effect" // koda_change
import { InstanceRef } from "@/effect/instance-ref" // koda_change
import { TestConfig } from "../fixture/config" // koda_change
...
let dispose = () => Promise.resolve() // koda_change
let runEffect = <A, E>(effect: Effect.Effect<A, E>) => Effect.runPromise(effect) // koda_change
...
async function buildTool() {
  ...
  for (const def of listed) {
    mcpTools[McpCatalog.toolName(SERVER, def.name)] = { def, client: client as unknown as Client, clientName: SERVER } // koda_change
  }
  ...
  const layer = Layer.mergeAll(
    ...
    }), // koda_change - production code mode captures config for sandbox policy
    Layer.succeed(InstanceRef, { directory: process.cwd(), worktree: process.cwd(), project: {} as any }), // koda_change
    AppNodeBuilder.build(Database.node), // koda_change - sandbox state uses the session database
  )
  const runtime = ManagedRuntime.make(layer) // koda_change
  dispose = () => runtime.dispose() // koda_change
  runEffect = (effect) => runtime.runPromise(effect) // koda_change
  return {
    tool: await runtime.runPromise(CodeModeTool.pipe(Effect.flatMap(Tool.init))), // koda_change
    description: describeCatalog(mcpTools, [SERVER]),
  }
}

const run = (code: string) => runEffect(tool.execute({ code }, ctx)) // koda_change
// Program failures die at the tool boundary; recover the defect for message assertions.
const runFailed = async (code: string) => {
  const exit = await runEffect(tool.execute({ code }, ctx).pipe(Effect.exit)) // koda_change
