import { afterAll, describe, expect, test } from "bun:test" // koda_change
...
import { Cause, Effect, Exit, Layer, ManagedRuntime, Schema } from "effect" // koda_change
...
const runtimes: { dispose(): Promise<void> }[] = [] // koda_change

afterAll(async () => {
  await Promise.all(runtimes.map((runtime) => runtime.dispose())) // koda_change
})
...
function mcpTool(
  ...
  return {
    ...
    clientName: name.split("_")[0] ?? name, // koda_change
  }
}
...
function build(
  ...
  const runtime = ManagedRuntime.make(harness({ mcpTools, servers: names, permission, trigger, sandbox })) // koda_change
  runtimes.push(runtime) // koda_change
  return runtime.runPromise(CodeModeTool.pipe(Effect.flatMap(Tool.init))) // koda_change
}
...
describe("code mode execute", () => {
  ...
  // koda_change start - code-mode must not advertise remote MCP tools in a network-restricted sandbox
