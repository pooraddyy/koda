...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.koda_TEST ?? "")'), {
      env: {
        koda_TEST: "set",
      },
    })
  ...
  test("can use a complete environment without inherited values", async () => {
    const key = "koda_TEST_INHERITED_ENV"
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "koda_TEST_SHELL"], {
      shell: true,
      env: {
        koda_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("koda_TEST_SHELL=ok")
  })
