...
describe("ProviderTransform.schema - gemini non-object properties removal", () => {
  ...
  test("sanitizes gemini schemas routed through the koda Gateway", () => {
...
describe("ProviderTransform.message - cache control on gateway", () => {
  ...
  test("koda gateway with openai gpt-5.6 applies caching options", () => {
    const model = createModel({
      providerID: "koda",
      api: {
        id: "openai/gpt-5.6",
...
describe("ProviderTransform.variants", () => {
  ...
  // koda_change start
  describe("@koda/koda-gateway", () => {
    test("mercury-2 uses server-provided variants from koda gateway", () => {
      ...
      const model = createMockModel({
        id: "koda/inception/mercury-2",
        providerID: "koda",
        api: {
          id: "inception/mercury-2",
          url: "https://gateway.koda.ai",
          npm: "@koda/koda-gateway",
        },
  ...
  describe("ProviderTransform.smallOptions", () => {
    describe("@koda/koda-gateway", () => {
      test("claude models use their default reasoning effort", () => {
        const model = createMockModel({
          id: "koda/anthropic/claude-sonnet-4",
          providerID: "koda",
          api: {
            id: "anthropic/claude-sonnet-4",
            url: "https://gateway.koda.ai",
      ...
      test("google models use their default reasoning effort", () => {
        const model = createMockModel({
          id: "koda/google/gemini-2.0-flash",
          providerID: "koda",
          api: {
...
// koda_change start - tests for reasoningSummary guard
describe("ProviderTransform.options - OpenAI Responses API params guard", () => {
  ...
  test("includes reasoningSummary for @koda/koda-gateway", () => {
    const result = ProviderTransform.options({
      model: gpt5Model("@koda/koda-gateway", "koda"),
      sessionID,
    })
    expect(result.reasoningSummary).toBe("auto")
  })
