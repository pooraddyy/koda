...
const paid = (providers: Record<string, { models: Record<string, { cost: { input: number } }> }>) => {
  ...
  if (!item) return 0 // koda_change - koda drops opencode provider without apiKey/auth
...
it.instance(
  // koda_change start - koda always has an auto-routed small-model fallback
  "getSmallModel falls back to koda auto when model IDs lack family metadata",
  Effect.gen(function* () {
    const model = yield* Provider.use.getSmallModel(ProviderV2.ID.make("test-provider"))
    expect(model).toMatchObject({ providerID: "koda", id: "koda-auto/small" })
  }),
  // koda_change end
  {
    config: {
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("nvidia")].options.headers).toEqual({
      "HTTP-Referer": "https://koda.ai/", // koda_change
...
it.instance(
  "configured variants remain authoritative", // koda_change
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    const providers = yield* list
    const model = providers[ProviderV2.ID.anthropic].models["claude-sonnet-4-6"]
    expect(model.variants!["high"]).toBeDefined()
    expect(model.variants!["high"].thinking).toBeUndefined() // koda_change
...
it.effect("plugin config providers persist after instance dispose", () =>
  Effect.gen(function* () {
    ...
    const configDir = path.join(dir, ".koda") // koda_change
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".koda") // koda_change
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() => markPluginDependenciesReady(Global.Path.config)) // koda_change
    yield* Effect.promise(() =>
      Bun.write(
