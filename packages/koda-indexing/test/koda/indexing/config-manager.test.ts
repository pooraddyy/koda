...
describe("CodeIndexConfigManager", () => {
  ...
  test("configures koda with hosted auth options and explicit model metadata", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "koda",
        openAiKey: undefined,
        kodaApiKey: "koda-token",
        kodaBaseUrl: "https://example.test/api/gateway/",
    ...
    expect(cfg.isFeatureConfigured).toBe(true)
    expect(cfg.getConfig().kodaOptions).toEqual({
      apiKey: "koda-token",
  ...
  test("requires koda model metadata from Cloud config", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "koda",
        openAiKey: undefined,
  ...
  test("uses configured dimension for koda models outside the fallback catalog", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "koda",
        openAiKey: undefined,
  ...
  describe("loadConfiguration restart checks", () => {
    ...
    test("requires restart when koda auth changes", () => {
      const cfg = new CodeIndexConfigManager(
        createInput({
          embedderProvider: "koda",
          openAiKey: undefined,
          kodaApiKey: "old-token",
          modelId: "mistralai/mistral-embed-2312",
          modelDimension: 1024,
        }),
      )
