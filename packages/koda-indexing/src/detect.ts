export const INDEXING_PLUGIN_NAMES = ["koda-indexing", "@koda/koda-indexing"] as const
...
function isPathSpecifier(value: string): boolean {
  ...
  if (normalized.includes("/.opencode/") || normalized.includes("/.koda/") || normalized.includes("/.koda/")) {
    return true
  }
...
function normalizePath(value: string): string {
  ...
  const scoped = parts.findIndex((part, i) => part === "@koda" && parts[i + 1] === "koda-indexing")
  if (scoped >= 0) return "@koda/koda-indexing"

  const workspace = parts.findIndex((part, i) => part === "packages" && parts[i + 1] === "koda-indexing")
  if (workspace >= 0) return "@koda/koda-indexing"
