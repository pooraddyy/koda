...
import { kodaEmbedder } from "./embedders/koda"
...
export class CodeIndexServiceFactory {
  ...
  public createEmbedder(): IEmbedder {
    ...
    if (provider === "koda") {
      if (!config.kodaOptions?.apiKey) throw new Error("koda API key is required for embedding.")
      if (!config.modelId) throw new Error("koda embedding model is required.")
      return new kodaEmbedder({
        apiKey: config.kodaOptions.apiKey,
        baseUrl: config.kodaOptions.baseUrl,
        organizationId: config.kodaOptions.organizationId,
