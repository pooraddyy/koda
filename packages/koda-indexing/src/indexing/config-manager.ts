...
export class CodeIndexConfigManager {
  ...
  private kodaOptions?: { apiKey: string; baseUrl?: string; organizationId?: string }
  ...
  private applyInput(input: IndexingConfigInput): void {
    ...
    this.kodaOptions = input.kodaApiKey
      ? { apiKey: input.kodaApiKey, baseUrl: input.kodaBaseUrl, organizationId: input.kodaOrganizationId }
      : undefined
  ...
  private captureSnapshot(): PreviousConfigSnapshot {
    return {
      ...
      kodaApiKey: this.kodaOptions?.apiKey ?? "",
      kodaBaseUrl: this.kodaOptions?.baseUrl ?? "",
      kodaOrganizationId: this.kodaOptions?.organizationId ?? "",
  ...
  public isConfigured(): boolean {
    ...
    if (provider === "koda")
      return !!(this.kodaOptions?.apiKey && this.modelId && this.currentModelDimension && hasStore)
  ...
  doesConfigChangeRequireRestart(prev: PreviousConfigSnapshot): boolean {
    ...
    // Auth changes
    if ((prev.kodaApiKey ?? "") !== (this.kodaOptions?.apiKey ?? "")) return true
    if ((prev.kodaBaseUrl ?? "") !== (this.kodaOptions?.baseUrl ?? "")) return true
    if ((prev.kodaOrganizationId ?? "") !== (this.kodaOptions?.organizationId ?? "")) return true
