...
const repo = process.env.GH_REPO ?? "pooraddyy/koda"
...
const order = ["Core", "TUI", "SDK", "Automation"] as const
const sections = {
  core: "Core",
  tui: "TUI",
  sdk: "SDK",
...
function section(areas: Set<string>) {
  const priority = ["core", "tui", "sdk", "plugin", "github"]
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/opencode packages/sdk packages/plugin github`.text()
