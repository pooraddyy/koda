 ...
 * If a hit is genuinely legitimate, fix the call site rather than weakening
 * the rule. The list is intentionally narrow so it stays low-noise.
 */
...
// Each entry: pattern (literal substring), reason, and optional path prefixes
// where the string is genuinely necessary.
const forbidden: { pattern: string; reason: string; allow?: string[] }[] = [
  { pattern: "opncd.ai/s/", reason: "legacy upstream share URL pattern" },
  {
    ...
    reason: "upstream repo URL -- should be koda-Org/koda",
    allow: ["README.md", "patches/"],
  },
  {
    pattern: "sst/opencode",
    reason: "old upstream org path -- should be koda-Org/koda",
    allow: [],
  },
