...
// inside the trusted koda serve process, so macOS Seatbelt can only confine their spawned
...
// outside the scanned directories. Runtime enforcement remains in @koda/sandbox.

import path from "node:path"
import { host, opaque } from "../packages/opencode/src/koda/sandbox/network-tools"

const root = path.resolve(import.meta.dir, "..")
const source = path.join(root, "packages", "opencode", "src")
const dirs = ["tool", "koda/tool", "mcp"]
const checks = [
...
const network = await Bun.file(path.join(source, "koda", "sandbox", "network.ts")).text()
...
const structure = [
  ...
  !network.includes("opaque.map((item) => item.id)")
    ? ["  koda/sandbox/network.ts must derive runtime opaque tool IDs from network-tools.ts"]
    : []),
  ...(!network.includes("host.map((item) => item.id)")
    ? ["  koda/sandbox/network.ts must derive host-executed tool IDs from network-tools.ts"]
    : []),
  // koda_change - v1.17.13 moved registry wiring from Layer.provide onto the LayerNode graph
  ...
  ...(!mcp.includes("SandboxNetwork.remote(tool)")
    ...
    : []),
  // koda_change - v1.18 preserves remote authority on the native MCP entry before adapting it to an AI SDK tool
...
if (invalid.length > 0 || drift.length > 0 || structure.length > 0) {
  ...
  console.error(
    "Use the @koda/sandbox network capability or classify an opaque client at the common tool boundary.",
  )
  process.exit(1)
}
