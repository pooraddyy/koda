...
/**
 * Verifies that every koda-specific change in shared upstream-owned source files
 * is annotated with a koda_change marker.
 *
 * Usage:
 ...
 *   - contains a koda_change marker comment           (inline annotation)
 *   - falls inside a koda_change start/end block      (block annotation)
 ...
 *
 * Exempt paths (no markers needed — entirely koda-specific):
 *   - packages/opencode/src/koda/**
 *   - packages/opencode/test/koda/**
 *   - Any path containing "koda" in directory or filename
 *   - Any path with a directory starting with "koda-" (e.g. koda-sessions/)
 *   - koda-specific annotation checker support files
 */
...
function isExempt(file: string) {
  ...
  if (norm.split("/").some((part) => part.includes("koda") || part.startsWith("koda-"))) return true
...
function isSource(file: string) {
  ...
  if (ext) return false
  return content(file).startsWith("#!") // koda_change
}
...
//   - revert: true when the file's diff removes any koda_change marker.
//     In that case the changes are reverting koda modifications back to the
...
//     koda construct (e.g. `unixSkip(` → `unix(`) often live in different
...
// Matches the start of a koda_change marker in JS, JSX, YAML, TOML, and shell comments.
const MARKER_PREFIX = /(?:\/\/|\{?\s*\/\*|#)\s*koda_change\b/
...
function coveredLines(text: string): { lines: string[]; covered: Set<number> } {
  ...
  // Whole-file annotation: first non-shebang non-empty line is a koda_change - new file marker.
  const first = lines.find((x) => x.trim() !== "" && !x.startsWith("#!"))
  if (first?.match(/(?:\/\/|\{?\s*\/\*|#)\s*koda_change\s*-\s*new\s*file\b/)) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/(?:\/\/|\{?\s*\/\*|#)\s*koda_change\s+start\b/)) {
      block = true
      covered.add(n)
      continue
    }

    if (line.match(/(?:\/\/|\{?\s*\/\*|#)\s*koda_change\s+end\b/)) {
      covered.add(n)
      block = false
      continue
    }
...
for (const file of files) {
  ...
  if (revert) continue // koda_change - file is reverting koda modifications back to upstream

  const text = content(file) // koda_change
...
if (violations.length === 0) {
  console.log("All shared upstream changes are annotated with koda_change markers.")
  process.exit(0)
}

console.error(
  [
    "Unannotated koda changes found in shared upstream files:",
    "",
    ...violations,
    "",
    "Every koda-specific change in shared upstream source files must be annotated.",
    "",
    "Checked paths:",
    ...SCOPES.map((scope) => `  - ${scope}/**`),
    "",
    "Inline (single line):",
    "  const url = Flag.koda_MODELS_URL || 'https://models.dev' // koda_change",
    "",
    ...
    "  - packages/opencode/src/koda/**",
    "  - packages/opencode/test/koda/**",
    "  - Any path containing 'koda' in the directory or filename",
    "  - Any directory starting with 'koda-' (e.g. koda-sessions/)",
    "  - script/upstream/**",
    "  - koda-specific annotation checker support files",
    "",
