 ...
 * remaining koda Promise facades are migrated away. It also prevents tests
...
const testAllow: Record<string, { count: number; reason: string }> = {
  ...
  "koda/config-resilience.test.ts": { count: 4, reason: "existing runtime integration test" },
  "koda/config-validation.test.ts": { count: 2, reason: "existing runtime integration test" },
  "koda/cli-shutdown.test.ts": { count: 1, reason: "mocked runtime boundary for shutdown unit tests" },
  "koda/plan-followup.test.ts": { count: 3, reason: "existing runtime integration test" },
  "koda/session-compaction-chunks.test.ts": {
    count: 2,
    reason: "disk-backed instance integration test cleanup",
  },
  "koda/session-fork-remap.test.ts": {
    count: 2,
    reason: "disk-backed instance integration test cleanup",
  },
  "koda/koda-sessions.test.ts": {
    count: 31,
    reason:
      ...
      "then drives the module-level kodaSessions seams and verifies the fence. " +
  ...
  "koda/session/platform-attribution.test.ts": { count: 2, reason: "existing runtime integration test" },
  "koda/session-prompt-queue.test.ts": { count: 6, reason: "prompt queue legacy instance bridge regression" },
  "koda/session-prompt-steering.test.ts": {
    count: 2,
    reason: "disk-backed prompt steering integration test cleanup",
  },
  "server/experimental-session-list.test.ts": { count: 2, reason: "koda session list integration test" },
  "koda/server/cloud-session-import.test.ts": { count: 5, reason: "full app cloud import transaction integration" },
  "koda/server/listener-runtime.test.ts": { count: 4, reason: "listener and AppRuntime integration test" },
  "tool/recall.test.ts": { count: 11, reason: "existing runtime integration test" },
}

const owned = (file: string) => file.startsWith("koda/") || file.startsWith("koda-sessions/")
