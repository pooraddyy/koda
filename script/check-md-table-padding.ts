 ...
 *
 * Enforcement scope (koda-owned paths only, to avoid upstream-sync churn):
 *   - Any top-level markdown file (TESTING.md, AGENTS.md, README.md, …)
 *   - Any path segment containing "koda" or starting with "koda-"
...
function skip(file: string) {
  ...
  if (parts.some((p) => p.includes("koda") || p.startsWith("koda-"))) return false
